...
async function removeResourceChecks(knex) {
  ...
  for (const check of checks.rows) if (check.definition.includes('tenant <> customer_tenant') || check.conname === 'co_management_in_app_resource_check')
...
exports.up = async function(knex) {
  ...
  await knex.raw(`CREATE OR REPLACE FUNCTION co_management_in_app_qualify_resource() RETURNS trigger AS $$ BEGIN
  ...
  if (await supportsTriggers(knex, TABLE)) {
    await knex.raw('DROP TRIGGER IF EXISTS co_management_in_app_qualify_resource ON ??', [TABLE]);
    await knex.raw('CREATE TRIGGER co_management_in_app_qualify_resource BEFORE INSERT OR UPDATE ON ?? FOR EACH ROW EXECUTE FUNCTION co_management_in_app_qualify_resource()', [TABLE]);
  }
  ...
  await knex.raw(`ALTER TABLE ?? ADD CONSTRAINT co_management_in_app_resource_check CHECK (
    (resource_type = 'ticket' AND ticket_id IS NOT NULL AND resource_id = ticket_id AND tenant <> customer_tenant) OR
    (resource_type = 'project_task' AND ticket_id IS NULL))`, [TABLE]);
  // Pending pre-rollout work joins recovery. Published native notices are not
  // replayed as new alerts; their cached content has no qualified read receipt.
  await knex.raw(`INSERT INTO co_management_event_consumers (tenant, event_id, consumer, status, completed_at, error_code)
    SELECT tenant, event_id, 'internal-notifications', CASE WHEN status = 'pending' THEN 'pending' ELSE 'cancelled' END,
...
exports.down = async function(knex) {
  if (await knex(TABLE).where('resource_type', 'project_task').first()) throw new Error('Cannot discard retained task notification receipts');
  await removeResourceChecks(knex);
  if (await supportsTriggers(knex, TABLE)) {
    await knex.raw('DROP TRIGGER IF EXISTS co_management_in_app_qualify_resource ON ??', [TABLE]);
  }
  await knex.raw('DROP FUNCTION IF EXISTS co_management_in_app_qualify_resource()');
  await knex.raw('ALTER TABLE ?? ALTER COLUMN ticket_id SET NOT NULL, ADD CHECK (tenant <> customer_tenant)', [TABLE]);

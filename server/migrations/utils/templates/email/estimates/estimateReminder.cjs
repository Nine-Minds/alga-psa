/**
 * Source-of-truth: estimate-reminder-email template.
 *
 * Sent to a client to remind them that an outstanding estimate is about to
...
const TEMPLATE_NAME = 'estimate-reminder-email';
const SUBTYPE_NAME = 'Estimate Reminder';

const SUBJECTS = {
  en: 'Reminder: Estimate {{estimate.number}} expires on {{estimate.validUntil}}',
};

const COPY = {
  en: {
    headerLabel: 'Estimate Reminder',
    greeting: 'Hello,',
    intro: 'This is a reminder that your estimate from <strong>{{company.name}}</strong> expires on <strong>{{estimate.validUntil}}</strong>.',
    ...
    closingNote: 'If you have any questions about this estimate, please don\'t hesitate to contact us.',
    thankYou: 'Thank you,',
    footer: 'Powered by Alga PSA',
    textHeader: 'Reminder: Estimate {{estimate.number}} expires on {{estimate.validUntil}}',
...
This is a reminder that your estimate from {{company.name}} expires on {{estimate.validUntil}}.

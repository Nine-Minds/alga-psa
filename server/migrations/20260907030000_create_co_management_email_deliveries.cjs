...
exports.up = async function (knex) {
  if (!await knex.schema.hasTable(TABLE)) await knex.schema.createTable(TABLE, table => {
    ...
    table.uuid('event_id').notNullable(); table.uuid('customer_tenant').notNullable(); table.uuid('relationship_id').notNullable();
    table.uuid('ticket_id').notNullable(); table.uuid('comment_id').notNullable(); table.uuid('thread_id').notNullable(); table.string('audience', 16).notNullable();
    table.string('status', 16).notNullable().defaultTo('pending'); table.integer('attempt_count').notNullable().defaultTo(0);
    table.timestamp('next_attempt_at', { useTz: true }).nullable().defaultTo(knex.fn.now()); table.timestamp('completed_at', { useTz: true }).nullable();
    table.timestamp('created_at', { useTz: true }).notNullable().defaultTo(knex.fn.now()); table.string('error_code', 100).nullable();
    table.primary(['tenant', 'delivery_key']); table.index(['tenant', 'status', 'next_attempt_at'], 'co_management_email_delivery_due_idx');
    table.check("audience IN ('requester', 'shared_it') AND tenant <> customer_tenant AND attempt_count >= 0");
...
exports.down = async function (knex) {
  if (await knex(TABLE).first() || await knex('co_management_event_consumers').where('consumer', 'co-managed-email').first()) throw new Error('Cannot discard retained co-managed email delivery history');
  await knex.raw('ALTER TABLE co_management_event_consumers DROP CONSTRAINT co_management_event_consumers_consumer_check');
  await knex.raw("ALTER TABLE co_management_event_consumers ADD CONSTRAINT co_management_event_consumers_check CHECK (consumer IN ('search-index', 'internal-notifications') AND attempts >= 0)");

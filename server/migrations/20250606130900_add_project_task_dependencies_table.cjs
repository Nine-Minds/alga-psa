exports.up = async function(knex) {
    ...
    await knex.schema.createTable('project_task_dependencies', table => {
        ...
        table.enum('dependency_type', [
            'finish_to_start',   // FS - Most common: B starts after A finishes
            'start_to_start',    // SS - B starts when A starts (parallel work)
            'finish_to_finish',  // FF - B finishes when A finishes (QA processes)
            'start_to_finish',   // SF - B finishes when A starts (rare)
            'blocks',            // Hard blocker
            'relates_to',        // Soft relationship
            'duplicates'         // Duplicate tasks
        ]).notNullable().defaultTo('finish_to_start');

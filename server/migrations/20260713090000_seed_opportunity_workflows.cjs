...
// Keep this migration self-contained. Setup images only package the core migration
// tree, so a migration cannot import its implementation from EE onboarding seeds.
// The onboarding seed contains the same definitions for tenants created later.
const WORKFLOWS = [
  {
...
exports.up = async function up(knex) {
  ...
  for (const row of tenants) {
    for (const workflow of WORKFLOWS) {
      await seedWorkflow(knex, row.tenant, workflow);
    }
  }
};

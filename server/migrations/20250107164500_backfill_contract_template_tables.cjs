    ...
    await trx
      .insert(function insertTemplates() {
        this.select({
          ...
          template_status: 'c.status',
          template_metadata: 'c.template_metadata',
    ...
    await trx
      .insert(function insertTemplateLines() {
        this.select({
          ...
          enable_overtime: knex.raw('COALESCE(terms.enable_overtime, cl.enable_overtime, false)'),
          overtime_rate: knex.raw('COALESCE(terms.overtime_rate, cl.overtime_rate)'),
          overtime_threshold: knex.raw('COALESCE(terms.overtime_threshold, cl.overtime_threshold)'),
          enable_after_hours_rate: knex.raw(
            'COALESCE(terms.enable_after_hours_rate, cl.enable_after_hours_rate, false)'
          ),
          after_hours_multiplier: knex.raw(
            'COALESCE(terms.after_hours_multiplier, cl.after_hours_multiplier)'
          ),
          ...
          .leftJoin({ terms: 'contract_line_template_terms' }, function joinTerms() {

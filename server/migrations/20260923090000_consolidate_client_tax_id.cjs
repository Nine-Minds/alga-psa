...
exports.consolidateTenant = async function consolidateTenant(knex, tenant) {
  ...
  await db.table('clients').where({ tenant })
    ...
    .update({ properties: knex.raw("jsonb_set(properties - 'tax_id', '{legacy_tax_id}', CASE WHEN jsonb_typeof(properties->'legacy_tax_id') = 'array' THEN (properties->'legacy_tax_id') || jsonb_build_array(to_jsonb(BTRIM(properties->>'tax_id'))) WHEN jsonb_exists(properties, 'legacy_tax_id') THEN jsonb_build_array(properties->'legacy_tax_id', to_jsonb(BTRIM(properties->>'tax_id'))) ELSE to_jsonb(BTRIM(properties->>'tax_id')) END, true)") });
  await db.table('clients').where({ tenant })

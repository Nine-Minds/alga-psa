const MIGRATION_TENANT = 'migration:20260719102000_add_marketing_permissions';
const TENANT_ENUMERATION_REASON = 'enumerate tenants for marketing permission backfill';

const PERMISSIONS = [
  { action: 'read', description: 'View marketing campaigns, content, posts, and sequences' },
  { action: 'manage', description: 'Manage marketing campaigns, content, posts, sequences, and forms' },
];
  ...
  for (const { tenant } of tenants) {
    ...
    for (const definition of PERMISSIONS) {
      let permission = await db.table('permissions')
        .where({ tenant, resource: 'marketing', action: definition.action })
...
exports.down = async function down(knex) {
  ...
  for (const { tenant } of tenants) {
    ...
    const permissionIds = await db.table('permissions')
      .where({ tenant, resource: 'marketing' })

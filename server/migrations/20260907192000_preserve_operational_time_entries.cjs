...
exports.up = async function(knex) {
  ...
  const customerEntries = knex('time_entries as e').join('tenants as t', 'e.tenant', 't.tenant').where('t.product_code', 'co_managed');
  const priced = await customerEntries.clone().whereRaw(`NOT (${UNPRICED.replace(/\b(billable_duration|service_id|contract_line_id|contract_line_source|contract_line_unresolved_reason|tax_rate_id|tax_region|invoiced)\b/g, 'e.$1')})`).first('e.entry_id');
  const linked = await customerEntries.clone().whereExists(knex('invoice_time_entries as i').select(knex.raw('1')).whereRaw('i.tenant::uuid = e.tenant AND i.entry_id = e.entry_id')).first('e.entry_id');
  ...
  await knex.raw("UPDATE time_entries e SET billing_mode = 'operational' FROM tenants t WHERE t.tenant = e.tenant AND t.product_code = 'co_managed'");

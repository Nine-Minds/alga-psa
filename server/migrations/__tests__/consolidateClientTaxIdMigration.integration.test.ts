...
beforeAll(async () => {
  wireLocalTestDbEnv(); db = await createTestDbConnection(); tenant = randomUUID(); otherTenant = randomUUID(); ids = Array.from({ length: 9 }, () => randomUUID()); otherClientId = randomUUID();
  ...
  await db('clients').insert(ids.map((client_id, i) => ({ tenant, client_id, client_name: `Tax ${i}`, is_inactive: false, tax_id_number: i === 1 || i === 3 ? 'CANONICAL' : i === 4 ? '  SAME  ' : i === 5 ? 'CANONICAL' : null, properties: i === 0 ? { tax_id: ' BACKFILL ' } : i === 1 ? { tax_id: 'LEGACY' } : i === 2 ? { tax_id: '   ' } : i === 3 ? { legacy_tax_id: 'OLDER-AUDIT', tax_id: 'NEW-LEGACY' } : i === 4 ? { tax_id: 'SAME' } : i === 5 ? { legacy_tax_id: ['OLDER-AUDIT', 'PRIOR-CONFLICT'], tax_id: 'NEW-LEGACY' } : i === 6 ? { tax_id: null } : i === 7 ? null : undefined, created_at: db.fn.now(), updated_at: db.fn.now() })));

...
const migration = require(path.resolve(__dirname, '../20260904120000_enforce_availability_scope_uniqueness.cjs'));
...
describe('availability scope uniqueness migration', () => {
  ...
  // distributed table already modified in the same transaction, so the dedupe
  // and the index creation must not share one.
  it('runs untransacted so Citus accepts the index after the dedupe', () => {

...
const nextConfig = {
  ...
  turbopack: {
    ...
    resolveAlias: {
      ...
      // Note: For Turbopack dev, we still use /src for HMR support
      // In production builds (webpack), these point to /dist
      '@alga-psa/core': '../packages/core/dist',
      '@alga-psa/types': '../packages/types/dist',
      '@alga-psa/validation': '../packages/validation/dist',
      '@alga-psa/db': '../packages/db/dist',
      ...
      '@alga-psa/ui-kit': '../packages/ui-kit/dist',
      '@alga-psa/shared': '../packages/shared/dist',

      // Mixed packages - main entry to dist
  ...
  webpack: (config, { isServer, dev }) => {
    ...
    config.resolve.alias = {
      ...
      // Pre-built packages - point to /dist for compiled JS (buildable code only)
      // Runtime subpaths (actions, components, hooks) resolve via package.json exports to /src
      '@alga-psa/core': path.join(__dirname, '../packages/core/dist'),
      '@alga-psa/types': path.join(__dirname, '../packages/types/dist'),
      '@alga-psa/validation': path.join(__dirname, '../packages/validation/dist'),
      '@alga-psa/db': path.join(__dirname, '../packages/db/dist'),
      ...
      '@alga-psa/ui-kit': path.join(__dirname, '../packages/ui-kit/dist'),
      '@alga-psa/shared': path.join(__dirname, '../packages/shared/dist'),

      // Packages with mixed buildable/runtime - main entry points to dist

...
exports.seed = async function (knex) {
    ...
    return db.table('interactions').insert([
        {
            tenant: tenantId,
            type_id: (await knex('system_interaction_types').where({ type_name: 'Call' }).select('type_id').first())?.type_id ?? null,
            contact_name_id: (await db.table('contacts').whereRaw("full_name ILIKE '%alice%'").select('contact_name_id').first())?.contact_name_id ?? null,
            client_id: (await db.table('clients').where({ client_name: 'Wonderland' }).select('client_id').first())?.client_id ?? null,
            user_id: (await db.table('users').where({ username: 'glinda' }).select('user_id').first())?.user_id ?? null,
            ticket_id: (await db.table('tickets').where({ title: 'Missing White Rabbit' }).select('ticket_id').first())?.ticket_id ?? null,
        ...
        {
            tenant: tenantId,
            type_id: (await knex('system_interaction_types').where({ type_name: 'Email' }).select('type_id').first())?.type_id ?? null,
            contact_name_id: (await db.table('contacts').whereRaw("full_name ILIKE '%alice%'").select('contact_name_id').first())?.contact_name_id ?? null,
            client_id: (await db.table('clients').where({ client_name: 'Wonderland' }).select('client_id').first())?.client_id ?? null,
            user_id: (await db.table('users').where({ username: 'glinda' }).select('user_id').first())?.user_id ?? null,
            ticket_id: (await db.table('tickets').where({ title: 'Missing White Rabbit' }).select('ticket_id').first())?.ticket_id ?? null,
        ...
        {
            tenant: tenantId,
            type_id: (await knex('system_interaction_types').where({ type_name: 'Meeting' }).select('type_id').first())?.type_id ?? null,
            contact_name_id: (await db.table('contacts').whereRaw("full_name ILIKE '%dorothy%'").select('contact_name_id').first())?.contact_name_id ?? null,
            client_id: (await db.table('clients').where({ client_name: 'Emerald City' }).select('client_id').first())?.client_id ?? null,
            user_id: (await db.table('users').where({ username: 'glinda' }).select('user_id').first())?.user_id ?? null,
            ticket_id: null,
        ...
        {
            tenant: tenantId,
            type_id: (await knex('system_interaction_types').where({ type_name: 'Note' }).select('type_id').first())?.type_id ?? null,
            contact_name_id: (await db.table('contacts').whereRaw("full_name ILIKE '%alice%'").select('contact_name_id').first())?.contact_name_id ?? null,
            client_id: (await db.table('clients').where({ client_name: 'Wonderland' }).select('client_id').first())?.client_id ?? null,
            user_id: (await db.table('users').where({ username: 'glinda' }).select('user_id').first())?.user_id ?? null,
            ticket_id: (await db.table('tickets').where({ title: 'Missing White Rabbit' }).select('ticket_id').first())?.ticket_id ?? null,

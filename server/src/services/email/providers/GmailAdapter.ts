  ...
  async listMessagesSince(startHistoryId: string): Promise<string[]> {
    try {
      ...
      if (gmailNotFound) {
        ...
        (historyError as any).responseBody = error?.response?.data;
        (historyError as any).requestId = error?.response?.headers?.['request-id'] || error?.response?.headers?.['client-request-id'];

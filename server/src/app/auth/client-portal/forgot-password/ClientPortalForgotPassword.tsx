...
interface ClientPortalForgotPasswordProps {
  branding: TenantBranding | null;
  portalDomain?: string;
}

const ClientPortalForgotPassword: React.FC<ClientPortalForgotPasswordProps> = ({ branding, portalDomain }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    await recoverPassword(formData.email, 'client', portalDomain);
    ...
    const checkEmailHref = `/auth/check-email?email=${encodeURIComponent(formData.email)}&type=forgot_password&portal=client`;
    router.push(appendPortalDomain(checkEmailHref, portalDomain));
  };
  ...
  return (
    <div className={`flex flex-col items-center pt-20 min-h-screen ${
      ...
      <div className="w-full max-w-md p-8 space-y-8 bg-card rounded-lg shadow-lg">
        ...
        <div className="text-center">
          <Link id="forgot-password-back-to-login-link" href={appendPortalDomain('/auth/client-portal/signin', portalDomain)} className="text-sm font-medium text-[rgb(var(--color-primary-500))] hover:text-[rgb(var(--color-primary-400))]">

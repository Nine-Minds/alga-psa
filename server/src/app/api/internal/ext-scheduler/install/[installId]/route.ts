...
async function loadEeRoute(): Promise<EeRouteModule | null> {
  ...
  if (!eeRouteModulePromise) {
    eeRouteModulePromise = import('@enterprise/app/api/internal/ext-scheduler/install/[installId]/route')
      .then((module) => module as EeRouteModule)
      .catch((error) => {
        console.error('[internal/ext-scheduler] Failed to load EE route', error);
        return null;
      });
  }
...
function eeUnavailable(): Response {
  return new Response(
    JSON.stringify({
      error: 'Extension scheduling is only available in the Enterprise Edition.',
      code: 'EE_REQUIRED',
    }),

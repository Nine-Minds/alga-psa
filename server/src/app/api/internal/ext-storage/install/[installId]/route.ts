...
async function loadEeRoute(): Promise<EeRouteModule | null> {
  ...
  if (!eeRouteModulePromise) {
    eeRouteModulePromise = import('@enterprise/app/api/internal/ext-storage/install/[installId]/route')
      .then(module => module as EeRouteModule)
      .catch(error => {
        console.error('[internal/ext-storage] Failed to load EE route', error);
        return null;
      });
  }
...
function eeUnavailable(): Response {
  return new Response(
    JSON.stringify({
      error: 'Extension storage is only available in the Enterprise Edition.',
      code: 'EE_REQUIRED'
    }),
    {
      status: 501,

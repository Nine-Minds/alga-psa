...
export async function GET(request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  ...
  const job = await db('migration_jobs').where({ migration_job_id: jobId }).first();
  if (!job) return NextResponse.json({ error: 'MIGRATION_NOT_FOUND' }, { status: 404 });
  const rows = await db('migration_staged_records').where({ migration_job_id: jobId }).select('entity_type', 'package_record_id', 'source_record_id', 'validation_state', 'validation_errors');

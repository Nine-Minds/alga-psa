...
export async function POST(request: Request) {
  ...
  const directory = await mkdtemp(join(tmpdir(), 'amp-spreadsheet-')); const inputPath = join(directory, name.toLowerCase().endsWith('.xlsx') ? 'source.xlsx' : 'source.csv'); const outputPath = join(directory, 'converted.amp'); let bytes = 0;
  ...
  try {
    const sourceOutput = createWriteStream(inputPath, { mode: 0o600 }); Readable.fromWeb(request.body as never).pipe(meter).pipe(sourceOutput); await finished(sourceOutput);
    ...
    const digest = createHash('sha256'); const storageInput = new PassThrough(); const packageStream = createReadStream(outputPath); packageStream.on('data', (chunk) => digest.update(chunk)); packageStream.pipe(storageInput);
    const stored = await StorageService.uploadStream(user.tenant, storageInput, `${name}.amp`, { mime_type: 'application/vnd.sqlite3', uploaded_by_id: user.user_id, metadata: { context: 'amp_migration_package', retention_days: 30, converted_from: name } }); const sha256 = digest.digest('hex');

...
import { downloadNamedConversationEditorFile, CoManagedSharedWorkError } from '@alga-psa/co-managed';
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ attachmentId: string }> }) {
  try {
    ...
    const result = await downloadNamedConversationEditorFile(await getConnection(session.user.tenant),

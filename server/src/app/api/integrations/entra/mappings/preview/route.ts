...
async function loadEeRoute(): Promise<EeRouteModule | null> {
  ...
  if (!eeRouteModulePromise) {
    eeRouteModulePromise = import('@enterprise/app/api/integrations/entra/mappings/preview/route')
      .then((module) => module as unknown as EeRouteModule)
      .catch((error) => {
        console.error('[integrations/entra/mappings/preview] Failed to load EE route', error);
        return null;
      });
  }

...
async function loadEeRoute(): Promise<EeRouteModule | null> {
  ...
  if (!eeRouteModulePromise) {
    eeRouteModulePromise = import('@enterprise/app/api/v1/platform-notifications/[notificationId]/reads/route')
      .then((module) => module as unknown as EeRouteModule)
      .catch((error) => {
        console.error('[v1/platform-notifications/:id/reads] Failed to load EE route', error);
        return null;
      });
  }

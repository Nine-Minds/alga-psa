...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      logger.error('[Stripe Payment Webhook] Failed to import PaymentService');
      return NextResponse.json(
        { error: 'Payment integration not available' },
        { status: 503 }
      );
    }

    // Create payment service for tenant
    const paymentService = await PaymentService.create(tenantId);

    // Dynamically import StripePaymentProvider for parsing
    let createStripePaymentProvider: any;
    try {
      const eeModule = await import('@ee/lib/payments');
      createStripePaymentProvider = eeModule.createStripePaymentProvider;
    } catch (importError) {
      logger.error('[Stripe Payment Webhook] Failed to import StripePaymentProvider');
      return NextResponse.json(
        { error: 'Payment integration not available' },
        { status: 503 }
      );
    ...
    // Parse the event using StripePaymentProvider

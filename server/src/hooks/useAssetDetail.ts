...
export function useAssetDetail(assetId: string) {
  ...
  const { 
    ...
    assetId ? ['asset', assetId, 'summary'] : null,
    ([_, id]) => getAssetSummaryMetrics(id)
  );

  // Fetch cached RMM data
  const { 
    data: rmmData, 
    error: rmmError, 
    ...
    assetId ? ['asset', assetId, 'rmm'] : null,
    ([_, id]) => getAssetRmmData(id)
  );

  const [isRefreshing, setIsRefreshing] = useState(false);

  // Manual refresh function
  const refreshRmmData = useCallback(async () => {
    if (!assetId) return;

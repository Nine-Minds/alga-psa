...
interface ProductChargeMockOptions {
  productRows?: Array<Record<string, any>>;
  taxRegion?: string | undefined;
  isTaxable?: boolean;
}

const installProductChargeMocks = (
  ...
  (engine as any).knex = vi.fn().mockImplementation((tableName: string) => {
    ...
    if (tableName === "contract_line_services as cls") {
      return buildQuery(null, productRows);
    }
...
describe("BillingEngine recurring product timing", () => {
  ...
  it("T063: recurring product charges preserve contract override precedence over catalog pricing after timing cutover", async () => {
    const engine = new BillingEngine();
    installProductChargeMocks(engine, {
      productRows: [
        {

...
describe("project invoice lifecycle contracts", () => {
  it("T012: standalone generation stamps project_id and persists charge + optimistic entry transition together", () => {
    ...
    // The auth wrapper is intentionally thin; the invoice header is assembled
    // in the transactional implementation.
    const invoiceCreate = invoiceGeneration.slice(
      invoiceGeneration.indexOf(
        "export async function createInvoiceFromBillingResultImpl",
      ),
    );
    const schedulePersistence = section(
      invoiceGeneration,
    ...
    expect(action).toContain(
      "billingEngine.calculateProjectBilling(projectId, entryIds)",
    );
    expect(action).toContain("{ projectId },");
    expect(invoiceCreate).toContain(
  ...
  it("marks only the exact material rows represented by generated product charges", () => {
    ...
    expect(materialPersistence).toContain(
      "charge.material_source_type === 'project'",
    );
    expect(materialPersistence).toContain(
      "charge.material_source_type === 'ticket'",
    );
    expect(materialPersistence).toContain(
    ...
    expect(materialPersistence).toContain(
      "updatedCount !== uniqueProjectMaterialIds.length",
    );
  ...
  it("T019: finalization issues project-earmarked deposit credit and application prefers the matching project", () => {
    ...
    expect(creditActions).toContain(
      "left.projectId === invoiceProjectId ? 0 : 1",
    );
  });
  ...
  it("T023: unfinalize preserves project linkage and caps while hard delete releases them exactly once", () => {
    ...
    const unfinalize = section(
      invoiceModification,
      "export const unfinalizeInvoice",
      "export const updateInvoiceManualItems",
    );
    ...
    expect(rollback).toContain(
      "metadata.project_billing_cap_rolled_back === true",
    );
    expect(rollback).toContain(
      "ProjectBillingCapUsage.getForUpdate(delta.configId, trx)",
    );
    expect(rollback).toContain(
      "{ billed: -billedRollback, writtenDown: -writtenDownRollback }",
    );
    expect(rollback).toContain("project_billing_cap_rolled_back: true");
    expect(invoiceModification).toContain(

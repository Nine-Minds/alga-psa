...
describe('QuickAskOverlay (streaming)', () => {
  it('expands to Chat and posts to the streaming completions endpoint', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Quick Ask input'), {
      target: { value: 'Ping' },
    });
  ...
  it('handles function proposal approval via /api/chat/v1/execute from expanded quick ask chat', async () => {
    expect(QuickAskOverlay).toBeDefined();

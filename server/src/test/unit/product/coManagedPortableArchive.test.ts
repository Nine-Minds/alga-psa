...
async function fixture(size = 32) {
  const root = await mkdtemp(join(tmpdir(), 'portable-archive-test-')); roots.push(root);
  ...
  return { root, bytes, file, context, manifest, directories, seal: () => sealPortableArchive({ context, manifest, files: [file], stagingRoot: root }, PASSPHRASE) };
}
...
describe('portable encrypted archive transport', () => {
  it('round trips an authenticated manifest and actual files across a frame boundary using only the customer passphrase', async () => {
    ...
    const sealed = await sealPortableArchive({ context: f.context, manifest: f.manifest, files: [f.file, empty], stagingRoot: f.root }, PASSPHRASE);
    ...
    try {
      ...
      opened = await openPortableArchive(sealed.path, PASSPHRASE, f.context, f.root);
  ...
  it('rejects wrong passphrases, context substitution, modified bytes, reordered or duplicated frames, truncation and appended content before returning a lease', async () => {
    ...
    try {
      ...
      const other = await sealPortableArchive({ context: { ...f.context, packageId: randomUUID() }, manifest: f.manifest, files: [f.file], stagingRoot: f.root }, PASSPHRASE);
      ...
      await expect(openPortableArchive(sealed.path, `${PASSPHRASE} wrong`, undefined, f.root)).rejects.toThrow(MESSAGE);
      await expect(openPortableArchive(sealed.path, PASSPHRASE, { ...f.context, sourceTenant: randomUUID() }, f.root)).rejects.toThrow(MESSAGE);
      for (let index = 0; index < variants.length; index++) {
        const path = join(f.root, `corrupt-${index}.alga`); await writeFile(path, variants[index]);
        await expect(openPortableArchive(path, PASSPHRASE, undefined, f.root)).rejects.toThrow(MESSAGE);
  ...
  it('rejects authenticated but inconsistent blob checksums and cleans up after extraction failure', async () => {
    ...
    try {
      ...
      try {
        ...
        await expect(openPortableArchive(path, PASSPHRASE, undefined, f.root)).rejects.toThrow(MESSAGE); expect(await f.directories()).toHaveLength(1);
      } finally { key.fill(0); }
    } finally { await sealed.dispose(); }
    expect(await f.directories()).toEqual([]);
  });
  ...
  it('rejects unsafe identities, duplicate membership, oversized files, symlinks and source hash changes without partial output', async () => {
    ...
    for (const files of [[{ ...f.file, id: '../../escape' }], [{ ...f.file, id: f.file.id.toUpperCase() }], [f.file, f.file], [{ ...f.file, size: PORTABLE_ARCHIVE_LIMITS.blobBytes + 1 }]]) {
      await expect(sealPortableArchive({ context: f.context, manifest: f.manifest, files, stagingRoot: f.root }, PASSPHRASE)).rejects.toThrow(MESSAGE);
      expect(await f.directories()).toEqual([]);
    }
    let accessed = false;
    const accessor = Object.defineProperty({}, 'payload', { enumerable: true, get() { accessed = true; return 'never execute'; } });
    await expect(sealPortableArchive({ context: f.context, manifest: accessor, files: [], stagingRoot: f.root }, PASSPHRASE)).rejects.toThrow(MESSAGE);

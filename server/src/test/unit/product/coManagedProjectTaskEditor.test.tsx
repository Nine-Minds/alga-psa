...
const mocks = vi.hoisted(() => ({ flag: vi.fn(), load: vi.fn(), statuses: vi.fn(), save: vi.fn(), list: vi.fn() }));
...
vi.mock('../../../lib/actions/coManagedProjectTaskActions', () => ({ getSharedProjectTaskEditorAction: mocks.load, getSharedProjectTaskStatusesAction: mocks.statuses,
  editSharedProjectTaskAction: mocks.save, listSharedProjectTasksAction: mocks.list }));
...
it.each([{ enabled: false }, { enabled: true, loading: true }, { enabled: true, error: new Error('unavailable') }])('does not load shared project UI with an unavailable release flag: %j', flag => {
  ...
  expect(mocks.load).not.toHaveBeenCalled(); expect(mocks.list).not.toHaveBeenCalled(); expect(screen.queryByRole('button')).toBeNull();
});
...
it('requires a reload on conflict and removes fields after access denial', async () => {
  ...
  expect(name()).toBeDisabled(); expect(save()).toBeDisabled(); fireEvent.click(screen.getByRole('button', { name: 'coManaged.policy.reload' }));

...
vi.mock('@alga-psa/tickets/actions/conversationAiActions', () => ({ prepareNamedConversationSynthesisAction: mocks.prepare, invokeNamedConversationAiAction: mocks.prepare }));
import { POST as synthesis } from '../../../app/api/tickets/conversation-synthesis/route';
import { POST as participation } from '../../../app/api/tickets/conversation-ai/route';
describe.each([{ name: 'synthesis', POST: synthesis }, { name: 'ai', POST: participation }])('conversation $name HTTP boundary', ({ name, POST }) => {
...
const request = (data: unknown = body, origin = 'https://psa.example.test') => new NextRequest(`https://psa.example.test/api/tickets/conversation-${name}`, {
...
it('rejects cross-origin and malformed requests before starting generation', async () => {
  ...
  expect((await POST(new NextRequest(`https://psa.example.test/api/tickets/conversation-${name}`, { method: 'POST', body: 'x' }))).status).toBe(403);
...
it('uses the deployment-aware public origin when Next normalizes the internal request URL', async () => {
  ...
  const response = await POST(new NextRequest(`http://localhost:3000/api/tickets/conversation-${name}`, {

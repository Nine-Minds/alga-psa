...
describe('search SQL helpers', () => {
  it('T021 builds weighted tsvector SQL using process_large_lexemes', () => {
    ...
    expect(fragment.sql).toContain("setweight(to_tsvector('english', public.process_large_lexemes(?)), 'A')");
    expect(fragment.sql).toContain("setweight(to_tsvector('english', public.process_large_lexemes(?)), 'B')");
    expect(fragment.sql).toContain("setweight(to_tsvector('english', public.process_large_lexemes(?)), 'C')");

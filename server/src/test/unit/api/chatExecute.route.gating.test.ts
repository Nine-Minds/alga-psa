...
const chatCompletionsHandleExecuteMock = vi.hoisted(() => vi.fn<
...
vi.mock('@product/chat/entry', () => ({
  ChatCompletionsService: {
    handleExecute: chatCompletionsHandleExecuteMock,
  },
}));

describe('POST /api/chat/v1/execute', () => {
  const originalEdition = process.env.EDITION;
  const originalPublicEdition = process.env.NEXT_PUBLIC_EDITION;

  beforeEach(() => {
    isExperimentalFeatureEnabledMock.mockReset();
    chatCompletionsHandleExecuteMock.mockReset();
  ...
  it('returns 403 when aiAssistant is disabled', async () => {
    ...
    vi.resetModules();
    const { POST } = await import('@/app/api/chat/v1/execute/route');

    const request = new NextRequest(
      new Request('http://example.com/api/chat/v1/execute', {
        method: 'POST',
        headers: {
    ...
    expect(chatCompletionsHandleExecuteMock).not.toHaveBeenCalled();
  });
});

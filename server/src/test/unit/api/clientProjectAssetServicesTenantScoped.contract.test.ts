...
describe('client, project, and asset API services tenant-scoped query contract', () => {
  ...
  it('keeps high-traffic service roots and joins behind facade helpers', () => {
    ...
    // The default-location join lives in joinDefaultLocation(); it must still go through the facade.
    expect(clientSource).toContain('joinDefaultLocation(db, dataQuery);');
    expect(clientSource).toContain("db.tenantJoinFirstMatching(query, 'client_locations', 'cl'");

  ...
  it('keeps identity mappings tenant isolated and makes a second apply a skip', async () => {
    ...
    await knex.transaction(async (trx) => {
      await trx('migration_identity_mappings').insert({
        ...
        // No backing migration_jobs row in this focused test; the provenance
        // FK column is nullable by design.
    ...
    await knex('migration_identity_mappings').where({ tenant, namespace: 'integration-test', source_record_id: sourceId }).delete();
  });
});

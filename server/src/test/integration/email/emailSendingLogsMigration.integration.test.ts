...
describe('email_sending_logs entity context migrations', () => {
  ...
  it('adds outbound threading diagnostic columns', async () => {
    await expect(knex.schema.hasColumn('email_sending_logs', 'provider_message_id')).resolves.toBe(true);
    await expect(knex.schema.hasColumn('email_sending_logs', 'rfc_message_id')).resolves.toBe(true);
    await expect(knex.schema.hasColumn('email_sending_logs', 'thread_id')).resolves.toBe(true);
    await expect(knex.schema.hasColumn('email_sending_logs', 'comment_id')).resolves.toBe(true);
    await expect(knex.schema.hasColumn('email_sending_logs', 'reply_token_hash')).resolves.toBe(true);
    await expect(knex.schema.hasColumn('email_sending_logs', 'reply_token_suffix')).resolves.toBe(true);
  });
  ...
  it('creates outbound threading indexes', async () => {
    const result = await knex.raw(
      `SELECT indexname FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'email_sending_logs'`
    );
    const indexNames = (result?.rows ?? []).map((row: any) => row.indexname);
    expect(indexNames).toContain('idx_email_sending_logs_tenant_provider_message');
    expect(indexNames).toContain('idx_email_sending_logs_tenant_rfc_message');
    expect(indexNames).toContain('idx_email_sending_logs_tenant_reply_token_hash');
  });
  ...
  it('migration files export up/down functions', async () => {
    ...
    const diagnosticsMigration = require('../../../../migrations/20260331110000_add_email_threading_diagnostics_columns.cjs');
    ...
    expect(typeof diagnosticsMigration.up).toBe('function');
    expect(typeof diagnosticsMigration.down).toBe('function');
  });
});

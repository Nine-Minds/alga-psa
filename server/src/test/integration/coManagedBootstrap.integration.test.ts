...
it('removes a published attachment idempotently without changing its message and purges only the selected object', async () => withAttachmentFixture(async ({
  ...
  const retained = await attachments.uploadCoManagedConversationAttachment(db, principal, resource, { ...request, attachmentId: randomUUID(), fileName: 'Keep.txt' }, upload);
  const original = await customer.table('comments').where('comment_id', root.commentId).first();
  await expect(attachments.removeCoManagedConversationAttachment(db, customerPrincipal, resource, attachmentReference(saved))).rejects.toBeDefined();
  for (const wrong of [{ ...attachmentReference(saved), threadId: randomUUID() }, { ...attachmentReference(saved), storeTenant: principal.tenant },
    { ...attachmentReference(saved), actorTenant: principal.tenant }])
    await expect(attachments.removeCoManagedConversationAttachment(db, principal, resource, wrong)).rejects.toBeDefined();
  const [receipt, duplicate] = await Promise.all([1, 2].map(() => attachments.removeCoManagedConversationAttachment(db, principal, resource, attachmentReference(saved))));
  expect(duplicate).toEqual(receipt); expect(receipt).toMatchObject({ ...attachmentReference(saved), removedAt: expect.any(String) });
  expect(await customer.table('comments').where('comment_id', root.commentId).first()).toEqual(original);
  expect(await attachments.listCoManagedConversationAttachments(db, customerPrincipal, resource, attachmentComment(root))).toEqual([retained]);
  await expect(attachments.downloadCoManagedConversationAttachment(db, principal, resource, attachmentReference(saved), download)).rejects.toBeDefined();
  expect(download).not.toHaveBeenCalled(); expect(objects.size).toBe(2);
  const remove = vi.fn(async path => { objects.delete(path); });
  expect(await cleanupCoManagedUploads(db, resource.tenant, remove)).toMatchObject({ purgedFiles: 1, failedFiles: 0, completedDrafts: 0 });
  expect(remove).toHaveBeenCalledExactlyOnceWith(`co-management/${resource.tenant}/${saved.attachmentId}`); expect(objects.size).toBe(1);
...
it('revokes requester file access on removal while retaining published draft receipts and preventing upload resurrection', async () => withPortalAttachmentFixture(async ({
  ...
  const draft = await drafts.beginCoManagedConversationDraft(db, principal, resource, request);
  const ready = await drafts.uploadCoManagedDraftAttachment(db, principal, resource, draftRef(draft), part.descriptor.attachmentId, part.content, upload);
  const published = await drafts.publishCoManagedConversationDraft(db, principal, resource, draftRef(draft), publishCustomer);
  const original = await customer.table('co_management_conversation_drafts').where('operation_id', draft.operationId).first();
  const target = portalAttachmentTarget(resource, published);
  ...
  expect(await drafts.beginCoManagedConversationDraft(db, principal, resource, request)).toMatchObject({ status: 'published', uploadedAttachmentIds: [] });
  expect(await drafts.publishCoManagedConversationDraft(db, principal, resource, draftRef(draft), publishCustomer)).toEqual(published);
  await expect(drafts.uploadCoManagedDraftAttachment(db, principal, resource, draftRef(draft), ready.attachmentId, part.content, upload)).rejects.toBeDefined();
  expect(await attachments.removeCoManagedConversationAttachment(db, principal, resource, attachmentReference(ready))).toEqual(removed);
  expect(upload).toHaveBeenCalledOnce(); expect(objects.size).toBe(0);
}));
...
it('removes private attachments only as their author in their owning organization', async () => withAttachmentFixture(async ({
  ...
  for (const [index, root] of roots.entries()) {
    const actor = index === 0 ? customerPrincipal : principal, other = index === 0 ? principal : customerPrincipal;
    const ready = await attachments.uploadCoManagedConversationAttachment(db, actor, resource, { attachmentId: randomUUID(), comment: attachmentComment(root),
      fileName: 'Private.txt', mimeType: 'text/plain', content: Buffer.from('Private file') }, upload);
    await expect(attachments.removeCoManagedConversationAttachment(db, other, resource, attachmentReference(ready))).rejects.toBeDefined();
    await attachments.removeCoManagedConversationAttachment(db, actor, resource, attachmentReference(ready));
  }
  const remove = vi.fn(async path => { objects.delete(path); });
  expect(await cleanupCoManagedUploads(db, resource.tenant, remove)).toMatchObject({ purgedFiles: 1 }); expect(objects.size).toBe(1);
  expect(await cleanupCoManagedUploads(db, principal.tenant, remove)).toMatchObject({ purgedFiles: 1 }); expect(objects.size).toBe(0);
}));

it('rolls attachment removal back with its caller and rejects staged-file removal through a colliding comment', async () => withConversationDraftFixture(async ({
  principal, resource, customer, create, attachments, drafts, file, upload, objects,
}) => {
  const part = file('Staged collision'), request = { operationId: randomUUID(), audience: 'shared_it' as const, content: { text: 'Draft' }, files: [part.descriptor] };
  const draft = await drafts.beginCoManagedConversationDraft(db, principal, resource, request);
  const staged = await drafts.uploadCoManagedDraftAttachment(db, principal, resource, draftRef(draft), part.descriptor.attachmentId, part.content, upload);
  await create(principal, { operationId: draft.operationId, audience: 'shared_it', text: 'Collision' });
  await expect(attachments.removeCoManagedConversationAttachment(db, principal, resource, attachmentReference(staged))).rejects.toBeDefined();
  expect(await customer.table('co_management_conversation_attachments').where('attachment_id', staged.attachmentId).first()).toMatchObject({ discarded_at: null, removal_actor_tenant: null });
  const root = await create(principal, { operationId: randomUUID(), audience: 'shared_it', text: 'Rollback parent' });
  const ready = await attachments.uploadCoManagedConversationAttachment(db, principal, resource, { attachmentId: randomUUID(), comment: attachmentComment(root),
    fileName: 'Keep.txt', mimeType: 'text/plain', content: Buffer.from('Keep') }, upload);
  await expect(db.transaction(async trx => {
...
it('requires current write authority for attachment removal and retains file reads during license lapse', async () => withAttachmentFixture(async ({
  principal, resource, sponsor, customer, create, attachments, upload, download,
}) => {
  const root = await create(principal, { operationId: randomUUID(), audience: 'shared_it', text: 'Retained file' });
  const ready = await attachments.uploadCoManagedConversationAttachment(db, principal, resource, { attachmentId: randomUUID(), comment: attachmentComment(root),
    fileName: 'Keep.txt', mimeType: 'text/plain', content: Buffer.from('Keep') }, upload);
  await expireCoManagedEntitlement(principal.tenant);
  await expect(attachments.removeCoManagedConversationAttachment(db, principal, resource, attachmentReference(ready))).rejects.toBeDefined();
  expect(await attachments.canManageCoManagedConversationAttachments(db, principal, resource, attachmentComment(root))).toBe(false);
  expect((await attachments.downloadCoManagedConversationAttachment(db, principal, resource, attachmentReference(ready), download)).attachment).toEqual(ready);
  await sponsor.table('co_management_staff_assignments').where('customer_tenant', resource.tenant).delete();
  await expect(attachments.removeCoManagedConversationAttachment(db, principal, resource, attachmentReference(ready))).rejects.toBeDefined();
  expect(await customer.table('co_management_conversation_attachments').where('attachment_id', ready.attachmentId).first()).toMatchObject({ discarded_at: null });
}));

it('rechecks session expiry after an attachment removal lock wait before marking the file discarded', async () => withAttachmentFixture(async ({
  principal, resource, customer, sponsor, create, attachments, upload,
}) => {
  const root = await create(principal, { operationId: randomUUID(), audience: 'shared_it', text: 'Wait for file lock' });
  const ready = await attachments.uploadCoManagedConversationAttachment(db, principal, resource, { attachmentId: randomUUID(), comment: attachmentComment(root),
    fileName: 'Keep.txt', mimeType: 'text/plain', content: Buffer.from('Keep') }, upload);
  await sponsor.table('sessions').where('session_id', principal.sessionId).update({ expires_at: db.raw("clock_timestamp() + interval '1 second'") });
  const blocker = await db.transaction();
  await tenantDb(blocker, resource.tenant).table('co_management_conversation_attachments').where('attachment_id', ready.attachmentId).forUpdate().first();
  let signal!: () => void; const waiting = new Promise<void>(resolve => { signal = resolve; });
  const listener = (query: any) => { if (query.sql.includes('co_management_conversation_attachments') && query.sql.includes('for update')) signal(); };
  db.on('query', listener);
  const attempt = attachments.removeCoManagedConversationAttachment(db, principal, resource, attachmentReference(ready)).catch(error => error);
  try {
    await Promise.race([waiting, attempt.then(() => { throw new Error('Removal completed without waiting'); })]);
    await blocker.raw('SELECT pg_sleep(1.1)'); await blocker.commit();
    expect(await attempt).toMatchObject({ code: 'CO_MANAGED_SHARED_WORK_FORBIDDEN' });
...
it('declares co-managed consumer obligations before publication and rolls transactional effects back with completion failure', async () => withCommentCreationFixture(async ({ principal, resource, customer }) => {
  ...
  expect((await customer.table('co_management_event_consumers')).map(item => item.consumer).sort()).toEqual(['co-managed-email', 'customer-internal-email', 'internal-notifications', 'requester-email', 'search-index']);
  ...
  await expect(consume(db, event, 'search-index', async (trx, current) => {
    ...
    await tenantDb(trx, resource.tenant).table('tickets').where('ticket_id', resource.id).update({ title: 'Rolled back effect' });
    throw new Error('effect failed');
  })).rejects.toThrow('effect failed');
  expect((await customer.table('tickets').where('ticket_id', resource.id).first()).title).toBe(originalTitle);
  expect((await customer.table('co_management_event_consumers')).every(item => item.status === 'pending')).toBe(true);
  ...
  expect(await recover(db, resource.tenant, replay)).toEqual({ queued: 4, cancelled: 0, failed: 0 });
...
it('recovers co-managed consumers that never saw Redis and cancels replay after source restriction without rerunning completed siblings', async () => withCommentCreationFixture(async ({ principal, resource, customer }) => {
  ...
  const row = await customer.table('co_management_event_outbox').where({ comment_id: receipt.commentId, event_type: 'TICKET_COMMENT_ADDED' }).first();
  const event = { id: row.event_id, eventType: row.event_type, payload: { tenantId: resource.tenant } };
  ...
  expect(await recover(db, resource.tenant, replay)).toEqual({ queued: 4, cancelled: 0, failed: 1 });
  ...
  const beforeRetry = replay.mock.calls.length;
  await due(); await recover(db, resource.tenant, replay);
  expect(replay.mock.calls.slice(beforeRetry).map(call => call.slice(1))).toContainEqual([row.event_id, failed.consumer]);
  expect(replay.mock.calls.slice(beforeRetry).some(call => call[2] === completed)).toBe(false);
  await customer.table('comment_threads').where('thread_id', receipt.threadId).update({ collaboration_audience: 'organization_private' });
  await due();
  expect(await recover(db, resource.tenant, replay)).toEqual({ queued: 0, cancelled: 4, failed: 0 });
...
it('commits actual co-managed search indexing with its consumer receipt and applies metadata-only deletion once', async () => withCommentMutationFixture(async ({ principal, resource, customer, read, mutateComment }) => {
  ...
  try {
    const receipt = await createSharedTicketComment(db, principal, resource, { operationId: randomUUID(), audience: 'shared_it', text: 'Search durable source' });
    const event = async (eventType: string) => { const row = await customer.table('co_management_event_outbox').where({ comment_id: receipt.commentId, event_type: eventType }).first();
      return { id: row.event_id, timestamp: new Date().toISOString(), eventType, payload: row.publication.payload } as any; };
    const added = await event('TICKET_COMMENT_ADDED');
    await Promise.all([handle(added), handle(added)]);
    const indexed = await customer.table('app_search_index').where({ object_type: 'ticket_comment', object_id: receipt.commentId }).first();
    expect(indexed.body).toContain('Search durable source');
    const item = (await read(db, principal, resource)).items.find(item => item.commentId === receipt.commentId)!;
    await mutateComment(db, principal, resource, { kind: 'delete', operationId: randomUUID(), expectedUpdatedAt: item.updatedAt,
      comment: { storeTenant: receipt.storeTenant, threadId: receipt.threadId, commentId: receipt.commentId } });
    const deleted = await event('TICKET_COMMENT_DELETED');
    await handle(deleted); await handle(deleted);
    expect(await customer.table('app_search_index').where({ object_type: 'ticket_comment', object_id: receipt.commentId }).first()).toBeUndefined();
    expect((await customer.table('co_management_event_consumers').where('consumer', 'search-index')).every(row => row.status === 'completed')).toBe(true);
...
it('rolls actual co-managed in-app consumers back when completion fails and emits channels only after a successful retry', async () => withCoManagedNotificationSubscriberFixture(async ({ customerPrincipal, resource, customer, sponsor, handle, observed, broadcast, hooks }) => {
  ...
  const name = `fail_consumer_${randomUUID().replaceAll('-', '')}`;
  await db.raw(`CREATE FUNCTION ??() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.consumer = 'internal-notifications' AND NEW.status = 'completed' THEN RAISE EXCEPTION 'completion rejected'; END IF; RETURN NEW; END $$`, [name]);
  await db.raw('CREATE TRIGGER ?? BEFORE UPDATE ON co_management_event_consumers FOR EACH ROW EXECUTE FUNCTION ??()', [name, name]);
  try {
    await expect(handle(event)).rejects.toThrow('completion rejected');
    expect(await sponsor.table('co_management_in_app_receipts').where('event_id', row.event_id)).toEqual([]);
    expect(await sponsor.table('internal_notifications')).toEqual([]);
    expect(observed).toEqual([]); expect(broadcast).not.toHaveBeenCalled(); expect(hooks).not.toHaveBeenCalled();
  } finally { await db.raw('DROP TRIGGER ?? ON co_management_event_consumers', [name]); await db.raw('DROP FUNCTION ??()', [name]); }
  await Promise.all([handle(event), handle(event)]);
  expect(await sponsor.table('co_management_in_app_receipts').where('event_id', row.event_id)).toHaveLength(2);
  await vi.waitFor(() => expect(observed).toHaveLength(2)); expect(observed.every(item => item.committed)).toBe(true);
  expect((await customer.table('co_management_event_consumers').where({ event_id: row.event_id, consumer: 'internal-notifications' }).first()).status).toBe('completed');
  expect(broadcast).toHaveBeenCalledTimes(2); await vi.waitFor(() => expect(hooks).toHaveBeenCalledTimes(2));
}));
...
it('routes co-managed consumer recovery through the actual job with isolated targets and skips unpublished work before its batch limit', async () => withCommentCreationFixture(async ({ principal, resource, customer }) => {
  ...
  const publish = vi.mocked(events.publishEvent);
  const pending = await db.transaction(trx => createSharedTicketComment(trx, principal, resource, { operationId: randomUUID(), audience: 'shared_it', text: 'Not yet published' }));
  await createSharedTicketComment(db, principal, resource, { operationId: randomUUID(), audience: 'shared_it', text: 'Published but not consumed' });
  await customer.table('co_management_event_consumers').update({ next_attempt_at: '2020-01-02T00:00:00Z' });
  const blocked = await customer.table('co_management_event_outbox').where({ comment_id: pending.commentId, event_type: 'TICKET_COMMENT_ADDED' }).first();
  await customer.table('co_management_event_consumers').where('event_id', blocked.event_id).update({ next_attempt_at: '2020-01-01T00:00:00Z' });
  const replay = vi.fn().mockResolvedValue(undefined);
  expect(await recoverCoManagedEventConsumers(db, resource.tenant, replay, { limit: 1 })).toEqual({ queued: 1, cancelled: 0, failed: 0 });
  expect(replay.mock.calls[0][1]).not.toBe(blocked.event_id);
  await customer.table('co_management_event_consumers').update({ next_attempt_at: '2020-01-01T00:00:00Z' });
  const connection = vi.spyOn(database, 'getConnection').mockResolvedValue(db);
  publish.mockClear();
  try { expect((await coManagedNotificationRecoveryHandler({ tenantId: resource.tenant })).consumers).toEqual({ queued: 10, cancelled: 0, failed: 0 }); }
  finally { connection.mockRestore(); }
  ...
  for (const options of targets) expect(options).toMatchObject({ strict: true, force: true, eventId: expect.any(String),
    channel: options!.targetSubscriber === 'search-index' ? 'global' : ['co-managed-email', 'customer-internal-email', 'requester-email'].includes(options!.targetSubscriber!) ? 'emailservice::v7' : 'internal-notifications' });
}));
...
it('does not consume foreign or mismatched co-managed event identities and rolls caller transactions back without completing work', async () => withCommentCreationFixture(async ({ principal, resource, customer }) => {
  ...
  const row = await customer.table('co_management_event_outbox').where({ comment_id: receipt.commentId, event_type: 'TICKET_COMMENT_ADDED' }).first();
  const event = { id: row.event_id, eventType: row.event_type, payload: { tenantId: resource.tenant } }, effect = vi.fn(async () => {});
  expect(await consume(db, { ...event, payload: { tenantId: principal.tenant } }, 'search-index', effect)).toBe(false);
  await expect(consume(db, { ...event, eventType: 'TICKET_COMMENT_DELETED' }, 'search-index', effect)).rejects.toThrow('identity mismatch');
...
it('queues customer technician recipients by stable identity and retries only unfinished current recipients', async () => withCustomerEmailQueueFixture(async ({
  customer, customerPrincipal, resource, recipient, request, enqueue, processEmail,
}) => {
  const teamId = randomUUID();
  await customer.table('teams').insert({ tenant: resource.tenant, team_id: teamId, team_name: 'Customer queue team', manager_id: customerPrincipal.userId });
  await customer.table('team_members').insert([recipient.userId, customerPrincipal.userId].map(user_id => ({ tenant: resource.tenant, team_id: teamId, user_id })));
  await customer.table('tickets').where('ticket_id', resource.id).update({ assigned_team_id: teamId });
  await Promise.all([enqueue(db, request), enqueue(db, request)]);
  const queued = await customer.table('co_management_customer_email_deliveries').orderBy('recipient_user_id');
  expect(queued).toHaveLength(2);
  expect(queued.every(row => row.status === 'pending' && row.attempt_count === 0)).toBe(true);
  expect(JSON.stringify(queued)).not.toContain('Customer technician queue source'); expect(JSON.stringify(queued)).not.toContain('@example.test');
  const first = queued[0].recipient_user_id;
  const send = vi.fn(async (delivery: any) => delivery.recipientUserId === first ? { status: 'failed' as const, retryable: true, errorCode: 'test_unavailable' } : { status: 'delivered' as const });
  expect(await processEmail(db, resource.tenant, send)).toEqual({ examined: 2, processed: 2 });
  expect(send).toHaveBeenCalledTimes(2);
  const firstMessageId = send.mock.calls.find(([item]) => item.recipientUserId === first)![0].messageId;
  await customer.table('users').where('user_id', first).update({ email: 'current-customer@example.test' });
  await customer.table('comments').where('comment_id', request.commentId).update({ note: 'Current customer retry content' });
  await customer.table('co_management_customer_email_deliveries').where('status', 'pending').update({ next_attempt_at: '2020-01-01T00:00:00Z' });
  send.mockImplementation(async delivery => {
    expect(delivery).toMatchObject({ recipientUserId: first, email: 'current-customer@example.test', messageId: firstMessageId,
      message: { note: 'Current customer retry content', resource: { tenant: resource.tenant, kind: 'ticket', id: resource.id } } });
    return { status: 'delivered' };
  });
  await Promise.all([processEmail(db, resource.tenant, send), processEmail(db, resource.tenant, send)]);
  expect(send).toHaveBeenCalledTimes(3);
  await enqueue(db, request); await processEmail(db, resource.tenant, send); expect(send).toHaveBeenCalledTimes(3);
  expect((await customer.table('co_management_customer_email_deliveries')).every(row => row.status === 'delivered')).toBe(true);
}));

it.each(['reassigned', 'inactive', 'no_role', 'audience', 'deleted', 'opted_out'] as const)('suppresses queued customer email after current %s changes', async condition => withCustomerEmailQueueFixture(async ({
  customer, resource, recipient, request, enqueue, processEmail,
}) => {
  await enqueue(db, request);
  if (condition === 'reassigned') await customer.table('tickets').where('ticket_id', resource.id).update({ assigned_to: null });
  if (condition === 'inactive') await customer.table('users').where('user_id', recipient.userId).update({ is_inactive: true });
  if (condition === 'no_role') await customer.table('user_roles').where('user_id', recipient.userId).del();
  if (condition === 'audience') await customer.table('comment_threads').where('ticket_id', resource.id).update({ collaboration_audience: 'organization_private' });
  ...
  expect(await customer.table('co_management_customer_email_deliveries').first()).toMatchObject({ status: 'skipped', attempt_count: 1, next_attempt_at: null });
}));

it('retains customer email delivery after departure and upgrade and preserves locks and retry exhaustion', async () => withCustomerEmailQueueFixture(async ({
  customer, resource, recipient, request, enqueue, processEmail,
}) => {
  await enqueue(db, request);
  await customer.table('co_management_relationships').where('relationship_id', resource.relationshipId).update({ state: 'terminated', ended_at: new Date() });
  ...
  await customer.table('co_management_customer_email_deliveries').update({ next_attempt_at: '2020-01-01T00:00:00Z', attempt_count: 9 });
  await processEmail(db, resource.tenant, send); await processEmail(db, resource.tenant, send);
  expect(send).toHaveBeenCalledTimes(2); expect(new Set(ids).size).toBe(1);
  expect(await customer.table('co_management_customer_email_deliveries').first()).toMatchObject({ status: 'failed', attempt_count: 10, next_attempt_at: null });
}));
...
it('recovers actual customer email discovery and scheduled delivery without resending completed native siblings', async () => withCustomerEmailQueueFixture(async ({
  ...
  const transport = await import('@alga-psa/jobs/handlers/coManagedCommentEmailTransport'), database = await import('@alga-psa/db');
  const connection = vi.spyOn(database, 'getConnection').mockResolvedValue(db);
  const send = vi.spyOn(transport, 'sendCoManagedCustomerCommentEmail').mockResolvedValue({ status: 'failed', retryable: true, errorCode: 'test_provider_unavailable' });
  try {
    const receipt = await createSharedTicketComment(db, customerPrincipal, resource, { operationId: randomUUID(), audience: 'organization_private', text: 'Customer-private production source' });
    const row = await customer.table('co_management_event_outbox').where({ comment_id: receipt.commentId, event_type: 'TICKET_COMMENT_ADDED' }).first();
    const event = { id: row.event_id, timestamp: new Date().toISOString(), eventType: row.event_type, payload: row.publication.payload };
    expect(await Promise.all([handle(event), handle(event)])).toEqual([true, true]);
    expect(send).toHaveBeenCalledTimes(1);
    expect((await customer.table('co_management_event_consumers').where({ event_id: row.event_id, consumer: 'customer-internal-email' }).first()).status).toBe('completed');
    await customer.table('co_management_customer_email_deliveries').update({ next_attempt_at: '2020-01-01T00:00:00Z' });
    send.mockResolvedValue({ status: 'delivered' });
    expect((await recover({ tenantId: resource.tenant })).customerEmails).toEqual({ examined: 1, processed: 1 });
    expect(send).toHaveBeenCalledTimes(2); expect(send.mock.calls[1][0]).toMatchObject({ recipientUserId: recipient.userId, message: { audience: 'organization_private' } });
    await handle(event); expect(send).toHaveBeenCalledTimes(2);
    expect(await handle({ ...event, id: randomUUID() })).toBe(false);
  } finally { send.mockRestore(); connection.mockRestore(); }
}));
...
it('discovers additional customer ticket resources and rechecks their current membership without granting read permission', async () => withCustomerEmailQueueFixture(async ({
  customer, customerPrincipal, resource, recipient, request, enqueue, processEmail,
}) => {
  await customer.table('ticket_resources').insert({ tenant: resource.tenant, ticket_id: resource.id, assigned_to: recipient.userId, additional_user_id: customerPrincipal.userId });
  await enqueue(db, request);
  expect(await customer.table('co_management_customer_email_deliveries')).toHaveLength(2);
  await customer.table('ticket_resources').where('ticket_id', resource.id).del();
  const send = vi.fn().mockResolvedValue({ status: 'delivered' });
  await processEmail(db, resource.tenant, send);
  expect(send).toHaveBeenCalledTimes(1); expect(send.mock.calls[0][0].recipientUserId).toBe(recipient.userId);
  expect(await customer.table('co_management_customer_email_deliveries').where('recipient_user_id', customerPrincipal.userId).first()).toMatchObject({ status: 'skipped' });
}));

it('suppresses close-paired customer comments both before discovery and before a queued send', async () => withCustomerEmailQueueFixture(async ({
  customer, resource, request, enqueue, processEmail,
}) => {
  await customer.table('comments').where('comment_id', request.commentId).update({ metadata: { closes_ticket: true } });
  await enqueue(db, request); expect(await customer.table('co_management_customer_email_deliveries')).toHaveLength(0);
  await customer.table('comments').where('comment_id', request.commentId).update({ metadata: {} });
  await enqueue(db, request); expect(await customer.table('co_management_customer_email_deliveries')).toHaveLength(1);
  await customer.table('comments').where('comment_id', request.commentId).update({ metadata: { closes_ticket: true } });
  const send = vi.fn(); await processEmail(db, resource.tenant, send); expect(send).not.toHaveBeenCalled();
  expect(await customer.table('co_management_customer_email_deliveries').first()).toMatchObject({ status: 'skipped' });
}));

it('rolls customer email discovery and completion back with its caller and sends only after owning commit', async () => withCustomerEmailQueueFixture(async ({
  customer, customerPrincipal, resource,
}) => {
  const { withTransaction } = await import('@alga-psa/db');
  const { createSharedTicketComment } = await import('../../lib/co-managed/createTicketComment');
  ...
  const event = { id: source.event_id, eventType: source.event_type, timestamp: new Date().toISOString(), payload: source.publication.payload };
  const send = vi.spyOn(transport, 'sendCoManagedCustomerCommentEmail').mockImplementation(async () => {
    expect((await customer.table('co_management_event_consumers').where({ event_id: source.event_id, consumer: 'customer-internal-email' }).first()).status).toBe('completed');
    return { status: 'delivered' };
  });
  try {
    await expect(withTransaction(db, async trx => { await handle(event, trx); expect(send).not.toHaveBeenCalled(); throw new Error('caller aborted'); })).rejects.toThrow('caller aborted');
    expect(await customer.table('co_management_customer_email_deliveries')).toHaveLength(0);
    expect((await customer.table('co_management_event_consumers').where({ event_id: source.event_id, consumer: 'customer-internal-email' }).first()).status).toBe('pending');
    expect(send).not.toHaveBeenCalled();
    await withTransaction(db, async trx => { await handle(event, trx); expect(send).not.toHaveBeenCalled(); });
    expect(send).toHaveBeenCalledTimes(1);
...
it('discovers production requester mail once, recovers it through the scheduled job and accepts a reply using its committed token', async () => withRequesterInboundFixture(async ({
  ...
  const transport = await import('@alga-psa/jobs/handlers/coManagedCommentEmailTransport'), database = await import('@alga-psa/db');
  const connection = vi.spyOn(database, 'getConnection').mockResolvedValue(db);
  const send = vi.spyOn(transport, 'sendCoManagedRequesterCommentEmail').mockResolvedValue({ status: 'failed', retryable: true, errorCode: 'provider_not_ready' });
  try {
    const source = await createSharedTicketComment(db, customerPrincipal, resource, { operationId: randomUUID(), audience: 'requester', text: 'Please send more details' });
    ...
    const event = { id: row.event_id, timestamp: new Date().toISOString(), eventType: row.event_type, payload: row.publication.payload };
    expect(await Promise.all([handle(event), handle(event)])).toEqual([true, true]);
    expect(send).toHaveBeenCalledTimes(1); const first = send.mock.calls[0][0];
    expect(first).toMatchObject({ recipient: { kind: 'requester_contact', contactId }, message: { commentId: source.commentId } });
    expect(await customer.table('co_management_requester_reply_tokens').where('token', first.replyToken).first()).toBeTruthy();
    expect(await customer.table('co_management_event_consumers').where({ event_id: event.id, consumer: 'requester-email' }).first()).toMatchObject({ status: 'completed' });
    await customer.table('co_management_requester_email_deliveries').update({ next_attempt_at: '2020-01-01T00:00:00Z' });
    send.mockResolvedValue({ status: 'delivered' });
    expect((await recover({ tenantId: resource.tenant })).requesterEmails).toEqual({ examined: 1, processed: 1 });
    expect(send).toHaveBeenCalledTimes(2); expect(send.mock.calls[1][0]).toMatchObject({ messageId: first.messageId, replyToken: first.replyToken });
    await handle(event); expect(send).toHaveBeenCalledTimes(2);
    emailData.body.text = `Requested details for the technician\n\n[ALGA-REPLY-TOKEN ${first.replyToken}]`;
    const reply = await run(); expect(reply).toMatchObject({ disposition: 'ack', outcome: 'replied', ticketId: resource.id });
    expect(await customer.table('comments').where('comment_id', (reply as any).commentId).first()).toMatchObject({ parent_comment_id: source.commentId, contact_id: contactId });
    expect(await handle({ ...event, id: randomUUID() })).toBe(false);
  } finally { send.mockRestore(); connection.mockRestore(); }
}));
...
it('rolls requester discovery and after-commit delivery back with an enclosing transaction, then recovers once', async () => withRequesterEmailQueueFixture(async ({
  ...
  try {
    const source = await createSharedTicketComment(db, customerPrincipal, resource, { operationId: randomUUID(), audience: 'requester', text: 'Transactional requester source' });
    const row = await customer.table('co_management_event_outbox').where({ comment_id: source.commentId, event_type: 'TICKET_COMMENT_ADDED' }).first();
    const event = { id: row.event_id, timestamp: new Date().toISOString(), eventType: row.event_type, payload: row.publication.payload };
    await expect(db.transaction(async trx => { expect(await handle(event, trx)).toBe(true); throw new Error('Abort requester event'); })).rejects.toThrow('Abort requester event');
    expect(send).not.toHaveBeenCalled(); expect(await customer.table('co_management_requester_email_deliveries')).toHaveLength(0);
    expect(await customer.table('co_management_event_consumers').where({ event_id: event.id, consumer: 'requester-email' }).first()).toMatchObject({ status: 'pending' });
    await handle(event, db); expect(send).toHaveBeenCalledTimes(1);
    expect(await customer.table('co_management_requester_email_deliveries').first()).toMatchObject({ status: 'delivered' });
  } finally { send.mockRestore(); }
}));

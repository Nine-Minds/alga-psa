...
test('Tactical Knox auth completes login when no TOTP is required', async ({ page }) => {
  ...
  try {
    tenantData = await createTenantAndLogin(db, page, {
      companyName: `Tactical Knox ${uuidv4().slice(0, 6)}`,
      baseUrl,
    });
    ...
    // Now test connection, which should perform checkcreds + login and persist the Knox token.
    await page.locator('#tacticalrmm-test-connection').click();

    const last4 = tactical.token.slice(-4);
    // The success alert can be scrolled out of view (clipped by container overflow), so assert
    // on the credentials status in the auth section which is always near the controls we used.
    await expect(page.getByText(/Knox token saved:/)).toContainText(last4, { timeout: 30_000 });

    // Validate the backend actually used the Knox token for the API call.

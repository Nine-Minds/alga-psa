...
export function AssetDetailDrawer({ assetId, isOpen, activeTab, onTabChange, onClose }: AssetDetailDrawerProps) {
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-500">
              Asset tag {asset.asset_tag} &bull; {asset.asset_type.replace('_', ' ')}
            </p>
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              id="asset-drawer-open-record"
              variant="secondary"
              size="sm"
              className="gap-2"
              onClick={() => router.push(`/msp/assets/${asset.asset_id}`)}
            >
        ...
        {maintenanceReport ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" {...withDataAutomationId({ id: 'asset-drawer-overview-metrics' })}>
            <MetricCard
              id="metric-maintenance-total"
              icon={<ListChecks className="h-4 w-4 text-blue-500" />}
              label="Active Schedules"
              value={maintenanceReport.active_schedules}
            />
            <MetricCard
  ...
  const maintenanceTab = useMemo(() => {
    if (!asset) {
      return renderEmptyState('Select an asset to review maintenance activity');
    }
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <MetricCard
            id="metric-schedules-total"
            icon={<ListChecks className="h-4 w-4 text-blue-500" />}
            label="Total Schedules"
            value={maintenanceReport.total_schedules}
          />
          <MetricCard
            id="metric-schedules-active"
            icon={<ShieldCheck className="h-4 w-4 text-emerald-500" />}
            label="Active"
            value={maintenanceReport.active_schedules}
          />
          <MetricCard
            id="metric-schedules-compliance"
            icon={<Layers className="h-4 w-4 text-violet-500" />}
            label="Compliance Rate"
            value={`${Math.round(maintenanceReport.compliance_rate)}%`}
          />
        </div>
                ...
                <li key={entry.history_id} className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Logged by {entry.performed_by || 'system'}
                    </p>
                    {entry.notes && (
  ...
  }, [asset, maintenanceHistory, maintenanceReport]);

  const ticketsTab = useMemo(() => {
    if (!asset) {
      return renderEmptyState('Select an asset to review linked tickets');
    }
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">
                    Linked {formatRelative(ticket.linked_at)}
                  </span>
                </div>
                <div className="text-xs text-gray-500">
                  {ticket.client_name && <span>Client: {ticket.client_name}</span>}
                  {ticket.assigned_to_name && (
                    <span className="ml-2">Assignee: {ticket.assigned_to_name}</span>
                  )}
                </div>
              </div>
              <Button
                id={`asset-ticket-open-${ticket.ticket_id}`}
                variant="ghost"
                size="sm"
                className="gap-2 self-start"
                onClick={() => router.push(`/msp/tickets/${ticket.ticket_id}`)}
              >
  ...
  }, [asset, tickets, router]);

  const configurationTab = useMemo(() => {
    if (!asset) {
      return renderEmptyState('Select an asset to inspect configuration details');
    }
  ...
  const documentsTab = useMemo(() => {
    if (!asset) {
      return renderEmptyState('Select an asset to manage documents');
    }
  ...
  const handleTabChange = (nextTab: string) => {
    if (TAB_ORDER.includes(nextTab as AssetDrawerTab)) {
      ...
      <div className="w-[560px] max-w-full space-y-6">
        <header className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-500">
              Stay in context while reviewing lifecycle and configuration
            </p>
          </div>
        </header>
...
function SectionTitle({ icon, title }: SectionTitleProps) {
  return (
    <div className="flex items-center gap-2 text-sm font-semibold text-gray-900">
      <span className="rounded-md bg-gray-100 p-1 text-gray-600">
        {icon}
      </span>
      {title}
    </div>
...
function renderTypeSpecificConfiguration(asset: Asset) {
  switch (asset.asset_type) {
    case 'workstation':
      if (asset.workstation && isWorkstationAsset(asset.workstation)) {
        return (
      ...
      break;
    case 'network_device':
      if (asset.network_device && isNetworkDeviceAsset(asset.network_device)) {
        return (
      ...
      break;
    case 'server':
      if (asset.server && isServerAsset(asset.server)) {
        return (
      ...
      break;
    case 'mobile_device':
      if (asset.mobile_device && isMobileDeviceAsset(asset.mobile_device)) {
        return (
      ...
      break;
    case 'printer':
      if (asset.printer && isPrinterAsset(asset.printer)) {
        return (

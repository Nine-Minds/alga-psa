...
import {
  ...
  formatDateTime,
  getUserTimeZone,
} from "server/src/lib/utils/dateTimeUtils";
...
import {
  ...
  updateComment,
  findCommentById,
} from "server/src/lib/actions/comment-actions/commentActions";
...
import {
  ...
  saveTimeEntry,
} from "server/src/lib/actions/timeEntryActions";
...
} from "server/src/lib/actions/ticketResourceActions";
...
const TicketDetails: React.FC<TicketDetailsProps> = ({
  ...
  const [createdByUser, setCreatedByUser] = useState<IUser | null>(
    initialCreatedByUser,
  );
  ...
  const [locations, setLocations] =
    useState<IClientLocation[]>(initialLocations);
  const [dateTimeFormat, setDateTimeFormat] =
    useState<string>("MMM d, yyyy h:mm a");
  ...
  const [isUpdatingBundleSettings, setIsUpdatingBundleSettings] =
    useState(false);
  const [
    isAddChildMultiClientConfirmOpen,
    setIsAddChildMultiClientConfirmOpen,
  ] = useState(false);
  const [pendingChildToAdd, setPendingChildToAdd] = useState<{
    ticket_id: string;
  ...
  const [currentTimeSheet, setCurrentTimeSheet] = useState<ITimeSheet | null>(
    null,
  );
  const [currentTimePeriod, setCurrentTimePeriod] =
    useState<ITimePeriodView | null>(null);

  const [team, setTeam] = useState<ITeam | null>(null);

  const [isChangeContactDialogOpen, setIsChangeContactDialogOpen] =
    useState(false);
  ...
  const [isTimeEntryPeriodDialogOpen, setIsTimeEntryPeriodDialogOpen] =
    useState(false);

  // ITIL-specific state for editing
  const [itilImpact, setItilImpact] = useState<number | undefined>(
    ticket.itil_impact || undefined,
  );
  const [itilUrgency, setItilUrgency] = useState<number | undefined>(
    ticket.itil_urgency || undefined,
  );
  ...
  const closeCurrentInterval = useCallback(async () => {
    try {
      ...
      if (currentIntervalId) {
        console.debug(
          "Closing known interval before navigation:",
          currentIntervalId,
        );
      ...
      console.debug(
        "No currentIntervalId available, checking for open intervals",
      );
      if (userId && initialTicket.ticket_id) {
        const openInterval = await intervalService.getOpenInterval(
          initialTicket.ticket_id,
          userId,
        );
        if (openInterval) {
  ...
  const handleClientClick = () => {
    if (client) {
      openDrawer(
        <ClientDetails client={client} isInDrawer={true} quickView={true} />,
      );
    }
  };
  ...
  const handleAgentClick = (userId: string) => {
    openDrawer(<AgentScheduleDrawer agentId={userId} />);
  };
  ...
  const handleAddAgent = async (userId: string) => {
    try {
      ...
      const result = await addTicketResource(
        ticket.ticket_id!,
        userId,
        "support",
        currentUser,
  ...
  const handleRemoveAgent = async (assignmentId: string) => {
    try {
      ...
      setAdditionalAgents((prev) =>
        prev.filter((agent) => agent.assignment_id !== assignmentId),
      );
        ...
        if (result === "success") {
          ...
          if (field === "assigned_to") {
            try {
              ...
              console.log(
                "Additional resources refreshed after assignment change",
              );
            } catch (resourceError) {
              console.error(
                "Error refreshing additional resources:",
                resourceError,
              );
            }
          }
  ...
  const handleBatchSaveChanges = async (
    ...
    if ("assigned_to" in normalizedChanges) {
      normalizedChanges.assigned_to =
        normalizedChanges.assigned_to &&
        normalizedChanges.assigned_to !== "unassigned"
          ? normalizedChanges.assigned_to
          : null;
    }
  ...
  const handleAddNewComment = async (
    ...
    try {
      ...
      const markdownContent =
        await convertBlockNoteToMarkdown(newCommentContent);
  ...
  const handleSave = async (updates: Partial<IComment>) => {
    ...
    try {
      ...
      const updatedCommentData = await findCommentById(
      ...
      if (updatedCommentData) {
        setConversations((prevConversations) =>
          prevConversations.map(
            (conv): IComment =>
              conv.comment_id === updatedCommentData.comment_id
                ? updatedCommentData
                : conv,
          ),
        );
    ...
    try {
      ...
      setConversations((prevConversations) =>
        prevConversations.filter(
          (conv) => conv.comment_id !== comment.comment_id,
        ),
      );
    } catch (error) {
  ...
  const handleAddTimeEntry = async () => {
    try {
      ...
      const timeSheet = await fetchOrCreateTimeSheet(
        userId!,
        currentTimePeriod.period_id,
      );

      if (!timeSheet) {
        console.error("Failed to fetch or create time sheet");
        toast.error(
          "Unable to add time entry: Failed to create or fetch time sheet",
        );
        return;
      }
      ...
      toast.error(
        "An error occurred while preparing the time entry. Please try again.",
      );
    }
  };
  ...
  const handleItilFieldChange = async (field: string, value: any) => {
    try {
      ...
      toast.success(
        `ITIL ${field.replace("itil_", "").replace("_", " ")} updated successfully`,
      );
    } catch (error) {
      console.error("Error updating ITIL field:", error);
      toast.error(
        `Failed to update ITIL ${field.replace("itil_", "").replace("_", " ")}`,
      );
    }
  };
    ...
    try {
      ...
      setTicket((prevTicket) => ({
        ...
        location: newLocationId
          ? locations.find((l) => l.location_id === newLocationId)
          : undefined,
      }));
  ...
  const handleRemoveChildFromBundle = useCallback(
    async (childTicketId: string) => {
      try {
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "Failed to remove ticket from bundle",
        );
      }
    },
  ...
  const handleUnbundleMaster = useCallback(async () => {
    ...
    try {
      const user = await getBundlingUser();
      await unbundleMasterTicketAction(
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to unbundle ticket",
      );
    }
  }, [ticket.ticket_id, getBundlingUser, router]);

  const performAddChildToBundle = useCallback(
    async (childTicketId: string) => {
      if (!ticket.ticket_id) return;
      const user = await getBundlingUser();
      await addChildrenToBundleAction(
        { masterTicketId: ticket.ticket_id, childTicketIds: [childTicketId] },
        user,
      );
  ...
  const handleAddChildToBundle = useCallback(async () => {
    ...
    try {
      const user = await getBundlingUser();
      const found = await findTicketByNumberAction(
      ...
      if (
        ticket.client_id &&
        found.client_id &&
        found.client_id !== ticket.client_id
      ) {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to add ticket to bundle",
      );
    }
  }, [
    ticket.ticket_id,
    ticket.client_id,
    addChildTicketNumber,
    getBundlingUser,
    performAddChildToBundle,
  ]);

  const bundleHasMultipleClients = useMemo(() => {
    if (!bundle?.isBundleMaster || !Array.isArray(bundle.children))
      return false;
  ...
  const handlePromoteChildToMaster = useCallback(
    async (childTicketId: string) => {
      if (!ticket.ticket_id) return;
      try {
        const user = await getBundlingUser();
        await promoteBundleMasterAction(
          {
            oldMasterTicketId: ticket.ticket_id,
            newMasterTicketId: childTicketId,
          },
          user,
        );
        ...
        toast.error(
          error instanceof Error ? error.message : "Failed to promote master",
        );
      }
    },
    [ticket.ticket_id, getBundlingUser, router],
  ...
  const handleToggleBundleMode = useCallback(async () => {
    ...
    try {
      ...
      const user = await getBundlingUser();
      const nextMode =
        bundle.mode === "link_only" ? "sync_updates" : "link_only";
      await updateBundleSettingsAction(
        { masterTicketId: ticket.ticket_id, mode: nextMode },
        user,
      );
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to update bundle settings",
      );
    } finally {
  ...
  return (
    <ReflectionContainer
      id={id}
      label={`Ticket Details - ${ticket.ticket_number}`}
    >
      <div className="bg-gray-100">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-5 min-w-0 flex-1">
            ...
            <h1
              className="text-xl font-bold break-words max-w-full min-w-0 flex-1"
              style={{
        ...
        <ConfirmationDialog
          ...
          onConfirm={async () => {
            ...
            try {
              ...
              console.error(
                "Failed to add child to bundle after confirmation:",
                error,
              );
              toast.error(
                error instanceof Error
        ...
        <div className="flex gap-6 min-w-0">
          <div
            className="flex-grow col-span-2 min-w-0"
            id="ticket-main-content"
          >
            <Suspense
              fallback={
                <div
                  id="ticket-info-skeleton"
                  className="animate-pulse bg-gray-200 h-64 rounded-lg mb-6"
                ></div>
              }
            >
              <div className="mb-6">
                {bundle?.isBundleChild && bundle?.masterTicket ? (
                  <div
                    className="mb-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-900"
                    id="ticket-bundle-child-banner"
                  >
                    This ticket is bundled under{" "}
                    <a
                      className="font-medium underline"
                      href={`/msp/tickets/${bundle.masterTicket.ticket_id}`}
                    >
                ...
                {bundle?.isBundleMaster ? (
                  <div
                    className="mb-3 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm text-indigo-900"
                    id="ticket-bundle-master-banner"
                  >
                    This ticket is the master of a bundle (
                    {Array.isArray(bundle.children)
                      ? bundle.children.length
                      : 0}{" "}
                    children). Mode: {bundle.mode || "sync_updates"}.
                ...
                {bundle?.isBundleMaster ? (
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-500 mb-2">
                      Children keep their current status; workflow fields are
                      locked on children. Inbound child replies are surfaced
                      below as view-only.
                    </div>
                    <div className="flex items-center gap-2 mb-3">
                      <Input
                        ...
                        onChange={(e) =>
                          setAddChildTicketNumber(e.target.value)
                        }
                            ...
                            >
                              <div className="min-w-0">
                                <a
                                  ...
                                  href={`/msp/tickets/${child.ticket_id}`}
                                >
                                  {child.ticket_number}
                                </a>
                              ...
                              <div className="flex items-center gap-2">
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handlePromoteChildToMaster(child.ticket_id)
                                  }
                                >
                                  Promote
                                </Button>
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleRemoveChildFromBundle(child.ticket_id)
                                  }
                                >
                                  Remove
                                </Button>
                ...
                <TicketInfo
                  ...
                  tags={tags}
                  allTagTexts={allTags
                    .filter((tag) => tag.tagged_type === "ticket")
            ...
            <Suspense
              fallback={
                <div
                  id="ticket-conversation-skeleton"
                  className="animate-pulse bg-gray-200 h-96 rounded-lg mb-6"
                ></div>
              }
            >
              <div className="mb-6">
                <TicketConversation
                  ...
                  externalComments={
                    bundle?.isBundleMaster ? aggregatedChildClientComments : []
                  }
                />
              </div>
            </Suspense>
          ...
          <div
            className={isInDrawer ? "w-96" : "w-1/4"}
            id="ticket-properties-container"
          >
            <Suspense
              fallback={
                <div
                  id="ticket-properties-skeleton"
                  className="animate-pulse bg-gray-200 h-96 rounded-lg mb-6"
                ></div>
              }
            >

...
export default function CoManagedCommentAttachments({ resource, comment }: { resource: CoManagedSharedResource; comment: CoManagedCommentReference }) {
  ...
  const identity = `${resource.tenant}:${resource.relationshipId}:${resource.id}:${comment.storeTenant}:${comment.threadId}:${comment.commentId}:${actor.tenant}:${actor.userId}`;
...
function Attachments({ resource, comment, actor }: { resource: CoManagedSharedResource; comment: CoManagedCommentReference; actor: { tenant?: string; userId?: string } }) {
  ...
  async function refresh() {
    ...
    try {
      ...
      if (page.actor.tenant !== actor.tenant || page.actor.userId !== actor.userId) throw new Error('Attachment session changed');
      setState(page); setReadError(false);
      if (!page.canUpload) clearFile();
    } catch {
      if (mounted.current && current === generation.current) { setState(null); setReadError(true); clearFile(); }
    } finally {
  ...
  async function upload() {
    if (inFlight.current || rejected || !state?.canUpload || !file) return;
  ...
  return <section aria-labelledby={`${id}-title`} className="space-y-2 border-t border-[rgb(var(--color-border-200))] pt-3">
    ...
    {state && <>
      {state.attachments.length > 0 && <ul className="space-y-1">{state.attachments.map(attachment => <li key={attachment.attachmentId} className="text-sm">
      ...
      {state.canUpload && !choosing && <Button id={`${id}-add`} size="sm" variant="ghost" onClick={() => setChoosing(true)}>{t('coManaged.attachments.add')}</Button>}

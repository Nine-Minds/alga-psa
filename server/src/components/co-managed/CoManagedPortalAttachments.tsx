...
function PortalAttachments({ ticketId, comment }: TicketConversationAttachmentsProps) {
  ...
  const identity = `${session?.session_id}:${session?.user?.tenant}:${session?.user?.id}:${ticketId}:${comment.thread_id}:${comment.comment_id}`;
...
function AttachmentList({ tenant, userId, target }: { tenant: string; userId: string; target: { ticketId: string; threadId: string; commentId: string } }) {
  ...
  return <section aria-labelledby={`${id}-title`} className="space-y-1">
    ...
    {state && <ul>{state.attachments.map(file => {
      const query = new URLSearchParams(destination.current);

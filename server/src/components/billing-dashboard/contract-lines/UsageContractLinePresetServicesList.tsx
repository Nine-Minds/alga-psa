// server/src/components/billing-dashboard/contract-lines/UsageContractLinePresetServicesList.tsx
'use client';
...
const UsageContractLinePresetServicesList: React.FC<UsageContractLinePresetServicesListProps> = ({ presetId, onServiceAdded }) => {
  ...
  const handleUnitChange = async (serviceId: string, newUnit: string) => {
    if (!presetId) return;
  ...
  const servicesAvailableToAdd = availableServices.filter(
    availService =>
      ...
      availService.billing_method === 'usage'
  );
...
export default UsageContractLinePresetServicesList;

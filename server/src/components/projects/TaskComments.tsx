...
const TaskComments: React.FC<TaskCommentsProps> = ({
  ...
  const handleEdit = (comment: IProjectTaskComment) => {
    setEditingCommentId(comment.project_task_comment_id!);
  ...
  const handleMentionInput = useCallback((text: string) => {
    // Look for @ followed by any characters at the end of the text or at cursor position
  ...
  const insertMention = (user: any) => {
    const ref = editingCommentId ? editContentEditableRef : contentEditableRef;
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      const mentionMatch = textContent.match(/@([^@\s]*)$/);

      if (mentionMatch) {
        // Calculate the position to replace
        const matchStart = textContent.lastIndexOf(mentionMatch[0]);
        const textNode = ref.current.childNodes[0] || ref.current;

        if (textNode.nodeType === Node.TEXT_NODE) {
          const newRange = document.createRange();
          newRange.setStart(textNode, matchStart);
          newRange.setEnd(textNode, matchStart + mentionMatch[0].length);
    ...
    setShowMentions(false);
    handleContentChange(!!editingCommentId);
  };
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
          {sortedComments.map((comment) => {
            ...
            const isCurrentlyEditing = editingCommentId === comment.project_task_comment_id;
            const isHovered = hoveredCommentId === comment.project_task_comment_id;

            return (
              <div
                key={comment.project_task_comment_id}
                className="bg-white border rounded-lg p-4 group hover:shadow-sm transition-shadow"
                onMouseEnter={() => setHoveredCommentId(comment.project_task_comment_id!)}

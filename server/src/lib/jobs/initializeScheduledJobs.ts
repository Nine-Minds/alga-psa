...
export async function initializeScheduledJobs(): Promise<void> {
  try {
    ...
    for (const tenantRecord of tenants) {
      ...
      try { await scheduleWorkflowDateTriggerScanJob(tenantId, '0 5 * * *'); } catch (error) { logger.error(`Failed to schedule workflow date trigger scan for tenant ${tenantId}`, error); }

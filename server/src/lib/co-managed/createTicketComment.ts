...
export async function createSharedTicketComment(db: Knex, actor: CoManagedSessionActor, resource: CoManagedSharedResource, request: CoManagedCommentCreateRequest) {
  return createCoManagedTicketComment(db, actor, resource, request, async (context, comment) => {
    ...
    const publish = async (eventType: 'TICKET_COMMENT_ADDED' | 'TICKET_RESPONSE_STATE_CHANGED', payload: Record<string, unknown>) => {
      ...
      await queueCoManagedConversationEvent(trx, { tenant: context.resource.tenant, eventId, ticketId: context.resource.id, commentId: comment.comment_id,
        threadId: comment.thread_id, audience: context.audience, publication: { kind: 'event', eventType, payload: complete } });
    };
    ...
    await publish('TICKET_COMMENT_ADDED', { ticketId: context.resource.id, ...(reference ? {} : { userId: context.actor.userId }), commentId: comment.comment_id,
    ...
    for (const event of buildTicketCommunicationWorkflowEvents({ ticketId: context.resource.id, messageId: comment.comment_id,
      ...
      await queueCoManagedConversationEvent(trx, { tenant: context.resource.tenant, eventId, ticketId: context.resource.id, commentId: comment.comment_id,
        threadId: comment.thread_id, audience: context.audience, publication: { kind: 'workflow', eventType: event.eventType, payload: event.payload, workflowContext,
          idempotencyKey: `co-managed-comment:${context.resource.tenant}:${comment.comment_id}:${event.eventType}` } });
    }
    await context.assertWriteAuthority(trx);
  });
}

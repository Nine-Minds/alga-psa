...
interface AlgaCompanyInfo {
  ...
  // The following fields were part of the old `AlgaClient = any` mapping,
...
interface QuickBooksClientInfo {
  quickbooks_client_id: string; // QBO Id
  Id?: string; // QBO Id can also be 'Id'
  SyncToken?: string; // QBO SyncToken
  client_name: string; // QBO DisplayName
...
interface ConflictResolutionFormData {
  ...
  alga_client_id_resolved: string;
  quickbooks_client_id_linked?: string;
  user_notes?: string;
}
...
interface ContextDataForForm {
  ...
  potentialQuickBooksMatches: QuickBooksClientInfo[]; // Changed from qb_client_name etc. to array
  sync_job_id: string;
  ...
  quickbooks_client_id_options?: { label: string; value: string }[];
...
type TriggerEventPayload = {
  realmId: string;
  client_id: string; // Corrected to match actual payload
...
export async function qboCustomerSyncWorkflow(context: WorkflowContext): Promise<void> {
  ...
  const emitFailureEventIfNeeded = async (errorMessage: string, clientIdForEvent?: string, realmIdForEvent?: string) => {
    ...
    const payload = {
      ...
      client_id: clientIdForEvent || triggerEvent?.payload?.client_id,
  ...
  let algaClientId = triggerEvent.payload.client_id;
  ...
  logger.info('Processing trigger event payload', { eventName: triggerEvent.name, realmId, algaClientId, originatingWorkflowInstanceId, successEventName, failureEventName, tenantId: tenant, executionId });


  if (!realmId || !algaClientId) {
    const errorMsg = 'CRITICAL WORKFLOW FAILURE: Missing realm id or client id.';
    logger.error(errorMsg);
    await emitFailureEventIfNeeded(errorMsg, algaClientId, realmId);
    return;
  }
  ...
  try {
    // 3. Data Fetching
    setState(WorkflowState.RUNNING);
    logger.info('Fetching Alga Client data', { algaClientId, tenantId: tenant });
    
    let algaClient: AlgaCompanyInfo | null = null; // Initialize to null
    let algaClientFetchedSuccessfully = false;
    while (!algaClientFetchedSuccessfully) {
        const fetchedClient: AlgaCompanyInfo | null = await actions.getClient({ id: algaClientId, tenantId: tenant });
        if (!fetchedClient) {
            logger.error('Alga Client not found', { algaClientId, tenantId: tenant });
            setState(WorkflowState.ERROR);
            const taskParamsClientNotFound: CreateTaskAndWaitForResultParams = {
                taskType: 'workflow_error',
                title: 'Workflow Data Error: Alga Client Not Found',
                description: `Could not fetch Alga Client data for ID: ${algaClientId}. Please ensure the client exists and is accessible, or provide corrected information if applicable.`,
                priority: 'high',
                assignTo: userId ? { users: [userId] } : undefined,
                contextData: {
                    message: `Could not fetch Alga Client data for ID: ${algaClientId}. The client may not exist or there was an issue with the data fetching action.`,
                },
            };
            const taskResolution: CreateTaskAndWaitForResultReturn = await actions.createTaskAndWaitForResult(taskParamsClientNotFound);

            if (!taskResolution.success || taskResolution.resolutionData?.action === 'cancel') {
                const errorMsg = `User cancelled or task resolution indicated no fix for Alga Client not found. TaskId: ${taskResolution.taskId}`;
                logger.warn(errorMsg, { error: taskResolution.error });
                await emitFailureEventIfNeeded(errorMsg, algaClientId, realmId);
                return; 
            }
            // If task allows providing a new algaClientId, it should be handled here.
            // For now, assume the task is about fixing the environment for the existing algaClientId.
            // Or, if taskResolution.resolutionData.newClientId exists, update algaClientId for next attempt.
        } else {
            algaClient = fetchedClient; // fetchedClient is AlgaCompanyInfo | null
            algaClientFetchedSuccessfully = true;
        }
    }

    if (!algaClient) {
        const errorMsg = 'CRITICAL WORKFLOW FAILURE: AlgaClient is null after fetch loop (client not found or other issue).';
        logger.error(errorMsg, { executionId, tenant, algaClientId });
        await emitFailureEventIfNeeded(errorMsg, algaClientId, realmId);
        return; 
    }

    data.set('algaClient', algaClient);
    logger.info('Alga Client data fetched successfully', { algaClientId, tenantId: tenant });

    // 4. Data Mapping
    setState(WorkflowState.RUNNING);
    logger.info('Mapping Alga Client data to QBO Customer format', { algaClientId, tenantId: tenant });

    // Fetch client location data for real address information
    let defaultLocation: any = null;
    try {
        const locationResult = await actions.get_client_default_location({ clientId: algaClientId });
        if (locationResult.success && locationResult.found && locationResult.location) {
            defaultLocation = locationResult.location;
            logger.info('Successfully fetched client default location', { algaClientId, locationId: defaultLocation.location_id });
        } else {
            logger.warn('No default location found for client', { algaClientId, message: locationResult.message });
        }
    } catch (locationError: any) {
        logger.error('Error fetching client location', { error: locationError.message, algaClientId });
    ...
    // TODO: Refine mapping based on actual AlgaClient and QboCustomer types
    const qboCustomerData: QboCustomerInput = {
        DisplayName: algaClient!.client_name, // algaClient is verified not null by the check above
        PrimaryEmailAddr: { Address: algaClient!.main_email_address },
        PrimaryPhone: { FreeFormNumber: algaClient!.main_phone_number },
    ...
    // Map Payment Terms
    // TODO: Review algaClient.paymentTerm, as AlgaCompanyInfo from project doc does not list it.
    // Assuming it might still be part of the object returned by actions.getClient or needs to be added to AlgaCompanyInfo.
    if ((algaClient as any)?.paymentTerm) { // Added optional chaining for algaClient itself
        let termMappedSuccessfully = false;
        let currentAlgaPaymentTerm = (algaClient as any).paymentTerm; // Use a variable that might be updated by task
        ...
        while (!termMappedSuccessfully) {
            try {
                ...
                if (qboTermId) {
                    ...
                    logger.warn('QBO Term ID not found for Alga term', { algaTerm: currentAlgaPaymentTerm, algaClientId, tenantId: tenant, realmId });
                    setState(WorkflowState.ERROR);
                    const taskParamsTermMappingMissing: CreateTaskAndWaitForResultParams = {
                        taskType: 'qbo_mapping_error', // Specific task type
                        title: `QBO Term Mapping Missing for Client ${algaClient!.client_name}`,
                        ...
                        contextData: {
                            ...
                            algaClientId: algaClientId,
                            algaClientName: algaClient!.client_name,
                ...
                logger.error('Error looking up QBO Term ID', { error: mappingError.message, algaTerm: currentAlgaPaymentTerm, algaClientId, tenantId: tenant, realmId });
                setState(WorkflowState.ERROR);
                const taskParamsTermLookupError: CreateTaskAndWaitForResultParams = {
                    taskType: 'workflow_error',
                    title: `Error looking up QBO Term for Client ${algaClient!.client_name}`,
    ...
    logger.info('Data mapping complete', { algaClientId, tenantId: tenant });
    ...
    while (!qboCredentialsFetchedSuccessfully) {
        ...
        if (!secretResult || !secretResult.success) {
            ...
            logger.error('Failed to fetch QBO credentials.', { algaClientId, realmId, message: errorMessage, errorDetails: secretResult?.errorDetails, tenantId: tenant, executionId });
            setState(WorkflowState.ERROR);
            const taskParamsSecretFetchError: CreateTaskAndWaitForResultParams = {
                taskType: 'workflow_error', // Specific task type
                title: `Failed to Fetch QBO Credentials for Client ${algaClient?.client_name || algaClientId}`,
    ...
    while(!mappingCheckedSuccessfully) {
        logger.info('Fetching QBO customer mapping from tenant_external_entity_mappings', { algaClientId, realmId, tenantId: tenant });
        const mappingResult = await actions.get_external_entity_mapping({
            algaEntityId: algaClientId,
        ...
        if (mappingResult && mappingResult.success && mappingResult.found && mappingResult.mapping) {
            ...
            logger.info('Found existing QBO mapping', { algaClientId, existingQboCustomerId, qboSyncToken });
            mappingCheckedSuccessfully = true;
        } else if (mappingResult && !mappingResult.success) {
            logger.error('Failed to fetch QBO customer mapping', { algaClientId, error: mappingResult.message, details: mappingResult.errorDetails });
            setState(WorkflowState.ERROR);
            const taskParamsMappingLookupError: CreateTaskAndWaitForResultParams = {
                taskType: 'workflow_error', // Specific task type
                title: `Failed to lookup QBO mapping for Client ${algaClient?.client_name || algaClientId}`,
                description: `The workflow failed to retrieve the QBO customer mapping for Alga Client ID ${algaClientId} and Realm ID ${realmId}. Error: ${mappingResult.message}. Please check the mapping system or provide details.`,
                priority: 'high',
                assignTo: userId ? { users: [userId] } : undefined,
                contextData: {
                    message: `The workflow failed to retrieve the QBO customer mapping for Alga Client ID ${algaClientId} and Realm ID ${realmId}. Error: ${mappingResult.message}`,
                },
            };
            ...
            logger.info('No existing QBO mapping found for client.', { algaClientId });
    ...
    while(!qboOperationSuccessful) {
        try {
            ...
            if (existingQboCustomerId) {
                // --- UPDATE PATH ---
                setState(WorkflowState.RUNNING);
                logger.info('Calling QBO Update Customer API', { algaClientId, qboCustomerId: existingQboCustomerId, tenantId: tenant, realmId });
                ...
                if (!qboSyncToken) {
                    ...
                    if (!qboSyncToken) {
                         logger.error('Missing qboSyncToken for QBO Customer update (after direct fetch attempt if applicable).', { algaClientId, qboCustomerId: existingQboCustomerId, tenantId: tenant, realmId });
                         setState(WorkflowState.ERROR);
                         const taskParamsMissingSyncToken: CreateTaskAndWaitForResultParams = {
                             taskType: 'workflow_error', // Specific task type
                             title: `Missing SyncToken for QBO Customer Update - ${algaClient!.client_name}`,
                             ...
                             assignTo: userId ? { users: [userId] } : undefined,
                             contextData: {
                                 message: `Cannot update QBO Customer ${existingQboCustomerId} (Alga ID: ${algaClientId}) because the QBO sync token is missing (after direct fetch attempt if user linked).`, // Adjusted message
                             },
                         };
                ...
                logger.info('QBO Update Customer API call successful', { algaClientId, qboCustomerId: existingQboCustomerId, tenantId: tenant });
                ...
                while(!duplicateCheckPassed) {
                    ...
                    if (performDuplicateCheck) {
                        ...
                        if (!displayNameForCheck && !emailForCheck) {
                            logger.warn('Cannot perform QBO duplicate check: both DisplayName and Email are missing. Skipping check.', { algaClientId, tenantId: tenant, realmId });
                        ...
                        try {
                            // Cast potentialDuplicatesResult.customers to QuickBooksClientInfo[]
                            const potentialDuplicatesResult = await actions.get_qbo_customer_by_display_or_email({
                                ...
                                realmId: realmId,
                                qboCredentials: qboCredentials
                            }) as { success: boolean; found: boolean; customers?: QuickBooksClientInfo[]; message?: string; errorDetails?: any; };


                            if (!potentialDuplicatesResult.success) {
                                logger.error('Failed to check for QBO duplicate customers (API Error).', { algaClientId, message: potentialDuplicatesResult.message, errorDetails: potentialDuplicatesResult.errorDetails, tenantId: tenant, realmId });
                                setState(WorkflowState.ERROR);
                                const taskParams: CreateTaskAndWaitForResultParams = {
                                taskType: 'workflow_error',
                                title: `Failed Duplicate Check for ${algaClient!.client_name}`,
                            ...
                            if (potentialDuplicatesResult.found && potentialDuplicatesResult.customers && potentialDuplicatesResult.customers.length > 0) {
                                logger.warn('Potential QBO duplicate customer(s) found', { algaClientId, count: potentialDuplicatesResult.customers.length });
                                ...
                                const contextDataForForm: ContextDataForForm = {
                                    alga_client_id: algaClient!.alga_client_id,
                                    client_name: algaClient!.client_name,
                                    ...
                                    main_phone_number: algaClient!.main_phone_number,
                                    main_email_address: algaClient!.main_email_address,
                                    website_url: algaClient!.website_url,
                                    date_created_in_alga: algaClient!.date_created_in_alga,
                                    last_modified_in_alga: algaClient!.last_modified_in_alga,
                                    current_quickbooks_link_status: algaClient!.current_quickbooks_link_status,
                                    alga_primary_address_full_string_display: defaultLocation 
                                        ...
                                        : algaClient!.primary_address_full_string || "Address not available",
                                    ...
                                    qb_client_name: primaryQbMatch?.DisplayName || primaryQbMatch?.client_name,
                                        ...
                                        "oneOf": [
                                          {
                                            ...
                                            "title": "Link Alga client to this existing QuickBooks client"
                                          },
                                          {
                                            "const": "CREATE_NEW_IN_QB",
                                            "title": "Create this Alga client as a new client in QuickBooks"
                                          }
                                        ]
                                      },
                                      "alga_client_id_resolved": { "type": "string", "default": "${contextData.alga_client_id}" },
                                      "quickbooks_client_id_linked": {
                                        "type": "string",
                                        "title": "Select QuickBooks Client to Link",
                                        "oneOf": (contextDataForForm.quickbooks_client_id_options && contextDataForForm.quickbooks_client_id_options.length > 0
                                                  ? contextDataForForm.quickbooks_client_id_options.map(opt => ({'const': opt.value, 'title': opt.label}))
                                                  : [{ 'const': '', 'title': 'No QuickBooks clients available to link'}])
                                      },
                                    },
                                    "required": ["resolution_action", "alga_client_id_resolved"],
                                    "dependencies": { "resolution_action": { "oneOf": [ { "properties": { "resolution_action": { "const": "LINK_TO_EXISTING_QB" } }, "required": ["quickbooks_client_id_linked"] }, {  "properties": { "resolution_action": { "const": "CREATE_NEW_IN_QB" } } } ] } }
                                  };
                                
                                const clientConflictUiSchema = {
                                    "ui:order": ["conflictContextInfo", "algaClientDisplay", "quickbooksClientDisplay", "resolution_action", "quickbooks_client_id_linked", "user_notes", "alga_client_id_resolved"],
                                    "conflictContextInfo": { "ui:widget": "RichTextViewerWidget" },
                                    "algaClientDisplay": { "ui:widget": "RichTextViewerWidget" },
                                    "quickbooksClientDisplay": { "ui:widget": "RichTextViewerWidget", "ui:visible": "${contextData.qb_client_name != null}" },
                                    "alga_client_id_resolved": { "ui:widget": "hidden" },
                                    "resolution_action": { "ui:widget": "radio" },
                                    "quickbooks_client_id_linked": { "ui:placeholder": "Choose a QuickBooks client", "ui:visible": "${formData.resolution_action === 'LINK_TO_EXISTING_QB'}" },
                                  };

                                const inlineTaskParams = {
                                    workflowContext: context, // Pass the whole context if needed by the action
                                    taskDefinitionId: "clientConflictResolutionForm_v5",
                                    title: `Resolve Client Conflict: ${algaClient!.client_name}`,
                                    description: `Potential QuickBooks duplicate(s) found for ${algaClient!.client_name}. Please review and choose an action.`,
                                    assignTo: { users: userId ? [userId] : undefined }, // from workflow context
                                    contextData: contextDataForForm,
                                    form: {
                                        jsonSchema: finalClientConflictJsonSchema, // Use the dynamically constructed schema
                                        uiSchema: clientConflictUiSchema,
                                    },
                                    priority: 'high',
                                ...
                                if (!taskResolution.success || !taskResolution.resolutionData) {
                                    const errorMsg = `Client conflict resolution task was cancelled or failed. TaskId: ${taskResolution.taskId}, Status: ${taskResolution.status}, Error: ${taskResolution.error}`;
                                    logger.warn(errorMsg, { algaClientId, realmId, executionId });
                                    await emitFailureEventIfNeeded(errorMsg, algaClientId, realmId);
                                    return;
                                }
                                ...
                                if (resolutionData.resolution_action === 'LINK_TO_EXISTING_QB' && resolutionData.quickbooks_client_id_linked) {
                                    existingQboCustomerId = resolutionData.quickbooks_client_id_linked;
                                    ...
                                    logger.info(`User chose to link Alga client ${algaClientId} to existing QBO customer ${existingQboCustomerId}. Proceeding to update.`, { algaClientId, existingQboCustomerId, executionId });
                                    duplicateCheckPassed = true; // Exit duplicate check loop
                                } else if (resolutionData.resolution_action === 'CREATE_NEW_IN_QB') {
                                    logger.info(`User chose to create a new QBO customer for Alga client ${algaClientId}. Proceeding to create.`, { algaClientId, executionId });
                                    duplicateCheckPassed = true; // Proceed to create
                                } else {
                                    // Should not happen if form validation is correct
                                    const errorMsg = `Invalid resolution action from client conflict form: ${resolutionData.resolution_action}. TaskId: ${taskResolution.taskId}`;
                                    logger.error(errorMsg, { algaClientId, realmId, executionId });
                                    await emitFailureEventIfNeeded(errorMsg, algaClientId, realmId);
                                    return;
                                }
                                ...
                                logger.info('No potential QBO duplicates found.', { algaClientId });
                                duplicateCheckPassed = true;
                            }
                        } catch (dupCheckError: any) {
                             logger.error('Unexpected error during QBO duplicate check invocation', { error: dupCheckError.message, algaClientId });
                             setState(WorkflowState.ERROR); // Keep ERROR state
                             const taskParamsDupUnhandled: CreateTaskAndWaitForResultParams = {
                                 taskType: 'workflow_error',
                                 title: `Error During QBO Duplicate Check - ${algaClient!.client_name}`,
                ...
                setState(WorkflowState.RUNNING);
                logger.info('Calling QBO Create Customer API', { algaClientId, tenantId: tenant, realmId });
                ...
                logger.info('QBO Create Customer API call successful', { algaClientId, newQboCustomerId: qboResult?.Customer?.Id });
            ...
            if (qboResult) { // Only process if qboResult is defined (i.e., an operation was performed in this iteration)
                ...
                if (!newQboCustomerId || !newQboSyncToken) {
                    logger.error('QBO API result missing Customer ID or SyncToken', { qboResult, algaClientId });
                    setState(WorkflowState.ERROR);
                    const taskParams: CreateTaskAndWaitForResultParams = {
                        taskType: 'workflow_error',
                        title: `Invalid Response from QBO API - ${algaClient!.client_name}`,
                        description: `The QBO API call succeeded but the response did not contain the expected Customer ID and/or SyncToken.`,
                        priority: 'high',
                        assignTo: userId ? { users: [userId] } : undefined,
                        contextData: {
                            message: `QBO API call for customer (Alga ID: ${algaClientId}) succeeded but the response was missing Customer ID or SyncToken.`,
                        },
                    };
                ...
                // Update Alga Client Record
                setState(WorkflowState.RUNNING);
                await actions.update_client_qbo_details({
                    clientId: algaClientId,
                    qboCustomerId: newQboCustomerId,
                    qboSyncToken: newQboSyncToken,
                    realmId: realmId
                });
                logger.info('Alga Client updated successfully', { algaClientId, qboCustomerId: newQboCustomerId });
                ...
                logger.error('qboResult is undefined after create path and not due to form link. This indicates an issue.', { algaClientId });
            ...
            logger.error('QBO API call failed within operation loop', { error: error.message, currentState: getCurrentState(), algaClientId, realmId: currentRealmIdForError });
            setState(WorkflowState.ERROR);
            ...
            const taskParamsQBOAPIFailed: CreateTaskAndWaitForResultParams = {
                taskType: 'workflow_error',
                title: `QBO Customer Sync Failed for ${algaClient?.client_name || `ID: ${algaClientId}`}`,
            ...
            if (!taskResolutionApiFail.success || taskResolutionApiFail.resolutionData?.action === 'cancel') {
                ...
                await emitFailureEventIfNeeded(finalErrorMsg, algaClientId, currentRealmIdForError);
                return; 
            }
    ...
    logger.info('QBO Customer sync successful', { algaClientId, qboCustomerId: finalQboCustomerId, tenantId: tenant, executionId });
    ...
    if (successEventName && originatingWorkflowInstanceId) {
      ...
      logger.error(`Outer catch block entered in qboCustomerSyncWorkflow. Error: ${errorMsg}`, { stack: outerError.stack, currentState: getCurrentState(), algaClientId: triggerEvent?.payload?.client_id, tenantId: tenant, executionId });
      if (getCurrentState() !== WorkflowState.ERROR) { 
          setState(WorkflowState.ERROR); 
      }
      await emitFailureEventIfNeeded(errorMsg, triggerEvent?.payload?.client_id, triggerEvent?.payload?.realmId);
  }

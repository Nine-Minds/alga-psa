...
/** Stable ledger consumer id for the webhook subscriber. */
const INBOUND_OUTBOX_WEBHOOK_CONSUMER = 'webhook';
...
async function handleTicketEvent(event: unknown): Promise<void> {
  ...
  // Durable inbound outbox events: webhook enqueues are external effects, so the
  // delivery is at-least-once with a bounded duplicate window — fenced
  // reservation -> enqueue -> fenced completion. A crash between reservation and
  // completion leaves an expired reclaimable reservation and redelivery retries
  // the enqueue (a duplicate webhook delivery inside the window is inherent to
  // external effects). See inboundEmailConsumerDedupe.ts.
  const tenantId = internalEvent.payload.tenantId;
  const isCandidate = typeof tenantId === 'string' && tenantId
    && INBOUND_OUTBOX_EVENT_TYPES.has(internalEvent.eventType);
  if (isCandidate) {
    let db: Awaited<ReturnType<typeof getConnection>>;
    try {
      db = await getConnection(tenantId);
    } catch (error) {
      logger.warn('[WebhookSubscriber] Tenant connection unavailable; delivering normally', {
        eventType: rawEventType,
        tenantId,
        error: error instanceof Error ? error.message : String(error),
      });
      try {
        await dispatchWebhookDelivery(internalEvent);
      } catch (dispatchError) {
    ...
    const outcome = await withInboundOutboxDelivery({
      event: { id: internalEvent.id, eventType: internalEvent.eventType, payload: internalEvent.payload },
      consumer: INBOUND_OUTBOX_WEBHOOK_CONSUMER,
      db,
      owner: newInboundDeliveryOwner(),
      effect: () => dispatchWebhookDelivery(internalEvent),
    });
    if (outcome.status === 'skipped') {
      logger.info('[WebhookSubscriber] Skipping already-delivered inbound outbox event', {
      ...
      logger.warn('[WebhookSubscriber] Inbound outbox delivery failed; recovery will retry', {
        eventId: internalEvent.id,
        eventType: rawEventType,
        tenantId,
        consumer: INBOUND_OUTBOX_WEBHOOK_CONSUMER,
...
async function dispatchWebhookDelivery(internalEvent: TicketWebhookBusEvent): Promise<void> {
  const publicEvents = publicEventsFor(internalEvent.eventType);
  if (publicEvents.length === 0) {
    logger.info('[WebhookSubscriber] no public events mapped', { eventType: internalEvent.eventType });
    return;
  }

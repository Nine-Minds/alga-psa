...
export class TimeSheetService extends BaseService<any> {
  ...
  async list(options: ListOptions, context: ServiceContext, filters?: TimeSheetFilterData): Promise<ListResult<any>> {
    ...
    if (filters) {
      ...
      if (filters.has_entries !== undefined) {
        const subquery = context.db('time_entries')
          .where('time_entries.time_sheet_id', context.db.raw(`${this.tableName}.id`))
    ...
    query.leftJoin('users', `${this.tableName}.user_id`, 'users.user_id')
      ...
      .select(
        ...
        context.db.raw(`CONCAT(approvers.first_name, ' ', approvers.last_name) as approver_name`),
        'time_periods.start_date as period_start',
        'time_periods.end_date as period_end'
      );

    // Add computed fields
    const timeEntrySubquery = context.db('time_entries')
      .where('time_entries.time_sheet_id', context.db.raw(`${this.tableName}.id`))
      .select([
        context.db.raw('SUM(billable_duration) / 60.0 as total_hours'),
        context.db.raw('SUM(CASE WHEN billable_duration > 0 THEN billable_duration ELSE 0 END) / 60.0 as billable_hours'),
        context.db.raw('COUNT(*) as entry_count')
      ])
      .first();

    query.select([
      context.db.raw(`(${timeEntrySubquery.clone().select('total_hours').toQuery()}) as total_hours`),
      context.db.raw(`(${timeEntrySubquery.clone().select('billable_hours').toQuery()}) as billable_hours`),
      context.db.raw(`(${timeEntrySubquery.clone().select('entry_count').toQuery()}) as entry_count`)
    ]);
  ...
  async getById(id: string, context: ServiceContext): Promise<any | null> {
    const timeSheet = await context.db(this.tableName)
  ...
  async bulkApprove(data: BulkApproveTimeSheetData, context: ServiceContext): Promise<any[]> {
    ...
    for (const timeSheetId of data.time_sheet_ids) {
      try {
        ...
        results.push({ success: false, error: error.message, time_sheet_id: timeSheetId });
      }
    }
  ...
  async addComment(id: string, data: CreateTimeSheetCommentData, context: ServiceContext): Promise<any> {
    ...
    const [comment] = await context.db('time_sheet_comments')
      .insert(commentData)
      .returning('*');
  ...
  async getTimeSheetComments(timeSheetId: string, context: ServiceContext): Promise<any[]> {
    const comments = await context.db('time_sheet_comments')
  ...
  async getTimePeriod(id: string, context: ServiceContext): Promise<any | null> {
    const period = await context.db('time_periods')
  ...
  async deleteTimePeriod(id: string, context: ServiceContext): Promise<void> {
    ...
    const hasTimeSheets = await context.db('time_sheets')
      .where({ period_id: id, tenant: context.tenant })
      .first();
  ...
  async getTimePeriodSettings(context: ServiceContext): Promise<any[]> {
    return context.db('time_period_settings')
      .where({ tenant: context.tenant })
  ...
  async createTimePeriodSettings(data: CreateTimePeriodSettingsData, context: ServiceContext): Promise<any> {
    ...
    if (data.is_active) {
      await context.db('time_period_settings')
    ...
    const [settings] = await context.db('time_period_settings')
      .insert(settingsData)
      .returning('*');
  ...
  async getScheduleEntries(context: ServiceContext, filters?: any): Promise<any[]> {
    let query = context.db('schedule_entries')
  ...
  async createScheduleEntry(data: CreateScheduleEntryData, context: ServiceContext): Promise<any> {
    ...
    const [entry] = await context.db('schedule_entries')
      .insert(entryData)
      .returning('*');
    ...
    if (data.assigned_user_ids && data.assigned_user_ids.length > 0) {
      ...
      await context.db('schedule_entry_assignees').insert(assigneeData);
    }
  ...
  async updateScheduleEntry(id: string, data: UpdateScheduleEntryData, context: ServiceContext): Promise<any> {
    ...
    if (data.assigned_user_ids !== undefined) {
      await context.db('schedule_entry_assignees')
        .where({ entry_id: id, tenant: context.tenant })
        .del();
  ...
  async search(searchData: TimeSheetSearchData, context: ServiceContext): Promise<any[]> {
    ...
    if (searchData.period_ids && searchData.period_ids.length > 0) {
      query.whereIn(`${this.tableName}.period_id`, searchData.period_ids);
    }
  ...
  private async hasTimeEntries(timeSheetId: string, context: ServiceContext): Promise<boolean> {
    const entry = await context.db('time_entries')
  ...
  private async getScheduleAssignees(entryId: string, context: ServiceContext): Promise<any[]> {
    return context.db('schedule_entry_assignees')

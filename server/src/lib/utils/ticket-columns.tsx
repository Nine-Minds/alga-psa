...
interface CreateTicketColumnsOptions {
  ...
  onQuickViewClick?: (ticketId: string) => void;
...
export function createTicketColumns(options: CreateTicketColumnsOptions): ColumnDefinition<ITicketListItem>[] {
  ...
  // Actions - show if we have any action handlers (like Contracts pattern)
  if (columnVisibility.actions && showActions && (onEditClick || onQuickViewClick || onDeleteClick)) {
    columns.push({
      key: 'actions',
      col: {
        title: 'Actions',
        ...
        width: '3%',
        sortable: false,
        render: (_value: string, record: ITicketListItem, _index: number) => (
          <div onClick={(e) => e.stopPropagation()}>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button id={`ticket-actions-menu-${record.ticket_id}`} variant="ghost" size="sm" className="h-8 w-8 p-0">
              ...
              <DropdownMenuContent align="end" className="bg-white z-50">
                ...
                {onQuickViewClick && (
                  <DropdownMenuItem
                    id={`quick-view-ticket-menu-item-${record.ticket_id}`}
                ...
                {onEditClick && (
                  <DropdownMenuItem
                    id={`edit-ticket-menu-item-${record.ticket_id}`}
                    data-ticket-id={record.ticket_id}
                    className="px-2 py-1 text-sm cursor-pointer hover:bg-gray-100 flex items-center"
                ...
                {onDeleteClick && (
                  <DropdownMenuItem
                    id={`delete-ticket-menu-item-${record.ticket_id}`}
                    data-ticket-id={record.ticket_id}

...
import { getAdminConnection } from './db/admin'; // Use admin connection for non-tenant access
...
export async function syncStandardTemplates(): Promise<void> {
    ...
    let knex;
    try {
        knex = await getAdminConnection(); // Get a non-tenant connection
            ...
            try {
                ...
                if (needsUpdate) {
                    ...
                    const compileResult = await compileStandardTemplate(
                        ...
                        fileContent,
                        knex // Pass the admin knex instance
                    );
        ...
        // Ensure the admin connection is destroyed if it was created
        if (knex) {
            await knex.destroy().catch(err => console.error('[Startup Task] Error destroying admin knex connection:', err));
            console.log('[Startup Task] Admin Knex connection destroyed.');
        }
    }
}

 ...
 *
 * Server actions for managing BlockNote-formatted notes on contacts.
...
export async function getContactNoteContent(contactId: string): Promise<ContactNoteContent> {
  ...
  try {
    ...
    const blockContent = await getBlockContent(contact.notes_document_id);
...
export async function deleteContactNote(
  ...
  try {
    ...
    if (!contact || !contact.notes_document_id) {

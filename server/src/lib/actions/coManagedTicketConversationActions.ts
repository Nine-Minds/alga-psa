...
export const getCoManagedTicketConversationScreenAction = withAuth(async (user, { tenant }, resource: CoManagedSharedResource, before?: CoManagedConversationCursor, requester?: TicketConversationReference, messageId?: string) => {
  const actor = await coManagedBrowserActor(user, tenant), { knex } = await createTenantKnex(tenant);
  ...
  const named = requester ? await getNamedTicketConversationMessages(knex, actor,
    { tenant: resource.tenant, ticketId: resource.id, relationshipId: resource.relationshipId }, requester, before, messageId) : null;
  ...
  const conversation = named ? { resource, items: named.items, nextBefore: named.nextBefore,
    ...('messageUnavailable' in named ? { messageUnavailable: named.messageUnavailable } : {}) } : await getCoManagedTicketConversation(knex, actor, resource, before);

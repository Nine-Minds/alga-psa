...
async function getLatestInvoicedEndDate(db: any, tenant: string, clientBillingPlanId: string): Promise<Date | null> {
  // First, get the client_id and plan_id associated with the clientBillingPlanId
  const planInfo = await db('client_billing_plans')
    .select('client_id', 'plan_id')
    .where({ client_billing_plan_id: clientBillingPlanId, tenant: tenant })
    .first();

  if (!planInfo) {
    console.warn(`Client billing plan ${clientBillingPlanId} not found for tenant ${tenant}`);
    return null; // No plan assignment found
  }

  const { client_id, plan_id } = planInfo;
...
export async function getClientBillingPlan(clientId: string): Promise<IClientBillingPlan[]> {
  ...
  try {
    ...
    const clientBillingPlan = await withTransaction(db, async (trx: Knex.Transaction) => {
    ...
    return clientBillingPlan.map((billing: IClientBillingPlan): IClientBillingPlan => ({
      ...billing,
...
export async function updateClientBillingPlan(clientBillingPlanId: string, updates: Partial<IClientBillingPlan>): Promise<void> {
  const session = await getSession();
  if (!session?.user?.id) {
    throw new Error('Unauthorized');
  }
...
export async function addClientBillingPlan(newBilling: Omit<IClientBillingPlan, 'client_billing_plan_id' | 'tenant'>): Promise<void> {
  const session = await getSession();
  if (!session?.user?.id) {
    throw new Error('Unauthorized');
  }
  ...
  try {
    ...
    await withTransaction(db, async (trx: Knex.Transaction) => {
      const existingBilling = await trx('client_billing_plans')
        .where(whereClause)
        .whereNull('end_date')
        .first();
      ...
      return await trx('client_billing_plans').insert(insertData);
    });
  } catch (error: any) {
...
export async function removeClientBillingPlan(clientBillingPlanId: string): Promise<void> {
  const session = await getSession();
  if (!session?.user?.id) {
    throw new Error('Unauthorized');
  }
  ...
  try {
    ...
    // --- Validation Start ---
    const latestInvoicedEndDate = await getLatestInvoicedEndDate(db, tenant, clientBillingPlanId);
    const now = new Date();
...
export async function editClientBillingPlan(clientBillingPlanId: string, updates: Partial<IClientBillingPlan>): Promise<void> {
  const session = await getSession();
  if (!session?.user?.id) {
    throw new Error('Unauthorized');
  }

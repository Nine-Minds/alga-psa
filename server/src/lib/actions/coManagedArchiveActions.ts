...
export const listArchiveWorkAction = withAuth(async (user, { tenant }, page: number = 0) => {
...
export const getArchiveHistoryAction = withAuth(async (user, { tenant }, resource: CoManagedSharedResource, page: number = 0) => {
  const actor = await coManagedBrowserActor(user, tenant), { knex } = await createTenantKnex(tenant);
  return getCoManagedArchiveHistory(knex, actor, resource, page);
});
export const listArchiveFilesAction = withAuth(async (user, { tenant }, resource: CoManagedSharedResource, page: number = 0) => {
  const actor = await coManagedBrowserActor(user, tenant), { knex } = await createTenantKnex(tenant);

...
const TaskDependencyModel = {
    addDependency: async (
        ...
        if (!tenant) {
            throw new Error('Tenant context is required for dependency operations');
        }
    ...
    removeDependency: async (knexOrTrx: Knex | Knex.Transaction, dependencyId: string): Promise<void> => {
        const tenant = await getCurrentTenantId();
        if (!tenant) {
            throw new Error('Tenant context is required for dependency operations');
        }

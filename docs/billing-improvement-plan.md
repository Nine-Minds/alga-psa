...
- `client_plan_bundles` - Contract assignments to clients (has start_date, end_date)
- `client_billing_plans` - Links to `client_bundle_id`
...
- Client (required) - ClientPicker/CustomSelect
...
**Validation:**
- Must have client, contract name, start date
...
- DataTable with: Client | Contract Name | Status | Monthly Value | Start Date | End Date | Actions
...
```sql
- schedule_id, tenant, client_bundle_id
...
**Alter:** `client_plan_bundles` add `po_number`, `po_amount`, `po_required`
...
- Mock contract assignments (IClientPlanBundle)
- Mock billing plans (IBillingPlan)
- Mock clients (IClient)

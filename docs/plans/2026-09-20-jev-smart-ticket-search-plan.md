...
- **Client**: `useSmartSearchStream` and `SmartSearchResults` in `ee/server/src/components/smartSearch/` are generic over row, scope, and metadata; the panel takes `rowId`, `hydrateRows`, `relevanceColumnIndex`, and the page's `i18nNamespace` so each entity's `smartSearch.*` strings name it. The CE shell and `useSmartSearchAvailability(entity)` live in `@alga-psa/ui`.

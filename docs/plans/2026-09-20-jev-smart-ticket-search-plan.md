...
- **Ticket facts in the state.** After the first live run Robert asked for status, closed flag, priority, board, assignee, team, and dates to reach Jev. Each candidate now carries them as named text and ISO 8601 date-times, and the question tells Jev to use them only when the query refers to such things. Verified live: "closed tickets on the General Support board" over all 66 tickets put exactly the 10 status-closed General Support tickets in Strong and the 2 closed Urgent Matters tickets in Unlikely. Input tokens roughly double (about 38k for 66 tickets).

- **Gating (Robert, after the first live run).** Access is one shared decision, `evaluateSmartTicketSearchAccess` in `ee/server/src/services/smartTicketSearch/smartSearchAccess.ts`, used by both the page probe and the route: ticket read permission, then the `release-v1-6-feature` flag (PostHog, evaluated server-side with tenant and user), then the AI Assistant add-on (`assertTenantAddOnAccess`), then the key. The route handler moved into the enterprise tree (`ee/server/src/app/api/tickets/smart-search/stream/route.ts`); the CE file is a delegator with a 404 stub, the same shape as the Hudu integration routes.

Live scoring against the real TypeSafe API was verified after the key was copied into the local server env. No key was available, so the streamed-bucket UI was exercised only through unit tests of the runner, the SSE route, and the client reducer. Manual steps 3 to 8 in the Testing section remain to be run once a key is configured.

# Projects smart search and the shared engine (2026-09-20)

Robert asked for the same smart search on the Projects list, gated the same way. Writing it a second time would have copied the runner, the SSE route, the stream hook, the panel, the probe, and the access decision, so those became one entity-neutral layer and tickets became its first entity:

- **Wire contract** moved to `packages/ui/src/lib/smartSearch/types.ts`: `SmartSearchEvent<TRow, TMetadata>`, `SmartSearchRequestBody<TScope> = { scope, query }`, the error codes, and `smartSearchStreamPath(entity)`. Each entity keeps a thin flavor (`packages/tickets/src/lib/smartTicketSearch/types.ts`, `packages/projects/src/lib/smartProjectSearch/types.ts`). `filters` became `scope`: what defines the candidate set, which for projects is not chip filters at all.
- **One route**: `/api/smart-search/[entity]/stream` (enterprise handler in `ee/server/src/app/api/smart-search/[entity]/stream/route.ts`, CE delegator and stub beside it). The middleware allow-list entry is now `/api/smart-search/`. An unknown entity is a 404.
- **Engine** in `ee/server/src/services/smartSearch/`: `runSmartSearch` (unchanged concurrency, failure policy, and events, parameterized by a `SmartSearchEntityDefinition`), `scoreBatch` (the relevance question and criteria come from the entity), `candidate.ts` (`assembleCandidate`: head facts, description, then ordered sections of child rows, one budget for all entities), `access.ts` (`evaluateSmartSearchAccess(user, { permissionResource })`: same four gates, permission resource per entity), `entities/ticket.ts` and `entities/project.ts`, and a registry.
- **Client**: `useSmartSearchStream` and `SmartSearchResults` in `ee/server/src/components/smartSearch/` are generic over row, scope, and metadata; the panel takes `rowId`, `hydrateRows`, `relevanceColumnIndex`, and the page's `i18nNamespace` so each entity's `smartSearch.*` strings name it. The CE shell lives in `@alga-psa/ui`. Availability is decided in each page's server component (`getSmartSearchAvailability(entity)` through the `@enterprise` alias, the CE stub answers false) and passed down as `smartSearchAvailable`, so the affordance is right on first paint rather than appearing after a client-side probe.

Projects-specific decisions:

- **Scope is the chip-filtered id list.** The Projects list loads every project once and filters in the browser, so there is no server-side filter type to re-evaluate. The page sends the ids of its chip-filtered projects (`{ projectIds }`, uuids only); the server re-authorizes them through the same per-row kernel decision `getProjects` applies (`authorizeProjectIds`) and hydrates rows by id with tags (`loadProjectListItemsByIds`, `ProjectModel.getByIds`). The keyword filter is not applied to the scope; the typed text is the query. A chip change (including a tag chip) changes the id list and offers a rerun.
- **What Jev reads per project**: number, name, client, contact, status, closed and inactive flags, manager, start/end/created/updated dates, budgeted hours, the description, then tasks newest-updated first (phase, name, status from the project status mapping, priority, assignee, due date, description), then recent task comments from `app_search_index` (ACL-filtered, climbing task → phase → project). Tasks take budget before comments; the budgets are the same numbers as tickets.
- **Truncation rule tightened**: the newest child is truncated rather than dropped only while the candidate has no child at all. A sliver of a comment behind a full task list was noise.
- **Page wiring**: Enter or the Smart search button runs it; the box keeps the typed text (also in the URL as the keyword param, as before); clearing the box, Escape, or Reset leaves smart mode; the debounced index search is skipped while smart mode is on and resumes on exit. Projects has no row selection, so the panel's selection hooks are optional.

Not verified live: the dev tenant has neither the release flag nor the AI Assistant add-on, and the dev server's app initialization currently aborts on a missing `CREDENTIAL_ENCRYPTION_KEY`, so the page could not be exercised in a browser. Coverage is unit tests: candidate assembly, packing and scoring, the runner against a fake entity, the access decision per entity, both entity definitions (state shape, scope normalization, prompt wording), the route (unknown entity, validation, denial mapping, streaming, abort), the delegator, the stream reducer, and source-contract tests on both pages. Manual steps 3 to 8 of the Testing section apply to the Projects page as well.

## Live smoke test (2026-09-20, dev stack)

Run in the dev browser pane after granting the dev tenant the AI Assistant add-on (`tenant_addons` row) and setting `DISABLE_FEATURE_FLAGS=true` in the local server env (release flags default off without PostHog). Two defects surfaced and were fixed:

- **Project comment join.** `app_search_index.parent_id` is text and `project_tasks.task_id` is a uuid; the first join failed with `operator does not exist: text = uuid`. The loader now joins through a derived table of the scoped projects' tasks that carries the id as text.
- **Index addressing drifts on large candidates.** With nine projects in one request (three of them 21-task template projects, about 12k tokens of state), the question `candidates[i]` was answered against the wrong candidate often enough that an empty project scored 77% for "road building and territory expansion" while Wonderland Expansion (roads and territories in its description and tasks) scored 62%; two identical template projects scored 64% and 39%. Reproduced outside the app with the same state. Two remedies both fixed it: batches of three, or an explicit `ref` field (`c0`, `c1`, …) as the first field of each candidate with the question naming it. The engine now stamps `ref` on every candidate and both entity questions name it, say "judge only that candidate", and say that an empty candidate is not evidence of relevance. After the change: Wonderland 97%, every other project 6% or lower; "api credentials and monitoring setup" put the three template projects (which carry "Find the Golden Key (API Credentials)" and "Wake the Dormouse (Activate Monitoring)" tasks) at 85 to 90% in the probe. Tickets re-verified through the shared engine: 54 tickets, 6 requests, the same six email-delivery tickets in Possible as before.

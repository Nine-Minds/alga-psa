...
Next implementation checkpoint: wire sponsor-authorized public provisioning controls, customer-admin acceptance, and allocation/user-seat enforcement. Provisioning cleanup has guarded acknowledgements but still needs the real cleanup worker. The initial administrator invitation uses the existing team setup flow; its co-managed seat/activation/flag handling needs completion. The reservation primitive has no public endpoint and starts no provisioning worker. Shared-work grants, consolidated queues, collaboration audiences, time/billing, provider SLAs, exit/export/upgrade, and the remaining end-to-end validation have not landed. Verify external price/issuer configuration and complete Citus validation before release.

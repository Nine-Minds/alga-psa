...
Next implementation checkpoint: implement allocation/user-seat enforcement and lifecycle mutation guards, including pending activation and license lapse. Provisioning cleanup has guarded acknowledgements but still needs the real cleanup worker and cancellation UI. The initial administrator invitation uses the existing team setup flow; its co-managed seat/activation/flag handling and explicit resend control need completion. Shared-work grants, consolidated queues, collaboration audiences, time/billing, provider SLAs, exit/export/upgrade, and the remaining end-to-end validation have not landed. Verify external price/issuer configuration and complete Citus validation before release.

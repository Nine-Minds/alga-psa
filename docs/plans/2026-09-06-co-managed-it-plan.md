...
Next implementation checkpoint: add seat purchase/allocation management and sponsor-authorized provisioning, including failure cleanup and customer-admin acceptance. The reservation primitive has no public endpoint and starts no provisioning worker. Shared-work grants, consolidated queues, collaboration audiences, time/billing, provider SLAs, exit/export/upgrade, and the remaining end-to-end validation have not landed. Verify external price/issuer configuration and complete Citus validation before release.

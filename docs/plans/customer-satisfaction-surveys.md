...
- `surveys.ceFallback.title`
- `surveys.ceFallback.description`
- `surveys.ceFallback.cta`
...
1. **Database Setup**
   ...
   - Add RLS policies
   - Create indexes
   - Add default survey template migration
   - Add email template migration

2. **Backend Implementation**
   - Implement `surveyActions.ts` (CRUD for templates/triggers)
   - Implement `surveyResponseActions.ts` (public response submission)
   - Create `surveyService.ts` (email sending logic)
   - Add event types to `events.ts`
   - Create `surveySubscriber.ts` (ticket closed handler)
...
4. **Basic UI**
   - `SurveySettings.tsx` (templates + triggers management)
   - `TemplateList.tsx` + `TemplateForm.tsx`
   - `TriggerList.tsx` + `TriggerForm.tsx`
   - `SurveyResponsePage.tsx` (public survey page)

5. **Email Template**
   - Design HTML email with rating buttons
...
- Basic admin UI to manage templates and triggers
...
### `server/src/empty/components/surveys/SurveySettings.tsx`
...
export default function SurveySettings() {
  ...
  return (
    <Card className="p-8 text-center space-y-4" id="survey-settings-upsell-card">
      <h2 className="text-xl font-semibold">
        {t('surveys.ceFallback.title')}
      </h2>
      <p className="text-muted-foreground">
        {t('surveys.ceFallback.description')}
      </p>
      <Button asChild id="survey-settings-upsell-button">
        <Link href="/upgrade">
          {t('surveys.ceFallback.cta')}
        </Link>
      </Button>
    </Card>
  );
...
### `server/src/empty/lib/actions/surveyActions.ts`

```typescript
'use server';

export async function getSurveyTemplates() {
  throw new Error('Surveys are only available in Enterprise Edition');
}

...
- The feature implementation was already committed on this branch through `08e3be2d5c`. This mitigation adds valid PDF/PNG byte-forwarding fixtures and removes the PDF renderer stub from the route integration test. The test now exercises the real Chromium-backed PDF renderer for BlockNote and stored-text fallback exports, and checks PDF structure, a page tree, and EOF.
...
- The byte-forwarding fixtures are valid: `file(1)` identifies the PNG as a 1×1 image, and `file(1)`/`pdfinfo` identify the PDF as a one-page PDF 1.4 document. Exported PDF responses from the integration route are generated by the actual Puppeteer renderer and checked for a page tree and EOF marker.

...
This collector stages bytes for the coordinated authenticated archive. The public download and installation restore paths are described below. Additional historical inline/media coverage remains part of the full package work.

## Published conversation files

`portableConversationExport.ts` selects ready customer-owned files attached to published canonical ticket comments or posted project-task comments. It uses the existing attachment query and purpose-specific path validation, retains the actual work/comment/root and native read permissions, and preserves each attachment's thread, audience and qualified author. A surviving reply can retain its file when the root is a soft-deleted tombstone. Pending/unpublished uploads, discarded/purged files, and MSP-private stores are excluded. Customer-owned published files remain exportable after departure.

The shared `portableBlobStaging.ts` transport handles both document and conversation bytes. Conversation files must match their previously stored SHA-256 as well as their declared size. After streaming, the collector rechecks its source and current authority; failures dispose the lease. The component contains no storage path or live relationship identity. Its restore policy requires converting the bindings to native ticket or project-task documents and rewriting file links, without restoring co-management download routes or trust. The native destination file conversion is implemented below; rewriting legacy co-management-specific attachment URLs still needs coverage.
...
`portableWorkflowExport.ts` captures five definition/configuration tables plus task business snapshots and task activity. It requires native workflow administration/read authority, checks each task against current record policy, and rejects field redactions. Existing workflow dependency analysis finds schema/action references; secret expressions remain names and never cause a provider lookup. Authored JSON and task form responses can contain literal sensitive values, so the complete archive must be encrypted, not only its vault section. Restore leaves workflows paused and requires explicit connection/secret review before activation. Execution histories, leases and dispatch queues are not restored by this component.

Task snapshots retain titles, original status, dates, customer-entered responses, and source-qualified author identity/name. Activity preserves actions, status transitions, attribution, submitted `formData`, and dismissal state. Opaque execution context, event/execution identifiers, live claim fields, and unknown activity-detail payloads are excluded. The trusted restore adapter remaps task/definition/history identities and writes these records to native `audit_logs` as `portable_workflow_task_restored` and `portable_workflow_task_activity_restored`. Their `record_id` groups a task's retained records; details carry the source tenant/task and original actor identity. They are historical audit records, accessible under destination audit permissions, and do not populate the actionable task inbox or resume a workflow after activation. Earlier authenticated packages with exactly the five configuration tables remain supported.
...
`prepareCoManagedPortableWorkspaceRecords` now supplies the trusted identity roster for all 115 projections. It remaps scalar/composite/alias identities, conditional associations, thread roots, work-item links and known nested approvers. The four global catalogs must map to existing destination definitions and are explicitly excluded from insertion. Users remain inactive, automation remains paused, and terminal meeting history is preserved. Operational time receives unpriced insertion defaults. The adapter performs no database writes or activation. Native file/vault/database insertion now has tested internal adapters. Installation admission and activation are described below, along with supported embedded-reference rewriting and its remaining coverage gaps.
...
Abandoned records remain after successful cleanup and are revisited daily: a provider request from a crashed process can finish after an earlier sweep. These tombstones deliberately survive tenant deletion and contain only restore identity metadata. Operators must restore the previous storage configuration to sweep attempts from another location. The existing application maintenance system also runs portable-restore-upload-cleanup every 15 minutes: PG Boss registers one global schedule, and EE uses its Temporal maintenance workflow. It is a system job and does not enumerate active tenants, so absent, suspended and deleted destinations remain recoverable. The handler skips a rolling schema until the journal table exists and reports failed sweeps to the runner for retry. The recovery kernel lives in the shared co-managed package and has no edition, product-tier or UI-flag gate. EE command paths remain compatibility re-exports. Local staging recovery runs separately in each application process, as described below.


## Local staging recovery

Archive encryption and extraction, native blob staging, remote meeting capture and operator input capture share private temporary-directory leases. Before customer bytes can be written, a 0600 marker is synced inside the 0700 directory. It contains only directory identity, ownership and a fixed two-hour expiry. Public transfer work remains limited to 30 minutes; prepared encrypted downloads also reject an expired lease before database admission and before delivery. An elapsed transfer deadline is checked directly, even if the event loop has not yet delivered its abort timer.

Each application process starts a local sweep at startup and every minute, independently of edition, subscription and the UI feature flag. Each pass scans at most 1,000 directory entries and retains its cursor between passes so unrelated temporary files cannot starve later entries. It removes only expired, correctly marked directories owned by the application UID with matching filesystem identity and private permissions. Symlinks, copied or malformed markers, and unmarked legacy directories do not authorize deletion. Ordinary completion still disposes staging immediately. Local failures are reported without paths or customer contents and retried on subsequent passes.

This recovery requires a running application process with access to the same temporary filesystem. It does not reclaim another replica's private filesystem through Temporal, and it does not delete unmarked staging left by earlier versions. A replica that never returns requires its normal filesystem/container lifecycle cleanup. The two-hour lease is a maximum staging lifetime, not a promise that an unused prepared download remains available after application restart.


## S3 restore transport

Portable restore supplies each authenticated source's exact byte count to the storage upload interface. The native streaming upload service also forwards its declared size. S3 uses this count as ContentLength, allowing the installed SDK's checksum stream to produce valid Node HTTP headers without buffering the entire file. The restore adapter still checks source size/checksum and complete stream consumption, then validates the provider's returned size, path and MIME type before native persistence.

A streaming PUT is attempted once: its input cannot be safely rewound after partial consumption. Replayable buffer PUTs retain retry behavior, and HEAD receipt reads retry independently without repeating a successful PUT. An unsuccessful restore continues to use its durable attempt journal for cleanup/retry. S3 capability admission now combines the configured maximum with the existing 5 GiB single-PUT transport ceiling and exposes the configured MIME allowance; this change does not add multipart uploads or bypass destination upload policy.

Focused loopback fixtures exercise the installed AWS SDK and Node HTTP transport, including actual chunked checksum framing, restored file bytes and deletion of only attempt-owned objects. These checks do not claim live S3 endpoint, IAM, bucket policy or multipart validation.


## Embedded document references

The restore file adapter now repairs explicit references to imported native files and documents after allocating their destination IDs. It handles Tiptap/ProseMirror media fields and link marks, BlockNote media, nested links and table cells, and Markdown/HTML URL destinations in declared document, comment, task-description and KB-template content columns. Serialized editor JSON remains serialized. Markdown and HTML parsers locate exact URL spans; replacement preserves surrounding formatting, quoting, entity escaping, labels, captions, titles and literal code. The transformers never render content or fetch a URL.

Only recognized root-relative native document routes with an authenticated imported mapping are changed. Legacy view/download routes accepting either file or document IDs reject an ambiguous source identity instead of guessing. Literal UUIDs, ordinary prose, external URLs and unrelated workflow/configuration objects remain authored values. Unknown/unmapped links and absolute URLs tied to the original installation remain unchanged and need additional coverage; this does not claim every historical or provider URL is portable. Destination file authority still comes from the restored document and association records, not from an embedded URL.


## Task conversation attachments

Task conversation files use the same immutable upload, checksum, removal and audience admission as ticket files, with exactly one parent column enforced in the database. Downloads distinguish `taskId` from `ticketId`; mixed or repeated parent parameters fail admission. A posted comment can accept files from its original author. A surviving reply retains its files after its root becomes a tombstone. The customer and MSP private stores stay separate, and current project scope, body redaction, tracked session and lifecycle rules apply to each operation. Task drafts and explicit disclosure still need their own publication protocol; the schema rejects attaching task files to ticket draft/disclosure operations.

The MSP retains independent bytes for eligible shared/requester task files and its own private task files. Retained parent identity is immutable, and archive reads use the MSP's retained evidence and local authorization after live project access ends. Customer backups include ready files from their canonical task comments, including customer-private comments, and exclude the MSP-private store. The authenticated conversation component adds `taskAttachments`; older archives without this field remain accepted. Each restored task file becomes a native document associated with the remapped task, with remapped thread/comment metadata and historical qualified authorship. Requester visibility follows the recorded canonical audience.

Focused PostgreSQL checks exercise the real encrypted export, native database restore and file transport after relationship termination, comparing restored bytes and checking every task association. This coverage does not include task composer drafts, explicit disclosure, requester-portal upload UI or all historical co-management attachment URLs.

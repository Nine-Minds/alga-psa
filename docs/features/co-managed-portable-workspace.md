...
The native integration case restores real records, file bytes and usable vault values into a new suspended tenant, including thread roots, generated phone normalization and customer/MSP handoff history. It checks caller rollback, caught mid-restore constraint failure, duplicate destination rejection and suspension recovery isolation. The installation coordinator below now supplies destination admission and committed retry identity for these adapters. Hosted destination activation, crash recovery for interrupted provider uploads, full worker activation review and remaining content coverage are still required before the complete backup/restore requirement is satisfied.
...
Restored workspaces initially remain suspended with inactive users and paused operational dispatch. Self-hosted activation is described below; hosted activation and review of authored inline references remain unfinished work. An interrupted process can still leave provider objects before the database commit; durable upload-attempt reconciliation is not implemented. Providers that do not support cancellation can keep an upload pending until their own timeout; the final database insertion checks the transfer deadline and rejects expired work. This command does not claim to complete those recovery or activation requirements.
...
The activation receipt is immutable. Repeating its operation UUID returns the receipt without resetting the password, rerunning setup or clearing a later suspension. A different operation UUID cannot activate the same tenant again. Failure rolls back license staging, new roles/configuration, password changes, activation receipt and suspension changes together. The installation license and MSP capacity are not changed. Hosted restored workspace activation and the wider worker/content review remain open; this self-hosted command does not complete those parts of the plan.

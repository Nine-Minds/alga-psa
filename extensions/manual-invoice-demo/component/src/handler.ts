...
const BUILD_STAMP = '2026-01-15T00:00:00Z';
...
export async function handler(request: ExecuteRequest, host: HostBindings): Promise<ExecuteResponse> {
  try {
    ...
    await safeLog(host, 'error', `[manual-invoice-demo] unhandled error build=${BUILD_STAMP} reason=${reason}`);
    return jsonResponse(
      {
        error: 'handler_failed',
        message: 'Manual invoice demo handler encountered an unexpected error.',
        detail: reason,
        build: BUILD_STAMP,
      },
      { status: 500 }
...
async function processRequest(request: ExecuteRequest, host: HostBindings): Promise<ExecuteResponse> {
  ...
  await safeLog(
    host,
    'info',
    `[manual-invoice-demo] request start tenant=${tenantId} requestId=${requestId} method=${method} url=${url} build=${BUILD_STAMP}`
  );
...
function validateCreateInput(
  payload: unknown
): { ok: true; input: CreateManualInvoiceInput } | { ok: false; errors: FieldErrors } {
...
async function handleCreateManualInvoice(request: ExecuteRequest, host: HostBindings): Promise<ExecuteResponse> {
  ...
  try {
    ...
    const reason = err instanceof Error ? err.message : String(err);
    await safeLog(host, 'warn', `[manual-invoice-demo] invalid JSON body; reason=${reason}`);
    return jsonResponse(

...
async function start() {
  try {
    ...
    healthServer = http.createServer((req, res) => {
      // SMOKE SHIM: stands in for server /api/email/webhooks/imap in durable
      // mode 'off' (prod default) -- same enqueue call the real route makes.
      if (req.method === 'POST' && req.url === '/smoke-imap-webhook') {
        let body = '';
        ...
        req.on('end', async () => {
          try {
            ...
            const queued = await enqueueUnifiedInboundEmailQueueJob({
              tenantId: payload.tenant ?? payload.tenantId,
              providerId: payload.providerId,
              provider: 'imap',
              pointer: {
                mailbox: p.mailbox || 'INBOX',
                uid: String(p.uid),
                uidValidity: p.uidValidity ? String(p.uidValidity) : undefined,
                messageId: p.messageId || payload.emailData?.id || undefined,
              },
            });
            logger.info('[SMOKE] shim enqueued V1 job', { jobId: queued.job.jobId, uid: p.uid });
            res.statusCode = 200; res.setHeader('content-type', 'application/json');
            res.end(JSON.stringify({ success: true, queued: true, jobId: queued.job.jobId }));
          } catch (e: any) {
            logger.error('[SMOKE] shim enqueue failed', e);
            res.statusCode = 503; res.end(JSON.stringify({ error: String(e?.message || e) }));
          }
        });
        return;
      }

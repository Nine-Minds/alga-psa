 ...
 *
 * This module lives in `@alga-psa/shared` — a package the workflow runtime and
 * the billing package both depend on — so there is exactly one implementation
 * and the worker never has to import into billing at runtime. It is exposed
 * through the package's `./lib/quoteTerms` export; the formatting dependency it
 * resolves is declared in `shared/package.json`.
 */

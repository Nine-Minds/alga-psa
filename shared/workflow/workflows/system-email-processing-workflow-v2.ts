/**
 * System-Managed Email Processing Workflow V2
 * This workflow processes inbound emails and creates tickets with email threading support
 * 
 * Changes in V2:
 * - Uses provider-specific inbound ticket defaults instead of hardcoded values
 * - Improved error handling for missing defaults
 * 
 * MVP Features:
...
export async function systemEmailProcessingWorkflowV2(context: WorkflowContext): Promise<void> {
  const { actions, data, logger, setState, events } = context;
  
  setState('PROCESSING_INBOUND_EMAIL');
  logger.info('🚀 Starting email processing workflow V2');

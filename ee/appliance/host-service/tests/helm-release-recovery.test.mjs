...
test('isBootstrapHookCollision only matches the immutable hook Job signature', () => {
  assert.equal(isBootstrapHookCollision('server-side apply failed for object msp/alga-core-sebastian-bootstrap batch/v1, Kind=Job ... spec.template ... field is immutable'), true);
  assert.equal(isBootstrapHookCollision('Job.batch "alga-core-sebastian-bootstrap" is invalid: field is immutable'), true);
  assert.equal(isBootstrapHookCollision('field is immutable'), false);
  assert.equal(isBootstrapHookCollision('alga-core-sebastian-bootstrap failed: BackoffLimitExceeded'), false);
  assert.equal(isBootstrapHookCollision(''), false);
});
...
test('clearBootstrapHookJob is a no-op when no Job exists', async () => {
  const { calls, runKubectl } = stubKubectl([[ 'get job', notFound('jobs.batch "alga-core-sebastian-bootstrap"') ]]);
  const result = await clearBootstrapHookJob({ runKubectl });
  assert.deepEqual(result, { ok: true, existed: false, deleted: false, waitedForActive: false });
  assert.equal(calls.length, 1);
});

test('clearBootstrapHookJob deletes a finished Job and waits for the object to be gone', async () => {
...
test('clearBootstrapHookJob gives a running Job time to finish, then deletes it; without a budget it deletes at once', async () => {
  let reads = 0;
  const active = stubKubectl([[ 'get job', () => { reads += 1; return json({ status: { active: reads < 3 ? 1 : 0 } })(); } ]]);
  const sleeps = [];
  const waited = await clearBootstrapHookJob({ runKubectl: active.runKubectl, sleep: async (ms) => sleeps.push(ms), waitForActiveMs: 60_000, pollMs: 7 });
  ...
  const now = await clearBootstrapHookJob({ runKubectl: immediate.runKubectl, waitForActiveMs: 0 });
  assert.equal(now.waitedForActive, false);
  assert.equal(now.deleted, true);
});

test('recoverAppRelease resumes every release, clears the hook Job, then forces a reset reconcile of alga-core', async () => {
  const { calls, runKubectl } = stubKubectl([[ 'get job', json({ status: { succeeded: 1 } }) ]]);
  const result = await recoverAppRelease({ runKubectl, at: '2026-09-18T14:22:56.000Z' });
  assert.equal(result.ok, true);
  assert.equal(result.clearedHookJob, true);
...
test('recoverAppRelease reports which step failed', async () => {
  ...
  assert.equal(result.step, 'clear-bootstrap-hook-job');
  assert.match(result.error, /timed out/);
});

// /api/recover must go through recoverAppRelease so the UI button is enough to
// clear a Stalled release (resetAt) and a leftover hook Job without SSH.
test('server /api/recover uses the shared recovery (reset + force + hook Job cleanup)', () => {

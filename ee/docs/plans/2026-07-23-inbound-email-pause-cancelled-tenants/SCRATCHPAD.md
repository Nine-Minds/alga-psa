...
- F028/T048: one-off operator backfill (kept out of the repo). It selects only live deletion rows that still have active, unpaused providers, then invokes the same suspension activity per tenant. An unexpected tenant failure is logged and does not stop later tenants; a nonzero exit means operations should rerun it.
...
- Operations runbook: after deploying the migration and application code, run the operator backfill (private runbook). A zero exit means all eligible providers were paused/processed; a nonzero exit should be investigated and the procedure safely rerun.

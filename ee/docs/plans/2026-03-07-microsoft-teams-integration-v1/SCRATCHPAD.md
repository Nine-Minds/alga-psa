...
- (2026-03-07) Verified Teams permission/compatibility/fallback regression coverage with:
  ...
  - `pnpm --dir server exec tsc -p tsconfig.json --noEmit --pretty false`
  - `pnpm --dir packages/integrations typecheck`
- (2026-03-07) No feature checklist items remain. Next unchecked test is `T497` (Tenant migration creates a default Microsoft profile, preserves existing Microsoft credentials, and leaves legacy email/calendar/MSP SSO consumers working).

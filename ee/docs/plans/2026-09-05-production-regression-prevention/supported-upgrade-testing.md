...
For a Citus service, set `UPGRADE_DB_BACKEND=citus` in addition to the isolated database settings. The runner installs the Citus extension only in its newly created database and sets four shards before opening migration connections. It fails if the baseline or upgraded core tables are not hash-distributed by tenant, and records both metadata snapshots in schema evidence. The default remains `postgres`; unknown backend names are rejected.

...
For a Citus service, set `UPGRADE_DB_BACKEND=citus` in addition to the isolated database settings. The runner installs the Citus extension only in its newly created database and sets four shards before opening migration connections. It requires tenants, users, clients, tickets and contracts to be hash-distributed by tenant, and records both metadata snapshots in schema evidence. The pinned baseline leaves usage_tracking and time_entries local; these tables must exist and their actual distribution state is recorded, but their operations cannot be presented as shard-execution evidence. The default remains `postgres`; unknown backend names are rejected.

The Citus mode is implemented but not yet validated against a live supported-release upgrade. It does not by itself establish the service topology or upgraded application behavior. F022 remains open until the retained-data browser/runtime journeys pass on the upgraded Citus database and the required CI evidence is wired and verified.

Set `UPGRADE_DB_BACKEND=citus` for the browser runner as well as the schema runner. It rejects mismatched backend evidence before consuming the fixture. The independent verifier requires both baseline and upgraded distribution snapshots when its caller requires Citus. Application-role grants use a transaction with Citus sequential mode so mixed local/distributed tables receive privileges on one connection.

Enterprise CI now runs the supported Citus upgrade after the PostgreSQL upgrade, using an isolated Citus service on the application network and reusing the same verified application image ID. The preparation helper reads the synthetic account from the original installation and copies the existing app role credential hash to the fresh Citus service without logging it or changing a shared role. The test service matches the fixture's MD5 authentication configuration. Backend-specific raw evidence uploads as `supported-citus-upgrade-execution`; parent readiness independently checks it as an additional mandatory requirement for application changes. Native GitHub execution of this wiring remains unverified.

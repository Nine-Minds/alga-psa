...
The candidate implementation uses `.github/workflows/production-regression.yml` as the sole PR/push entry point for the seven test workflows. Its always-running **Production regression readiness** job consumes nine independently verified execution verdicts from the same run and candidate revision. Unit, Node, workspace, integration/infrastructure/database, CE/EE browser/API, Temporal and Citus results must be complete. Documentation-only omissions require independent diff validation; browser collection cannot satisfy browser execution. Existing manual and scheduled child entry points remain available. Reusable workflow calls change displayed child check names. The inspected main rulesets currently require only the extension/ESLint and circular-dependency guards, which remain unchanged. F008 must validate the native readiness check and require it alongside those guards before enforcement is claimed. This parent is candidate validation, not yet the immutable release-promotion gate.
...
- Create a synthetic existing-tenant fixture from the user-approved v1.5.0 schema, pinned to commit `f3579f3a317cf51df5f4489e5dcd8f649bb71289`, upgrade through the actual migration path, then execute usage/invoice/ticket checks. Load baseline migrations and their helper files from that commit, and seed synthetic records before applying candidate migrations. Keep this separate from clean-install coverage; preserve the baseline migration history so edited historical migrations are not silently rerun.

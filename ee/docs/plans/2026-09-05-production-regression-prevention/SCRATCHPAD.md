...
### 2026-09-07 — repair and revalidate the complete tooling lane

- A clean full run at 5da86c90e1 executed 487 tests and found one failure: the selection harness copied the production integration runner without its new sharding dependency. After supplying it, the focused runtime test exposed uncommitted temporary manifest edits that correctly violated the runner's clean-source requirement. The fixture now copies the required helper and commits intentional fixture changes before validating execution outcomes; assertions remain intact.
- Focused selection behavior passes in 41.15 seconds. Committed the repair as 4d02008fae, then reran the complete tooling lane. All 47 required files / 487 tests pass in 138.83 seconds with zero failures/skips/cancellations, complete discovery and identical clean before/after source. Evidence: evidence/node-tooling-local-4d020.json, with raw artifact hashes.
- Refreshed PR3343 state read-only: published head remains d8cf156c731bdfb9414ee6435152a676e0bcecf4. Its full integration and server unit jobs are cancelled; server-colocated and workspace aggregate jobs failed. Queued repairs are not yet native-verified. No push attempted; publication approval remains pending.

### 2026-09-07 — define immutable release component identity checks

- Added scripts/lib/release-component-manifest.mjs. A versioned manifest binds edition, exact source revision and named image digests. The consuming deployment supplies the required component inventory independently; missing/duplicate/unknown components, mutable tags, malformed digests, wrong editions and stale revisions fail validation.
- Deployment comparison requires complete immutable observations and rejects substituted images, duplicates, missing components and invalid manifests. The adapter supplying observations must resolve actual runtime image identity; a desired spec is not deployment proof. Matching digest declarations do not attest build provenance or passing tests, and the returned scope explicitly names component identities.
- Four behavioral tests pass (scripts/tests/release-component-manifest.test.mjs), covering valid order-independent inputs and invalid manifest/readback cases. The generic node-tooling inventory includes this test path. No source-string assertions were added.
- F020 remains open: required components must be resolved from the selected deployment target, then build provenance, candidate execution, promotion and runtime readback must be wired and native-verified. This library establishes the identity contract without inventing a supported release or deployment target. Those requested details and push approval remain pending.

### 2026-09-07 — derive component requirements from rendered releases

- Added renderedReleaseComponents: consumes the entire rendered Kubernetes resource array/List, enumerates Deployment/StatefulSet/DaemonSet/ReplicaSet/Job/CronJob/Pod container and init-container images, and assigns namespace/workload/group/container identities. Hook Jobs are included. Unknown kinds fail explicitly rather than silently omitting operator-managed workloads; names, images, duplicate identities and empty release inputs are validated. Secret/ConfigMap data is not copied to output.
- Seven focused tests pass across component extraction and immutable manifest/deployment comparison. An extracted inventory cannot accept a manifest that omits init containers. These tests use structured resource behavior, not source-text assertions.
- Actual local Helm renders succeed for the main chart (nine image-bearing components) and email-service with bootstrap enabled (two). The default email-service tag is rejected by immutable manifest validation as expected. Inventory: evidence/rendered-release-component-inventory.json. No deployment or registry operation was performed.
- F020 remains open: this connects required component identity to rendered configuration but does not choose the production target, resolve digests, prove builds/tests or implement runtime readback/promotion. Lifecycle hooks require phase-aware evidence rather than assuming all hook pods are simultaneously live. Push approval remains pending.

### 2026-09-07 — complete the local unit baseline and repair compound tax behavior

- The full clean a8fd74 run completed all 2,699 files: 14,564 passing assertions, no assertion failures, 15 TODOs and six platform/capability skips. The execution verifier correctly rejected readiness. Vitest list omits skipped/TODO cases, so the raw report also exposes them as unexpected relative to collection. Preserved raw artifacts under /tmp/alga-server-unit-a8fd74-final and hashes/source metadata in evidence/server-unit-local-a8fd74.json. The 15 TODOs comprise 14 tax cases and one no-client contract-line selection case; six secret-store checks need Linux/procfs or a usable subordinate UID namespace. This is local macOS Node 25 evidence, not native CI.
- The complete tooling lane at the same clean revision passed 494 tests across 49 files, zero skips/failures, with complete discovery and identical clean before/after source. Evidence: evidence/node-tooling-local-a8fd74.json.
- Re-enabling the composite-tax TODO exposed a real contradiction with the editor's documented semantics: compound components should tax base plus previous taxes, while independent components should always use the original base. Corrected service and preview calculations, replaced the existing test expectation that encoded the defect, and re-enabled the skipped case using integer cents. Added mixed-component and non-positive-base regressions; zero bases no longer yield NaN and composite/threshold paths follow the simple-rate non-positive-amount policy.
- Before changes, temporary copies of the expanded behavioral tests produced six service failures and one rendered-preview failure. After applying the fixes, five focused files execute 42 passing assertions; 13 other pre-existing tax TODOs remain. Evidence and attribution limits: evidence/tax-compound-regression.json. No source-string test was added, no skip rule weakened, and no broad plan flag changed. Current-candidate native CI and publication approval remain pending.

### 2026-09-07 — replace the missing-client selection TODO with runtime coverage

- Replaced the no-client placeholder with empty/whitespace/null/undefined cases. The existing test targeted a legacy server helper, so added behavioral checks against the separate billing and scheduling eligibility implementations used by current flows. The original implementations failed all 12 new input cases; missing client scope still reached database access.
- All three eligibility helpers now return no candidates before querying when the client ID is absent or blank. The legacy UI wrapper continues to reject missing tenant context before reaching this guard, verified for both valid and empty client IDs. This does not assert a real-database data leak: database I/O is substituted, and the tested boundary is early validation.
- Focused validation executes 52 tests across five files with no skips/failures, covering existing selection/projection/profile behavior as well as the missing-client cases. Evidence: evidence/contract-selection-missing-client.json. This resolves one TODO; 13 tax TODOs and six environment-dependent secret-store skips from the full baseline remain. No broad plan flags changed; native CI/publication remain pending.

### 2026-09-07 — execute seven tax scenarios previously left as TODOs

- Replaced seven bodyless TaxService TODOs with runtime assertions: overlapping holidays and inclusive boundaries, exempt clients, non-positive simple-rate amounts, fractional-cent rounding, reverse-charge precedence, progressive-bracket boundaries and separately supplied taxable/non-taxable portions. Fixtures now reset mock implementations and seed explicit defaults before each case, preventing inherited settings from supplying accidental coverage.
- Three focused files execute 52 passing assertions with six TODOs remaining. No product change was needed for these cases. Evidence: evidence/tax-todo-runtime-coverage.json. Mixed-portion coverage deliberately describes the service calls it exercises; it is not invoice-orchestration proof.
- Remaining tax placeholders need stronger/different boundaries: transaction-date and currency SQL selection, international selection, multi-item invoice orchestration, tax caps and subscriptions crossing tax periods. The current service/type inspection found no tax-cap input, so that placeholder cannot be fulfilled by inventing a mock cap property. These gaps remain visible rather than being deleted or counted as passing. Native CI, full-suite readiness and publication approval remain pending.

### 2026-09-07 — move date/currency tax coverage to PostgreSQL and fix numeric hydration

- Added taxService.rateSelection.db.test.ts to the existing automatically discovered workspace database lane. Real migrated PostgreSQL queries cover active/inactive rates, inclusive starts/exclusive ends, regional and tenant isolation, universal plus matching currency rates, and default-association date/currency eligibility. Synthetic rows live only inside per-test transactions. Context and profile/reverse-charge setup are substituted; SQL results are not.
- The first fixture attempt used a duplicate globally unique client UUID across tenants; corrected that fixture without changing production constraints. The subsequent expanded before run executed 17 cases with six failures: PostgreSQL numeric values escaped the default-rate calculation as strings. Normalized the simple-rate percentage to a number for positive and non-positive paths; fractional percentages retain precision, and monetary rounding remains unchanged.
- All 17 database cases pass after the fix. Nearby unit/component files execute 59 passing assertions with four TODOs. A read-only check confirms zero remaining synthetic fixture tenants. Local execution reused the task-owned migrated database through a temporary configuration that bypasses bootstrap; canonical native CI still uses workspaceDbGlobalSetup and has not run this candidate.
- Removed only the date and currency unit placeholders, replacing them with an explicit reference to the executed DB suite. International policy selection, multi-item invoice orchestration, caps and subscription periods remain open. Evidence: evidence/tax-rate-postgres-regression.json and the unattributed regression ledger record. No broad plan flag changed; publication approval remains pending.

### 2026-09-07 — verify invoice tax persistence and promote it into Tier-1 selection

- Located existing multi-region invoice-persistence coverage in billingInvoiceGeneration_tax.test.ts. Executed all nine cases in fresh task-owned PostgreSQL/Redis containers: all pass. Strengthened the fractional-cent case to assert exactly two persisted service identities and their individual net/tax amounts (1000/89 and 500/40 cents), in addition to the existing exact invoice totals. All nine cases pass again in 19.65 seconds.
- Added this file to the infrastructure runner's mandatory Tier-1 selection (now five files). Updated the actual temporary-Git/Vitest runner test to execute six full-fixture files across three shards and verify five Tier-1 files, explicitly including invoice tax. The runtime runner test passes, including existing missing/stale shard rejection checks.
- Replaced the multi-item tax TODO with a reference to the executed invoice suite. Three tax placeholders remain: international policy selection, caps and subscription periods. The invoice case supplies billing charges and substitutes auth/transaction wrappers; it does not prove upstream recurring charge generation or a browser journey. Evidence: evidence/invoice-tax-tier1-verification.json. Both disposable containers were removed after execution. Native publication/effective gate enforcement remain pending; no broad plan flags changed.

### 2026-09-07 — independently verify infrastructure coverage and candidate source

- Reproduced an aggregate false positive using real passing Tier-1 reports: changing their selection label to full let five internally consistent files stand in for six repository files. The aggregate previously trusted the first shard's required inventory.
- Extracted infrastructure selection policy so the runner uses its reconciled collection and the aggregate independently uses repositoryTestFiles from its candidate checkout. Both require the five-file floor; full mode requires the complete checkout inventory. The aggregate records the independent expected set separately from the reported set and rejects disagreements. GITHUB_SHA is now required and must match a clean aggregate checkout; both shard source snapshots must explicitly be clean with empty change lists.
- The actual temporary-Git/Vitest test passes full three-shard and Tier-1 runs and rejects relabelled incomplete coverage, dirty source flags/nonempty changes, changed checkout, missing/stale candidate identity, invalid mode, failed jobs and existing raw-shard substitutions. Evidence: evidence/infrastructure-independent-aggregate.json. This is local behavioral gate verification; the workflow's selector-dependent skip condition and global required protections remain open. No broad plan flag changed; native publication remains pending.

### 2026-09-07 — always emit the infrastructure verdict

- The infrastructure aggregate now uses always() and consumes selector outcome, event and base revision. It rejects failed/cancelled/missing selection; scheduled full runs permit their deliberately skipped selector. An independent Git diff allows not-applicable only for documentation-only/identical changes with successful selection and a skipped Tier-1 job. Missing change evidence or changes outside the reliable import graph require full coverage.
- Checkout fetches history for that independent comparison. Skipped jobs do not trigger downloads for nonexistent shard artifacts or emit full-suite metrics. A not-applicable verdict explicitly records its reason and contains no invented passing tests.
- The actual temporary-Git/Vitest test passes documentation, runtime, missing-base, selector failure and scheduled cases alongside existing shard/source integrity checks. Workflow actionlint and diff checks pass. Evidence: evidence/infrastructure-prerequisite-verdicts.json. Other global workflow prerequisites and effective required protections remain unresolved; no broad plan flags changed. Publication approval remains pending.

### 2026-09-07 — validate the complete tooling lane after infrastructure gate changes

- Full tooling execution at clean 5c479b245cb643563463927d5d3345241da3118f passed all 494 assertions across 49 required files with zero skips/failures/cancellations, complete discovery and unchanged clean before/after source. Evidence: evidence/node-tooling-local-5c479.json, with raw artifact hashes.
- Refreshed PR3343 read-only. Published head remains d8cf156c731bdfb9414ee6435152a676e0bcecf4: 59 successful, two failed, two cancelled and one skipped check conclusions. Recent local repairs therefore still lack native-candidate execution evidence. No publication retry was attempted.
- Updated the partial global inventory with the completed unit collection and fresh tooling events. It still fails with 90 unmatched files against the mixed-revision artifacts. That is not a current orphan count: newer assigned tests are absent from older lane collections, while legacy browser suites remain the largest unresolved group. Evidence: evidence/inventory-after-infrastructure-gates.json. No broad plan flags changed; publication approval remains pending.

### 2026-09-07 — replace stale passing artifacts after malformed shard metadata

- Added an actual-runner regression that first writes a valid passing aggregate, then supplies malformed shard inventory/file lists. Before the fix, a non-array allFiles value threw during shard reconciliation, leaving the previous aggregate marked passed even though the process exited nonzero.
- Reconciliation exceptions now produce an explicit failed aggregate and incomplete combined execution report. Counts are null when reconciliation cannot substantiate them; no passing state is retained from a previous invocation.
- The full focused infrastructure runner test passes (10.92 seconds), including malformed allFiles/expectedFiles/executedFiles cases and the existing coverage, source, prerequisite and skip scenarios. Evidence: evidence/infrastructure-malformed-artifact-lifecycle.json. Diff checks pass. No broad plan flags changed; native publication remains pending.

### 2026-09-07 — execute Linux secret-store coverage without silently skipping ownership repair

- Confirmed the four procfs-dependent cases execute in Linux. The cached runtime initially had no test configuration, then the first config mount used an unshared /tmp path; corrected the local harness with an explicit config mounted from the ignored workspace output directory. The selected Linux baseline passed nine cases and skipped the two ownership cases because unshare capability was unavailable (31 other cases intentionally filtered out).
- Added an explicit test-only ownership mode: direct root in containers or noninteractive sudo on a non-root CI runner. Ordinary local runs keep the unprivileged namespace path. Probes and repair scripts operate only on generated temporary fixtures, remove probe directories and restore the original runner UID on exit. Explicit mode throws if capability is unavailable, instead of skipping. Enabled it for the Unit Tests workflow.
- The final complete Linux file run passes 42 tests, zero skips/failures, in 19.14 seconds. A separate missing-tools run fails with the intended capability error. Workflow actionlint and diff checks pass. All containers were automatically removed. Evidence: evidence/secret-store-linux-ownership.json.
- Root-container behavior is verified; the hosted non-root sudo branch still requires native CI. The three tax TODOs and broader unit/global readiness remain open. No broad plan flag changed; publication approval remains pending.

### 2026-09-07 — migrate anonymous portal discovery into production browser coverage

- Replaced the legacy 11-case discovery file with four canonical journeys. Preserved meaningful form selection, confirmation, reset, callback and redirect behavior; strengthened malformed-email validation and MSP link navigation. Removed arbitrary SVG/body checks and the unsupported claim that unknown users receive email.
- Headed enterprise Chromium passed all four cases without retries in 10.1 seconds against the existing local production app. No credentials, session injection, route interception or account seeding were used; the sole submission used a unique example.invalid address. The initial root Playwright invocation failed collection due to duplicate installations; rerunning with the harness-pinned installation passed.
- Both editions collect the new suite. This reused a cached image, so it does not establish candidate-build or native CI readiness. Email delivery, existing-account enumeration parity and callback propagation into delivered mail remain unproven by these UI checks. Evidence: evidence/portal-discovery-migration.json. No broad plan flags changed.

### 2026-09-07 — preserve aggregate diagnostics for malformed shard inventories

- Shared shard reconciliation now validates uploaded file-list shapes before iteration. Invalid input returns a failed serializable verdict with unknown counts instead of throwing. Workspace and Temporal consumers can continue evaluating sibling suites.
- New runtime regression failed before the change on null shard input. Tests also exercise malformed required, expected and executed inventories, and verify that the workspace aggregate preserves seven unaffected suite verdicts. All 17 focused checks pass, including the actual temporary-checkout infrastructure runner (10.21 seconds total).
- An initial attempt to invoke the infrastructure regression with Vitest collected no files; this file is a Node test that launches Vitest itself, and was subsequently executed correctly with node --test. Evidence: evidence/shared-shard-malformed-inventory.json. No broad plan flags changed; native candidate CI remains unverified.

### 2026-09-07 — run server-rendered locale regression in production Playwright

- Migrated the unmatched serverRenderedLocale.e2e.test.ts into the canonical production browser suite. Preserved all five legacy assertions across four journeys and added post-render browser checks. Locale packs supply expected copy; assertions exercise returned HTTP HTML, response cookies and live browser state, not source-string presence.
- German/French/English headers, Polish cookie precedence, English fallback prevention and absence of guessed locale persistence all pass in headed enterprise Chromium without retries (four tests, 5.6 seconds). The initial import.meta path lookup conflicted with the harness module transformation; the corrected file-relative lookup collected and ran successfully.
- Both editions collect the suite. The existing cached local image is not candidate-build evidence, and authenticated preference hierarchy remains outside this suite. Evidence: evidence/server-rendered-locale-migration.json. No broad plan flags changed; native CI remains unverified.

### 2026-09-07 — exercise persisted tax settings and default billing profiles
...
### 2026-09-07 — replace two non-asserting login diagnostics
...
### 2026-09-07 — full six-file tenant database lane passes on Citus
...
### 2026-09-07 — project survey subject migration with database constraints
...
### 2026-09-07 — real project survey response persistence

- Response writes now use the locked invitation subject, persisting project_id separately from ticket_id. Tenant-scoped project/ticket metadata feeds negative-feedback payloads; legacy events require exactly one subject and matching numbering.
- Six PostgreSQL behavioral tests exercise real token lookup, writes, duplicate protection, rating validation and foreign-tenant metadata suppression. Against pre-fix response actions, two project submission cases fail the subject constraint; corrected actions pass all six. Twelve legacy event contract cases also pass. Evidence: evidence/project-survey-response-persistence.json.
- Tests clone migrated definitions into rollback-only schemas and apply the project migration when needed. Publisher capture verifies payloads, not delivery. Full migration-chain, invitation/email journey and analytics/UI remain open. Project dispatch stays skipped until the complete journey is repaired; no broad flags or native CI claims changed.

### 2026-09-07 — project invitation journey and restored dispatch coverage

- Invitation service now accepts exactly one subject, loads real projects, persists project_id, and emits project lifecycle identity. Subscriber sends projectId. New project email template/migration and dev seed cover all eight languages, sharing layout while preserving ticket template output.
...
### 2026-09-07 — native production browser success and refreshed CI failure audit

- Native run 34151013512 completed both production browser editions and fresh-install aggregate successfully on merge candidate cfb7f4ea (parents main 9c7890a8 and published PR head 7b170294). Downloaded raw reports/collections and an independent complete candidate Git tree. Current stricter verifier re-evaluates all reports successfully: CE browser 37, EE browser 41, API 349 per edition; zero failures/skips/flakes/missing assertions. Each edition collected all 18 browser files. Evidence: evidence/native-browser-7b170.json. This supersedes the earlier unknown native browser status, but does not validate newer local commits.
- Refreshed remaining failures: integration shard2 569 pass/5 redaction skips, shard3 485 pass/29 email workflow skips, shard4 432 pass/3 invoice production skips. Redaction source explicitly targets retired server interpreter; port its five storage-time/retention assertions to real Temporal activities, not seeded read-only substitutes. Email cases need equivalent runtime port.
- Native unit job ran 2700 files, 14607 passing assertions and 3 TODOs, then failed Sheets read with HTTP 503. Treat omitted coverage separately from reporting availability. Evidence: evidence/native-7b170-remaining-failures.json. Local survey dispatch, Temporal and Citus fixes remain unpublished/unverified natively. No broad flags changed; full goal still not achieved.

### 2026-09-07 — port snapshot storage assertions to real Temporal activities

- Moved three retired-interpreter assertions out of the skipped redaction suite into an active integration suite calling projectWorkflowRuntimeV2StepCompletion with real persistence models and PostgreSQL. Four cases cover nested redaction/byte counts/linkage/retry idempotency, multibyte JSON bounds, default and configured retention with tenant/run isolation.
- Canonical server config passes all four after independent disposable migration/seed bootstrap (14.78s). Fixtures use rollback-only cloned schemas; admin connection/retry wrapper are the only activity substitutions. Evidence: evidence/workflow-snapshot-storage-port.json. New file automatically participates in integration shard discovery.
- Two original skipped cases remain: invocation input/output storage and run-studio action reads. Current action activity stores raw output and reuses it for successful invocation replay; blindly redacting output would change retry results. Resolve that persistence distinction before claiming complete redaction coverage. No skip policy relaxed, no broad flags changed, no native candidate pass claimed.

### 2026-09-07 — real run-history action read after Temporal snapshot write

- Ported the retired run-steps assertion into the active snapshot suite. Actual activity writes precede actual listWorkflowRunStepsAction reads; tests assert snapshot linkage, secret masking, tenant settings redaction pointers, foreign-run rejection and denied-permission rejection. No pre-seeded snapshot substitutes for activity writes.
- All five canonical integration cases pass after fresh disposable migrations/seeds (16.60s). Evidence: evidence/workflow-history-read-port.json. Authentication/permission decisions and DB connection boundaries are substituted; action/model queries remain real.
- Removed the equivalent retired skipped case. Exactly one redaction-suite case remains: invocation input/output diagnostics must be reconciled with current successful-action retry storage semantics. Full CI/plan remains incomplete.

### 2026-09-07 — real invocation input diagnostics and successful retry guard

- Added an active integration case for executeWorkflowRuntimeV2ActionStep with actual registry/mapping/persistence and a synthetic registered action. Handler receives original secretRef input; persisted input_json is masked; successful retry returns the original result and does not repeat the handler. All six canonical cases pass after fresh migration/seed bootstrap (16.88s). Evidence: evidence/workflow-invocation-input-retry.json.
- The remaining output diagnostic gap cannot be fixed by masking output_json in place: successful retries parse that same persisted value. Next implement a protected replay representation separate from diagnostic output, with migration/read/export/upgrade handling and a sensitive-output regression. Existing encryption patterns are in packages/integrations/src/lib/xero/xeroOAuthVerifierCipher.ts and CalendarProviderService; no new encryption implementation added yet.
- The combined legacy invocation test remains skipped until output protection and replay semantics are both verified. No requirements narrowed or broad flags changed.

### 2026-09-07 — encrypted replay representation and additive storage

- Added actionReplayCipher: AES-256-GCM, fresh nonce, domain-separated key derivation and authenticated tenant/invocation context. Caller supplies key material; no production secrets read. Fifteen behavioral tests cover JSON preservation, nonce variation, tamper/wrong-key/context rejection, malformed versions and legacy tenantless identity.
- Added nullable replay_output_encrypted JSONB column via migration; existing outputs preserved. Down refuses any protected results. Actual envelope persistence/decryption and lossless downgrade guard pass on PostgreSQL and a tenant-distributed Citus fixture. Evidence: evidence/workflow-replay-storage-foundation.json.
- Not wired to worker yet. Next: protected field in internal invocation model, exclusion from list/export reads, key preflight before effects, encrypt full retry result while storing redacted diagnostics, legacy fallback, and a real sensitive-output/retry regression. Mixed-worker upgrade and key rotation must be addressed; original combined invocation skip stays until complete. No broad flags changed.

### 2026-09-07 — exclude protected replay data from diagnostic reads

- Internal invocation model now types replay_output_encrypted. listByRun strips that field and returns a diagnostic-only type; internal idempotency lookup retains it.
- Two real database/action regressions initially fail because history/export spread the private field. After fix all eight integration cases pass following fresh migrations/seeds (16.13s). Tests decrypt the internal record and assert no replay field/ciphertext in either response. Evidence: evidence/workflow-replay-read-boundary.json.
- Source search found no other non-test raw invocation reads bypassing the model in inspected packages/server/EE/shared roots. Worker write integration, key preflight and upgrade semantics remain next; original output-storage skip remains until the real worker replay behavior passes. No native or broad completion claim.

### 2026-09-07 — worker encrypted replay and active output regression

- Worker success now writes redacted diagnostics and encrypted original replay output atomically. Successful retries decrypt the recorded result, while legacy rows retain their existing replay behavior. Key-ring and migration preflight precede reservation/effects. Rotation supports retaining historical keys; missing retired keys reject replay without repeating effects.
- Real activity/database suite: before narrowly reverting worker integration, 4 fail / 9 pass; after integration, 13 pass, including new-key writes and replay after old-key retirement. Existing activity unit suite: 13 pass. Evidence: evidence/workflow-replay-worker-integration.json.
- Removed the final obsolete skipped redaction suite after porting all five assertions into active snapshot/history/invocation coverage. This does not resolve the separate 29 skipped email-workflow cases. No broad plan flags changed.
- Rollout requires migration and key configuration, then draining old workers before enabling new writers; old workers cannot interpret redacted output_json as the original retry result. Documented in workflow-replay-rollout.md. Actual deployment rehearsal and native CI remain outstanding.
- Broader Temporal engine lane passes all 44 cases in 10 files (16.58s), including production workflow-index bundling. Ephemeral servers required local socket permission; initial sandbox run failed at server startup. Engine activity stubs do not independently prove encrypted database writes.

### 2026-09-07 — shipped email definition on real Temporal engine

- Added seven engine scenarios for reply/new-ticket branching, unmatched sender acknowledgement suppression, missing defaults, attachment/acknowledgement failure continuation and parsing failure to manual resolution. Production definition, Temporal scheduling/control flow and expression mapping are real; node/action effects and projections are fixtures. Registered in engine config and independent discovery.
- Found an incidental diagnostic defect: normalizeRuntimeError exposed Temporal's generic ActivityFailure message to human-task context, losing the actionable cause. Preserve the wrapped activity cause message while retaining existing error categorization. Corrected the synthetic context fixture to include targetAuthorUserId: null, matching the real action.
- Before fix: 6 pass / 1 fail. After: 51 engine cases pass across 11 files (including all seven new cases), 32 runtime unit tests pass, and 10 discovery/gate tests pass. Evidence: evidence/email-definition-temporal-engine.json.
- The 29 retired email-workflow cases remain skipped until parsing, persistence, node semantics, context resolution, attachment idempotency and remaining assertions have active equivalent coverage. Do not count the fixture action outputs as evidence of those behaviors. No broad flags changed; full native CI remains outstanding.

### 2026-09-07 — real email nodes, parser schema and sanitized output

- Added five database/activity cases: real parsing and stored metadata/replay, two HTML converter fallback cases, and two parser failure/unavailable HTML boundary cases. Ported three retired assertions, reducing that skipped suite from 29 to 26 cases.
- Found two defects hidden by stubbed outputs: parsedEmailReplySchema rejected the parser's normal tokens:null result, causing FAILED invocation plus silent fallback; emitted node sanitizedHtml contained executable HTML on both success and fallback. Allow nullable tokens and sanitize node HTML with a lazy server-only DOMPurify/jsdom instance. Keep raw diagnostics distinct; this is not a claim that every parser consumer is sanitized.
- Direct shared runtime dependencies use already-locked DOMPurify/jsdom versions. Lock update is minimal. Sanitizer helper typechecks in isolation. Before: unsafe HTML regression fails; after sanitizer, schema reproducer exposes null rejection; final 18 database cases pass (17.00s). Existing parser 12, activity 13 and production workflow-entry engine 2 cases pass. Evidence: evidence/email-node-storage-port.json.
- Fresh worker image packaging/native CI and remaining retired assertions remain required. No broad plan flags changed.

### 2026-09-07 — joined Temporal email execution with activity persistence

- Added six real Temporal/database scenarios running the shipped definition through actual node/action activities, invocation schemas/idempotency/encrypted replay, snapshot projection and run completion. New/reply paths verify persisted payload/time/metadata/context/defaults/ticket identity; fault paths verify durable failure records, continuation and manual resolution.
...
### 2026-09-07 — application email start and final retired-suite replacement

- Added a real publish/start-action test for the source email template. Verifies version/tenant/payload persistence and Temporal launch arguments, rejection of invalid email input without execution, intentional failed-run audit persistence, and event-source mapping into the workflow payload. Existing auth/Temporal-client substitutes remain; separate engine/database suites execute workflows.
...
### 2026-09-07 — isolated historical invoice locale matrix baseline

- Extended active immutable-invoice integration coverage with nine combinations: v1 / partially missing / entirely missing snapshots × English / French / unavailable locale. Generates actual previews and PDFs, asserts localized unknown-rate/coverage labels and totals, and verifies snapshots/charges/view model remain unchanged during rendering. Restores snapshot data and client properties afterward.
- Full regression passes (24.60s). Evidence includes all nine HTML/PDF/text artifact hashes: evidence/invoice-historical-locale-baseline.json. Poppler content-order extraction keeps wrapped cell labels intact; geometric reading order had inserted the adjacent amount between French label words. No label assertion weakened.
- Both original opt-in cases stay open: this is a standard-template historical baseline, not UI authoring or nested six-column detail coverage. Entry dates, task/ad-hoc labels and other generation variants remain for the wider custom-layout matrix. Native/full-plan completion remains unproven.

### Billed-time date sort authoring regression

Added behavioral coverage to `TransformsWorkspace.integration.test.tsx`: selects the billed-time source, adds date descending sorting through UI controls, checks evaluated preview row order across year/month boundaries, then unmounts/resets/imports/remounts and checks the same order. Source rows must remain unchanged. Full suite: 17 passed, 0 failed (2.43s). Evidence: `evidence/invoice-designer-date-authoring.json`. This is component integration with a mocked feature flag, not authenticated browser persistence or PDF layout evidence. Both remaining opt-in invoice layout acceptance cases remain outstanding; no broad completion flags changed.

### Production browser layout persistence journey (execution pending)

Added `e2e-tests/tests/invoice-designer-persistence.spec.ts`: real administrator sign-in, new named layout, billed-time sample/source selection, date descending authoring, UI save, tenant-scoped database AST verification, cache removal and full reload/reopen, evaluated date-order and unchanged saved AST assertions. No template is seeded and no application responses are intercepted. CE and EE Playwright collection each discover the case. Runtime execution is not yet verified: `release-v1-6-feature` gates billed-time sample/source choices and defaults false; an isolated production installation with the enabled release configuration is needed. Current task-owned Docker containers were enumerated, but existing email-app credentials/data were not accessed or modified. Do not mark this journey or the two remaining custom/nested invoice acceptance cases complete from collection alone. Next: provision/configure an isolated enabled-release installation, execute headed, fix any runtime failures, then extend to custom/nested tables and generated-invoice/PDF assertions.

### Explicit enabled-release browser build configuration

The existing client hook supports `NEXT_PUBLIC_FORCE_FEATURE_FLAGS`; the CE and EE Dockerfile.build files now expose it as a build argument with an empty default. Fresh-install CI builds with only `release-v1-6-feature:true`, and supplies `E2E_BUILD_FEATURE_FLAGS` to record the declared image configuration in Playwright metadata. This does not change normal-build defaults, server-side evaluation, or attest image contents by itself. README documents local build/run requirements. Related component/hook suites passed 19 tests; EE browser collection contains the expected flag metadata. No production rebuild or browser runtime pass yet, so the acceptance work remains incomplete.

### Local production build started; disposable designer database ready

Started EE production image build from `git archive HEAD` at bacc825f06, tag `alga-regression-designer:bacc825f06`, Docker context colima-alga-regression, 6144 MB Node heap (8 GB VM), billed-time release override enabled. Live exec session **20762**, log `/tmp/alga-designer-production-build.log`; last observed building upstream workspace packages. Re-poll that handle before starting or restarting anything. New owned database `designer_browser_82cc` was migrated/seeded through the real immutable invoice suite: 1 passed in 45.15s, log `/tmp/alga-designer-browser-db-bootstrap.log`, artifacts `/tmp/alga-designer-browser-invoice-baseline`. No login password has yet been configured for this new installation and no browser app started. PR3343 readback still points to 7b170294 (59 successful/10 failed/1 skipped checks), not local head. See `evidence/designer-production-build-attempt.json`.

### Production build memory failure; Nx graph diagnostic running

Exec 20762 is terminal: exit 102. `/tmp/alga-designer-production-build.log` reports ResourceExhausted / cannot allocate memory during `npx nx build-deps server` (149.9s in that step). The log explicitly starts `server:build` / full `next build --webpack` during the purported upstream dependency step. Installed Nx helper declares `dependsOn: ["^build"]`; do not guess the dependency cycle or patch it until the resolved graph is inspected. Host has no callable Nx CLI in root/server node_modules, so started diagnostic Docker build from extracted committed bacc825f06 at `/tmp/alga-designer-build-graph`; replaces full build/dependency step with `NX_DAEMON=false npx nx graph --file=/tmp/designer-project-graph.json`. Tag `alga-regression-designer-graph:bacc825f06`, live exec **6817**, log `/tmp/alga-designer-build-graph.log`. Re-poll this handle; on success read graph from the diagnostic image and find server dependency cycles. No runtime app was started and no browser login credentials were configured.

### Survey regression moved to its server boundary

Moved `packages/surveys/src/actions/surveyResponseActions.db.test.ts` to `server/src/services/surveyResponseActions.db.test.ts`, adjusting relative imports only. It imports server surveyService and migrations, which creates a surveys->server dependency when owned by the package. All 17 behavioral cases pass against designer_browser_82cc (5.49s); workspace-db config still discovers the new path. Logs `/tmp/alga-survey-server-relocation.log`, `/tmp/alga-survey-relocation-collection.json`. Nx graph proof remains pending. Diagnostic graph attempt6817 terminated after a zstd prebuilt download timeout/fallback script failure; retried once, **live exec86583**, log `/tmp/alga-designer-build-graph-retry.log`. The diagnostic source remains bacc825f06 (before relocation), enabling before/after comparison once ready.

### Verified two regression-induced Nx dependency cycles repaired

Diagnostic image build86583 completed. Actual before graph had surveys->server and server->temporal-workflows; survey relocation alone removed only the first. Also moved `server/src/test/integration/workflowRuntimeV2.snapshotStorage.integration.test.ts` into `ee/temporal-workflows/src/__tests__/integration/`, adjusting imports/repository-root calculation only. Full suite passes28/28 (24.12s), collection via existing server config confirmed. Both suites stay in their existing DB lanes. Actual Nx before/after task graph:60->58 tasks; before includes server:build and temporal-workflows:build, after has only server:build-deps for those applications. Cycle guard passes3known/0new with unchanged baseline. Graphs in test-results/designer-graph; evidence `evidence/regression-test-build-dependency-repair.json`. No processes remain running. Next: commit relocations then retry production image build from new committed source; old production attempt20762 is terminal and diagnostic image is not a runnable application image.

### Fixed dependency build passes; intended EE application build running

Started new production build from committed813ba16d8d: **live exec19202**, `/tmp/alga-designer-production-build-fixed.log`, tag `alga-regression-designer:813ba16d8d`. Dependency step now passes57upstream tasks in16.57s; proceeded to intended `npm run build:ee` stage. Re-poll19202 before any restart. Initialized synthetic source-login credentials ONLY in new designer_browser_82cc database; mode0600 `/tmp/alga-designer-runtime.env` and `/tmp/alga-designer-browser.env` hold matching auth/database settings (do not print). Script `/tmp/alga-designer-initialize.cjs` refuses repeat initialization. Database/Redis network is alga-regression-browser. Planned new container alga-regression-designer-app, host53010; none started yet. Once build succeeds, run with private runtime env file on that network, built Next production server, then headed Playwright invoice-designer-persistence with browser env file. Runtime flag is baked into this image, not enabled by E2E metadata.

### Dedicated VM resized after actual EE build memory failure

Build19202 exited102: dependency step passed, but intended full EE compile ran out of memory in8GB VM at120.8s. Host has48GB. Restarted ONLY colima profile alga-regression with24GB RAM/4CPUs; restored the10previously running task-owned containers (email-service was already stopped, left stopped). New build **live exec87007**, source813ba16d8d, sameimage tag, 12288MB heap matchingCI; log `/tmp/alga-designer-production-build-24gb.log`. Re-poll before restart. Runner/discovery behavior checks also passed8/8; log `/tmp/alga-relocation-runner-checks.log`. Synthetic designer credentials remain in private tempfiles; no new application container started yet.

### Production compile continuing; recovered diagnostic-image disk space

Revalidated build87007 live (same process, intended EE compile, no new error at last poll). Found VM disk100% full; removed ONLY completed diagnostic image alga-regression-designer-graph:bacc825f06 after graph evidence preservation. VM recovered6.5GB free; host has roughly12GB free, so do not expand disk blindly. Diagnostic image no longer available; saved before/after project/task JSON remain in test-results/designer-graph. Added required logging/email-disabled/hocuspocus-optional settings to the private synthetic runtime env file, for using the shipped entrypoint. No app launched or browser executed.

### EE application build passed; final Docker assembly needed disk cleanup

Attempt87007 completed actual EE compile, generated74static pages, collected traces, and copied built output into final stage. It then exited102: no space left while copying runtime node_modules (Dockerfile line137). Not an application/compiler failure. After failure VM had15GB free. Inspected dangling images: removed only27a798b0ddb3, recovering~7GB (22GB free). Docker refused removal of5804ee43c0ef because stopped container b51a3a0fcb1e references it; did NOT force/remove container. New retry **live exec77107**, same committed813ba16d8d/source/tag/12GB heap, log `/tmp/alga-designer-production-build-space-retry.log`. Re-poll77107;87007 is terminal. No browser/application container started yet.

### User-directed fast local feedback loop now in use

User explicitly requested host testing during implementation and deferring Docker/Colima image builds until final validation. No Docker builds are active;77107 terminated at final export/unpack with no space. Added playwright.local.config.ts and npm run test:local: same headed journeys, no retries, development lifecycle/releaseValidation:false, separate test-results/local outputs, 240s test/90s assertion budgets for cold compilation. CI production config and budgets unchanged. Host Next direct CLI avoids Nx and full build; **live host session33064**, port53010, log `/tmp/alga-designer-host-ready.log`, dist `.next/designer-host`, DB127.0.0.1:55432/designer_browser_82cc, Redis56379. Source private runtime env via shell (Node --env-file propagates invalid NODE_OPTIONS through Next); fixed synthetic LOG_LEVEL=WARNING and supplied CREDENTIAL_ENCRYPTION_KEY. Auth providers returns200. Temporal unavailable: this host setup is only designer/UI/persistence validation. Earlier browser61531 failed connection refusal during startup fix;8783 interrupted for missing encryption key;95897 timed out at dashboard while host cache hit ENOSPC. fstrim on dedicated VM released24GiB of already-free blocks, recovering~26GB host space, no files/volumes deleted. Latest **live browser exec1106**, `/tmp/alga-designer-host-browser-space-fixed.log`; re-poll before rerunning. No browser pass claimed yet.

### Host browser authentication and automatic restart diagnosis

Local Playwright collection and plan validation pass. Browser attempt 63407 exposed a host-origin mismatch: credentials set a session cookie on 127.0.0.1, but the callback redirected to localhost without that cookie. Attempt 91735 used consistent localhost with a dual-stack listener and reached the dashboard (200), then failed connecting to billing. The server log explicitly reports an automatic memory-threshold restart; this is stronger evidence for connection refusals than the initial IPv4-resolution hypothesis. Restarted the task-owned server with 12GB Node heap, preserving the compilation cache and synthetic isolated database. Host session 65069 and browser session 19194 are running; logs /tmp/alga-designer-host-12gb.log and /tmp/alga-designer-host-browser-12gb.log. No production Docker build restarted.

### Host designer journey passes; development action manifest repaired

After a verified Next auto-restart at 8GB, the 12GB run exposed RangeError: Invalid string length in FlightClientEntryPlugin.createActionAssets while serializing billing server actions. Dashboard manifest was 224,520,089 bytes for 1,178 actions; action-entry module IDs reached 215,829 characters. Set deterministic module IDs only for development server Webpack compilations. Same dashboard action count now serializes to 414,041 bytes with IDs at most 6 characters; billing compiles with 1,951 actions in 777,208 bytes. No actions removed; production settings unchanged. Reference/evidence: evidence/host-development-action-manifest.json.

Browser run 75356 reached the designer and exposed ambiguous dropdown locators (wrapper and combobox). Narrowed field/direction locators to role=combobox. Run 51007 authored/saved a DB row but timed out on return navigation during cold development activity; background preview errors while the pipeline was incomplete are not evidence of a save failure. Warm run 21557 passed the full authenticated save/cache-clear/reopen/AST persistence journey in 36.4 seconds with retries=0. Log /tmp/alga-designer-host-browser-warm.log; report e2e-tests/test-results/local/results.json. Host session 65069 remains available with compiled routes. Cold navigation stability and production/native acceptance remain open; do not mark custom/nested invoice acceptance or overall plan complete.

### Fast host Microsoft authentication guard validation

Revalidated existing host session 65069; no Docker build. Microsoft OAuth popup rejection suite first ran 3 passed / 1 failed due opener execution-context destruction during navigation. Warm run 47717 passed all 4 cases in 9.7s, retries=0, including popup failure payload/closure and absence of an authenticated session. Teams endpoint/OpenID override suites passed 16 existing cases in 346ms. Added two signed-token cases for incorrect issuer and expiry, each preceded by a verified valid-token control so network/config failure cannot masquerade as rejection. Both suites now pass 18 cases. Evidence: evidence/host-microsoft-auth-guards.json. Missing teams_integrations in designer_browser_82cc is an EE schema preparation gap; no partial migration applied or product behavior weakened. T028/F034, cold navigation stability and current native CI stay open.

### Fresh EE host schema verified through billing and browser persistence

Created new owned host_ee_test_82cc database using CE migrations overlaid by EE collisions, matching scripts/run-additional-workspace-tests.mjs. Actual immutable invoice suite passed in 33.8s (generated source records, preview/PDF/accounting and persisted snapshot assertions). Verified teams_integrations, teams_notification_deliveries, teams_audit_events, teams_conversation_references and microsoft_profiles exist. Prepared only this database's synthetic browser account; private mode-0600 configs /tmp/alga-host-ee-runtime.env and /tmp/alga-host-ee-browser.env. Original designer DB/config remain untouched.

Stopped old task host session 65069 and started host session 7488 against the EE database, reusing compilation cache and 12GB heap. Browser session 69743 passed real-login designer persistence on the first attempt (2.1m including route compilation), zero retries. No missing Teams relation error recurred. Log /tmp/alga-host-ee-browser.log; evidence/host-ee-schema-bootstrap.json records migration digest and report timings. Removed owned temporary migration overlay after bootstrap. README now explains CE+EE schema selection for host runs. This verifies a fresh plain PostgreSQL EE schema only, not Citus, upgrades, production packaging or current native CI.

### Designer table binding persistence added to real browser journey

Extended existing persistence spec: add dynamic table via palette, select authored billedTimeByDate transform output in inspector, add Date column via preset, save; assert exactly one persisted dynamic table, source binding and date path column. After cache removal/reload, select saved table on canvas and verify reopened inspector binding, while retaining sort-output and whole-AST immutability assertions. Host EE run 79377 passed, retries=0; evidence/host-designer-table-persistence.json records actual timing. Prior run 29368 failed as the long-lived dev worker crossed its heap threshold and restarted; do not count it as passing or claim host stability fixed. No Docker builds. Full custom two-table/nested layout/generated-invoice/PDF acceptance and native CI remain open.

### Two-table/six-column authoring and cache-policy correction

Expanded browser persistence case through real UI: create primary charges table, create separate billedTimeByDate table, remove default columns and add Date/Ticket/Description/Hours/Rate/Amount presets in order. Verify two persisted table bindings, all six expression paths, reopened inspector count/binding, transform date ordering and unchanged saved AST. Run 56613 passed in 146.8s with fresh compilation, retries=0. Repeat 48665 failed after the dev worker hit its memory threshold (server log line45973); no stability claim. Prior run 11350 failed before designer navigation due another restart.

Found next.config.mjs replaced Next's cache object, losing its maxMemoryGenerations:0 and cache-version metadata. Now preserve incoming cache settings/build dependencies while changing cache location. This is a configuration correction, NOT a proven cure for memory restarts; high heap usage persists. Subsequent comment-only config edit caused an explicit restart after the repeat run had already suffered its automatic memory restart. Evidence: evidence/host-designer-two-table-persistence.json. Generated-invoice/PDF, nested-group acceptance, repeat-run stability and native CI remain open. No Docker builds.

### Host production build and authored-layout PDF acceptance pass

Stopped task dev server 7488. Built application revision 7274946079 directly on ARM macOS with Next --webpack, NEXT_DIST_DIR=.next/host-production, 12GB heap and 2 static workers. Build session 98136 passed, 74 static pages; log /tmp/alga-host-production-build.log. Next configuration skips type validation; no full type/native/container claim. Started production host session 91205 on port53010 against host_ee_test_82cc using /tmp/alga-host-ee-runtime.env. No Docker build. Reuse this server for browser testing; application edits require a host rebuild.

Expanded designer browser fixture to create a billing-capable isolated actor, authored Line Items Table + six-column sorted billed-time detail + Totals, generate invoice via real authenticated API from approved source work, and download PDF through UI. Client layout preference is fixture setup pointing at the UI-authored template. Reused/extracted readInvoiceDownload without weakening existing invoice document assertions. First run found incorrect fixture assumptions (generic charges table and expected grouped525 amount); switched to actual Line Items Table/Totals and generated invoice total. Existing ownership/PDF case passed in 2.8s in that run. Final designer-to-PDF journey 94942 passed in 6.9s with production timeouts and retries=0: public ticket labels, four descending dates, mixed-rate text, amounts, total, private exclusion, unchanged snapshots. Evidence/host-production-designer-pdf.json. Full nested/locale/custom acceptance, Docker packaging/native CI and development memory stability stay open.

### Nested UI layout and three-locale generated PDF validation

Added billed-time-by-ticket preset through designer UI, selected its nested table via outline and added Description as sixth column. Saved AST proves group registry path ticketGroups and nested registry path group.entries; flat/nested columns each6 and primary charges/totals retained. First failures corrected ambiguous outline/breadcrumb locator and an incorrect literal-binding-ID assumption (generated collection.ticketGroups correctly resolves to ticketGroups). Nested run99931 passed in6.9s. Expanded client-locale fixture checks for fr and zz-unavailable, with separate PDF files and locale restoration. Final run12955 passed1journey in10s, production timeouts/retries0, using the unchanged host production app7274946079. Verifies eight entry dates (four flat/four nested), descending flat date subsequence, mixed-rate labels, public tickets, amounts/total, private exclusion and snapshot immutability. Evidence/host-production-nested-locales.json includes PDF hashes.

This is not full CLOSURE parity: historical v1/missing snapshots, task/ad-hoc identities, hour-block and long multi-tax layout matrix remain. PDF text checks do not prove all visual layout properties. No Docker build or production app rebuild; native CI/container packaging remain open.


### Historical authored-layout PDF matrix exposed two runtime defects

Expanded the existing UI-authored flat/nested layout journey with explicit v1, partial and missing snapshot fixtures after real invoice generation, in English/French/unavailable-locale fallback. Fixture changes are tenant/invoice scoped and restored in finally. Verify localized detail notices, available row counts, invoice totals, no private text, and unchanged snapshots/charges. These are historical rendering fixtures, not evidence of snapshot production or migration backfill.

The initial browser run failed downloading a no-snapshot PDF: the designer saves transforms.sourceBindingId=timeEntries directly, but evaluator only accepted absent optional historical collections through declared binding IDs. Added direct-path support restricted to documented historical collections, preserving malformed and unknown binding diagnostics. Three focused runtime cases reproduced the failure before the fix.
...
Canonical browser discovery at39401169ed passes underCI=true:88files, server15/32cases, EE53/411, deploy1/2, productionCE19/38 andEE19/42. Seven disabled registrations are payment-links3, portal-handoff2 anddeployment2. No runtime execution claim. Complete tooling passes510 tests/54files in148.2s, zero failure/skip/todo; actionlint and15 focused discovery tests pass. Evidence: evidence/current-browser-discovery.json. Publication remains pending the already requested approval.


### Authored-layout long multi-tax PDF coverage

Extended the real UI-authored invoice journey with addLongInvoiceSources:74 approved time entries, two actual service tax rates, generation through authenticated invoice API. Refactored shared PDF reader to expose page text while preserving existing readInvoiceDownload API. Asserts148 flat/nested dates, repeated Date/Ticket headers on detail pages, localized mixed-rate/total text, no private content and unchanged snapshots/charges. Independent expected subtotal1132500/tax167250/total1299750 differs from older cycle-id fixture because this API's explicit August service window excludes September usage. Both charge tax rates10/20 asserted.

Final headed run1374 passes2 journeys in29.4s, retries0, using unchanged host production buildc5938cdad5. Long PDFs: English12pages, French13, fallback12. Existing invoice ownership/PDF case passed2.4s after reader refactor. Evidence: evidence/host-long-invoice-layout.json. No Docker/app rebuild. Task/ad-hoc/hour-block/authoritative-preview parity and global readiness gate remain incomplete; no broad plan flags changed. Publication approval remains pending.


### Project task identity and immutable authored-layout PDF coverage

Added tenant-owned project/status/phase/task sources to standalone Playwright fixture, without selecting pre-existing project data. Three tasks: two same public names, one empty title, first with two hourly service rates. Actual invoice API captures8 links including4 task snapshots with3 distinct IDs. Authored flat/nested PDF verifies11 same-name occurrences (three rows in flat Description/nested Ticket/nested Description plus two group headings), localized unnamed-task fallback,16dates,180serviceamount and1600.50total. Independent subtotal145500/tax14550/total160050 asserted.

After generation, explicit fixture-only edits replace project task names and descriptions. PDFs still show frozen public names/fallback, no PRIVATE/EDITED, and snapshots/charges unchanged; sourcefields restoredfinally. Final headed run63859 passes in31.6s including all historical/long cases, retries0, with unchanged host appc5938cdad5; no Docker/app build. Evidence: evidence/host-task-invoice-layout.json. Empty-title setup does not prove authoring UI accepts empty names. Hour-block/ad-hoc/preview-canvas parity/native CI/full goal remain open. Publication approval remains pending.


### Prepaid allocation through UI and authored invoice PDF

Added an owned three-hour block, service and timesheet. Two real UI saves (one and three hours) trigger FIFO allocation, leaving120 then0 minutes. Approval remains explicit fixture setup, covered separately by the approval journey. UI billing selects the full client/window, previews and generates recurring hourly/usage plus one hour overage. Assert two allocation rows, both entries invoiced, zero-dollar block information, subtotal102500/tax10250/total112750, PDF consumption line/total and no private notes.

Fixture diagnosis: September entries require the September billing cycle; automatic invoicing defaults to periods ending by today, so explicitly apply through2026-10-02 using the real date input. Moving prepaid time into August excluded it from the seeded September cycle; corrected without changing application behavior or weakening monetary assertions. Headed run76148 passes in36.9s (one journey including all prior PDF matrices), retries0, reused host appc5938cdad5. Evidence: evidence/host-prepaid-invoice-layout.json. No app/Docker build. Legacy ad-hoc/authoritative-preview-canvas parity and broad plan/native CI remain incomplete. Previously requested publication approval remains pending.


### Historical missing-identity fallback PDF matrix

Added disclosed historical snapshot variants to the real authored-template journey: ad_hoc with missing work identity/title, unnamed synthetic task identity and unnamed ticket. Keep actual mixed overtime; mark other snapshotsv1 to validate unavailable historical rates. English/French/unsupported locale PDFs assert localized other-work/task labels, mixed and unavailable rates, eight flat/nested dates, charge/total preservation, no private strings and unchanged stored snapshots/charges. Original snapshot/locale fields restored. Description column binds title, so the initially added expectation that snapshot description appears was corrected after inspecting the saved authoring bindings; no app change.

Host headed session22722 passed the complete journey including prior history/long/task/prepaid cases with no app or Docker rebuild; see evidence/host-historical-fallback-layout.json for duration and PDF hashes. Native CI and authoritative preview/canvas parity remain outstanding. No broad plan completion flags changed. Publication approval remains pending.


### Node workflow aggregate gate

Added always-running Node workflow execution gate with needs on browser discovery, tooling and appliance. It downloads current-run candidate-named raw artifacts and independently reconciles Node event streams against current checkout candidates/exclusions, plus all five real Playwright collection reports. Rejects non-success prerequisite jobs, missing/malformed artifacts, dirty/stale source, incomplete events, empty mandatory collection and omitted files. Producers now record sourceRoot so cross-machine absolute report paths normalize correctly. Browser collection remains explicitly executionVerified:false. Artifact download may continue only so the final verifier can report missing evidence; the verifier itself is blocking.

Behavioral fixture runs actual Node and Playwright collectors, then damages their evidence to verify failures; also covers expired vs valid owned exclusions. This is a per-workflow building block for F007, not the repository-wide gate, effective branch protection or native-CI proof. Broad F007/F018 flags remain false. Full host tooling validation follows the implementation commit. No Docker build; publication approval remains pending.

Canonical host validation atf16a103450 passes: tooling511tests/55files (158.9s), appliance239tests/47files, allfivebrowsercollections. Actual standalone aggregate CLI replays those raw reports and passes, with browser executionVerified:false. Ten focused tests and actionlint pass. Evidence: evidence/host-node-workflow-gate.json. Read-only PR refresh confirms unchanged7b170294a755dfebba34cb8bbfc1069b6d8ba8bc draft with59success/10failure/1skipped. No native enforcement claim.


### Repository-wide candidate readiness orchestration

Added production-regression.yml parent for PR/push and manual full/tier1 builds. Seven existing workflows are reusable and no longer independently trigger the same PR/push, avoiding duplicate executions. Original manual/scheduled entries retained. Parent selector permits Temporal/Citus omission only for known docs-only diff; unknown diff runs them. Existing integration/browser selectors remain authoritative within their own lanes. Final always-running readiness job downloads only nine small aggregate verdicts from this run (not Docker image artifacts), requires successful reusable outcomes and exact revision/scope/member sets, positive complete execution counts and no failures/skips/todos/pending/flaky/interrupted data. Missing artifact downloads continue only so the blocking verifier emits an explicit failure.

17 focused decision/CLI behavioral tests pass in0.5s: per-workflow failure/cancel/skip/missing, absent/stale/wrong-scope/malformed/empty/partial/duplicate verdicts, missing edition, docs-only reason and selector integrity, real CLI artifact reads/exit statuses and dirty checkout. Actionlint passes all8 workflows. Native GitHub workflow-call execution remains unverified; child check names change, so F008 effective-protection migration remains required and no readiness enforcement claim is made. F007/F018 flags remain false pending representative native candidate validation. Full canonical tooling follows the implementation commit. No Docker build. Publication approval remains pending.

GitHub reusable-workflow semantics checked against official reference: https://docs.github.com/en/actions/reference/workflows-and-actions/reusing-workflow-configurations and https://docs.github.com/en/actions/how-tos/reuse-automations/reuse-workflows. Local reusable paths bind to caller commit; secrets inherited, contents-read permissions retained, parent and fresh-install concurrency groups use distinct prefixes.

Canonical host tooling at a9f291053c passes528tests/56files in154.8s, zero failures/skips/todos/cancellations. Evidence: evidence/host-production-readiness-gate.json. Read-only legacy branch protection and effective ruleset inspection: legacystrict=true, no status contexts; active rulesets require only Run ext-v2 guard and ESLint and Check for new circular dependencies, both integration15368. These workflows were not changed, so existing effective check names remain intact. No main test-suite readiness check is currently required. Evidence: evidence/main-required-checks-readiness.json. No protection mutation performed. Native orchestration and effective required readiness remain open.


### All landed browser files form the mandatory floor

Audit found fresh-install gate pinned only4 of19 landed journey files. New regression removes a journey from candidate inventory and both CE/EE raw collection/results reports. Baseline verifier incorrectly passed for deleted inbound-email; expanded floor now rejects every one of19 deletions with explicit missing-file errors for both editions. Fixture list is independent of verifier list. This closes deletion of whole landed journey files, not deletion of individual assertions within a surviving file. F018 native enforcement remains pending.

Also replaced Node aggregate broad SHA artifact pattern with three explicit downloads; under the new shared parent run, the old pattern could fetch unrelated workspace reports. Consumer paths unchanged. Focused fresh-install/global readiness/Node aggregate suites pass21tests in1.1s; actionlint and diff checks pass. Evidence: evidence/browser-floor-deletion-regression.json. No Docker/app build or redundant full suite rerun; prior fulltooling528test evidence remains scoped to a9f291053c. Publication approval remains pending.


### Exact release manifest binding for test results

Added release-test-evidence library and verify-release-promotion CLI. Canonical SHA256 covers all recorded metadata and sorted component set; image/source/build changes invalidate prior results. Consumer owns requiredComponents/requiredChecks. Every successful check must carry the same manifest digest as the wrapper, and runtime observations must match approved images. Non-JSON metadata is rejected. CLI takes policy, manifest, evidence, runtime observations and output JSON paths; does not read runtime itself or publish/deploy anything.

Focused component/rendered-resource/binding tests and real CLI process pass. Tests replace only email image while matching runtime to replacement, then prove old evidence and wrapper-only retagging fail. Simulated new per-check digest succeeds as identity-contract validation, not a claim of real replacement smoke. Evidence: evidence/host-release-test-binding.json. Producer binding to actual smoke, build provenance/schema completeness, runtime adapter and release pipeline wiring remain incomplete; F020/T015 full acceptance stays open. No Docker builds. Publication approval remains pending.


### Runtime image readback adapter and collector
...
### Published native readiness verifier for Argo integration

Added read-only verify-published-readiness CLI and library. Requires full source SHA, fixed production-regression.yml workflow, matching repository/push run and exact successful Production regression readiness job. Paginated API collections must be complete; latest matching run supersedes older success; job reads pin run attempt and final reread rejects concurrent rerun. HTTP/malformed/missing/ambiguous data fails closed. CLI optionally uses GITHUB_TOKEN without logging it.

Focused behavioral API fixtures pass; public read-only probe for published7b170294a755dfebba34cb8bbfc1069b6d8ba8bc returnsHTTP404 and correctly fails, proving no qualifying verification rather than assuming success. This does not distinguish missing workflow from access restriction. Evidence: evidence/published-readiness-verifier.json. Official contracts: https://docs.github.com/en/rest/actions/workflow-runs and https://docs.github.com/en/rest/actions/workflow-jobs.

Next external edit must require this check on the resolved full commit before BOTH existing-image and rebuilt-image promotion, including standalone build/release-pointer paths; adding it only after build-app in the composite would miss earlier release-pointer promotion. Preserve manual approval and add actual manifest-bound preview smoke separately. No external workflow patch applied yet. No Docker build or publication.
...
### Host Graph calendar reaches real recovery; non-UTC delta gap remains

Started task-local native emulator+HTTPSproxy19750 (Graph52620/control52621/callback52622). Existing c5938cdad5 build uses compiledOAuthorigin53010. Host apps86483(port53010) and56718(port53011 callbacktarget) now use corrected tunnels, privatefilesystemsecrets and REDIS_PREFIX=alga-provider-host-82cc:. Prior sharedgroup had recentlyactive externalconsumer103 in addition to hostprocesses,acknowledging scheduleevents; isolatedprefix progresses. Startupscript/tmp/start-alga-provider-host-82cc.sh; endpoints/tmp/alga-provider-host-82cc.env; noDocker. Original20677stopped.

Calendar browserfixture accepts E2E_CALENDAR_CALLBACK_BASE_URL (defaultCompose unchanged), and clicks viewportnoonpoint to avoid parentautoscroll stale coordinates. Oldclickfails; newreaches actualschedulepersistence/outboundsync. Finalsession86777 terminalexit1: OAuth,subscription,UIoutboundcreate/delete,inboundcreate,outage503/errorUI,tokenrefresh/recovery allpass; vendorcorrection neverreacheslocaltitle. DiagnosedcalendarView/delta400fromcore UTC-onlyconstraint; recovered event dateTime hasZ buttimeZone America/New_York. Do not inventtimezonebehaviororweakenassertion. F034/F037 remainopen. Evidencehost-calendar-provider-partial.json, runbookhost-provider-testing.md. Next: implement verified nonUTC Graphdate semantics andrerun completejourney.
...
### Exact current-source supported upgrade gate passed

- Native webpack enterprise build session88708 exited0 at58aa45fc957650d49e2d3eec79e94b3d0741fd95 into server/.next/upgrade-current (12GB heap,2workers). No source changes during build or tests. Build skips full type validation and reported workflowInference export/dynamic dependency warnings; these require separate review, not hidden by upgrade pass.
- Schema session21170 passed baseline1028+upgrade31, source before/afterclean, database upgrade_current_82cc_20260908; output /var/folders/8g/3xyjqdpd4hx2h39h4qb2lyvm0000gn/T/alga-supported-upgrade-nRX5VJ.
...
### Teams meeting adapter wire coverage

- Added packages/emulators/msgraph/tests/teamsMeetingAdapter.test.ts: actual token acquisition, guarded endpoint resolution and create/update/delete meeting adapters against an ephemeral Graph host. Tenant config and background subscription scheduler are mocked; this is not DB/browser/SSO evidence.
...
### Rebuilt emulator full development gate and enforcement refresh

- Rebuilt msgraph natively (subsecond npm build), stopped owned oldhost16479 and restarted updated host73913 on52620/control52621. This brings the explicit unsupported-route diagnostic into the running vendor process.
- First clean98a7 gate51426 failed during a memory-threshold restart of the long-running app; preserved raw evidence /tmp/alga-teams-98a7-reused-app-failure. Stopped that parent and created fresh app98460 on53016,12GiB/no Node source maps, matching the CI recreation lifecycle. Unchanged gate41921 passed1case~1.8min with clean identical before/after revision and no retry/skip; includes meeting persistence and real bot isolation. Evidence/host-teams-rebuilt-emulator-gate.json.
-12GiB is now demonstrated for the fresh-app lane, not repeated tests on one indefinitely retained process. Current fresh app98460 remains live; start a fresh process before another full gate if matching CI.
- Refreshed main protection through required_status_checks, repository rulesets, and effective rules/branches/main APIs. Classic contexts/checks empty; effective required checks remain only ext-v2 guard/ESLint and no-new-circular-dependencies. Fresh-install ruleset is evaluate, not enforced. Evidence/main-enforcement-refresh.json. PR3343 remains draft/open at7b170294. No publication or rule mutations. F001/F008 remain incomplete pending current CI, owner/enforcement work.

### CI runner registration and pending decisions

- Verified unit-tests.yml includes ../packages in server unit scope, so the Graph tests also execute via server/vitest.config.ts. Initial focused server runner88550 passed70/7; exact CI per-file isolation run65204 (singleFork=false,maxWorkers1,SKIP_DB_TESTS1) also passed70/7 in2.67sec. Raw report /tmp/alga-msgraph-server-isolated.json. No full unit/coverage claim.
- Full browser collection37722 passed at cleanf6771d1738741ecff21bc742e7dd5a630680636c. Evidence/graph-server-runner-registration.json records both boundaries; collection is not broad browser execution.
- Refreshed explicit publication approval question for committed branch/PR3343 after prior auto-review rejection; no answer has arrived, no push attempted. Also requested a product decision on the three unresolved tax TODOs (existing policy vs explicit owned expiring exclusions). Existing code/docs provide no cap input or defined subscription/international policy; no policy invented and no TODO removed.
### Calendar contract drift detection and error-envelope fix

- Reviewed Microsoft's create-event and Graph error documentation on 2026-09-08; added a consumed-field contract independent of emulator implementation, executed against real HTTP responses. Mutations of event start type, create status and error envelope are rejected.
- Red run: the new behavioral test failed on missing Graph error.message (27 calendar cases passed, one failed). Corrected wire error serialization to add a developer message when a Graph error object has only its code; string OAuth error envelopes remain unchanged.
- Green native server CI-runner configuration: 71 tests / 7 files passed, 2.92 seconds, no Docker build. Log `/tmp/alga-graph-contract-suite.log`. Command: server Vitest with SKIP_DB_TESTS=1, ../packages/emulators/msgraph/tests, --poolOptions.forks.singleFork=false --maxWorkers=1.
- F037/T031 remain incomplete: this establishes calendar response/error shape coverage and deliberate mismatch detection, not independent auth/callback parity across all providers or live sandbox verification. Publication approval is still pending; no push attempted.

### OAuth envelope and callback handshake contract checks

- Reviewed Microsoft authorization-code response and webhook-validation documentation (links in emulator README). Added token-envelope checks to real code/refresh/client-credential HTTP flows; deliberate expiry-type and bearer-type mutations fail. Added seven real subscription handshake cases and verified refused callbacks persist no subscription.
- Red callback run: 7 failures, 28 passes (`/tmp/alga-graph-callback-contract-red.log`). Emulator omitted the request content type and accepted any 2xx/plain token regardless of response media type. Fixed validation to send text/plain; charset=utf-8, require HTTP 200 with text/plain and the exact decoded token, and apply a ten-second deadline including response body consumption. Invalid callback URLs now return validation failure through the existing subscription error path.
- Callback-only change full suite passed 78/7, 2.79 seconds. After OAuth assertions, combined run hit UND_ERR_SOCKET on the calendar control reset before test execution (35 failures, 43 passes); preserved `/tmp/alga-graph-auth-callback-contract-green.log` despite its misleading preselected filename. Subsequent full run passed 78/7 in 2.67 seconds; exact failing seed 1788853414239 replay passed 78/7 in 2.75 seconds (`/tmp/alga-graph-auth-callback-contract-seed-replay.log`). Root cause of that socket failure is unresolved; do not count the failed attempt as green or claim proven flake-free execution.
- All runs used native server Vitest CI fork options, no Docker builds. F037/T031 remain open for notification payload, auth-error and other-provider independent parity; no live sandbox verification, SSO or complete scope/PKCE parity claim. README now carries the explicit capability matrix. Publication approval remains pending.

### Real application webhook handshake checks

- Added four Playwright HTTP cases against the running application (calendar, email, Teams recordings, Teams calls). They validate HTTP 200/plain text, exact once-decoded opaque tokens containing spaces, plus, literal percent escapes and Unicode; a second compiled-route call has a ten-second timeout. Initial development compilation has a separate 120-second allowance and is not release-latency evidence.
- Existing native app on 53016 was confirmed listening before execution. First run: 3 passed, email warm call ECONNRESET; application log confirms memory-threshold restart. Failure preserved in `/tmp/alga-product-webhook-contract-first-failure` and `/tmp/alga-product-webhook-contract.log`. After restart all 4 passed in 10.5 seconds (`/tmp/alga-product-webhook-contract-rerun.log`). This does not establish stable full-suite memory behavior or current production-image coverage.
- Strengthened Teams evidence verifier to require the original product journey and all four named callback cases in enterprise-chromium. Matching collected/executed reports with only callbacks in the correct file, or a missing callback, now fail. Ten behavioral verifier tests pass natively. The expanded five-case clean-source Teams gate remains to execute; F034/F037 remain open.
- No Docker builds or publication attempted.

### Put webhook contracts in the production lane after measured memory failure

- Rebuilt Graph natively at ce16cdaac2 and started a fresh 12 GiB Teams app. Full expanded gate: all four callbacks passed, original product journey failed after a confirmed Next memory-threshold restart. Raw failed gate retained at `/tmp/alga-teams-ce16-memory-failure`; log `/tmp/alga-teams-full-ce16.log`. This invalidates the five-case development composition as a reliable CI choice.
- Moved webhook cases to `e2e-tests/tests/microsoft-webhook-validation.spec.ts`, the mandatory production browser suite. Production requests use ten-second timeouts from the first call; the local runner alone permits initial route compilation. Community checks the three EE-only route denials and the shared email handshake; enterprise checks all four handshakes. Added the file to the permanent fresh-install floor. Teams development keeps the original product journey and now verifies its exact collected title so unrelated same-file checks cannot substitute for it.
- Relocated enterprise cases passed natively: 4 tests, 11.2 seconds, `/tmp/alga-product-webhook-production-lane.log`. This used local development config, not a current production image. Community execution remains for CI. Fresh-install fixture initially failed correctly when the new mandatory file was absent; updated its complete-run fixture. Fresh-install + Teams verifier tests: 13 passed (`/tmp/alga-webhook-lane-tooling-fixed.log`); browser-discovery behavioral test also passed. No Docker builds or publication.

### Clean Teams gate passes after production callback separation
...
### OAuth offline-access parity catches accidental persistent access

- Added real HTTP authorization/token contract cases for explicit Calendars.ReadWrite with offline_access, without it, and with the nonmatching offline_access_extra scope. Microsoft authorization-code documentation conditions refresh tokens on offline_access. Red run: 2 failures /36 passes (`/tmp/alga-graph-offline-access-red.log`): emulator incorrectly returned refresh tokens without the required scope.
- Corrected token issuance to create/store/return refresh tokens only for delegated offline-access grants. App-only grants no longer create and immediately discard one. Updated same-second token identity fixture to explicitly request offline_access for its refresh behavior. Legacy omitted-scope default remains documented; this does not establish full consent, scope or PKCE parity.
- Native server CI-config Graph suite passed 81 tests /7 files in 2.95 seconds (`/tmp/alga-graph-offline-access-green.log`). Package build and tsc --noEmit both passed. No Docker build. F037/T031 remain open for remaining provider contracts and current CI. Running emulator process still predates this rebuilt bundle and must be restarted before any new application-level validation of this change.

### Live metrics workbook audited; isolated copy awaiting approval

- Read actual Google Sheets metadata and bounded headers through the connected Drive tools. Live metrics!A1:AB1 has 28 columns exactly matching the current HEADER exporter; browser_readiness!A1:R1 has 18 columns exactly matching BROWSER_HEADER. Historical metrics rows remain present; browser rows from 2026-09-07 are visible. Header equality is not historical-reader or full T019 workbook evidence. Recorded evidence/live-metrics-header-audit.json.
- No existing isolated validation workbook was identified. Created My Drive root ChatGPT folder 1LnvKWwRyRoTXZEP4Fu3wv5vRzOcxhEPm. Automatic approval review rejected copying the full original metrics workbook into that folder because the potentially sensitive historical payload and destination lack explicit authorization. No copy was created and no original cells were changed. Asked user asynchronously for approval of that exact full-workbook copy and title. Do not bypass through another copy/export route.
- Next after approval: native-copy original workbook, verify different spreadsheet ID, then validate synthetic success/failure/cancelled/missing/retry-only metrics against preserved history and separate PR/main/nightly views. F024/F025/T019 remain incomplete. Publication approval is independently still pending.

### Live chart formulas still omit readiness dimensions

- Read native CellData for chart_data!A1:AN3 and charts!A1:H5. Actual QUERY formulas use metrics!A2:P and filter by suite only, omitting run_kind, execution gate and methodology fields. The current integration summary shows 100% with 146 skipped tests. This is direct evidence that additive headers alone did not complete F025. Bounded native readback preserved in evidence/live-metrics-chart-audit.json.
- Replaced obsolete A:T percentage-only chart setup advice in docs/reference/test-metrics.md with explicit PR/main/nightly execution views, status/count visibility, legacy separation and methodology boundaries. Included a concrete proposed A:AB query, clearly marked unexecuted pending isolated-workbook validation. No original workbook cells or charts modified. F024/F025/T019 remain open.
- The copy approval request is still pending; do not bypass the rejection by copying/exporting historical data through another path. Publication approval also remains pending.

### Stripe callbacks verified with the independent provider SDK

- Replaced the Stripe smoke suite's homemade HMAC signature verifier with actual stripe.webhooks.constructEvent. Added rejection checks for altered raw bytes, wrong endpoint secret, and a stale but correctly signed delivery. The historical-event redelivery case verifies provider SDK acceptance of a current delivery signature. Reviewed https://docs.stripe.com/webhooks/signature.
- Declared stripe ^19.1.0 as an emulator dev dependency, matching server's range; existing root lock already resolves 19.3.1, so no package version change. npm ls confirms resolution. Native server CI-config tests passed 22/2 in 704 ms (`/tmp/alga-stripe-sdk-contract.log`); package typecheck passed (`/tmp/alga-stripe-sdk-typecheck.log`).
- This supplies independent Stripe callback-auth validation for F037/T031. Other provider contract coverage and live sandbox limitations remain; no claim of complete parity. No Docker builds, workbook copy or publication attempted.

### QBO OAuth errors separated from accounting Faults

- Independent Intuit OAuth client references show token failures using OAuth error fields. Added four real HTTP cases: absent Basic auth, wrong client secret, invalid refresh grant and unsupported grant type. Red run: 4 failures /10 passes because all failures used accounting Fault envelopes (`/tmp/alga-qbo-oauth-contract-red.log`).
- Added typed QboOAuthError and token-response serialization. Accounting QboWireError/Fault behavior remains exercised by existing entity tests. Green native server CI-config QBO suite: 14 passed in 461 ms; package typecheck passed. Logs `/tmp/alga-qbo-oauth-contract-green.log` and `/tmp/alga-qbo-oauth-typecheck.log`. Xero source already separates token error bodies from accounting errors; source inspection alone is not independent Xero parity evidence.
- No Docker builds. Running QBO emulator remains an earlier bundle; application-level current-source proof requires a rebuild/restart and current CI. F037/T031 remain incomplete; publication and isolated workbook copy approvals are still pending.

### Xero validation contract corrected; combined provider suites pass

- Read official XeroAPI/Xero-OpenAPI xero_accounting.yaml, including Error schema and examples. Added typed runtime shape expectations to the existing invoice rejection case. Red: 1 failed /12 passed because ErrorNumber and Message were absent (`/tmp/alga-xero-error-contract-red.log`).
...
### 2026-09-08 — Independent inference package removes remaining cycle

Shared already imports EE email adapters, so putting the inference algorithm there created a reverse edge. Moved the unchanged injected algorithm into packages/workflow-inference, with explicit AJV/OpenAI dependency declarations and native build/typecheck targets. Both edition wrappers consume the independent package. Full Nx graph generated using the installed base-checkout Nx runtime: three known cycles, zero new cycles. Eight behavior tests, production-alias webpack bundle, package typecheck and native tsup build pass. No baseline exemptions or Docker builds. CI must still verify the published source.

### 2026-09-08 — T019 isolated native workbook validation complete

Validated real exporter-generated success/failure/interrupted/missing-report rows across PR/main/nightly, browser retry-only rejection and forged-passed downgrade, visible measured/source counts, exact-methodology filtering with a negative selector, and preserved historical A:P chart readers. Evidence is in isolated-workbook-readiness-views.json, isolated-workbook-retry-coverage.json, and isolated-workbook-methodology-history.json. T019 is complete for the isolated workbook. Chrome visual review of readiness, methodology and browser evidence found clipped identifiers; native formatting now wraps cells and highlights failed gates. This does not deploy replacement views to the source workbook, supply externally missing CI runs, or complete F024/F025 rollout. Original live workbook remains unchanged.
...
### 2026-09-08 — Restore real Vitest types in the server

CI typecheck on 964c67be64 failed because server/src/types/vitest.d.ts replaced Vitest with a partial ambient any API, hiding beforeAll/afterAll and typed importActual. Replaced it with an import of installed Vitest declarations. Fixed the now-visible SSO mock signatures (limiter argument, cookie arguments, nullable signing secret) and retained explicit toBeDefined checks before accessing subscriber mock calls. Native full server tsc --noEmit passes with the CI app heap allowance of 14336 MB; initial 6144 MB attempt exhausted heap. All 23 affected SSO route tests pass under server Vitest 3 in 730 ms. Logs: /tmp/alga-server-real-vitest-types-final.log and /tmp/alga-sso-real-vitest-types-tests.log. No tests excluded and no Docker builds. Current-source GitHub verification remains pending.

Current published CI additionally identified two skipped invoice acceptance cases requiring manual UI data in invoiceTicketProduction.integration.test.ts (473 other shard assertions passed), and an enterprise browser image archive that docker load rejected as incomplete. The latter occurred before application startup despite a successful download step and 86 GB free disk. These are unresolved; neither is counted as passing product coverage.

### 2026-09-08 — Self-contained saved invoice layout regression

Converted the manual INVOICE_TICKET_CUSTOM opt-in case to an unconditional integration test. It bootstraps its own disposable database, generates real invoice sources, saves a sorted custom layout through saveInvoiceTemplate, reloads it, and retains both table, amount, French preview/PDF and source-edit sentinel assertions. Native focused execution passed using test_invoice_sorted_82cc and existing Poppler tooling; the nine other cases were intentionally deselected by the focused test-name filter, not evidence of full-suite execution. Log: /tmp/alga-invoice-sorted-native.log. The separate historical locale matrix still needs its own repeatable setup; visual editor authoring is not claimed by an action-level fixture.

### 2026-09-08 — Historical invoice matrix owns its fixtures

Removed the remaining INVOICE_TICKET_CLOSURE manual opt-in and hard-coded wired database. The historical locale test now saves its own six-column nested/sorted template and independently generates task-identity, hour-block and long multi-tax invoices in the same disposable database. Reused the existing generation assertions through generateProductionVariant; no cross-test files or shuffled execution order are prerequisites. All 18 variant/locale combinations pass in the focused native test (38.72 s), retaining preview, persisted snapshot/charge invariance, localized label, date, money and multi-page checks. Content-order (-raw) Poppler extraction avoids adjacent columns splitting wrapped French labels; the actual generated PDF was inspected to confirm the label existed before changing extraction. Full unfiltered 10-case native verification is running; do not treat this focused result as full-suite or published-source CI proof.

Full unfiltered invoice file verification completed: 10/10 tests passed, zero failures, pending or TODO assertions, under native server Vitest 3 with test_invoice_complete_82cc. Raw report: /tmp/alga-invoice-complete-native.json; log: /tmp/alga-invoice-complete-native.log. No image builds.

### 2026-09-08 — Workflow snapshot secret-input isolation

Reproduced both CI snapshot failures with nextauth_secret=synthetic-ambient-provider-key: rotation could not authenticate the stored result and missing-key still invoked the action. getSecret correctly prioritizes provider material over the NEXTAUTH_SECRET fallback changed by these tests. Scoped the test secret adapter to its two replay inputs while delegating every other secret lookup to the real provider. Key parsing, AES-GCM encryption, activity execution, database persistence and disclosure checks remain real. All 28 tests pass, zero pending/TODO, with the same ambient key present, in 26.33 seconds. Red: /tmp/alga-snapshot-ambient-key-red.log. Green: /tmp/alga-snapshot-ambient-key-green.json and .log. Database test_snapshot_keys_82cc, existing native infrastructure, no image build.

### 2026-09-08 — Image transfer integrity checks

Enterprise browser job 102070296735 reported its image download successful, then docker load failed with an incomplete deflate archive before any product test. The runner had 86 GB free, so there is no evidence of local disk exhaustion. Updated only image downloads to actions/download-artifact v8.0.1 with explicit digest-mismatch: error and added gzip -t on the producer before upload. Official action metadata confirms Node 24 and the strict digest input: https://raw.githubusercontent.com/actions/download-artifact/v8.0.1/action.yml . The workflow parses successfully. This improves integrity diagnostics and rejects corrupt transfers; it does not prove that the observed transfer failure is resolved. Actual new-source CI transfer and browser execution remain required.

### 2026-09-08 — Independent Graph basic notification parity

Microsoft changeNotification documentation requires subscription/tenant GUIDs and subscriptionExpirationDateTime. Added an independent Zod contract used against real mail, calendar, meeting-artifact and call-record HTTP callbacks. Four existing tests failed against the previous emulator: missing tenant/expiry and non-GUID subscription IDs. Corrected common notification metadata and derived GUID subscription IDs from the deterministic seeded ID stream. Mail callbacks additionally match the actual subscription ID/expiry and OAuth token tenant claim. Deliberately missing required fields and malformed GUID/date/change-type values are rejected. All 81 Graph tests in seven files pass in 1.26 s and package typecheck passes. Logs: /tmp/alga-graph-notification-contract-red.log, /tmp/alga-graph-notification-contract-green.log and /tmp/alga-graph-notification-typecheck.log. README records source and limitations. F037/T031 remain open for other resource-specific/auth contracts and current-source CI; no live sandbox or full Entra tenant-model claim. Existing running emulator processes have not been rebuilt or restarted.

Current CI community browser job 102070296645 completed: 40 browser cases passed, invoice-designer-persistence failed (Add Work Item picker option detached during click), and invoice-generation was flaky (first-attempt invoice generation failure). The strict browser gate rejected both rather than counting retries as ready. All 349 API E2E tests in 21 files passed against the built application. Source log: /tmp/alga-browser-ce-964.log. These browser failures remain unresolved; the unit coverage job 102062061268 is still live.

### 2026-09-08 — Picker ticket-number search defect

CI trace and error context show the invoice designer hour-block step searched a visible ticket number, then got No work items found. The actual picker query only matched title. A real PostgreSQL action test reproduced zero results for number while title passed. Grouped title OR ticket-number ILIKE within tenant/exclusion predicates. Four native tests pass in 19.54 s, covering title, number, already-added exclusion and foreign-tenant denial. Added the case to the Tier-1 floor and regression ledger with before/after evidence. No selector bypass, forced click or retries added. Full current-source browser execution remains pending; designer-save and invoice-generation failures are separate unresolved paths.


### 2026-09-08 — Browser trace diagnosis and unfinished-transform preview

Complete community server logs correlate invoice run 79b740c3-c861-49c2-88d9-fb0dd8105823 with a Knex acquisition timeout during durable session validation, resulting in AuthenticationError and failedCount=1. This is not a login-selector timeout. Pool exhaustion cause remains unconfirmed; revocation must continue to fail closed. Designer retry save is followed by back-navigation GET and overlapping preview requests to the original new-template URL, then an empty editor; the navigation race is still unconfirmed. Do not count the retry as ready.

Preview digest 1419326533 independently reproduces with real workspace export: setting a source/output before adding an operation throws outside the action's error boundary. Added a structured diagnostic return and a behavioral recovery assertion after clearing the edit. Before: 7 pass/1 fail; after: all 11 tests across four preview suites pass without Docker or database. Evidence: invoice-preview-unfinished-transform.json. This fixes the proven preview 500, not the unresolved navigation or pool-acquisition causes. Local development server on 53017 uses separate .next-regression-debug-453 output and existing host_ee_test_82cc infrastructure for diagnosis only, not release attestation.


### 2026-09-08 — Reproduced and repaired designer save navigation

Host reproduction with matching release-v1-6-feature:true confirms save returned success and a persisted template, yet the URL stayed at templateId=new through a 90-second assertion. Pausing authoritative preview calls while saving, and retaining that pause until successful navigation unmounts the editor, fixes the complete existing designer journey: 1 pass, no retries, 1.5 minutes. The same matched host run passed both recurring and manual invoice-generation journeys; an earlier run without the release flag is not equivalent evidence. Added component assertions for no new preview calls during navigation and restoring editing/preview after a failed save. All 55 tests across three designer/editor suites pass. No auth bypass, route interception, browser timeout relaxation in production config, or Docker image build. Native test config labels development evidence explicitly. Evidence: invoice-designer-save-navigation.json. Current-source CI and the separate CI connection-acquisition failure still require verification.


Full server typecheck passes after the navigation change (14336 MB allowance), /tmp/alga-designer-navigation-typecheck.log is empty. The temporary host app was stopped and its separate generated Next output removed.

CI unit job 102062061268 completed normally: 2706 files pass/1 fails, 14703 tests pass/1 fails/3 TODOs. The sole failure is taxCalculation.test.ts retaining the old remainder-across-gap expectation after e1a0295918 correctly honored threshold minima. The fixture defines bands 1001–5000 and 5001–unbounded; at 6000 the taxable amounts are 3999 and 999, whose independently rounded taxes are 400 and 150 (550 total, effective rate 9.17%). Corrected only the stale expectation/comment. Native reproduction: 5 pass/1 fail before; both tax suites now 30 pass/0 fail/3 existing policy TODOs. Reports: /tmp/alga-tax-calculation-red.json and /tmp/alga-tax-calculation-green.json. No tax policy or production calculation changed in this correction.


### 2026-09-08 — Cross-runner inventory and browser discovery gaps

Refreshed repository discovery from 30 actual completed CI collections, including browser legacy runners. Initial 151 unmatched candidates comprise 134 mobile tests in the separate green Mobile checks workflow and 17 additional identities requiring reconciliation. Downloaded same-revision mobile collection instead of misclassifying those as orphans. The comparison is explicitly older CI collection versus newer candidate, not same-revision readiness. Citus did not reach its database lane; the newer picker test is absent from the older revision; four legacy email suites and the visual/manual baseline need further classification. Existing i18n baseline has an owned expiring exclusion. Full global gate remains incomplete.

Proved a genuine discovery defect with actual Playwright collection: e2e-tests/upgrade-tests/retained.spec.js was silently ignored by the browser candidate filter. Added upgrade discovery and its explicit runner to collection and Node aggregate revalidation. Renamed collaborativeEditing.e2e.test.ts to collaborativeEditing.playwright.test.ts so the existing browser runner actually collects it. Two behavioral tool tests pass after the pre-fix missing-file assertion failed. Evidence: global-inventory-browser-gaps.json.

Current-source run 34235374733 is live. Nx tooling job 102091593709 failed before tests while Puppeteer downloaded Chrome (ECONNRESET). That lane does not launch a browser, so skip Puppeteer download there only; other runtime lanes retain their browser setup. Community build 102091914737 also failed with npm ECONNRESET before compilation; no product-code diagnosis is inferred. Current mobile run 34235374300 is green. Preserve the live expensive run before publishing the next batch.
...
Clean 40518dad8b manual collection passes: four legacy email files/21 cases and one invoice visual file/one case. The first sandboxed run reported empty visual cases because its database reachability probe could not connect; rerun with permitted local networking and REQUIRE_DB=1 passes against existing PostgreSQL. Only collection ran, no hooks, test bodies, database reset or Docker build. Evidence: manual-inventory-native.json.


### 2026-09-08 — Same-run repository inventory gate

Wired repository inventory into the production regression parent/readiness workflows. It downloads only the named collection/evidence artifacts from this run, preserves separate directories, checks clean matching before/after revisions, compares actual Vitest file and registered-case reports, reads raw Playwright collection and Node event streams, and reconciles all tracked candidates. Missing/stale/empty artifacts, orphaned tests and expired manual reviews fail. It labels collection as executionVerified=false. Documentation-only changes receive an explicit independently recomputed not-applicable decision because conditional runtime lanes do not execute; unknown changes still widen.

Moved the existing four Mobile checks jobs into a reusable workflow invoked once by the parent (same PR/main/master/develop triggers remain at the parent). Added mobile before/after source snapshots and require mobile job success. Manual registration reuses the existing workspace-DB runner after its execution; it starts no extra database or browser. Readiness now requires the inventory verdict as its thirteenth separate requirement. Named operational ownership remains an open user clarification.

Validation: full artifact-boundary fixture uses real Node runner events plus serialized Vitest/Playwright producer formats and passes; it rejects stale revisions, missing/empty reports, orphaned candidates and expired manual reviews. All 17 readiness tests pass, including CLI source validation and collection-versus-execution checks. actionlint passes all three changed workflows. A compatibility audit of 22 prior real CI Vitest reports finds exact file/registered-case agreement in every bundle (including 2,707 server-unit files/14,704 cases). That prior-source audit is format evidence only. F004/F006 remain open until current-source GitHub verifies the complete repository.

Canonical native tooling run at clean 8b1cb1aad0 passes all 649 tests across 69 required files in 154.3 seconds. No skips, TODOs, cancellations or failures; before/after source is identical and clean. Evidence: native-node-repository-inventory.json. Current unit CI job 102091593804 remains live (started 13:59:16 UTC, 90-minute bound); no new push while it runs. Existing tax policy/exclusion clarification remains unresolved (prior requests recorded above); do not invent caps, international or subscription-period behavior or delete TODOs to force green.

Native mobile source-boundary validation at clean cb3b2a27e3 passes 901 tests in 134 files, no skips/TODOs/pending/failures. Regenerated the ticket editor, collected both raw reports, ran the complete suite, and read the reports through the new global inventory adapter. Both source snapshots match the same clean commit. Evidence: mobile-inventory-source-native.json. Host Node 25 is development evidence; GitHub retains its Node 20 mobile runtime.


### 2026-09-08 — Xero refresh-token contract recovery

Independent Xero OAuth FAQ review found immediate refresh-token deletion and indefinite unused-token validity in the emulator. Added two actual HTTP regressions with private virtual clocks: lost-response retry at 29 minutes must succeed, old-token reuse after 30 minutes must fail without a sliding deadline, recovered token remains usable, and unused refresh expires after 60 days. Both tests fail before the core correction. Full native Xero suite passes 15/15 across two files afterward (882ms), without Docker builds or shared clock aging. Evidence: xero-refresh-lifecycle.json. Existing client/secret and redirect checks stay intact; no application security override changed.

Published batch b0c83651c866a69cdf9e0c9e7599777e51a77b3d is running in GitHub 34244599182. Previous unit job completed all 2,707 files/14,706 executable cases successfully; strict execution gate fails only on the three tax TODO identities. PR description updated. This Xero follow-up is separate and will be batched after live CI diagnostics.

Xero offline-consent follow-up: official auth-flow docs require offline_access before issuing refresh tokens. New real HTTP case fails before correction (refresh token returned for accounting.invoices alone). Token response now makes refresh_token optional, preserves exact requested scope instead of injecting default privileges, and allocates refresh state only with offline consent. Existing client-binding fixture now explicitly requests its required scope; all authentication rejection assertions remain. Full Xero native suite passes 16/16 (790ms), package typecheck passes. Logs: /tmp/alga-xero-offline-{red,green,types}.log. F037 remains open for current-source CI and wider protocol coverage.


### 2026-09-08 — Close cached-build masking of Temporal imports

Current Citus job 102123396283/run34244599182 passes eight runtime cases but fails Temporal collection on unbuilt @alga-psa/storage. A native resolver audit rejecting workspace dist imports then exposed authorization and email dependencies masked by cached local builds. Vitest now uses the worker tsconfig workspace paths as a fallback after explicit overrides, with a guard rejecting remaining workspace dist imports. Added storage and email source aliases; no dependency installation or worker production configuration change.

Validation: all 27 cases in the six-file Temporal database suite pass on existing Citus; all 263 fast-readiness cases pass; engine collection succeeds with 51 cases in 11 files (no engine-runtime claim). No skipped/pending failures or Docker builds. Evidence: temporal-source-resolution.json; /tmp/alga-temporal-source-{database,readiness}.json and /tmp/alga-temporal-source-engine-collection.json. Current-source CI remains required; do not push while the expensive current run is live solely to validate one import.

### 2026-09-08 — Reproduce reported Microsoft all-day payload escape

Issue #3348 reports Google-style date fields forwarded to Graph. Independent Microsoft event documentation requires dateTimeTimeZone, midnight all-day boundaries, and matching zones. Four new HTTP cases failed against the permissive emulator; validation now rejects invalid creates and patches before mutating state. The real adapter then reproduced HTTP 400 UnableToDeserializePostBody. Source fix converts date-only values to Graph midnight dateTime, sets isAllDay on updates, and maps inbound all-day boundaries back to date-only values with exclusive end preserved.

All 87 Microsoft emulator cases across seven files pass natively in 3.80s; emulator typecheck passes. Includes real adapter create/update, title-only edit, timed/all-day transitions, and inbound fractional-second date mapping. Evidence: microsoft-all-day-contract.json. Logs: /tmp/alga-mscalendar-contract-red.log, /tmp/alga-mscalendar-adapter-{red,green}.log. UI rendering/editor timezone semantics remain unfinished; this does not close #3348 or F034/F037. Current CI34244599182 is still live; retain follow-up batch until it completes.

### 2026-09-08 — Preserve exclusive all-day ends in shared mapping

Tracing #3348 beyond the adapter found date-only end parsed as T23:59:59Z, extending the interval by almost a day. Two provider round-trip regressions failed before correction. End now uses exclusive UTC midnight, and outbound detection uses exact UTC midnight rather than local hours/minutes (including seconds/milliseconds to avoid misclassifying nearly-midnight timed events). All three cases pass in separate native processes under UTC, Europe/Berlin, and America/Los_Angeles; no Docker/database required. Each provider repeats three synchronization cycles without date drift. Evidence: calendar-date-roundtrip.json; logs /tmp/alga-calendar-mapping-{red,green}.log. Added actual reported incident #3348 to regression ledger with partial status. Browser display/editor and full current CI remain required. Existing schema still cannot distinguish exact UTC-midnight timed events from all-day sentinels; do not claim complete all-day semantics.

### 2026-09-08 — Editor all-day display/save boundary

Added an EntryPopup behavioral regression reproducing 02:00 display for a UTC-midnight all-day event in Berlin. Editor initialization now converts imported all-day sentinels to local calendar midnight and saves them back to UTC date boundaries. Moving the start retains calendar-day count across DST rather than elapsed milliseconds; explicit time changes remain timed events. Title-only save and one-day move assertions pass. Three existing component suites pass 13/13 in each of UTC, Europe/Berlin, America/Los_Angeles. Evidence: calendar-editor-dates.json; logs /tmp/alga-calendar-editor-{red,green}.log. The test stubs pickers/actions and is not browser evidence. Grid accessors, drag/resize, and real Playwright validation remain unfinished; accessor-only conversion would mismatch existing drag persistence, so implement them together next.

### 2026-09-08 — Grid and drag/resize date boundaries

Added behavioral coverage invoking actual ScheduleCalendar accessors and resize/drop callbacks through the existing stubbed widget. Berlin grid accessor assertion fails before correction. Grid now supplies local-midnight dates; resize converts local date boundaries back to UTC sentinels, and all-day drop uses target calendar date plus original UTC day count. This avoids preserving elapsed DST hours or original local 02:00 offsets. Three component suites pass 14/14 in each of UTC, Berlin, Los Angeles. Evidence: calendar-grid-dates.json, /tmp/alga-calendar-grid-{red,green}.log. This is not rendered cell-span/pointer-gesture evidence. Tooltip/detail formatting and real browser journey remain to verify. CI34244599182 now runs production browser jobs for both editions; both images built successfully, known Citus follow-up remains unpublished.

### 2026-09-08 — Add real all-day Microsoft browser journey

Extended Microsoft Playwright integration cases with an additional all-day throttling/recovery scenario; existing three timed fault cases remain. Browser timezone is explicitly Europe/Berlin. New assertions verify stored exclusive UTC dates, actual editor midnight time fields, unchanged date boundaries after title save, and remote isAllDay/date preservation after refresh/retry, retaining existing duplicate mapping and deletion checks. Real DateTimeField uses Select time combobox inside dtf-fields; selectors follow that rendered structure. Enterprise Playwright discovery succeeds with four cases; no execution claim. Existing 53010/53011 app processes are next-server16.2.12 from this server directory, but their source/emulator freshness still needs establishing before native execution. Evidence: calendar-all-day-browser-collection.json. Published CI34244599182 remains in production browser execution on b0.

### 2026-09-08 — Activate coordinator plus three agents

User explicitly requested continued parallel implementation. Agents qbo_protocol and stripe_protocol own their emulator packages only, no shared files/builds/services/commits. calendar_review owns independent calendar review then bounded semantics tests; root owns shared persistence/config/CI/browser integration. QBO fixed non-expiring refresh tokens against advertised TTL (2 red, 17 green); Stripe snapshots event data so replay does not mutate after payment recovery (2 red, 24 green). Both package typechecks pass. Root reviewed diffs and executed both suites together: 41/41, five files, two workers,2.22s. Evidence parallel-qbo-stripe-validation.json, logs /tmp/alga-{qbo-lifecycle,stripe-snapshot}-{red,green,types}.log and /tmp/alga-qbo-stripe-combined.log.

Native calendar setup findings: app53017 source4141329, emulator52630/control52631/callback52632. First cold popup timeout, next run OAuth succeeds but Next dev restarts at80% of12GB heap. Restarted owned app with20GB heap after checking48GB host RAM. Next failure null subscription traced to missing NODE_EXTRA_CA_CERTS on new emulator; emulator restarted with local CA trust, no assertion changes. Current browser retry session33628/log /tmp/alga-calendar-current-browser-trusted.log, appsession7763, emulatorsession76390. Native validation remains unproven. CI b0 unit job102123231800 completes2707files/14706 executable tests green; gate fails same3taxTODOs. Full job log/tmp/alga-unit-b0-final.log.

### 2026-09-08 — Native all-day browser passed; explicit provenance follow-up

Trusted emulator callback run33628 passed1/1 in3.9min with realheadedBerlin browser. Scope is existing4141329 all-day behavior before explicit field follow-up. Log /tmp/alga-calendar-current-browser-trusted.log, preserved result/tmp/alga-calendar-trusted-passed-results.json, evidencecalendar-all-day-native-browser.json. Coordinator/agents review found real timed midnight intervals wrongly inferred all-day inUI. Agent calendar_review added explicit provenance tests then mapping/helper changes (23/23 in eachUTC/Berlin/LA). Agent qbo_protocol owns shared model recurrence propagation + dedicatedDB cases; root owns migration/types/schema/API/UI. Agent stripe_protocol owns offset-free Graph timezone parser/newtests using official CLDR Windowsmapping. These follow-ups are uncommitted/inprogress; earlier browser pass does not prove them. Root migration20260908170000_add_schedule_entry_all_day.cjs defaultsfalse without backfill. Dedicated DB suite planned schedule_all_day_82cc_20260908; do not recreate host_ee_test_82cc.

### Coordinator and three-agent review round — 2026-09-08

Maintained explicit ownership/dependencies in `tasks.json`. Calendar reviewer found and fixed a further all-day recurrence editor timezone boundary defect (two pre-fix failures; 11 focused component cases pass separately in UTC, Europe/Berlin and America/Los_Angeles). Mobile review executed the actual generator against conflicting root/mobile and nested dependencies, proving local resolution and fail-closed root fallback. Citus investigation ruled out the observed startup race; added safe phase diagnostics without claiming a fix. Teams development sign-in remains unresolved; preceding suites show no source-password mutation in source review. Full server TypeScript check passed before the final editor change; workflow actionlint and plan validation passed. Current full browser/CI validation remains outstanding. No Docker builds performed in this review round.

### Actual enterprise calendar path and CI timezone protection

The current native all-day browser failed at persisted `is_all_day`, including after rebuilding shared/dist. Traced the remaining failure to the separate EE mapper and Microsoft adapter used by enterprise routes. Extended existing mapper and real-HTTP adapter behavioral tests to both implementations. The EE mapper produced seven pre-fix failures; 32 cases now pass in each of UTC/Berlin/Los Angeles. The EE adapter failed all-day creation with Graph400 before the fix; all45 Graph calendar cases now pass. CI now explicitly repeats focused UI/mapping suites in Berlin and Los Angeles; default UTC alone cannot catch the recurrence editor bug. Added a read-only Teams source-auth preflight (five real-crypto tests plus a passing isolated real-DB probe), clearly distinguished from Next compiled runtime. Citus role provisioning now reports only fixed phase and sanitized code. Full green CI remains unproven.

### Merged current main and native validation

Committed reviewed batch849ae8edb3 and merged origin/main1ea8c942f6 cleanly into db09315e1b. Full canonical Node tooling654/654 in70files and mobile901/901 in134files passed with clean identical before/after snapshots. Exact timezone CI selection now passes51cases in Berlin and51in Los Angeles, including both actual mapper implementations. Full server TypeScript passed after merge. Upstream AvailabilitySettings component tests exposed a missing jsdom scrollIntoView API; supplied a test-local restored spy and awaited real animation callbacks, then six related UI files/30cases passed without unhandled errors. Browser validation remains pending after a cold OAuth popup timeout; no application pass claimed.
...
### Shared worker secret ownership and process proof

R035 implementation now uses UID1000 across isolated server/email/authored/Temporal workers and a full shared `/data/secrets` root. A root-only initializer operates on empty job-owned volumes, copies only synthetic app secrets (including admin-password alias), enforces0700roots/0600files, and makes file storage writable. Existing `/run/secrets` mounts remain for actual shell entrypoints; filesystem-provider app reads use initialized `/data/secrets`. Eight native initializer tests pass. Sourceprovider suite passes37cases with6existing platform-specific skips, including new independent-process rotation test. CI now runs write(server),rotate(each worker),read(email),delete(server) through actual public provider exports and verifies UID/modes.

Peer review found serverimages did not contain core dist for this Node diagnostic path and nonrootNextneeded additional writablepaths. Images now copy the builder's compiledcore beside existingpackage.json, give UID1000ownership of `.next`,serverparent andtsconfig while retaining actualentrypoints. Stagedpackage containing onlypackage.json+dist passes three independentprocesses write→rotate→delete without externaldependencylinks. No Docker build/run performed. CleanLinuxcandidate execution still required; F031 remains incomplete.

### Consumer recovery and retained process readiness

QBO realconsumer nativeHTTP coverage now verifies refresh/persist/retry, revokedrefresh rejection with no overwrite/replay, and second401 stopping afterone refresh; fullQBO29/29 andtypecheck pass. Teams bothactualsaveactions nowprove organizer404 preserves existingrow and laterlookup recovers;4/4 pass package andserver runners. Database/credential infrastructure remains private mocks inthese consumer tests; no newapplicationbug wasfound.

Provider readiness nowretains sanitizedorigin/process/candidate JSON underCIlogs. Reviewreproduced two falsepasses: redirectfollowingwithhistoricaljournal, and stalejournalwithoutnewrequest. Rejectredirects and requirefreshmatchingsequences fixboth;10nativeHTTP/readinesscasespass. These are containerconfiguration/network probes, notproviderworkflow executionproof. Actualauthoredworkflow proof isstillR036inprogress. Currentrun34266401070stillhasrealserverunit/integrationshard4steps active; do notcancelmerelybecauseobservationtimesout.

The serverunit job in34266401070finished:14895passed,1failed,3taxTODO. Soleexecutedfailure was an obsolete source-stringT072 assertion expectinghardcodedGraphURL. Removedsource-onlytest and carriedT072intoactualworkspace/EEHTTPbehaviorcases;4package/7serverfocusedcasespass. Taxpolicy stillrequiresuserinput. Integrationshard4remainslive.

Actualcompiledworker nowexecutes pinnedstate.set/control.return via productiondispatcher: TemporalCOMPLETED, persistedrunSUCCEEDED, bothexactstepsSUCCEEDED andsnapshotstate native-ready. FinalnativeCLIrun gracefullystopsworker/Redis anddropsownDB. Priorlookupfailure was harness assigning undefinednamespaceasstring, notanestablishedproductbug. R041isaddingthecorrespondingexistingcandidate-container CIcheck; nativeproof alone doesnotcloseF031.

Candidateauthoredexecution helper nowruns against existingworker/services using onlyproductionexports andexplicitownedDBidentity guards. Separateprocess nativevalidation passes exactsteps,state,completion andscopedcleanup whilepreservinganunrelatedtenant. CI invokesit insideworkflow-worker with directownedpostgresadminconnection andretains passJSON plusprivatefailurediagnostics with theexistingjoblogs. Non-COMPLETEDworkflows areterminated andfixturesretained toavoid deletingunderpotentiallyrunningactivity; thatfailurepath isnotyetfault-tested. Current-source Linux image/CI execution remainsoutstanding; featureflagsremainfalse.

Run34266401070isnowterminalfailure. Finalintegrationshard4passed528tests/72files; allfourshardspassed. CE/EEproductionimagebuildspassed. InventoryfailureisabsentAPIevidencefrombrowserjobblockedbyalgasimrunnerfailure, notnewunregisteredTemporaltests. Knowncodefailures have reviewedlocalfixes;3taxTODOremainpolicydependent. Publishcompletedbatchnowthatnousefuljobswillbecancelled.

### Current-source image failure and remaining behavior assertions

Run34273284490revealed Temporalimage aptfailure beforecompilation: bullseye-security metadataexpired. Changedonlybase to node22-bookworm, keepingfreshness/signaturechecks. Officialcompatibilitymetadata andcurrentpublicBookwormsecurityInRelease(validuntilSept15) supportthechange; noDocker/aptlocalexecutionclaim. PreserveotherliveCIjobs andbatchthisfix.

Calendarbrowsernowassertsall-dayregionplacementandabsencefromtimedgrid afterimport/vendorupdate;4casescollect, browserexecutionpending. ExplicitDBflag assertionremainsbecausecurrentUIalso puts multi-dayspansintoheader. ProviderparityinventorynowincludesactualQBOrefresh/retry andTeams404recoverycoverage withtheirprivateinfra limits. Real event.wait failure-retention validationisR044inprogress.


Native failure-retention proof now passes against the actual compiled worker: unpublished event.wait hits the bounded deadline, only its execution is terminated, diagnostic rows remain, and unrelated tenant survives. The combined harness exits 0, gracefully stops worker/Redis, and drops its own database. Sanitized evidence records source and script hashes. Candidate image CI remains pending. Current run additionally exposed clean-build dependency gaps: QBO consumer test resolves an unbuilt DB package before mocking; worker Temporal compilation lacks the shared jsdom declarations. Agents are fixing these independently while useful CI continues.

Reviewed clean-build fixes: QBO test aliases resolve real source before infrastructure mocks; hiding DB/core dist per process reproduces the original error and all 29 cases pass after the fix. Shared now owns @types/jsdom alongside jsdom; exact sanitizer compilation reproduces missing declarations before and passes after. Temporal noEmit and actual worker manifest selection pass. Lockfile has only the intended one-line workspace dependency addition. These are native checks, not a claim that candidate image CI is green.


### Parallel review of remaining requirements

F025 review found browser rows lacked the standard run category. Added run_kind/event_name as columns S:T without moving A:R; missing evidence remains incomplete within its original trend. All35 related native tests pass, including legacy schema extension and retry/incomplete behavior. Documentation describes historical blank categories; no live workbook writes.

F022 audit found no concrete implementation gap within documented single-node scope: migrated distributed billing/workflow coverage and v1.5.0 browser upgrade checks already exercise tenant boundaries, retained usage and invoice amounts. Eight lightweight verification cases pass; current-source runtime CI is still required. Read-only effective main rules refresh confirms only extension/ESLint and circular-dependency checks are required, both with strict freshness false. Readiness enforcement remains incomplete and no rules were changed.

QBO review reproduced a cross-company authorization false pass: a realm-one token could read realm-two. Bound authorization codes, access tokens and refresh credentials to selected realm; middleware rejects foreign reads/writes before mutation. Corrected three fixtures to independently authorize their companies. Peer review found no introduced implementation issue. Default parallel runner passed29/29 both normally and with application dist hidden; TypeScript passes. Two earlier local HTTP failures (closed vendor sockets and unexpected authorize400) remain undiagnosed despite unchanged-source successful reruns; no concurrency reduction or retry policy added. R048 remains in progress pending currentCI/follow-up. Exact403/SIM_REALM_MISMATCH is emulator-specific, not claimed vendor error parity.

Bounded QBO transient investigation: three default-parallel instrumented runs each pass29/29; each has32 distinct listener ports, zero port reuse, zero Undici errors and no unexpected authorization response. Earlier two failures remain unexplained; no speculative lifecycle/concurrency changes. Browser fixture review confirms actual selected-company OAuth with no cross-company mint-token assumption. Current published revision CI now passes allfour integration shards, workspaceDB, allinfra shards (infra2:137tests/22files), Temporal, Citus, and CEbuild. Serverunit, Nx andEEbuild still live; holdpush. Separately confirmed potential partial-start listener leak is being behaviorally tested.

R049 startup cleanup: actual occupied control port reproduced both masked bind error and reachable leaked vendor listener. Fix handles Express5 listen callback error and closes previously started listeners on startup failure, retaining original error or both errors if cleanup fails. Regression verifies TCP connection refusal, retry and HTTP response after releasing collision, and shutdown refusal. Full host28/28 native tests and package typecheck pass. No normal shutdown semantics change; QBO transient cause remains unresolved. Published revision now also has EEimage success; browserjobs correctly reject failedbuildmatrix before execution. Remaining serverunit/Nx jobs are live.

Combined native emulator verification at b2dd7cfa76: rebuilt host and QBO packages, then default suites pass host28, QBO29, Xero22, Graph92, Stripe26, webhook3, SMTP4 and suite2 (206total). No retry/concurrency changes or failures this round. Unchanged provider dist and installed dependencies reused; no clean image proof. Nx published CI now passes; serverunit remains the only live test job.

Published source beb79b951a serverunit job102220017077 completed:2738files and14906executed tests all pass, plus3taxTODO. The execution gate correctly fails only the outstanding international/cap/multi-period tax policy cases. No new executed-test regression. Image failures have reviewed local fixes. Await final inventory/aggregate completion before publication.

Isolated workbook validation at e67fdb4584 now exercises current exporter run categories. Added only validation_browser_trends tab9060806 in approved copy1v7rjj0_8tMOGbibDghxcjmSgN1Ejvyvjkyz0jOUHl7I. Native QUERY formulas at V2/V8/V14 each return passed, failed retry-only, and incomplete runs for pr/main/nightly; incomplete counts remain blank. Existing copied history and original workbook untouched. Connector evidence saved; native UI opened and saved, screenshot fit check remains unperformed. CurrentCI34279976654 has new deeper image failures: EE Teams runner cases, standalone Temporal dompurify override, and final workflow-worker compilation. Three agents assigned separate fixes.

CurrentCI fixes: EE Teams organizer tests now isolate unrelated notification builders/publisher with fail-on-use mocks, retaining real action/token/Graph behavior. Safe clean-resolution simulation reproduces both EE failures before and4/4passes after; full Graph92/92 and canonical server4/4pass. Standalone shared manifest now references $dompurify for its override; actual npm Arborist EOVERRIDE reproduced before and all45directedges pass afterward, security constraint^3.4.13 unchanged. Worker import checker AST fix is under review, including imported JSON compatibility. No currentCIgreen claim.

R052 worker build validator: actual esbuild bundle of installed dunder-proto/get.js reproduces regex false imports from JSDoc. TypeScript AST extraction accepts the same bundle while preserving real static/dynamic/template forbidden/unresolved checks. Review added real Node JSON-import execution to preserve JSON module compatibility. Six focused behavioral cases and full native workflow-worker build pass. Existing workspace-unit discovery includes this test file. No new dependency or image build locally; actual candidateCI remains pending.


Current-source run 34287571495 at published 9c1f2908e3203ff46c74182bd1c61a0677effdf6 validates all three repaired image builds (algasim, workflow-worker, Temporal). All four integration shards and Nx pass. Community production browser job 102272580006 passes 42 browser cases and 349 API cases across 21 files; log retained at /tmp/alga-9c1-browser-community.log. Enterprise job 102272579983 failed: PostgreSQL retained-ticket restore assertion at line39 cannot find the title after reload despite its DB assertion passing; retained invoice and usage cases pass. Subsequent Citus setup fails connecting to the isolated destination, and another later phase requires investigation. Agents are investigating separate causes. Server unit job102266589201 remains live; no whole-run green claim or feature completion inferred from these partial results.

R053–R055 native review: retained-ticket failure screenshot explicitly reports PostgreSQL connection exhaustion. Fixed concurrent cold initialization so callers share one pending pool; added five behavioral lifecycle cases. Coordinator independently ran all 12 new/existing DB lifecycle tests successfully; agent DB typecheck passes. Citus now receives fixed-port ingress routing with host-path SQL readiness (three native TCP behavior tests pass). Teams captures the recreated server’s latest paired credentials and uses them in auth preflight and browser tests (three native helper tests pass). All six Node tests pass together. Cross-review found no additional lifecycle issue across ingress, upgrade overrides, and server recreation. Docker topology and full enterprise recovery still require CI; no local Docker run. Server unit CI remains live, so publishing waits to preserve its evidence.

Current enterprise job primary suites passed 47 browser cases without retries and 349 API cases across 21 files. Later PostgreSQL upgrade has two passing cases and one failed ticket case; Citus/Teams browser phases did not start. R048 clean CI evidence verifies the actual PR merge revision 3c046458a0fca6cc43c63f9ac791154410a8a1e3, QBO29/29 and Xero22/22. Earlier native HTTP transients did not recur. Review also found nonfatal Teams webpack EACCES: R056 adds an isolated UID1000 disk cache bind mount in that development phase only. No local Docker build; actual mount behavior pending CI.

Run34287571495 is terminal failure with no active jobs. Server unit job102266589201 completed normally at2026-09-09T00:14:04Z:2745files and14981executed tests pass, three tax-policy TODOs remain. Execution gate rejects incomplete TODO coverage; no executed-test failure. Counts reflect actual PR merge candidate (not merely raw branch head). Reviewed next batch can now be published without cancelling useful CI.

Published49a688cca7ef2454236919e1574633e9a791190c to draft PR3343; new regression run34294404179 is live. Full prior-head PR audit additionally found TypeScript readonly NODE_ENV writes in two Teams test files and ext-v2 React hook lint errors on a plain server predicate. R057–R058 fix those using supported Vitest environment helpers and a predicate rename. Teams20/20 and Graph92/92 behavioral cases, both package typechecks and action ESLint pass; coordinator reviewed. Current live run predates these local follow-ups; do not cancel it to publish. R059 stages two isolated registration-collection workers while preserving serial test execution; ten-file63identity native sample shows7.98s vs5.88s, exact inventories equal. Memory and full CI collection performance remain under investigation.

Evidence acceptance audit: completed run34287571495 inventory job102286982139 passed with4052candidate files,35runners,0unmatched/0failures.4046mandatory-runner files,5explicitmanual files,1ownedexclusion; executionVerified=false. F004/T004 accepted with evidence/ci-repository-inventory.json plus9native behavioral inventory cases, while F006 and release readiness remain open. Accepted R034/R035 worker startup/image identity/shared-secret phases and R041 real authored-workflow success at tested merge3c046458a0; limits retained in tasks. R039 audit found incomplete replica coverage: previous7records prove service classes only. Added configured replica-count/running-state/label checks, individual docker exec probes, per-container identity/results, and stale-success removal. Combined32native routing/topology tests and actionlint pass; actual container validation remains pending. New code remains local while run34294404179 executes.

Current run34294404179 browser jobs102293311555(EE)/102293311640(CE) both fail starting temporal-dev: --wait times out180seconds. Normal browser bootstrap is skipped; EE later upgrade lacks installation credentials, and Teams cannot find skipped Playwright Chromium. Citus host-path readiness passes; Teams writable-cache setup, recreated credential capture, and actual-source auth preflight pass. Accepted bounded R004/R054/R055/R056 repairs while keeping full upgrade/Teams journeys pending. R060 investigates Temporal cause without timeout inflation. Replica-readiness cross-review tightened candidate image binding and stale-evidence cleanup; combined36native tests pass.

R060 exact failure: current Temporal frontend binds container IP and default namespace registers; loopback health probe targets unbound address. Changed only probe address to temporal-dev:7233, retaining timeout and namespace semantics. Native real-server bound-address/wrong-loopback/missing-namespace proof saved; no Docker. Prior9c eventually also showed unhealthy despite proceeding, so do not claim prior Temporal-dev health was verified. R061 makes server/browser bootstrap independent of unrelated worker failure and guards each dependent phase on actual prerequisites. A real native Chromium launch passes; initial capture reuses already-tested safe credential helper. Mandatory worker and aggregate gates remain unchanged.

Canonical native tooling at clean4c860140c5773f2ecac450dff15675dac5270b48 passes741tests across78required files in170.6seconds,0fail/skip/todo/cancel. Before/after source identical and clean; real Node reporter reconciles exact file inventory and execution. Evidence: evidence/native-node-tooling-4c860140.json. Current49a non-regression audit confirms10passing workflows and only known TypeScript/ext-v2 failures, both fixed locally by0045d185d7. UnitCI102287758791 remains live; no publication until terminal. Native Temporal proof process cleanup confirmed.

Run34294404179 is terminal failure (updated2026-09-09T01:32:28Z) with no active jobs. Unitjob102287758791 completed normally:2746files/14986executed tests pass plus3taxpolicyTODOs; the gate correctly rejects TODO completeness. Repository inventory102303317865 passes; final readiness102303409611 fails. The five new pool lifecycle cases execute successfully in clean CI, but retained-ticket browser recovery remains unverified because Temporal readiness prevented bootstrap. Reviewed local batch is ready to publish without cancelling useful CI.

F006/T006 and F007/T007 accepted by requirement-specific audits: 27 native reconciliation/workspace-gate tests and 19 Node25 aggregate cases pass. Completed CI34294404179 tested mergeffb14efc77a861aa4dcb340ff5fd18764c47566b publishes2746expected/executed files,14986passed cases plus3TODO; unit aggregate and final readiness correctly reject incomplete execution. Final readiness artifact10084410917 explicitly rejects failed/missing browser/upgrade/Teams evidence. Expired quarantine proof composes inventory expiry rejection with mandatory inventory gate rejection. This accepts decision/reporting behavior, not F002/F008/F018 or whole-plan readiness. Current b413 TypeScript and ext-v2 checks pass. F024 audit identified missing artifact/project/rerun/configuration reporting; R062–R064 address these while CI continues.

R062–R064 metrics batch:67 combined native behavioral tests pass across6files. Verified archive receipts bind actual bytes to sanitized build records before archive removal; manifests verify exact CE/EE component inventory and loaded image identity, source/run/attempt. Producer and recorder reject invalid supplied provenance as incomplete; optional native runs retain null. Five additive U:Y fields preserve project/rerun/configuration identity; actual appendRows mock HTTP verifies suffix-only header extension and historical-row preservation. Coordinator found and agent fixed CLI output alias deletion, with regression coverage. Actionlint and plan validators pass. Current CI34299806767 predates this batch; live manifest/Sheets readback and full readiness remain unverified.

Canonical native tooling at cleanfa6f6e1cb44c3f61afbb7b462f06270c1fa27a55 passes764tests across79required files,0fail/skip/todo/cancel,in167.6seconds. Initial sandboxed a198 run exposed loopback EPERM plus one real isolated browser-runner fixture missing the new verifier dependency; fixed and confirmed focused test before the full socket-enabled rerun. Before/after source clean and identical. Evidence: evidence/native-node-tooling-fa6f6.json. No Docker run; current published CI predates metrics batch.

Current b413 CI34299806767 background-worker startup passes for both CE102309228264 and EE102309228309; provider topology step fails in both, blocking primary browser execution while independent bootstrap/API phases proceed. Direct active-job log API returns404 and annotations are empty. Static audit identifies build-only email-service config rejected by helper missing image check, despite existing loader supporting generated tags. R065 implements matching image resolution and safe diagnostic phases; exact runtime log confirmation pending.

R065 native repair verified: build-only email-service fixture fails before then passes with generated project-service image resolution;24 topology cases pass. EE config explicitly enables enterprise profile, independently confirmed by native configuration-only Compose rendering and pinned2.36 source inspection; ps -q returns fullIDs as required. Added fixed phase/service diagnostics without raw Docker data. Current CI predates repair and direct logs remain unavailable; no claim of actual all-replica readiness yet.

F037/T031 accepted by bounded protocol-evidence audit: explicit provider capability/model-limit inventory, independently sourced Graph calendar/OAuth/notification contract validators with deliberate malformed payload rejection, Stripe SDK signature/tamper/age validation, QBO OAuth/accounting and Xero contract evidence. Unsupported Graph operations explicitly501; absent live-sandbox verification remains tracked as optional limitation. Recorded native/CI emulator suites cover these unchanged provider tests. PROTOCOL_PARITY.md now distinguishes verified worker/service-class evidence from pending replica checks. This does not accept T028/F034 browser/SSO requirements or full release readiness.

F009 accepted independently of branch protections: mandatory infrastructure execution and aggregate have no continue-on-error, critical five-file floor cannot be excluded, explicit exclusions require owner/reason/issue/future expiry and remain visible. Current CI34299806767 tested merged7d864473db09832671427a71a4220601709376c passes infrastructure aggregate:65required files,497passed,0failed/skipped/todo/pending. Nine native infrastructure/exclusion/expiry audit cases pass. T008 effective protected-branch mergeability and F008 remain unverified; no claim that passing workflow alone enforces merge policy.

Current b413 enterprise CI confirms PostgreSQL capacity exhaustion during upgrade: 237 connection-limit failures overlap the invoice/ticket/usage HTTP500 window; pool ownership is not identified. Citus schema and browser gate pass. Teams credentials succeed, but dashboard remains compiling beyond the navigation deadline; the EE server also attempts to reach a stopped Temporal frontend. R066/R067 fix independently reproduced general/admin initialization and cleanup races: 25 combined native cases and package typecheck pass. R068 preserves phase logs and grouped connection counts before replacement. R070 adds isolated candidate PgBouncer routing with unchanged pool/auth settings, real app-user readiness and cleanup before Citus. R069 retains Temporal and warms an authenticated dashboard before unchanged browser assertions. All 37 focused native readiness/topology cases and independent integration review pass. No local Docker; current unit CI remains active, so publication is held.

Clean native tooling at 082de56a31a324bc3aebaa17c9fd49eea62c2469 passes 781 tests across 81 required files in 167.2 seconds, zero fail/skip/TODO/cancel, identical clean before/after source. Evidence: native-node-tooling-082de.json. F022/T017 accepted from current same-revision Citus migration/runtime and supported-upgrade artifacts: schema and 3/3 first-attempt browser journeys pass, including tenant isolation, usage persistence and invoice accounting; 8 Citus runtime plus 27 Temporal database cases pass. Single-node mixed-distribution limits remain explicit. R060 pinned Temporal health fix is confirmed by actual healthy container status and successful worker startup. Whole CI, PostgreSQL upgrade, Teams, and new local runtime repairs remain separate pending requirements.

Run 34299806767 is terminal failure, updated 2026-09-09T02:48:57Z, with no active jobs. Unit job 102304102871 passes all 2746 files and 14986 executed cases; the completeness gate rejects three tax-policy TODOs. Two-worker registration collection completes; observed pre-execution interval 19m56s versus 27m57s previously is not a controlled benchmark. Independent bootstrap after provider failure now reaches all later phases, including the passing Citus upgrade. Reviewed local follow-ups can now be published without cancelling useful CI.

Current cb884 run34304828149 passes Node tooling and browser discovery but Temporal engine has one concurrent email workflow failure (60 cases pass). Coordinator plus three agents continues: ordinary-client clock repair, independent manifest review (no actionable defects), and stateful upgrade retry repair. Approved-copy browser identity validation passes exact A1:Y8 native readback, preserving legacy header/history and distinct project/run-attempt/configuration identities plus full synthetic manifest JSON. Evidence isolated-browser-identity.json is explicitly synthetic; original workbook untouched. CI remains live; no push or local Docker.

R073/R074 close a concrete missing-export implementation gap with read-only observed-run reconciliation. Three-agent implementation/review plus coordinator integration yields83 native behavioral tests, including collector-to-reconciler CE/EE pass and removed-export failure. Exact runtime identities, safe missing/cancelled outcomes, bounded remote reads, recorder states and legacy schemas are preserved. Manual workflow and actionlint pass; live execution, automatic reporting and original scorecard rollout remain open. No local Docker or original workbook writes. Current cb884 CI remains live with only known Temporal failure; local clock repair remains queued.

Canonical native tooling at clean886b65362d52a9017945891ec5b598478f75636f passes864 tests across83 required files in162.1 seconds, zero failure/skip/TODO/cancel and identical clean before/after source. Evidence native-node-tooling-886b.json. Both new reconciliation files are discovered and executed by the real harness. Current cb884 CI has reached CE/EE browser jobs102324221156/102324221161 after successful image builds; unit jobs remain live. No push while useful CI runs.

Real original browser_readiness read observes563 populatedrows including20columnheader; completed34299806767 has2legacy incomplete runrows. Snapshot replay exposed GitHub run.pull_requests mutable currenthead/base, so collector no longer mistakes those for historical source evidence. Fixed immutable runhead mergeparent relationship and explicit checkout limitation; both legacy exports remain visible but attempt-unverified.86 combined native cases pass. Evidence real-browser-metrics-reconciliation.json records actual source fields and replay limits; original workbook unchanged.

F024/R064 accepted from actualcurrentCI:42CE+47EE first-attempt journeys,clean testedmergef71e845a2c5d66fb634a01daa39161c82dd3e4ff;8/9service archive manifests retained. OriginalSheets exact91row×25column projection matchesbothartifacts, and562historicalrows+20legacyheadings compareunchanged acrossreadbacks. Evidence ci-browser-metrics-cb884.json. No perreplica acceptance yet (recordsawaitfinalcontainerartifact), andAPI/upgrade/Teams/unitCI stillpending. Collector also now toleratesabsentmutablePRassociation whilevalidating immutable runhead parent;33collectorcasespass.

Run34304828149 terminalfailure updated2026-09-09T03:46:42Z; noactivejobs. Bothprimarybrowser suites42CE/47EE pass, andall4CE/5EEconfiguredreplicasverified (R039/R065accepted). Unit2748files/15004casespass but3taxTODOrejected. EErootnpmci failsECONNRESET beforelaterphases; R075retrywrapper9nativecasespasses,retains10mincap andonlyoneclassifiedretry. Temporalclockfix61nativecases alreadyqueued. Finalreadiness102329595262 rejects failures; reviewedbatchreadytopublishwithoutcancellingusefulCI.

R071 acceptedfromcurrenta0d077 CI34308610232:Temporalengine61/61cases12filesfullselection atclean3db700f653e28ddd322dfdb652f1689a54f6b65d andgatepassed. F031foundationacceptedfromboth-edition productionjourneys/processrouting and newnativeconcurrentisolationboundary. TwoactualStripehosts withsameIDsremainindependent whileBcallbackheld andAreset; package27+canonicalserverunit1filteredcase+typecheckpass. T025remainsopenfornewtestpublishedCIverification; no claim of fullparallelAlga browserstacks.
...
Teams lifecycle first native attempt failed before new assertions: profile secret was written to owned filesystem but inherited env-only read chain hid it from readiness. Independent review confirms correct product behavior and CI already uses env,filesystem. Agent retry uses matching isolated chain/root; do not treat first run as lifecycle coverage or product regression. Current CI34313560297 remains live; no publication. Provider evidence audit found per-test emulator observations were not projected into metrics; R089 addresses bounded truthful summary without inventing vendor auth/parity claims.

Reviewed provider projection now records sanitized per-attempt observations bound to clean candidate/validated archive context.18 native checks pass; actual historical CE/EE replay yields2/11 observed provider journeys and40/36 missing-attachment unavailable entries, unchanged after abort/status consistency repair. No overall-pass or OAuth/parity claim. Added Teams UI lifecycle assertions passed natively through delete and Graph/DB readback; existing later bot delivery still fails and is under investigation. Commit these bounded reviewed results; freeze tracked edits for full native Node verification.

Canonical native Node suite on clean unchanged b80be7e884ffb8b2e05ab51870ae199f202b2c5f passed926/926 across85required files,0fail/skip/TODO/cancel,180.79seconds. evidence/native-node-tooling-b80be7.json retains counts, exact file inventory, clean source snapshots and full-report hash. This does not resolve Teams bot delivery or prove current-source CI. Tracked-edit freeze released after completion.

Full native Teams scenario now passed1expected,0unexpected/skip/retry,193.83seconds, including profile recovery, meeting CRUD and prelinked bot tenant isolation. Preserve earlier generic bot fetch failures and changed diagnostics: final live run included GET405 across localhost/IPv4/IPv6 and unauthenticatedPOST401 before signed bot delivery. No root cause/permanent product fix, SSO, repeatability or clean CI claim. Owned resources cleaned. R085 bounded implementation verified; R069 and broad F034/T028 remain pending current CI/auth scope.

Main protection refresh at2026-09-09T05:58:42Z confirms only ext-v2/ESLint and circular-dependency rules enforce checks; fresh-install remains evaluate-only. Prepared exact main-only classic required-status-check PATCH using4observed GitHub Actions15368 contexts and existing strict:true. Proposal preserves oldrulesets and records bypass semantics, stale-state reread and actual mergeability rehearsal requirements; no remote mutation. Activation waits for repaired baseline/current candidate green. Separately R090 closes visiblebucket gap in primary and supported-upgrade journeys; original Citus query error could otherwise remain invisible despite Add Usage passing.

Current561a6 run34313560297 is terminalfailure:15025unitpass,1invoice-template failed-save failure,3taxTODO. Primary42CE/47EE and349API each pass; PG/Citusupgrade3each pass. Teams diagnostic now confirms HTTP200 dashboard-marker-missing, expected to be addressed by unpublished hydrated-browser warmup. R080 accepted on retained-phase pressure observations; new product/UI/runtime tasks R091–R094 preserve outstanding CI validation. Coordinator and three agents continue independent bounded work; no local Docker.

Reviewed native fixes: invoice filter2red then52componentpasses and primarybrowser1pass; project status event named-inputred then9realDBpasses, full server tsc exit0. Matching CI HocuspocusJWT key verified by actual production token consumers for CE/EE and13tests. Invoice-template CI failure reproduced with delayed read and fixed test readiness3/3. No retries or deadlines relaxed. Native upgrade invoice+usage pass twice; ticket first hit devheaprestart, then coldcompile-related deadline; fullsuite warm-page validation remains pending, with all failure history retained.

Final cross-suite discovery review collected4bucketDB,9project-status,3invoice-template and3upgradebrowser cases through actual runner CLIs; required inventories include them. Native full server typecheck passed. R095 addresses separately proven CI marketing-worker publicURL propagation; scheduled background failure predates successful primary browser completion and is not the Teams dashboard cause.

Final native upgrade run2pass/1fail196.18s after bounded authenticated read-only ticket/client precompile. Invoice962.50 and visiblebucketusage assertions pass; ticket120-second deadline now occurs late in secondtenant foreign-ticket navigation. No devmemoryrestart. Stop repeated native runs without new evidence; preserve fixed timeout/retries and fullsuite failure. Owned resources cleaned. Production candidate CI remains required for R090.

Canonical full native Node tooling passed934/934 across85required files in159.85s on clean unchanged d683f3611992fcb5a849c4b4f6de420727e23e21. Zero fail/skip/TODO/cancel. Earlier sandbox attempt879pass/55fail from localhost bind EPERM preserved separately; exactsource suite passed with localnetwork allowed. Reviewed candidate ready for next PR CI publication, not fullplan completion.

Published c523418b5d candidate run34320694357 revalidated live. Early lightweight accounting failed only missingVitest dependency: unchanged runtimecase relocated to installed fullNode lane, no-deps26/26 +runtime1/1 verified. Separate localegate found2newbucketkeys; catalogfixed and exactcheckerpasses. R098 investigates silently ignored Teams reschedulewarning with native behavioral tests. CurrentCI untouched. Concurrent Stripe isolation newproof from preceding561run now verified1collected/1pass clean9ba03b9, preserved in ci-concurrent-provider-isolation-561a6.json; not simultaneousfullAlga stacks.

Independent and coordinator T015 review confirms exact simulated-runtime wording is satisfied by existing behavioral/CLI tests and current934test native suite. Mark only T015implemented with explicit simulation evidence; F020 actualpipeline/promotion/deployment remainsfalse. This corrects underreported test acceptance without narrowing release rollout requirements.

T025 accepted compositionally after actual CI verified previously pending concurrent-isolation assertion: production-built authenticated provider journeys and callback/DB effects, independent per-replica routing and blocked fallback evidence, and two real HTTP emulator scenarios with colliding IDs, held callback, signed delivery, independent credentials/faults and reset. Exact concurrenttest source reviewed and CI1/1passed. This proves concurrent provider-fixture isolation, not simultaneousfullAlga stacks or vendorSSO/parity; broad readiness/pipeline flags remain unchanged.

R098 native fullTeams denial/recovery journey firstattemptpass1/1 in121.44s,0retry/skip/flaky; wrapperandbrowser sourceunchanged. Actual Graph403/localcommit warning, unchangedprovider/meeting then reopenedjoinidentity and successfulconvergence/delete/bot. Cross-review strengthenedcomponenttests toassert reopenednewtime and rejectedresize rollback/no-warning;11pass, scopedtsc exit0. Ownedresourcescleaned. Actual publishedNodeartifact10091920517 proves934/934 and85exactrequired files oncleanmergecec08351189624fb4db8db7ec453e788157fbdad. Overallrun34320694357 stilllive, no push.

Current Citus artifact verifies4bucketcases and12totalruntimepasses, completing R083 single-nodequeryrepair. Nx affectedunitjob nowfailedbilling3 andXero1: package3 reproducedfrommissingmatcher/one-shotmockleak, fixedexplicitsetup with10passes underbothrunners. Xero5sCI timeout not locallyreproduced; nativeprofile487ms/553ms spentdynamicclientimport, movedtostaticcollection and23packagecasespass preserving5sdeadline. R101 actualMicrosoftNextAuth signedauthority/callback harness begins withoutproductionendpointoverride orliveconsentclaim.

Actual MicrosoftNextAuth nativecallback passed using test-only canonicalauthoritytransport and real app routes/options: correcttenant/user/email session, exactlyone Microsoft user_auth_accounts link keyedbyoid distinctfromsub, freshwrongstate refused beforeadditional tokenexchange with no session/linkmutation. Authority enforcedPKCES256. ObservedJWKS0 andnoncefalse; signedfixturetokens do not prove signature/nonceverification. Earlierdiscovery/authmethodfixture failures retained; allownedresourcescleaned. DurableCLI/CI registration remains R101; helpersalone not recurringlane. Evidence native-microsoft-nextauth-callback.json usesactualdc1d23a98e source, dirty.

Reusable Microsoft callback CLI passed with ownedfixture/process/DBcleanup; originaltestedhashes preserved in native-microsoft-nextauth-callback-cli.json. Coordinator wired existingEEimage one-shotCI lane afterTeams, read-only sourcebinds and independent rawreport/source/image/mount/exitcode reconciliation; no localDocker. Cross-review found cancellationcleanup gap; subsequent real subprocess SIGTERM/SIGINT tests fix it without claiming earliernativefullcallback retested. R101 andF034/T028 remainpendingactualcurrentCI.

Canonical native Node tooling passed997/997 across89required files oncleanunchanged a1127052371b27a3558b1902156c177a51ec5148 in161.98s; zero fail/skip/TODO/cancel. native-node-tooling-a112705.json. CI34320694357 remainslive onc523: CE42firstattemptpass, EE45firstattempt+2calendarretrypasses, shard2all476pass including9projectstatuscases. Calendar localmapping immediateassertion races subscribercommit afterremoteDELETE becomesvisible; retain exactemptyrequirement via boundedpoll. This is independentofprimaryinvoice/bucketbehavior.

R103 closes reporting-only providerobservation gap: freshinstall independently applies committedfulljourney/provider policy to rawreports and verifiedarchive context. Currentc523 CEreplayfails solelyunsupportedSMTP; EEtwoflakycases remainnon-green and HTTPjournals otherwisecomplete. Actual SMTPDATA journal native2red before, now7/7SMTP plus28/28host and both typechecks. Metadata-only250/rejection/abort and pending/drop/reset evidence; no fakeHTTP, recipient/body capture or preDATA claim. Providerhelper/freshgate/metrics19pass. FullcurrentCI/releaseenforcement and F036/T030 remainopen.

Canonicalnative93e79e1004/1004 across90required files, cleanunchanged before/after,161.50s;0fail/skip/TODO/cancel. R103localimplementationcomplete, current-sourceCIpending. R090nowverifiedactualproductionPG/Citusupgrades3/3each+schema oncleancec083 forpublishedc523 withvisiblebucketassertions; EEAPI349/349. Doesnotprove standaloneHocuspocusfixwithoutcase/logevidence. Fullplan remainsopen; preserveactiveCIbeforepush.

Currentc523serverunit15031pass/1locale-fail/3taxTODO; localeexact5case nativepass confirmsalreadyqueuedcatalogfix. Shard1 survey60sbootstrapdeadline failedbefore3cases; selfcontainedfixture nowomitsdevseeds, ownsDB/alignedenv andawaitscleanup. Native3/3pass11.75s; historicalnative/sharedrolelimitationsrecorded ratherthanclaimingisolationforallattempts. Teamswarmdashboardpasses, actualtestfailssettings90s withSyntaxError/ChunkLoadError on80sHTTP200layoutchunk; bodymissing, no speculativetimeout/heapfix. R105 awaitsfinalserverdiagnostics.

R106 bounds Dockerdiagnosticclients withkill-after, preservespartiallogs/exitcodes, captureshostmemory/disk independentlyandprioritizesserverlogs. Currentrunstillauthoritativelylive; no cancel/push. Currenttestlocale5/5nativepass, existing3taxTODOs reconfirmedanduseraskedforactualrulesorownedexpiringdeferral. Teamsactualchunkcauseawaitsserverlogs; do nottransformnavigationdeadlineintoapass.

R105 closed by decision rather than by tuning. Causal chain established: Teams emulator overrides are denied under NODE_ENV=production by design (emulatorMode.ts gates where the bot secret is POSTed and whose JWTs are accepted), forcing the journey onto a development server; that server compiles an 81.7MB app/msp/layout chunk, crosses Next's used_heap > 0.8 * heap_size_limit and restarts mid-response, delivering HTTP 200 whose gzip body decodes to an exact 46,727,168-byte prefix of the valid 81,713,954-byte chunk. CI lane already at 12GiB so no runner headroom remains; guard not touched. Captain chose the PRD's quarantine option over chunk reduction and over tenant-config endpoint resolution.

F009's quarantine had no implementation until now. scripts/lib/quarantine.mjs plus a committed scripts/lib/quarantine.json registry: a quarantined requirement still executes, uploads and is reported, but stops vetoing readiness while its entry is valid. Hard failures for missing owner/reason, absent or malformed expiry, passed expiry, unknown requirement, duplicate entry, and any attempt to quarantine a P0 journey; fresh-install-execution-gate marked p0Journey. Readiness CLI prints quarantined outcomes with owner and expiry ahead of the failure list, and the committed registry is itself covered by a test that fails once an entry goes out of date. The CLI fixture writes an empty registry so it keeps exercising full enforcement. 27 cases pass; 64 of 71 script suites pass, the 7 failures being pre-existing absent-node_modules environment failures untouched by this change. teams-development-execution quarantined to 2026-10-21 under F034/T028. Teams coverage deferred, not obtained.

F008 prepared but deliberately not applied. production-regression.yml is absent from origin/main and thirteen other PRs are open against main without it; a required check a branch never produces is never reported and never satisfied, so enabling before merge would block all thirteen. scripts/readiness-enforcement.mjs refuses to enable while the workflow is off origin/main, adds its own ruleset rather than editing the two active guards, and rolls back in one call. Live readback confirms only ext-v2/ESLint and circular-dependency rulesets are active; fresh-install-e2e, code-review and tenant_management sit in evaluate and gate nothing. Release Bot bypass and non-strict freshness documented in f008-enforcement.md rather than left implicit. T008 remains open until a deliberately failing candidate is observed to be refused.

Three tax it.todo placeholders removed after checking each against the product: international rates are region codes already covered by rateSelection.db; tax caps have no concept in the tax service or schema (cap_amount is project billing); period-spanning tax has nothing to exercise since calculateTax resolves a single date. None were missing tests for existing behavior. Caps and period-spanning tax carried to board card 8d576a75-24b2-42cd-9758-f6a9e1aa9d76 as a build-or-decline decision, not restored as placeholders.

Merged origin/main after 68 commits of drift. This also explains a stretch of pushes that produced no CI at all: these workflows run against refs/pull/3343/merge and GitHub cannot build that ref while a branch conflicts, so the absence of runs was the conflict, not a delivery failure. One textual conflict in AccountingMappingDialog, resolved onto main's SearchableSelect prop name while keeping this branch's stale-target clearing. Correction to that merge commit's message: it claims this branch's onValueChange would never have fired, which is wrong -- CustomSelect takes onValueChange, so the branch was correct for the component it used, and the prop only became wrong once main swapped the component.

The merge broke five cases git could not show. AccountingMappingDialog.targetKind.test.tsx did not conflict, but main replaced CustomSelect with SearchableSelect and the test still drove the old component through document.getElementById plus keyDown. Rewritten to match main's own passing search test: open by the trigger's visible placeholder, click the option. Verified locally at 844/844 across 100 files against CI's 5 failed / 839 passed, plus a clean package typecheck.

Server unit on the merge commit reported 5 failed / 15051 passed with no locale failure and no TODOs, so both the translation repair and the tax placeholder removal are confirmed in CI. CE browser passed, confirming the SMTP replay repair. All nine image builds, both upgrade browser gates, Citus, Temporal, every workspace shard and all integration and infrastructure shards passed.

The enterprise Playwright step failed with 46 passed and 0 unexpected: one retry-only pass, which F017 refuses to let satisfy the gate. invoice-designer-persistence timed out at 124.1s and passed on retry at 116.5s; the previous run's artifact shows the same case passing first attempt at 119.067s against the 120s default. It is a chronic coin flip at that boundary, not a merge regression, and it is the largest spec in the suite and one of the few heavy ones that never carried an explicit budget while twelve peers use 300s. Given the same budget, it now sits at 41% utilisation and no other spec exceeds 19%, so no comparable timeout flake remains.

[SUPERSEDED IN PART -- the viewport explanation below is wrong and was reverted; see the correction at the end of this file.] Turbopack resolved the Teams chunk truncation outright. No SyntaxError, no ChunkLoadError, no memory restart; the lane starts, authenticates, warms the dashboard and runs the journey for 74.8s. The 81.7MB single chunk was a dev-webpack artifact, and every earlier mitigation had stayed inside webpack while both next configs already treated Turbopack as the primary path. Underneath it was an ordinary automation problem: the Microsoft profile dialog overflows the 1280x720 window instead of scrolling internally, so its first capability checkbox is below the fold and Playwright reports it visible, enabled and stable but outside the viewport. Only this lane is affected because it forces release-v1-6-feature on an enterprise development build. Lane moved to 1280x1200, verified by launching a real browser under that config rather than by reading it. Forcing the click was rejected as asserting on an element a user at that size also could not reach; the overflow is tracked separately as a product question.

A false success was nearly recorded here. The Teams step reported completed/success while its verdict artifact said status failed with counts passed 0 failed 1, because continue-on-error masks the step conclusion. Read the verdict, never the step.

First live exercise of the F009 quarantine, and it behaved to specification: teams-development-execution reported quarantined-failing with owner and expiry, its own failures still visible, contributing nothing to the top-level failure list, while every genuine failure still vetoed and the P0 fresh-install journey floor vetoed as designed. supported-upgrade, supported-citus-upgrade and microsoft-callback failed only with "Required workflow: failure" -- their own verdicts were sound and they clear once the browser job passes.

Open question deliberately left for the enforcement rehearsal rather than guessed: thirteen pull requests are open on branches predating this work, and whether they report the required check after merge depends on whether GitHub reads pull_request workflows from head or from the merge of head and base. An attempt to settle it from existing check-runs was inconclusive because those records are historical. The rehearsal now opens a disposable pull request from a pre-merge commit to answer it by observation.

Correction to the entry above: the viewport explanation was wrong and the 1280x1200 change is reverted. A taller window reproduced the failure identically. Two further theories also failed: it is not the component (plain visible input with a proper label association) and it is not CSS delivery (every stylesheet returned 200, local Next chunks and the unpkg-hosted Radix and react-big-calendar sheets alike). The failure screenshot shows Radix and component styling present while layout utilities are absent, so the page renders as a long unstyled flow with no dialog for the control to sit inside, which is exactly what "visible, enabled and stable but outside of the viewport" describes. Working theory is that Tailwind is not processed under Turbopack in this lane; that is a theory, not a finding. Net bundler position: Turbopack traded a memory-dependent chunk failure for a deterministic styling one, and the lane stays on Turbopack because making Tailwind work in a development lane is far more tractable than reducing an 81.7MB webpack chunk. Forcing the click was rejected at every step.

Worth naming the pattern: three confident explanations in a row, each discarded by the next piece of evidence, and one of them was acted on before it was checked. The screenshot settled in one look what the logs had not in three attempts. Look at the artifact before theorising about it.

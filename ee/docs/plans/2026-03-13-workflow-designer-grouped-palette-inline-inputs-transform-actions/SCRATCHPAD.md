...
- (2026-03-13) The next checklist item (`F079` / `T079`) remains blocked on scope clarity: the current workflow designer has no existing step duplicate/copy affordance to preserve, so there is no baseline behavior to validate or carry forward without inventing a new feature beyond the current slice.

...
- Citus FK hazard: `users` is distributed inline in its CE migration (`server/migrations/20251108123000_add_service_category_audit_columns_v2.cjs`) — correction: this used to cite `ee/server/migrations/citus/20250805000006_distribute_users.cjs`, but that folder was never executed and has since been removed; an FK from a not-yet-distributed table to `users` fails. `user_cost_rates` therefore has NO FKs (convention: `20251110223310_create_appointment_requests.cjs`, incl. `exports.config = { transaction: false }`); integrity in the model layer.
...
### 2026-07-02 — Final report test closure and build

- Added report action coverage for uncosted/unattributed labor at summary/client/agreement levels (T037/T048/T098), line-detail reconciliation (T050/T091), zero-hour fixed allocation residual behavior (T058), tenant-bound raw query isolation (T062/T063), unconverted revenue (T087), unbilled/uncosted materials (T088/T089), legacy NULL-`item_id` residual behavior (T092), no-covering-client-contract unattributed cost (T094), interaction/appointment client attribution (T095), zero-time revenue EHR behavior (T083), large fixture smoke (T084), and a comprehensive hand-computed fixture across hourly/fixed/material/ticket flows (T082).
- Added static source guards for inclusive date boundaries, revenue-by-invoice-date/cost-by-work-date timing, effective-date rate resolution, and deterministic single-row rate resolution (T065/T066/T068/T090/T097).
- Added `invoiceService.static.test.ts` to guard the localized invoice generation contract for `invoice_time_entries.item_id`; the broad invoice generation integration path previously hung, so the regression is covered by focused source contract plus the existing item-link tests (T028).
- Existing `userCostRate.test.ts` covers the advisory-lock-before-overlap contract used to serialize concurrent writers; this is the meaningful unit-level coverage for T096 without standing up concurrent database transactions.
- Full root build completed successfully: `npm run build` passed after dependency builds and Next/Turbopack production build. Build emitted existing broad dynamic-file-pattern warnings in documents/extensions tracing, but exited 0 (T081).
- Commands run: `cd server && npx vitest run ../packages/billing/src/actions/profitabilityReportActions.test.ts ../packages/billing/src/actions/profitabilityReportActions.static.test.ts --coverage.enabled=false`; `cd server && npx vitest run ../packages/billing/src/services/invoiceService.static.test.ts --coverage.enabled=false`; `cd server && npx vitest run ../packages/billing/src/models/userCostRate.test.ts ../packages/billing/src/models/userCostRate.behavior.test.ts --coverage.enabled=false`; `npm -w @alga-psa/billing run typecheck`; `npm run build`.
- F056 completed: plan artifacts now reflect implemented work, tests, commands, and the remaining build-script caveat for standalone `@alga-psa/reporting` build despite full root build success.

### 2026-07-02 — Post-review fixes (i18n keys, material double-count, allocation window)

- i18n (AC9): all 62 report strings were keyed `profitability.*` in `ProfitabilityReport.tsx` while every locale pack nested them under `contractReports.profitability.*` — every key missed and rendered English defaults in all languages. Component re-keyed to `contractReports.profitability.*` (66 call sites); `find-missing-i18n-keys.cjs` now reports zero missing keys; `validate-translations.cjs` passes with 0 errors/warnings.
- Material revenue double-count (D13): the engine emits `invoice_charges` rows for billed materials AND stamps `billed_invoice_id` (invoiceGeneration.ts ~2217), so summary/client/agreement revenue counted the money twice (revenue CTE + material-fact `qty×rate`). Material-fact revenue is now excluded at summary/client/agreement grain (the charge row is authoritative) and kept only at ticket grain, where charges carry no ticket linkage. Mocked fixtures updated to model the engine's material charge row.
- Allocation window (D8/F036): fixed/bucket ticket allocation weighted by labor facts fetched for the *report range*, so an arrears invoice (June invoice, May service period) never found its weighting hours and silently stayed an agreement residual. Added a dedicated `ticket_allocation_weights` query spanning the union of charge windows, allocation charges/exact facts now carry `client_contract_id`, and `getTicketProfitability` surfaces revenue-only ticket rows (with ticket metadata from the weights query) for tickets whose hours fall outside the report range — consistent with D5's revenue-by-invoice-date basis.
- Verification: billing typecheck clean; 37/37 mocked action/static tests; 6/6 ProfitabilityReport component tests; new real-DB integration suite `server/src/test/integration/billing/profitabilityReporting.integration.test.ts` (12 tests incl. an arrears-allocation regression and a count-once material regression) — runs in `test:integration:tier1` via the `src/test/integration/billing` glob.

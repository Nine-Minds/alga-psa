...
- Agreement-level "Unattributed" row per client collects everything with no agreement mapping: time entries with NULL `contract_line_id`, time entries whose contract line's contract has no covering `client_contracts` row (§7.4), and material cost (D13). Production audit (2026-07-02): ~60% of existing time entries have NULL `contract_line_id` — Unattributed is the *common* case on real data today, not an edge case; the UI must present it neutrally, not as an error state.
...
Generation is strictly **1 time entry : 1 charge** in the current engine (`calculateTimeBasedCharges` emits one `ITimeBasedCharge` per entry; consolidation exists only for fixed charges, which never call `linkAndMarkSourceBillingRecord`). New link rows are therefore exact 1:1. Pre-existing invoices whose link rows have NULL `item_id` remain at agreement level (D8 — legacy rule). Report queries nonetheless tolerate N link rows sharing an `item_id` defensively (by-`billable_duration` apportionment, no double counting) in case future engine changes introduce consolidation.
...
- **Hourly (legacy, NULL `item_id`):** stays at **agreement level** as a tagged residual — no per-ticket apportionment. Production audit (2026-07-02): exactly 2 invoices in 1 tenant have `invoice_time_entries` link rows at all, so the previously specified pool-identification heuristic (required because `invoice_charges` has no charge-type column; a wrong pool silently corrupts historical ticket revenue) was all risk and no payoff — cut. Installs with meaningful legacy volume see those invoices at agreement grain, clearly tagged; ticket-level revenue is exact from the first post-upgrade invoice.
...
`sent`, `paid`, `partially_applied`, `overdue`, plus legacy statuses `open`, `completed`, and `Unpaid` — none of these are in the current `InvoiceStatus` type but they exist on older rows (`open`/`completed` are included by every existing revenue filter, `revenue.ts:102`; `Unpaid` found on 3 production invoices dated 2024-09, evidently imported); dropping them would shrink reported revenue on old tenants versus today's reports. Excluded: `draft`, `cancelled`, `pending`, `prepayment`. Credits reduce cash, not earned revenue — `credit_applied` is ignored for margin.
...
3. `getAgreementProfitability({start, end, clientId?})` → per-`client_contract` rows + "Ad-hoc / manual" + "Unattributed" rows per client; each row optionally expandable to per-`contract_line` detail. Per-line revenue: `invoice_charge_details.config_id` → `contract_line_service_configuration.contract_line_id` for detail-bearing charges; `invoice_time_entries.item_id` → `time_entries.contract_line_id` for hourly; charges resolvable to neither stay at agreement level (line breakdown shows an "unassigned to line" residual so rows still sum to the agreement). Cost/hours: `time_entries.contract_line_id` → `contract_lines` → `contracts` → `client_contracts` joined on `contract_id` (+ tenant) — the engine's canonical replacement for the **dropped** `client_contract_lines` table (dropped in `20251207140000_drop_redundant_client_contract_tables.cjs`; pattern: `fetchDiscounts`, `billingEngine.ts:4640`). **De-duplicated**: `client_contracts` is a junction table, so if multiple rows ever exist for one `contract_id`, pick the one whose date range covers the entry's `work_date`, tiebreak by latest `start_date` then `client_contract_id`, so each entry's cost lands in exactly one agreement (production today: 0 duplicate and 0 NULL `contract_id` rows — the de-dup is defensive). An entry whose contract line's contract has no `client_contracts` row has a line but no agreement — its cost folds into the client's "Unattributed" row so agreement reconciliation still sums.
...
| Legacy-invoice ticket revenue (NULL `item_id` rows) | Kept at agreement level as a tagged residual (production audit: only 2 such invoices exist); no apportionment heuristic to get wrong |

...
- **(2026-04-09)** `ContractOverview.tsx` has a local `formatFrequency()` that maps frequency keys to display strings ('Weekly', 'Monthly', etc.). These should use `t()` rather than a local map. Same pattern in `ContractHeader.tsx`.

...
## Smoke Test — merge round 73ba75100c (2026-08-17 evening, agent:Smoke Test)

NOTE: hub CLI socket (/tmp/ghostty-pane-ide/cli.sock) vanished mid-run in a /tmp wipe — workflow-add-fact unreachable; facts recorded here instead.

- MERGE-ROUND BLOCKER (mainline-inherited): EC BKT Jul-2026 window generation fails on merge tip 73ba75100c with `Internal error: recurring invoice detail ... is missing servicePeriodRecordId` (assertRecurringPeriodLinked, packages/billing/src/services/invoiceService.ts:96; requireRecurringServicePeriodLinkage defaults true). Pre-merge 4a0fac8a90 generated the SAME window fine (INV-000039, 13:06 UTC). Bisection: fails identically with (a) restored pre-generation state and (b) my test entry deleted — trigger is neither. billingEngine/compute*/invoiceService diffs vs main tip 1d8f6beb79 show ZERO functional changes on the recurring-timing path (profile columns + comments only) ⇒ defect inherited from main (PR #3175 merged all-red), NOT a merge-resolution fault. Repro: /msp/billing?tab=invoicing → Emerald City BKT Ready Jul window → Generate → [billing.recurringBillingRun.invoiceFailure] in server log.
- Smoke Replenish (2-profile client) "Smoke Advance Line" Jul window: generation produced no error AND no invoice (zero-suppressed or silently skipped) — needs its own look.
- glinda@emeraldcity.oz password on shared dev DB 6472 is now `SmokeMergeRound2026!` (pbkdf2 salt:hex, pepper NEXTAUTH_SECRET=dummy from server/.env.local, 10000 iters/64/sha512). Banner passwords stopped matching because SIBLING dev servers from other worktrees (ports 3330+3983, started 15:28) share DB 6472 and rotate glinda at their boots. On banner mismatch, set the hash directly.
- /tmp was wiped mid-smoke (~20 screenshots + all scripts lost, twice). Durable evidence now at /home/robert/alga-smoke-evidence/billing-profiles-merge-20260817/ (screenshots + results.json + run.log). Do not stage evidence in /tmp on this rig.
- POST-MERGE UI VERIFICATION (all PASS on 73ba75100c): profiles card renders EC 3 profiles; Billing→Contracts sub-tab (after "Show all") shows Billing Profile column with Smoke Bucket line → "Emerald City — Preschool"; ticket TIC1006 picker shows School; time entry via UI resolved contract banner "Smoke Bucket Line (Bucket)" and persisted contract_line_source=explicit, billable 60; Spend-by-profile card renders (empty state) on EC, ABSENT on 1-profile client (D6 regression PASS); Create Client regression PASS (client 77453d34 "Smoke MergeProfile Client 20260817" + default profile + tax settings created).
- Dev-DB mutations left in place: INV-000039 DELETED (bisection) and NOT regenerated — EC Jul window back to unbilled, entries 6eef6b8e/315f942e invoiced=f; new EC DRAFT entry bff61a83 ("consolidated evidence") unbilled, bills in Sep window; new client 77453d34 left as artifact; "Emerald City — Annex" profile created+deleted via UI (exercise only).
- alga-dev browser panes were NOT drivable from this agent run (every pane id rejected: "belongs to a different host"; list-browsers empty under any scope). Drove the real UI with Playwright + system Chrome (/usr/bin/google-chrome) against :3419 instead — same real browser fidelity, disclosed step-down from the alga-dev card-space tab.

## Smoke Test round 2 — merge tip 73ba75100c deep-dive (2026-08-17 late evening, agent:Smoke Test)

Board socket still down; facts recorded here. Evidence: /home/robert/alga-smoke-evidence/billing-profiles-merge2-20260817-181302/ (live screenshots 01-22, captured action payloads/responses, run logs, results JSON).

- DEFECT #1 ROOT CAUSE PROVEN (mainline-inherited, PR #3180 will ship it to main users): EC BKT Jul-window "missing servicePeriodRecordId" = main's weighted-burn computeBucketCharges stamps bucket OVERAGE charges with servicePeriodStart/End + billingTiming + config_id (bucket_id) but NEVER a servicePeriodRecordId (computeBucketCharges.ts:444-467, identical on main 1d8f6beb79). invoiceService.ts other-charges loop then: requiresRecurringServicePeriodLinkage=true (line id present) -> shouldPersistDetail=true (serviceId+config_id+period fields) -> link returns 0 (early-out on null record id) -> assertRecurringPeriodLinked throws (invoiceService.ts:103-107). Whole generation tx aborts; verified live 3x via UI (2026-08-17 22:13 etc) + engine harness dump (bucket charge $600 overage, no record id). Both colliding sides came from main (assert from #2976, emission from #3175); invoiceService diff vs main is profile-columns-only. Any main tenant with a bucket overage in a persisted recurring window hits this. Trigger = overage charge present (pool-covered-only periods emit no charge; INV-000039 pre-merge had no bucket charge which is why it passed).
- DEFECT #2 (mainline-inherited, blocks whole-client recurring generation): assertClientCadenceWindowFullyMaterialized (invoiceGeneration.ts:967) lacks cl.is_active filter; materializer BY DESIGN creates zero rows for inactive lines (proven by test clientBillingCycleRecurringServicePeriods.test.ts:259 expecting 0 rows for 'Inactive Line'). Any client with an inactive recurring contract line therefore permanently fails every client-cadence generate with "Recurring service periods were not materialized for this recurring execution window." Verified live on Smoke Replenish (line 20260807-...-0005 is_active=f, 0 rsp rows): PO dialog -> Allow overages -> Continue -> that error, 20ms, pre-engine. Byte-identical on main + pre-merge + merge. Fix: add .where('cl.is_active', true) to the activeRecurringLineRows query.
- MYSTERY SOLVED (prior pass's "Replenish silent no-op"): it was Defect A — PO-overage dialog "over by $NaN" BLOCKS generation awaiting Allow/Skip choice; prior script never clicked through. The $NaN here fires even though the contract has NO PO (po_amount NULL): getPurchaseOrderOverageForSelectionInput returns an action-error object (client lacks tax settings -> preview tax fails) which the truthy-object guard passes through to formatCurrency(undefined). Defect A is fixed on a DIFFERENT branch (feature-po-overage-dialog-nan...), NOT in this branch/PR #3180.
- Engine-side D4 re-verified ON MERGE TIP (read-only harness, real BillingEngine against dev DB): EC Jul window charges carry billing_profile_id=1d6444e9 (Preschool), billing_profile_source=contract on BOTH the hourly time charge and the bucket charge - contract assignment beats the ticket's School profile. Persist/UI-level D4 remains blocked by defect #1 on this tip.
- Merged-surface spot checks PASS on 73ba75100c: profiles card renders all 3 EC profiles (Default/Preschool/School), Contracts sub-tab shows Billing Profile column with Smoke Bucket -> "Emerald City — Preschool", Spend-by-profile card present; failed generations roll back cleanly (rsp stays generated, no invoice rows); reconstructed server/src/lib/jobs files load fine ("Scheduled jobs initialized", 0 SyntaxError in server log).
- Read-only repro harness for both defects preserved at evidence dir (repro-harness.vitest.test.ts; run from server/ with REPRO_CLIENT=replenish|ec). It runs the REAL merged engine against shared dev DB 6472 - no writes.

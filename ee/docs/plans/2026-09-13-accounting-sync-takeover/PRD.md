...
Final mitigation: manual Xero exports must select the connected organisation when no Xero default has been saved (including when the saved global default belongs to QBO). Use the same selection outcome as settings and catalogs; retain ambiguity rejection, explicit targets, and reject disconnected exports before creating an unusable batch. Validate the composed selection-to-batch behavior and retain the real DB export/reconciliation acceptance checks.

...
Extend inbound email processing so that:
...
- Support provider paths currently in scope for inbound email processing (Gmail, Microsoft, MailHog test path).
...
2. Workflow extracts embedded image payloads (data URLs and referenced CID images).
...
2. Workflow extracts embedded image payloads and persists them as ticket documents.
3. Workflow stores original reply email MIME as `.eml` on that ticket.
...
- Add a dedicated original-email processing action that downloads/builds MIME bytes and persists one `.eml` document per inbound message.
...
- Persist all workflow-generated files under system attribution (same as existing inbound attachment path).

...
- **(2026-09-23) Stale-URL race fix: a list that mirrors filters into the URL owns the single write (`ownsUrl`).** Next 16 (16.2.12) patches `history.replaceState` so every null-state call dispatches an `ACTION_RESTORE` in `startTransition`; applying a view used to write filters (carrying the *old* view) and then `view=<new>` as two writes, while `fetchTickets` dispatched an `ACTION_SERVER_ACTION` from the same tick. The server-action reducer captures `state.canonicalUrl` when it starts, so an interleaving could commit the earlier URL and `HistoryUpdater` re-canonicalised it 100–450 ms later, showing the previous view. Tickets and Projects now pass `ownsUrl: true`; their `onApply(next, meta)` builds one URL (filters + `meta.viewId`) and does one `replaceState`, and `useListViews` does not append a second `view=` write on apply/clear/delete. Lists whose filters live in React state (Clients, Contacts, Assets) keep the hook's `writeViewParam` fallback. Verified: 40 live cycles across 3 ticket views showed 0 mismatches (9/40 before the fix), the real-router Playwright test `named-list-views-tickets.playwright.test.ts` passed (30 applies + reload), and Projects/Clients/Contacts/Assets passed apply/Default/reload/delete on :3768.
- **(2026-09-23) Round 2: the action-queue race is closed by the single write; no further production change needed.**
  - **Task 1 conclusion — sound.** One `replaceState(null, …)` followed by `fetchTickets` in the same tick cannot leave the older `canonicalUrl` committed, because Next's action queue runs the enqueued server action *after* the pending restore commits, and the server-action reducer snapshots `actionQueue.state` at that point:
    - `history.replaceState(null, …)` is patched in `node_modules/next/dist/client/components/app-router.js:268-279`; it calls `applyUrlFromHistoryPushReplace` (`:237-247`), which runs `startTransition(() => dispatchAppRouterAction(ACTION_RESTORE))` synchronously. `startTransition`'s callback is synchronous, so the restore is enqueued before `fetchTickets` (a `'use server'` import, `packages/tickets/src/actions/optimizedTicketActions.ts:3726`) reaches `callServer` (`app-call-server.js:14-26`).
    - `createMutableActionQueue` defines `action` as an **async** function (`app-router-instance.js:169-172`), so `runAction` (`:71-103`) updates `actionQueue.state` in a `.then(handleResult)` **microtask**, not synchronously. The server action is therefore enqueued behind the still-pending restore.
    - For `ACTION_RESTORE` there is no deferred promise (`:113`), and a restore discards whatever was pending (`:143-154`). With only one write there is nothing to discard, so `runRemainingActions` (`:49-70`) never advances past the pending restore. When the restore commits, it starts the queued server action with `prevState = actionQueue.state` = the restored URL.
    - `serverActionReducer` snapshots that URL at `server-action-reducer.js:70` (`fetch(state.canonicalUrl, …)`) and `:264` (`currentUrl = new URL(state.canonicalUrl)`), so its settled state carries the new URL. `HistoryUpdater` (`app-router.js:38-71`) writes that same `canonicalUrl` with an `__NA` state (which the patched `replaceState` short-circuits), so the address bar keeps `?view=`.
    - The original defect needed a **second** restore: `R1 (filters+old view) → S → R2 (view=new)`. `R2` discarded `R1`, and `R1`'s discarded `handleResult` ran `runRemainingActions`, which advanced `pending` to `R1.next = S` and started `S` **before** `R2` committed. `S` snapshotted the pre-apply URL, was never discarded, and re-canonicalised it when it settled. `ownsUrl` removes that second restore.
  - **Task 2 — no gaps.** Every `updateURLWithFilters` call site in Tickets and Projects is a single `replaceState`: apply (`TicketingDashboardContainer.tsx:1045`, `Projects.tsx:429`), filter/search/board change (`:864`, debounced fetch at `:884-887`), paging/sort/page-size (`:795`, `:803`, `:909`, `:776`), and Default view/clear (via `onApply(next, { viewId: null })`). Each writes once and, where it fetches, writes before dispatching. The hook's fallback `writeViewParam` is disabled for these lists (`useListViews.ts:179-205`), so no path produces `R1 → S → R2`.
  - **Task 3 — tests.** Added `T-FILTER` to `server/src/test/e2e/named-list-views-tickets.playwright.test.ts`: 12 plain filter changes (search typing) plus a Default-view clear, each followed by a 3 s poll of `location.search`. It runs against the real Next router.
  - **Not verified live:** the :3768 dev server was down (only `alga-psa-local-test` postgres/redis were up) and the work order forbids starting it, so the Playwright suite (T-URL and T-FILTER) was not executed this round; it is written and typechecked. Static checks: `packages/list-views` typecheck + 25 unit tests, `packages/tickets` typecheck + `urlSync.contract` (6 tests), `server` typecheck, and `listViews.integration.test.ts` (7 tests) all pass. Other lists were re-checked by wiring only (Projects owns its URL like Tickets; Clients/Contacts/Assets keep the one-write hook fallback).
- **(2026-09-23) Round 3: re-audit confirms no further production change; T-FILTER gains a reload assertion.**
  - **Tickets write paths — one write per action, write-before-fetch.** `handleListViewApply` (`TicketingDashboardContainer.tsx`) clears the debounce, then `updateURLWithFilters(next.filters, 1, next.pageSize, 'replace', meta.viewId)` (one `replaceState`) and only then `fetchTicketsRef.current(…)`. Every other `updateURLWithFilters` caller is likewise a single `replaceState` that precedes its fetch: plain filter/search (`:864`, debounced 300 ms before `:884-887`), paging (`:795`), page size (`:803`), sort (`:909`), the remembered-page-size effect (`:776`). Default view and delete-active-view both route through `onApply(next, { viewId: null })`, i.e. the same single-write apply. Board-tab moves use `pushState` once (`:425`).
  - **Projects write paths — same shape.** `handleListViewApply` (`Projects.tsx:429`) clears the 300 ms debounce and writes once; plain filter/search (`:328`) is the only other writer. Projects filters client-side, so no server action shares the tick.
  - **No stale-state effect rewrites the URL.** The remembered-page-size effect (`TicketingDashboardContainer.tsx:757-784`) is guarded by `hasExplicitPageSizeInUrl` and by `normalizedPageSize === pageSize`, and `useUserPreference.setValue` updates its value synchronously (only the server save is debounced), so it cannot fire a second write after an apply. `syncFromUrl`/`popstate` set state only.
  - **Clients, Contacts, Assets are safe on the `writeViewParam` fallback.** None of them mirrors filters into the URL — their filter state is React state (`ClientListView`/`Contacts`/`AssetDashboardClient`), and their `onApply` only calls setters. So `applySummary` produces exactly one history write (the hook's `writeViewParam`), and because `onApply` triggers any refetch in a later effect it cannot run a server action ahead of that write. Their only other history calls are navigations (`router.push` to a detail) or the Assets quick-add `intent` write, which passes a state-bearing `window.history.state` (Next short-circuits it, no `ACTION_RESTORE`); neither touches `?view=` or the filters. No change.
  - **T-FILTER** now reloads after the 12 refined-filter cycles and polls 3 s for the applied `?view=` to survive, mirroring T-URL, since the reported defect explicitly includes reload/shared-link reopen.
  - **Live status:** :3768 was down again this round (`curl -sf http://localhost:3768` failed; only `alga-psa-local-test` postgres/redis were up), so T-URL and T-FILTER were **not** run live. They remain typechecked only; T-FILTER has still never executed against the real router. Static checks all pass: `packages/list-views` typecheck + 25 unit tests, `packages/tickets` typecheck + 864 tests (incl. `urlSync.contract`), `packages/projects` typecheck + 325 tests, `server` typecheck (`--max-old-space-size=8192`), and `listViews.integration.test.ts` (7 tests).

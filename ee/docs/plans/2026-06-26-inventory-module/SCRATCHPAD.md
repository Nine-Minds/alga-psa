...
- **154/155 features** implemented (typecheck-clean, committed). Only deferred: **F089** (serial/MAC picker inside the ticket/project material dialog — that dialog lives in the tickets/projects UI, outside the inventory module; the materials consume hook already handles non-serialized products; the serialized-via-materials picker is a small tickets-UI follow-up).

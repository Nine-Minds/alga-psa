...
import { AutopayWorkerService } from '@ee/lib/payments/AutopayWorkerService';
...
export async function prepareInvoiceAutopay(input: { tenantId: string; invoiceId: string }) {
  try { return await (await AutopayWorkerService.create(input.tenantId)).prepareInvoiceAutopay(input.invoiceId); }
...
export async function executeAutopayAttempt(input: { tenantId: string; invoiceId: string; attemptId: string }) {
  try { return await (await AutopayWorkerService.create(input.tenantId)).executeAttempt(input.attemptId); }
  catch (error) { return failValidation(error); }
}
...
export async function reconcileAutopayAttempt(input: { tenantId: string; invoiceId: string; attemptId: string; noIntentFound?: boolean }) {
  return (await AutopayWorkerService.create(input.tenantId)).reconcileAttempt(input.attemptId, input.noIntentFound === true);
}

export async function evaluateEnrollmentForInvoice(input: { tenantId: string; invoiceId: string; attemptId: string }) {
  return (await AutopayWorkerService.create(input.tenantId)).evaluateEnrollmentForInvoice(input.attemptId);
}

export async function createRetryAttempt(input: { tenantId: string; invoiceId: string; previousAttemptId: string; retryAt: string }) {
  return (await AutopayWorkerService.create(input.tenantId)).createRetryAttempt(input.previousAttemptId, input.retryAt);
}

export async function finishAutopayWithFallback(input: { tenantId: string; invoiceId: string; attemptId?: string; reason: string }) {
  return (await AutopayWorkerService.create(input.tenantId)).finishAutopayWithFallback(input.invoiceId, input.attemptId, input.reason);
}

export async function cancelAutopayAttempt(input: { tenantId: string; invoiceId: string; attemptId: string }) {
  return (await AutopayWorkerService.create(input.tenantId)).cancelAttemptById(input.attemptId);
}
...
export async function listAutopayReconcileWork() {
  ...
  const results = await Promise.all(tenants.map(async ({ tenant }: { tenant: string }) =>
    (await AutopayWorkerService.create(tenant)).listAutopayReconcileWork().then((items: any[]) => items.map((item) => ({ ...item, tenantId: tenant })))
  ));

...
export function AccountManagerPickerModal({
  visible,
  updating,
  updateError,
  onSelect,
  ...
  const fetchUsers = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(t("managerPicker.unableToLoad", { defaultValue: "Unable to load users" }));
        return;
      }
      ...
      <View style={{ backgroundColor: colors.background, borderTopLeftRadius: 16, borderTopRightRadius: 16, paddingBottom: spacing.xl, maxHeight: "70%" }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: spacing.lg, paddingBottom: spacing.sm }}>
          <Text style={{ ...typography.title, color: colors.text }}>
            {t("managerPicker.title", { defaultValue: "Assign account manager" })}
          </Text>
        ...
        <View style={{ paddingHorizontal: spacing.lg, paddingBottom: spacing.sm }}>
          <TextInput
            placeholder={t("managerPicker.searchPlaceholder", { defaultValue: "Search users" })}
          ...
          <ScrollView style={{ paddingHorizontal: spacing.lg }} keyboardShouldPersistTaps="handled">
            {users.length === 0 && !loading ? (
              <Text style={{ ...typography.body, color: colors.textSecondary, paddingVertical: spacing.sm }}>
                {t("managerPicker.noResults", { defaultValue: "No matching users" })}
              </Text>
            ) : null}
            {users.map((user) => {
              ...
              return (
                <Pressable
                  ...
                  accessibilityLabel={t("managerPicker.assignTo", { defaultValue: "Assign to {{name}}", name: displayName })}
                  disabled={updating}
                  ...
                  style={({ pressed }) => ({
                    ...
                    borderColor: colors.border,
                    backgroundColor: colors.card,
                    opacity: updating ? 0.65 : pressed ? 0.95 : 1,
                ...
                >
                  ...
                  {updating ? <ActivityIndicator size="small" /> : null}
                </Pressable>
              );
            })}
          </ScrollView>

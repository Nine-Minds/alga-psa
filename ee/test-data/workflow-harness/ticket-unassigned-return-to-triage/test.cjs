...
module.exports = async function run(ctx) {
  ...
  const marker = '[fixture ticket-unassigned-return-to-triage]';
  ...
  const title = `Fixture unassigned triage ${randomUUID()}`;
  ...
  await ctx.http.request('/api/workflow/events', {
    method: 'POST',
    json: {
      eventName: 'TICKET_UNASSIGNED',
      correlationKey: ticketId,
      payloadSchemaRef: 'payload.TicketUnassigned.v1',
      payload: {
        ticketId,
        previousAssigneeId: user.user_id,
        previousAssigneeType: 'user',
        fixtureTriageStatusId: status.status_id,

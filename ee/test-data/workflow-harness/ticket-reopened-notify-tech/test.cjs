...
module.exports = async function run(ctx) {
  ...
  const marker = '[fixture ticket-reopened-notify-tech]';
  ...
  const title = `Fixture reopened notify ${randomUUID()}`;
  ...
  const commentFound = comments.find((c) => typeof c.note === 'string' && c.note.includes(marker) && c.is_internal === true);
  if (!commentFound) {

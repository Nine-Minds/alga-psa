...
describe('EE calendar Microsoft consumer profile resolution tenant-scoped query contract', () => {
  it('uses tenantDb for tenant-known Microsoft profile resolution roots', () => {
    expect(source).toContain('tenantDb(db, tenant).table(table)');
    ...
    expect(source).not.toContain("db('microsoft_profiles').insert");
    expect(source).not.toContain("db('microsoft_profile_consumer_bindings')");

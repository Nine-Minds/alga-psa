 ...
 * They are constants so a tuning pass changes one file.
 */
export const SMART_SEARCH_BUDGETS = {
  /** Title + number + client + description + comments for one ticket. */
  tokensPerTicket: 3_000,
  /** The description is truncated to this before comments fill the remainder. */
  ...
  /** Ticket ids loaded from the database per round trip while building candidates. */
  candidateLoadChunk: 200,
  /** Comments fetched per ticket before budgeting; the budget keeps far fewer. */
  commentsPerTicketFetchLimit: 40,
} as const;

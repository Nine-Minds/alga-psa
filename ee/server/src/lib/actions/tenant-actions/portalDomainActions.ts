...
export async function enablePortalDomainAction(): Promise<PortalDomainStatusResponse> {
  const { knex, tenant } = await ensurePermission(UPDATE_ACTION);
  const current = await getPortalDomain(knex, tenant);

  if (!current || !current.domain) {
    throw new Error('No disabled custom domain to re-enable.');
  }

  if (current.status !== 'disabled') {
    throw new Error('Custom domain is not disabled.');
  }
  ...
  const updated = await updatePortalDomain(knex, tenant, {
    status: 'pending_dns',
    statusMessage: `Re-enabling custom domain. Waiting for DNS verification of ${current.domain}.`,
    lastCheckedAt: now,
    verificationDetails: {
      expected_cname: canonicalHost,
      requested_domain: current.domain,
    },
    certificateSecretName: null,
    lastSyncedResourceVersion: null,
  });
  ...
  analytics.capture('portal_domain.enable', {
    tenant_id: tenant,
    status: status.status,
  });

...
// Sync engine and the helpers it shares with this module; they live outside
// 'use server' because every export here is a callable RPC endpoint.
import {
  buildConfiguredTaniumClient,
  runTaniumDeviceSync,
  sanitizeError,
} from '../../integrations/tanium/sync/deviceSyncEngine';
...
export const triggerTaniumFullSync = withAdvancedAssetsAccess(async (_user, { tenant }) => {
  const permitted = await hasPermission(_user as any, 'system_settings', 'update');
  if (!permitted) return { success: false, error: 'Forbidden' };

  // Same signature and result shape as before; the work moved to a plain module
  // so the scheduled job can call it without a session or an acting user.
  return runTaniumDeviceSync({ tenant });
});

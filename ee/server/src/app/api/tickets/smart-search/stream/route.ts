/**
 * Smart ticket search stream (enterprise).
 *
 ...
 * runner scores the chip-filtered ticket set against the query.
 *
 * Reached through the delegator at server/src/app/api/tickets/smart-search/
 * stream/route.ts, which answers 404 in community edition. Access is decided
 * by evaluateSmartTicketSearchAccess (permission, release flag, AI add-on,
 * key) before a token is spent. Abort flows one way: the request's signal or
 * a cancelled body aborts every TypeSafe call the runner holds open.
 */
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const access = await evaluateSmartTicketSearchAccess(user as IUserWithRoles);

 ...
 * slow to undo. "Sync the remaining N" stays disabled until the pilot client's
 * run has actually completed — not merely been started.
 */
export function PilotSyncControl({ onPilotStarted }: PilotSyncControlProps): React.JSX.Element {
  ...
  const handlePreflight = React.useCallback(async () => {
    ...
    setMessage(null);
    try {
      const result = await runEntraPreflight({ managedTenantId: selected.managedTenantId });
      if ('error' in result) {
  ...
  return (
    <div className="space-y-3" id="entra-pilot-control">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          {pilotBusy
            ...
            : t('integrations.entra.pilot.actions.syncOne')}
        </Button>
        <Button
      ...
      {!pilotCompleted ? (
        <p className="text-sm text-muted-foreground" id="entra-pilot-gate-note">
      ...
      {preflight ? <ContactPreflightReport report={preflight} /> : null}
    </div>
  );
}

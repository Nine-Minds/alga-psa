...
export function ManagedTenantUserFilterPanel({ mapping }: { mapping: EntraConfirmedMapping }): React.JSX.Element {
  ...
  const load = React.useCallback(async () => {
    ...
    setDraft(result.data.override ? savedOverride : { memberUsersOnly: true, licensedUsersOnly: true });
  ...
  const reset = async () => {
    setBusy(true); setStatus('');
    try { const result = await updateEntraManagedTenantUserFilter({ managedTenantId: mapping.managedTenantId, override: null }); if ('error' in result) setStatus(result.error); else { setDefaults(result.data.defaults); setOverride(null); setDraft({ memberUsersOnly: true, licensedUsersOnly: true }); setPreview(null); setStatus(t('integrations.entra.userImportFilter.saved')); } } finally { setBusy(false); }
  };

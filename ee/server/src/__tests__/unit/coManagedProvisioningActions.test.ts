...
const mocks = vi.hoisted(() => ({ permission: vi.fn(), prepare: vi.fn(), schedule: vi.fn(),
...
vi.mock('@alga-psa/co-managed', () => ({ prepareCoManagedProvisioning: mocks.prepare, CoManagedProvisioningError: class extends Error {} }));
...
beforeEach(() => { vi.resetAllMocks(); mocks.user.user_type = 'internal'; mocks.permission.mockResolvedValue(true);
  mocks.prepare.mockResolvedValue({ operation_id: 'operation', state: 'queued' }); mocks.schedule.mockResolvedValue({ enqueued: true }); mocks.rows = { 'home-sponsor:tenants': [{ product_code: 'psa' }] }; });

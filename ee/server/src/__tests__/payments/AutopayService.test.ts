...
vi.mock('../../lib/payments/PaymentService', () => ({ PaymentService: { create: vi.fn(async () => ({ recordAutoPaySuccess, expireActiveLinksForInvoice, getOrCreatePaymentLink: vi.fn() })) } }));
...
function makeKnex(updatedAt = new Date()) {
  const attempt = {
  ...
  const rows: Record<string, any> = {
    ...
    invoices: { invoice_id: 'invoice-1', status: 'sent', total_amount: 5000, credit_applied: 0, currency_code: 'USD', client_id: 'client-1', invoice_number: 'INV-1' },

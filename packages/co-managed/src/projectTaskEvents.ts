...
export async function retainCoManagedTaskCommentEvent(trx: Knex.Transaction,
  input: { tenant: string; eventId: string; taskId: string; commentId: string; kind: 'create' | 'edit' | 'delete' | 'audience' }) {
  input = { ...input };
  if (!trx.isTransaction || ![input.tenant, input.eventId, input.taskId, input.commentId].every(isCoManagedUuid) || !['create', 'edit', 'delete', 'audience'].includes(input.kind)) throw new CoManagedSharedWorkError();
  ...
  if (!source || input.kind !== 'audience' && Boolean(source.deleted_at) !== (input.kind === 'delete')) throw new CoManagedSharedWorkError();
  const eventType = coManagedTaskCommentEventTypes[input.kind === 'create' ? 0 : input.kind === 'edit' || input.kind === 'audience' ? 1 : 2];

...
import { withCoManagedTicketRoutingNotification, type CoManagedTicketRoutingNotification } from './ticketRoutingNotifications';
...
export interface CoManagedRoutingEmailDelivery {
  tenant: string; recipientUserId: string; email: string; recipientName: string; subtypeId?: number;
  messageId: string; message: CoManagedTicketRoutingNotification;
}
const TABLE = 'co_management_ticket_routing_recipients';
const key = (row: any) => ({ event_id: row.event_id, recipient_user_id: row.recipient_user_id, channel: 'email' });
// LEVERAGE: pattern comment-email-completion — source-qualified Routing email shares the existing bounded retry policy; receipt identity differs.
...
export async function processCoManagedRoutingEmailDeliveries(db: Knex, tenant: string,
  send: (delivery: CoManagedRoutingEmailDelivery) => Promise<CoManagedEmailDeliveryResult>, limit = 30) {
  if (!isCoManagedUuid(tenant) || !Number.isSafeInteger(limit) || limit < 1 || limit > 300) throw new Error('Invalid Routing email batch');
  ...
  const rows = await due(db).orderBy('next_attempt_at').orderBy('event_id').orderBy('recipient_user_id').limit(limit);
  let processed = 0;
    ...
    try {
      const done = await withTransaction(db, async trx => {
        const delivered = await withCoManagedTicketRoutingNotification(trx, { kind: 'notification_recipient', tenant, userId: candidate.recipient_user_id },
          candidate.event_id, 'email', async (context, message) => {
            const to = await coManagedInternalEmailRecipient(context, 'Ticket Assigned');
            if (to && to.subtypeId === undefined) throw new Error('Routing email notification subtype is not configured');
            ...
            const identity = `${tenant}:${row.event_id}:${row.recipient_user_id}:email`;
            const messageId = `<co-managed-routing-${createHash('sha256').update(identity).digest('hex')}@notifications.alga.invalid>`;

...
async function sourceThread(context: CoManagedSharedWorkContext, reference: CoManagedThreadReference) {
  ...
  const thread = await home.table('co_management_private_threads').where({ thread_id: reference.threadId, customer_tenant: resource.tenant,
    relationship_id: resource.relationshipId, resource_type: 'ticket', resource_id: resource.id }).select('*', { created_at_exact: trx.raw('created_at::text') }).forUpdate().first();
  ...
  const drafts = await home.table('co_management_conversation_drafts').where({ customer_tenant: resource.tenant, relationship_id: resource.relationshipId,
  ...
  const files = await home.table(FILES).where({ customer_tenant: resource.tenant, relationship_id: resource.relationshipId,
    ticket_id: resource.id, thread_id: reference.threadId }).whereNull('discarded_at').select('*', { created_at_exact: trx.raw('created_at::text') }).orderBy('attachment_id').forShare();
...
function checkedPlan(row: any, source: Awaited<ReturnType<typeof sourceThread>>, context: CoManagedSharedWorkContext) {
  ...
  for (const file of manifest) {
    if (!file || Object.keys(file).some(key => !['sourceId', 'targetId', 'sourceCommentId', 'path', 'copied'].includes(key)) || typeof file.copied !== 'boolean' ||
      ...
      file.path !== disclosedAttachmentPath(context.resource.tenant, context.actor.tenant, row.operation_id, file.targetId)) conflict();
  }
...
export async function discloseCoManagedPrivateTicketThread(db: Knex, inputActor: CoManagedSessionActor, inputResource: CoManagedSharedResource,
  ...
  try {
    ...
    const withTransfer = <T>(work: (context: CoManagedSharedWorkContext, row: any) => Promise<T>) => withAuthority(db, actor, resource, async context => {
      ...
      if (!row || row.request_hash !== requestHash || row.customer_tenant !== resource.tenant || row.relationship_id !== resource.relationshipId || row.ticket_id !== resource.id ||
    ...
    await withAuthority(db, actor, resource, async context => {
      ...
      const manifest: TransferFile[] = source.files.map(row => {
        const targetId = randomUUID(); return { sourceId: row.attachment_id, targetId, sourceCommentId: row.comment_id,
          path: disclosedAttachmentPath(resource.tenant, actor.tenant, operationId, targetId), copied: false };
      });
      await home.table(TABLE).insert({ tenant: actor.tenant, operation_id: operationId, customer_tenant: resource.tenant, relationship_id: resource.relationshipId, ticket_id: resource.id,
    ...
    while (await withTransfer(async (context, row) => {
      ...
      if (!file || pending.path !== disclosedAttachmentPath(resource.tenant, actor.tenant, operationId, pending.targetId)) conflict();
    ...
    return await withTransfer(async (context, row) => {
      ...
      for (const file of source.files) {
        ...
        if (prepared.path !== disclosedAttachmentPath(resource.tenant, actor.tenant, operationId, prepared.targetId)) conflict();
        await customer.table(FILES).insert({ tenant: resource.tenant, attachment_id: prepared.targetId, customer_tenant: resource.tenant, relationship_id: resource.relationshipId,
          ticket_id: resource.id, thread_id: operationId, comment_id: mapping[file.comment_id], actor_tenant: file.actor_tenant, actor_user_id: file.actor_user_id,
    ...
    if ((error as { code?: string })?.code === '23505' && ['co_management_thread_transfers_pkey', 'comment_threads_pkey', 'comments_pkey', 'co_management_conversation_attachments_pkey']

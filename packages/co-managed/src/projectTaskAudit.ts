...
/** Called only inside an admitted task writer; retain qualified immutable
...
export async function recordCoManagedProjectTaskAudit(context: CoManagedSharedWorkContext, input: {
  ...
  if (context.action !== 'update' || context.resource.kind !== 'project_task' || !context.trx.isTransaction) throw new CoManagedSharedWorkError();
  ...
  await owner.table('audit_logs').insert({ audit_id: randomUUID(), tenant: context.resource.tenant, table_name: 'project_tasks', record_id: context.resource.id,

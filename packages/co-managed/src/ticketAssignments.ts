...
async function retainedRouting(context: CoManagedSharedWorkContext) {
  ...
  const relationship = await owner.table('co_management_relationships').where({ relationship_id: key.relationship_id, state: 'active' }).whereNull('ended_at').forShare().first('sponsor_tenant', 'sponsor_client_id', 'visibility_mode');
  ...
  if (!work) return { relationship, work: null, reference: null, canCollaborate: false };
  const refQuery = tenantDb(context.trx, relationship.sponsor_tenant).table('co_managed_ticket_references').where({ ...key, customer_tenant: context.resource.tenant, work_id: work.work_id, client_id: relationship.sponsor_client_id });
  ...
  const canCollaborate = Boolean(!work.grant_revoked_at && work.can_collaborate || boardGrant?.can_collaborate);
  return { relationship, work, reference, canCollaborate };
}
async function option(context: CoManagedSharedWorkContext, routing: Awaited<ReturnType<typeof retainedRouting>>, assignee: CoManagedTicketAssignee) {
  if (!routing.reference || !routing.work || !routing.canCollaborate) denied();
  const board = await tenantDb(context.trx, routing.relationship.sponsor_tenant).table('boards').where({ board_id: routing.reference.board_id, is_inactive: false }).forShare().first('board_id');
  if (!board) denied();
  return coManagedAssigneeOption(context, routing.relationship, assignee, { boardId: routing.reference.board_id, hidden });
}
async function state(context: CoManagedSharedWorkContext, canEdit: boolean) {
  ...
  return { resource: context.resource, canEdit: canEdit && !!reference, canAssign: canEdit && !!reference && routing.canCollaborate,
    revision: work?.revision, hasAssignment: !!(reference?.assigned_to || reference?.assigned_team_id), mspAssignment };
}
...
export async function listCoManagedTicketAssignees(db: Knex, inputActor: CoManagedSessionActor, inputResource: CoManagedSharedResource, kind: 'user' | 'team', afterId?: string) {
  ...
  return withWork(db, actor, resource, 'update', write => withWork(write.trx, actor, resource, 'read', async read => {
    ...
    if (!routing.reference || !routing.canCollaborate) denied();
...
export async function assignCoManagedTicket(db: Knex, inputActor: CoManagedSessionActor, inputResource: CoManagedSharedResource, input: CoManagedTicketAssignmentRequest) {
  ...
  if (!input || !isCoManagedUuid(input.operationId) || !Number.isSafeInteger(input.expectedRevision) || input.expectedRevision < 1 || input.expectedRevision >= 2147483647 || Object.keys(input).some(key => !['operationId', 'expectedRevision', 'assignee'].includes(key))) invalid();
  ...
  try { return await withWork(db, actor, resource, 'update', write => withWork(write.trx, actor, resource, 'read', async read => {
    ...
    if (work.revision !== request.expectedRevision) throw new CoManagedTicketAssignmentError('TICKET_ASSIGNMENT_CONFLICT');
    ...
    await tenantDb(write.trx, routing.relationship.sponsor_tenant).table('co_managed_ticket_references').where('reference_id', reference.reference_id).update({
      assigned_to: selected?.kind === 'user' ? selected.id : null, assigned_team_id: selected?.kind === 'team' ? selected.id : null, updated_at: write.trx.raw('clock_timestamp()') });
    await owner.table('co_management_ticket_work').where({ relationship_id: resource.relationshipId, ticket_id: resource.id }).update({ revision: work.revision + 1 });
    await recordCoManagedWorkAudit(write, { operation: 'co_managed_ticket_assignment', operationId: request.operationId,
      changes: { msp_assignment: selected ? `${selected.organizationName} · ${selected.name}` : null }, details: { assignment_reference_id: reference.reference_id, work_revision: work.revision + 1, assignee: request.assignee } });

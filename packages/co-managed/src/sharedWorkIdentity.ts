...
export async function lockCoManagedSessionIdentity(trx: Knex.Transaction, actor: CoManagedSessionActor): Promise<AuthorizationSubject> {
  ...
  const session = await home.table('sessions').where({ session_id: actor.sessionId, user_id: actor.userId }).whereNull('revoked_at').forShare().first('session_id');
  if (!user || !session) throw new CoManagedSharedWorkError();
...
export async function authorizeCoManagedWorkRecord(trx: Knex.Transaction, actor: CoManagedSessionActor,

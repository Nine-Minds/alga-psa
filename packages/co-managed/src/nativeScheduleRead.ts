...
function scheduleView(row: any, assignments: string[], actor: CoManagedAuthenticatedActor, fields: readonly string[], source: ScheduleSource) {
  const hidden = (...names: string[]) => isCoManagedReadFieldHidden(fields, names.flatMap(name => [name, `values.${name}`, `schedule_entries.${name}`]));
 ...
 * become a membership oracle. Recurring expansion is a separate native concern. */
export async function readCoManagedNativeSchedules(db: Knex, tenant: string, identify: () => Promise<CoManagedAuthenticatedActor>,
  options: { id?: string; start?: string; end?: string; userId?: string } = {}
  ...
  return withTransaction(db, async trx => {
    ...
    for (const hint of hints) {
      try {
        ...
        const view = scheduleView(row, assignments, actor, policy.redactedFields, source);

import { describe, expect, it } from 'vitest';
import {
  decodeInvoiceTemplatePathExpression,
  encodeInvoiceTemplatePathExpression,
  parseInvoiceTemplateToken,
} from './templateInterpolationFilters';

describe('templateInterpolationFilters', () => {
  it('parses plain path tokens', () => {
    expect(parseInvoiceTemplateToken('invoice.total')).toEqual({ path: 'invoice.total' });
  });

  it('parses currency filter tokens with whitespace tolerance', () => {
    expect(parseInvoiceTemplateToken('invoice.total | currency')).toEqual({
      path: 'invoice.total',
      filter: 'currency',
    });
  });

  it('rejects unsupported filters', () => {
    expect(parseInvoiceTemplateToken('invoice.total | percent')).toBeNull();
  });

  it('encodes and decodes filtered path expressions', () => {
    const encoded = encodeInvoiceTemplatePathExpression('total', 'currency');
    expect(encoded).toBe('total|currency');
    expect(decodeInvoiceTemplatePathExpression(encoded)).toEqual({
      path: 'total',
      filter: 'currency',
    });
  });
});

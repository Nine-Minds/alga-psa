// server/src/components/billing-dashboard/HourlyPresetConfiguration.tsx
'use client'
...
import { Button, ButtonProps } from '@alga-psa/ui/components/Button'; // Import ButtonProps
...
// Define the expected shape of the plan object returned by getContractLinePresetById for Hourly
type HourlyPlanData = IContractLinePreset & {
...
interface HourlyPresetConfigurationProps {
  presetId: string;
  className?: string;
}

export function HourlyPresetConfiguration({
  presetId,
  className = '',
  ...
  const fetchPlanData = useCallback(async () => {
    ...
    try {
      ...
      const fetchedPlanResult = await getContractLinePresetById(presetId);
      ...
      const servicesWithConfigsResult = await getContractLineServicesWithConfigurations(presetId);
  ...
  const handleSavePlanBasics = async () => {
    ...
    try {
      ...
      if (plan?.preset_id) {
        const result = await updateContractLinePreset(plan.preset_id, planData);
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        setSaveError(t('preset.hourly.errors.failedToSaveServiceConfiguration', {
    ...
    try {
        ...
        if (planChanged) {
            const result = await updateContractLinePreset(presetId, planUpdatePayload);
            if (isReturnedActionError(result)) {
                setSaveError(getErrorMessage(result));
                return;
            }

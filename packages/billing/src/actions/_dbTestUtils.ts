...
// Local DB bootstrap for billing DB-backed suites. Deliberately does NOT import
// server/test-utils: a billing -> server import edge introduces a new project
// cycle the circular-deps guard rejects. Mirrors the sanctioned pattern in
...
// packages/billing/src/actions -> repo root is four levels up.
...
// getSecret() falls back to these env vars — rewire them to the local test
// Postgres + real ./secrets files. Local-only: CI already sets correct DB_*
// step env.
...
export async function createTestDbConnection(): Promise<Knex> {
  ...
  // dozens of migrations; on plain Postgres each one ERRORs server-side. An
  // empty stand-in catalog makes every probe succeed with is_distributed=false.
  ...
  // (idle_in_transaction_session_timeout, lock_timeout) on the app role; in
  // tests they turn legitimate lock waits into spurious timeouts. Reset them.

...
export async function retainHostedPsaUpgradeCandidate(trx: Knex.Transaction, tenant: string,
  ...
  if (await trx('license_state').first('id')) throw new Error('A hosted subscription cannot replace self-host tenant licensing');
  ...
  if (candidates.length !== 1) throw new Error('Exactly one active customer PSA subscription is required');
  ...
  if (!customer || !price || !proPriceIds.includes(price.stripe_price_external_id) || !subscription.stripe_subscription_item_id ||
      ...
      subscription.metadata?.tenant_id !== tenant) throw new Error('The customer subscription is not a current PSA seat entitlement');

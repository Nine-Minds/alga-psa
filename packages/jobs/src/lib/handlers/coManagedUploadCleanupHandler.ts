...
import { cleanupCoManagedUploads, cleanupCoManagedThreadTransfers, storeCoManagedArchiveFiles } from '@alga-psa/co-managed';
...
export async function coManagedUploadCleanupHandler(input: { tenantId: string; limit?: number }) {
  ...
  return { ...uploads, archiveFiles: await storeCoManagedArchiveFiles(db, input.tenantId, input.limit), transfers: await cleanupCoManagedThreadTransfers(db, input.tenantId, remove, input.limit) };
}

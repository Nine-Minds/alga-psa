...
export const assetWarrantyEndSource: DateTriggerSource = {
  ...
  async findOccurrences(knex: Knex, tenant: string, fromDate: string, toDate: string) {
    const rows = await tenantDb(knex, tenant).table('assets as a')
      ...
      .select('a.asset_id', 'a.client_id', 'a.name as asset_name', 'c.client_name')
      .select(knex.raw("(a.warranty_end_date AT TIME ZONE COALESCE(ts.settings->>'timezone', 'UTC'))::date::text as occurs_on"))
    ...
    return rows.map((r) => {
      const occursOn = String(r.occurs_on);

...
export async function saveEntraDirectTokenSet(
  ...
  await secretProvider.setTenantSecret(tenant, ENTRA_DIRECT_SECRET_KEYS.accessToken, tokens.accessToken);
  await secretProvider.setTenantSecret(tenant, ENTRA_DIRECT_SECRET_KEYS.refreshToken, tokens.refreshToken);
  await secretProvider.setTenantSecret(tenant, ENTRA_DIRECT_SECRET_KEYS.tokenExpiresAt, tokens.expiresAt);
  await secretProvider.setTenantSecret(tenant, ENTRA_DIRECT_SECRET_KEYS.tokenScope, tokens.scope || '');
}

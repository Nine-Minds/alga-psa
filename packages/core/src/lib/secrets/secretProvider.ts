...
let filesystemProviderInstance: ISecretProvider | null = null;
 ...
 */
function validateProviderConfiguration(readChain: ProviderType[], writeProvider: ProviderType): void {
  // Validate read chain provider names
  const invalidReadProviders = readChain.filter(type => !SUPPORTED_PROVIDER_TYPES.includes(type));
  ...
  if (!SUPPORTED_PROVIDER_TYPES.includes(writeProvider)) {
 ...
 */
async function buildSecretProviders(): Promise<ISecretProvider> {
  const readChainEnv = getEnvVar('SECRET_READ_CHAIN') || 'env';
  const writeProviderEnv = getEnvVar('SECRET_WRITE_PROVIDER') || 'filesystem';

  const readChain = readChainEnv.split(',').map(s => s.trim()).filter(s => s.length > 0) as ProviderType[];
  ...
  const readProviders = await Promise.all(readChain.map(type => getProviderInstance(type)));

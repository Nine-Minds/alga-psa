 ...
 */
export const fetchWorkflowTask = withAuth(async (
  user,
  _ctx,
  taskId: string
 ...
 */
export const fetchTaskFormSchema = withAuth(async (
  _user,
  { tenant },
  formId: string
 ...
 */
export const fetchTaskFormData = withAuth(async (
  _user,
  { tenant },
  taskId: string
  ...
  try {
    ...
    const task = await withTransaction(db, async (trx: Knex.Transaction) => {
      const task = await tenantDb(trx, tenant).table("workflow_tasks")
 ...
 */
export const submitTaskForm = withAuth(async (
  _user,
  { tenant },
  taskId: string,
  ...
  try {
    ...
    await withTransaction(db, async (trx: Knex.Transaction) => {
      ...
      await tenantDb(trx, tenant).table("workflow_tasks")
        .where("task_id", taskId)
        .update({
          status: "completed",
 ...
 * @returns Promise resolving to a boolean indicating success
 */
export const cancelWorkflowTask = withAuth(async (
  _user,
  { tenant },
  taskId: string
): Promise<boolean> => {
 ...
 * @returns Promise resolving to a boolean indicating success
 */
export const reassignWorkflowTask = withAuth(async (
  _user,
  { tenant },
  taskId: string,
 ...
 */
export const fetchDashboardWorkflowTasks = withAuth(async (
  user,
  _ctx,
  limit: number = 5,

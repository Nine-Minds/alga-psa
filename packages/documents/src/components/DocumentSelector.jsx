...
import { Search, X, Check, FileText, Video, File } from 'lucide-react';
import Spinner from '@alga-psa/ui/components/Spinner';
import LoadingIndicator from '@alga-psa/ui/components/LoadingIndicator';
import { Input } from '@alga-psa/ui/components/Input';
import { getAllDocuments, createDocumentAssociations, getDocumentsByFolder } from '../actions/documentActions';
...
export default function DocumentSelector({ id, entityId, entityType, onDocumentSelected, onDocumentsSelected, singleSelect = false, isOpen, onClose, typeFilter, title, description }) {
    ...
    const loadDocuments = async (pageToLoad) => {
        try {
            ...
            if (selectedFolder !== null) {
                ...
                response = await getDocumentsByFolder(selectedFolder, false, // includeSubfolders
                pageToLoad, pageSize, {
    ...
    if (!id || !entityId || !entityType) {
        ...
        return (<Dialog isOpen={isOpen} onClose={onClose} data-testid="document-selector-dialog" title={t('documents.selector.configErrorTitle', 'Configuration Error')}>
                <DialogContent>
    ...
    return (<Dialog isOpen={isOpen} onClose={onClose} title={resolvedTitle}>
            <DialogContent>
                <ReflectionContainer id={id} label={t('documents.selector.reflectionLabel', 'Document Selector')}>
                    <div className="space-y-4">
                        {/* Description */}
                        {description && (<Text as="p" size="2" className="text-gray-600">
                                {description}
                            </Text>)}

                        {/* Search Bar */}
                        <div className="relative">
                            <Input id={`${id}-search`} type="text" placeholder={t('documents.selector.searchPlaceholder', 'Search documents...')} value={searchTerm} onChange={handleSearchChange} onKeyPress={handleSearchKeyPress} className="pl-10"/>
                            <Search className="absolute left-3 top-2.5 h-5 w-5 text-gray-400"/>
                        </div>

                        {/* Error Message */}
                        {error && (<div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded flex items-center">
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="w-64 flex-shrink-0 border-r border-gray-200 pr-4">
                                ...
                                <FolderTreeView selectedFolder={selectedFolder} onFolderSelect={(folder) => {
            ...
            setCurrentPage(1);
                ...
                return (<div key={document.document_id} id={`${id}-document-${document.document_id}`} className={`relative cursor-pointer rounded-lg border-2 overflow-hidden transition-all aspect-square ${selectedDocuments.has(document.document_id)
                        ? 'border-primary-500 ring-2 ring-primary-500'
                        : 'border-gray-200 hover:border-primary-300'}`} onClick={() => toggleDocumentSelection(document.document_id)}>
                                                        {/* Document Preview/Icon */}
                                                        {isImage && (document.thumbnail_file_id || document.file_id) ? (<img src={document.thumbnail_file_id
                            ? `/api/documents/${document.document_id}/thumbnail`
                            : `/api/documents/view/${document.file_id}`} alt={document.document_name} className="w-full h-full object-cover" onError={(e) => {
                            if (document.file_id && e.currentTarget.src.includes('thumbnail')) {
                                e.currentTarget.src = `/api/documents/view/${document.file_id}`;
                            }
                        }}/>) : isVideo && document.thumbnail_file_id ? (<div className="relative w-full h-full">
                                                                <img src={`/api/documents/${document.document_id}/thumbnail`} alt={document.document_name} className="w-full h-full object-cover"/>
                                                                <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                                                                    <Video className="w-12 h-12 text-white opacity-80"/>
                                                                </div>
                                                            </div>) : (<div className="w-full h-full flex flex-col items-center justify-center bg-gray-50">
                                                                {isVideo ? (<Video className="w-12 h-12 text-gray-400 mb-2"/>) : isPdf ? (<FileText className="w-12 h-12 text-red-400 mb-2"/>) : (<File className="w-12 h-12 text-gray-400 mb-2"/>)}
                                                        ...
                                                        {/* Selected Indicator */}
                                                        {selectedDocuments.has(document.document_id) && (<div className="absolute top-2 right-2 bg-primary-500 text-white rounded-full p-1.5 shadow-lg">
                                                        ...
                                                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 via-black/50 to-transparent p-2 pt-8">
                                                            <Text as="p" size="1" className="text-white font-medium truncate" title={document.document_name}>
                                                                {document.document_name}
                                                            </Text>
                                                        </div>
                                                    </div>);
            })}
                                            {documents.length === 0 && !isLoading && (<div className="col-span-full text-center py-8">
                                                    <File className="w-12 h-12 text-gray-400 mx-auto mb-2"/>
                                                    <Text as="div" size="2" color="gray">
                                                        {t('documents.empty.default', 'No documents found')}
                                                    </Text>
                                                    {selectedFolder && (<Text as="div" size="1" color="gray" className="mt-1">
                        ...
                        defaultValue: `in folder "${selectedFolder}"`
                                        ...
                                        {/* Pagination */}
                                        <Pagination id={`${id}-pagination`} currentPage={currentPage} totalItems={totalPages * pageSize} itemsPerPage={pageSize} onPageChange={handlePageChange} variant="full" className={totalPages <= 1 ? 'hidden' : ''}/>
                                    </>)}
                            </div>
                        </div>
                        ...
                        <div className="pt-4 border-t mt-4">
                            {/* Loading State */}
                            {isSaving && (<div className="flex justify-center mb-3">
                                    <LoadingIndicator text={t('documents.selector.saving', 'Saving...')} spinnerProps={{ size: 'sm' }}/>
                                </div>)}

                            <div className="flex justify-end space-x-2">
                                <Button id="cancel-document-selection-button" variant="outline" onClick={onClose} disabled={isSaving}>
                                    {t('common.cancel', 'Cancel')}
                                </Button>
                                <Button id="save-document-selection-button" onClick={handleSave} disabled={selectedDocuments.size === 0 || isSaving}>
                                    {singleSelect

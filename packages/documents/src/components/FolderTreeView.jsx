...
export default function FolderTreeView({ onFolderSelect, selectedFolder, onFolderDeleted, isCollapsed = false, onToggleCollapse }) {
    ...
    async function handleDeleteFolder(path, e) {
        e.stopPropagation();
        if (!confirm(t('documents.folders.deleteConfirm', {
            name: path,
    ...
    function renderFolderNode(node, level = 0) {
        ...
        <div id={`folder-item-${node.path.replace(/\//g, '-')}`} className={`
            flex items-center gap-2 py-2 px-3 cursor-pointer group
            hover:bg-gray-100
            ${isSelected ? 'bg-blue-50' : ''}
          `} style={{ paddingLeft: `${level * 20 + 12}px` }} onClick={() => onFolderSelect(node.path)}>
          {hasChildren && (<button id={`folder-toggle-${node.path.replace(/\//g, '-')}`} onClick={(e) => {
                    e.stopPropagation();
          ...
          {isExpanded ? (<FolderOpen className="w-4 h-4 text-blue-500"/>) : (<Folder className="w-4 h-4 text-gray-500"/>)}

          <span className="text-sm flex-1">{node.name}</span>
          <span className="text-xs text-gray-500">
            {node.documentCount}
          </span>
          <button id={`folder-delete-${node.path.replace(/\//g, '-')}`} onClick={(e) => handleDeleteFolder(node.path, e)} className="p-1 opacity-0 group-hover:opacity-100 hover:bg-red-100 rounded transition-opacity" title={t('documents.folders.deleteAction', 'Delete folder')}>

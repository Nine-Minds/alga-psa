...
export default function DocumentsPage() {
    ...
    const { value: isFiltersPaneCollapsed, setValue: setIsFiltersPaneCollapsed } = useUserPreference(FILTERS_PANE_COLLAPSED_SETTING, {
        defaultValue: false,
    ...
    const handleFolderNavigate = (folderPath, clearShowAll = true) => {
    ...
    return (<div className="p-6">
      <div className="mb-6">
        <div className="flex items-center gap-2 text-2xl font-semibold">
          <button onClick={() => handleFolderNavigate(null)} className="flex items-center gap-2 hover:text-blue-600 transition-colors">
            <span>Documents</span>
          </button>
          {currentFolder && (<>
              {currentFolder.split('/').filter(p => p).map((part, index, parts) => {
                ...
                return (<React.Fragment key={path}>
                    ...
                    <button onClick={() => handleFolderNavigate(path)} className="hover:text-blue-600 transition-colors">
                      {part}
                    </button>
                  </React.Fragment>);
            })}
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1">
          <Card className="p-4">
            {displayError ? (<div className="text-center py-4 text-red-500 bg-red-50 rounded-md">
                {displayError}
              </div>) : (<Documents id='documents' documents={[]} gridColumns={3} userId={currentUserId} searchTermFromParent={filterInputs.searchTerm} filters={filterInputs}/>)}
          </Card>
        </div>
      </div>
    </div>);

...
import { Download, Trash2, FileText, Image, File, FileSpreadsheet, FileType, FileCode, Unlink, EyeOff, Video, Eye, X, Play, FolderInput } from 'lucide-react';
import { ReflectionContainer } from '@alga-psa/ui/ui-reflection/ReflectionContainer';
function VideoPreviewComponent({ fileId, mimeType, fileName, onClick }) {
    ...
    if (canPlay === false || !isBrowserSupported) {
        ...
        return (<div className="max-w-full h-48 rounded-md border border-[rgb(var(--color-border-200))] cursor-pointer transition-all hover:border-[rgb(var(--color-border-300))] bg-gray-50 flex flex-col items-center justify-center group" onClick={onClick}>
    ...
    return (<div className="relative group">
            <video className="max-w-full h-auto rounded-md border border-[rgb(var(--color-border-200))] cursor-pointer transition-opacity group-hover:opacity-75" style={{ maxHeight: '200px', objectFit: 'contain' }} onClick={onClick} controls={false} muted preload="metadata">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer" onClick={onClick}>
...
function VideoModalComponent({ fileId, documentId, mimeType, fileName }) {
    const { t } = useTranslation('common');
    ...
    if (videoError) {
        return (<div className="text-center p-8">
                ...
                <p className="text-xs text-[rgb(var(--color-text-500))] mb-4">
                    {t('documents.videoCodecWarning', 'Chrome may not support this video codec. Try downloading or use Safari/Edge.')}
                </p>
                <Button id={`download-video-${fileId}`} onClick={async () => {
    ...
    return (<div>
            <video ref={videoRef} className="max-w-full max-h-[70vh] object-contain" controls autoPlay={false} preload="metadata" onError={() => setVideoError(true)}>
                <source src={`/api/documents/view/${fileId}`} type={mimeType} onError={handleSourceError}/>
                {t('documents.videoTagUnsupported', 'Your browser does not support the video tag.')}
            </video>
            <div className="text-center mt-4">
                ...
                <Button id={`download-video-fallback-${fileId}`} onClick={async () => {
            const downloadUrl = getDocumentDownloadUrl(documentId);
            const filename = fileName || 'download';
            try {
                await downloadDocument(downloadUrl, filename, true);
...
class PreviewLoadingQueue {
    queue = [];
    running = 0;
    maxConcurrent = 3; // Limit concurrent preview generations
    async add(task) {
...
function DocumentStorageCardComponent({ id, document, onDelete, onDisassociate, onMove, hideActions = false, showDisassociate = false, showMove = false, onClick, isContentDocument = false, forceRefresh }) {
    const { t } = useTranslation('common');
    ...
    return (<>
        <ReflectionContainer id={id} label={`Document Card - ${document.document_name}`}>
            <div ref={cardRef} className={`bg-white rounded-lg border border-[rgb(var(--color-border-200))] shadow-sm p-4 h-full flex flex-col transition-all hover:border-[rgb(var(--color-border-300))] ${(isContentDocument || !document.file_id) ? 'cursor-pointer' : ''}`} onClick={(isContentDocument || !document.file_id) && onClick ? (e) => {
            ...
            onClick();
        } : undefined} role={(isContentDocument || !document.file_id) ? "button" : undefined} tabIndex={(isContentDocument || !document.file_id) ? 0 : undefined}>
                <div className="flex-1">
                    <div className="flex items-start justify-between mb-3">
                        <div className="flex-1 min-w-0 mr-2">
                            ...
                            <p className="mt-1 text-xs text-[rgb(var(--color-text-500))] truncate">
                                ...
                                {document.entered_at && (<span className="ml-1">
                                        • {new Date(document.entered_at).toLocaleDateString()}
                                    </span>)}
                                {!document.entered_at && document.updated_at && (<span className="ml-1">
                                        • {new Date(document.updated_at).toLocaleDateString()}
                                    </span>)}
                            </p>
                            {document.type_name && (<p className="mt-1 text-xs text-[rgb(var(--color-text-500))] truncate">
                    ...
                    <div className="space-y-1">
                        {document.mime_type && (<p className="text-xs text-[rgb(var(--color-text-500))]">
                                {document.mime_type}
                            </p>)}

                        {document.file_size && (<p className="text-xs text-[rgb(var(--color-text-500))]">
                                Size: {(document.file_size / 1024).toFixed(1)} KB
                            </p>)}
                    </div>

                    {/* Preview Content */}
                    {!isInView ? (<div className="mt-4 flex justify-center h-48 items-center bg-gray-50 rounded">
                            <span className="text-sm text-gray-400">Loading...</span>
                        </div>) : isLoading ? (<div className="mt-4 flex justify-center">
                            <Spinner size="sm"/>
                        </div>) : previewContent.error ? (<div className="mt-4 flex items-center space-x-2 text-[rgb(var(--color-text-500))]">
                            <EyeOff className="w-4 h-4"/>
                            <p className="text-sm">{t('documents.previewUnavailable', 'Preview unavailable')}</p>
                        </div>) : (<div className="mt-4 preview-container">
                            {/* For videos, show FFmpeg thumbnail if available, otherwise show video preview */}
                            {document.mime_type?.startsWith('video/') && !previewContent.previewImage ? (<VideoPreviewComponent fileId={document.file_id || ''} mimeType={document.mime_type || ''} fileName={document.document_name} onClick={handleFullSizeView}/>) : previewContent.previewImage ? ((() => {
                ...
                const isPreviewableInModal = document.mime_type?.startsWith('image/') ||
                    ...
                    document.mime_type === 'application/pdf';
                return (<div className="relative group">
                                            <img src={previewContent.previewImage} alt={document.document_name} className={`max-w-full h-auto rounded-md border border-[rgb(var(--color-border-200))] ${isPreviewableInModal ? 'cursor-pointer transition-opacity group-hover:opacity-75' : ''}`} style={{ maxHeight: '200px', objectFit: 'contain' }} onClick={isPreviewableInModal ? handleFullSizeView : undefined} role={isPreviewableInModal ? "button" : undefined} tabIndex={isPreviewableInModal ? 0 : undefined}/>
                                            {isPreviewableInModal && (<div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer" onClick={handleFullSizeView}>
                                                    <div className="bg-black bg-opacity-50 text-white p-2 rounded-full pointer-events-none">
                                                        <Eye className="w-6 h-6"/>
                                                    </div>
                                                </div>)}
                                        </div>);
            })()) : previewContent.content ? (<div className="text-sm text-[rgb(var(--color-text-700))] max-h-[200px] overflow-hidden p-3 rounded-md bg-[rgb(var(--color-border-50))] border border-[rgb(var(--color-border-200))]" style={{
                    display: '-webkit-box',
                    WebkitLineClamp: '8',
                    WebkitBoxOrient: 'vertical',
                    whiteSpace: 'pre-wrap'
                }} dangerouslySetInnerHTML={{ __html: previewContent.content || '' }}/>) : null}
                        </div>)}
                </div>

                {!hideActions && (<div className="mt-4 pt-3 flex flex-col space-y-1.5 items-end border-t border-[rgb(var(--color-border-100))]">
                        <Button id={`download-document-${document.document_id}-button`} variant="ghost" size="sm" onClick={async (e) => {
                e.stopPropagation();
            ...
            }} disabled={isLoading} className="text-[rgb(var(--color-text-600))] hover:text-[rgb(var(--color-text-900))] hover:bg-[rgb(var(--color-border-100))] inline-flex items-center">
                            <Download className="w-4 h-4 mr-2"/>
                            {t('documents.download', 'Download')}
                        </Button>
                        {showDisassociate && onDisassociate && (<Button id={`disassociate-document-${document.document_id}-button`} variant="ghost" size="sm" onClick={(e) => {
                    e.stopPropagation(); // Prevent event bubbling to parent
                    handleDisassociate();
                }} disabled={isLoading} className="text-[rgb(var(--color-text-600))] hover:text-orange-600 hover:bg-orange-50 inline-flex items-center">
                                <Unlink className="w-4 h-4 mr-2"/>
                                {isLoading ? t('common.loading', 'Loading...') : t('documents.detach', 'Detach')}
                            </Button>)}
                        {showMove && onMove && (<Button id={`move-document-${document.document_id}-button`} variant="ghost" size="sm" onClick={(e) => {
                    e.stopPropagation(); // Prevent event bubbling to parent
                    onMove(document);
                }} disabled={isLoading} className="text-[rgb(var(--color-text-600))] hover:text-purple-600 hover:bg-purple-50 inline-flex items-center">
                                <FolderInput className="w-4 h-4 mr-2"/>
                                {isLoading ? t('common.loading', 'Loading...') : t('documents.move', 'Move')}
                            </Button>)}
                        {onDelete && (<Button id={`delete-document-${document.document_id}-button`} variant="ghost" size="sm" onClick={(e) => {
                    e.stopPropagation(); // Prevent event bubbling to parent
                    handleDelete();
                }} disabled={isLoading} className="text-[rgb(var(--color-text-600))] hover:text-red-600 hover:bg-red-50 inline-flex items-center">
        ...
        <ConfirmationDialog id={`${id}-delete-confirmation`} isOpen={showDeleteConfirmation} onClose={() => setShowDeleteConfirmation(false)} onConfirm={confirmDelete} title={deleteTitle} message={deleteMessage} confirmLabel={t('documents.deletePermanently', 'Delete Permanently')} cancelLabel={t('common.cancel', 'Cancel')} isConfirming={isLoading}/>

        {/* Disassociate Confirmation Dialog */}
        {onDisassociate && (<ConfirmationDialog id={`${id}-disassociate-confirmation`} isOpen={showDisassociateConfirmation} onClose={() => setShowDisassociateConfirmation(false)} onConfirm={confirmDisassociate} title={removeTitle} message={removeMessage} confirmLabel={t('documents.detach', 'Detach')} cancelLabel={t('common.cancel', 'Cancel')} isConfirming={isLoading}/>)}

        {/* Full Size View Modal */}
        {showFullSizeModal && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75" onClick={() => setShowFullSizeModal(false)}>
                <div className={`relative bg-white rounded-lg shadow-xl overflow-hidden ${document.mime_type === 'application/pdf'
                ? 'w-[95vw] max-w-6xl h-[90vh]'
                : 'max-w-[90vw] max-h-[90vh]'}`}>
                    <div className="absolute top-4 right-4 z-10">
                        <Button id={`${id}-close-modal-button`} variant="ghost" size="sm" onClick={() => setShowFullSizeModal(false)} className="bg-black bg-opacity-50 text-white hover:bg-opacity-75 rounded-full p-2">
                            <X className="w-4 h-4"/>
                        </Button>
                    </div>
                    <div className="p-4">
                        ...
                        <div className="flex justify-center items-center" onClick={(e) => e.stopPropagation()}>
                            {document.mime_type?.startsWith('image/') ? (<img src={`/api/documents/view/${document.file_id}`} alt={document.document_name} className="max-w-full max-h-[70vh] object-contain"/>) : document.mime_type?.startsWith('video/') ? (<VideoModalComponent fileId={document.file_id || ''} documentId={document.document_id} mimeType={document.mime_type || ''} fileName={document.document_name}/>) : document.mime_type === 'application/pdf' ? (<iframe src={`/api/documents/view/${document.file_id}`} className="w-full border-0" style={{ height: 'calc(90vh - 120px)', width: '100%' }} title={document.document_name}/>) : (<div className="text-center p-8">
                                    <p className="text-[rgb(var(--color-text-500))]">{t('documents.previewUnavailable', 'Preview unavailable')}</p>
                                    <Button id={`${id}-download-modal-button`} onClick={async () => {
                    const downloadUrl = getDocumentDownloadUrl(document.file_id);
                    const filename = document.document_name || 'download';
                    try {
                        await downloadDocument(downloadUrl, filename, true);
...
const DocumentStorageCard = memo(DocumentStorageCardComponent, (prevProps, nextProps) => {
    return (prevProps.document.document_id === nextProps.document.document_id &&

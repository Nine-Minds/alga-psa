 ...
 */
export function DocumentSkeletonCard({ id }) {
    return (<ReflectionContainer id={id} label="Document Skeleton Card">
      <div className="bg-white rounded-lg border border-[rgb(var(--color-border-200))] shadow-sm p-4 h-full flex flex-col">
        <div className="flex-1">
          ...
          <div className="mt-4 preview-container">
            <div className="bg-gray-200 rounded-md animate-pulse" style={{ height: '150px', width: '100%' }}></div>
          </div>
        </div>
 ...
 */
export function DocumentsGridSkeleton({ count = 6, gridColumns = 3 }) {
    const gridColumnsClass = gridColumns === 4
        ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'
        : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3';
    return (<div className={`grid ${gridColumnsClass} gap-4`}>
      {Array.from({ length: count }).map((_, index) => (<div key={`skeleton-${index}`} className="h-full">

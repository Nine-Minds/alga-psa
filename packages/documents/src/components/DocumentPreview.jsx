...
const DocumentPreview = ({ document, className }) => {
    ...
    // Loading state
    if (isLoading) {
        return (<div className={`flex items-center justify-center h-48 bg-gray-100 rounded-md ${className}`}>
                <span className="text-gray-600">Loading preview...</span>
            </div>);
    }
    // Error state
    if (error) {
        return (<div className={`flex items-center justify-center h-48 bg-red-50 rounded-md ${className}`}>
                <span className="text-red-600">{error}</span>
            </div>);
    }
    // Image preview
    if (mime.startsWith('image/')) {
        return (<div className={`relative w-full h-48 ${className}`}>
                <Image src={`/api/documents/view/${document.file_id}/preview`} alt={document.document_name} fill className="object-contain rounded-md"/>
            </div>);
    }
    // PDF preview
    if (mime === 'application/pdf' && preview) {
        return (<div className={`h-48 bg-gray-100 rounded-md overflow-hidden ${className}`}>
                {preview.previewImage ? (<div className="relative w-full h-full">
                        {/* Preview image */}
                        <Image src={preview.previewImage} alt={`Preview of ${document.document_name}`} fill className="object-contain"/>
                        {/* Page count overlay */}
    ...
    if ((mime === 'text/markdown' || document.document_name.endsWith('.md')) && preview?.content) {
        return (<div className={`h-48 p-4 bg-white rounded-md overflow-auto prose prose-sm max-w-none ${className}`} dangerouslySetInnerHTML={{ __html: preview.content }}/>);
    }
    // Text preview (including JSON)
    if ((mime.startsWith('text/') || mime === 'application/json') && preview?.content) {
        return (<div className={`h-48 p-4 bg-gray-100 rounded-md overflow-auto ${className}`}>
    ...
    if (mime.startsWith('video/')) {
        if (preview?.previewImage) {
            return (<div className={`relative w-full h-48 ${className}`}>
                    <Image src={preview.previewImage} alt={`Thumbnail of ${document.document_name}`} fill className="object-contain rounded-md bg-black"/>
        ...
        return (<div className={`flex items-center justify-center h-48 bg-gray-900 rounded-md ${className}`}>
                <div className="text-center">

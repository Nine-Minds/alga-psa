...
const Documents = ({ id = 'documents', documents: initialDocuments, gridColumns, userId, entityId, entityType, isLoading = false, onDocumentCreated, isInDrawer = false, uploadFormRef, searchTermFromParent = '', filters, namespace = 'common' }) => {
    const { t } = useTranslation('common');
    const router = useRouter();
    const searchParams = useSearchParams();
    const [documentsToDisplay, setDocumentsToDisplay] = useState(initialDocuments);
    ...
    const [currentContent, setCurrentContent] = useState(DEFAULT_BLOCKS);
    ...
    const { value: folderViewMode, setValue: setFolderViewMode } = useUserPreference(DOCUMENT_VIEW_MODE_SETTING, {
        defaultValue: 'grid',
    ...
    const { value: folderGridPageSize, setValue: setFolderGridPageSize } = useUserPreference(DOCUMENT_GRID_PAGE_SIZE_SETTING, {
        defaultValue: 9,
    ...
    const { value: folderListPageSize, setValue: setFolderListPageSize } = useUserPreference(DOCUMENT_LIST_PAGE_SIZE_SETTING, {
        defaultValue: 10,
    ...
    const [selectedDocumentsForMove, setSelectedDocumentsForMove] = useState(new Set());
    ...
    const [documentFolderPath, setDocumentFolderPath] = useState(null);
    // Move document modal
    ...
    const prevInitialDocumentsRef = useRef('');
    ...
    useEffect(() => {
        ...
        if (searchTermFromParent) {
            const filtered = initialDocuments.filter(doc => doc.document_name.toLowerCase().includes(searchTermFromParent.toLowerCase()));
    ...
    const handleFolderCreated = async (folderPath) => {
        try {
            // Create the folder in the database
            await createFolder(folderPath);
            // Show success toast
            toast.success(t('documents.messages.folderCreated', {
                name: folderPath,
    ...
    const handleMoveDocuments = async (targetFolder) => {
        if (selectedDocumentsForMove.size === 0)
            return;
        try {
            await moveDocumentsToFolder(Array.from(selectedDocumentsForMove), targetFolder);
            const count = selectedDocumentsForMove.size;
            const folderName = targetFolder || t('documents.folders.root', 'Root');
            toast.success(t('documents.messages.moveDocumentsSuccess', {
                count,
    ...
    const handleDocumentFolderSelected = (folderPath) => {
    ...
    const handleDisassociate = useCallback(async (document) => {
        if (!entityId || !entityType)
            return;
        try {
            ...
            setError(t('documents.messages.removeAssociationFailed', 'Failed to remove document association'));
        }
    }, [entityId, entityType, onDocumentCreated]);
    const handleMoveDocument = useCallback((document) => {
        setDocumentToMove(document);
        setShowMoveFolderModal(true);
    }, []);
    const handleMoveFolderSelected = async (folderPath) => {
        if (!documentToMove)
            return;
        try {
            await moveDocumentsToFolder([documentToMove.document_id], folderPath);
            toast.success(t('documents.messages.moveDocumentSuccess', {
    ...
    const handleSaveNewDocument = async () => {
        try {
            if (!newDocumentName.trim()) {
                setDrawerError(t('documents.validation.nameRequired', 'Document name is required'));
                return;
            }
            setIsSaving(true);
            setDrawerError(null);
            ...
            setDrawerError(t('documents.messages.createFailed', 'Failed to create document'));
        }
        finally {
            setIsSaving(false);
        }
    ...
    const handleSaveChanges = async () => {
        try {
            ...
            setDrawerError(t('documents.messages.saveFailed', 'Failed to save document'));
        }
        finally {
            setIsSaving(false);
        }
        ...
        const loadContent = async () => {
            if (selectedDocument?.document_id) {
                ...
                try {
                    ...
                    setError(t('documents.messages.loadContentFailed', 'Failed to load document content'));
                    setCurrentContent(DEFAULT_BLOCKS);
                }
                finally {
                    setIsLoadingContent(false);
    ...
    const moveHandlersRef = useRef(new Map());
    const handleDocumentClick = async (document) => {
        ...
        try {
            ...
            toast.error(t('documents.messages.downloadFailed', 'Failed to download document'));
        }
    };
    const getOrCreateClickHandler = (document) => {
    ...
    const deleteHandlersRef = useRef(new Map());
    const disassociateHandlersRef = useRef(new Map());
    const getOrCreateDeleteHandler = (document) => {
        const key = document.document_id;
    ...
    const getOrCreateDisassociateHandler = (document) => {
        if (!entityId || !entityType)
            return undefined;
        const key = document.document_id;
    ...
    const getOrCreateMoveHandler = (document) => {
        const key = document.document_id;
    ...
    const renderDocumentCards = () => {
        return documentsToDisplay.map((document) => {
            ...
            return (<div key={document.document_id} className="h-full">
          <DocumentStorageCard id={`${id}-document-${document.document_id}`} document={document} onDelete={getOrCreateDeleteHandler(document)} onDisassociate={getOrCreateDisassociateHandler(document)} onMove={getOrCreateMoveHandler(document)} showDisassociate={Boolean(entityId && entityType)} showMove={inFolderMode} forceRefresh={editedDocumentId === document.document_id ? refreshTimestamp : undefined} onClick={getOrCreateClickHandler(document)} isContentDocument={!document.file_id}/>
        </div>);
        });
    };
    // Folder mode: show folder tree sidebar and new layout
    if (inFolderMode) {
        return (<ReflectionContainer id={id} label="Documents">
        <div className="flex flex-col h-[calc(100vh-200px)]">
          ...
          <div className="border-b border-gray-200 p-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {/* New Document Button */}
              <Button id={`${id}-new-document-btn`} variant="default" onClick={handleCreateDocument}>
                <FileText className="w-4 h-4 mr-2"/>
                New Document
              </Button>

              {/* Upload Button */}
              <Button id={`${id}-upload-btn`} variant="default" onClick={() => setShowUpload(true)}>
                <Plus className="w-4 h-4 mr-2"/>
                Upload
              </Button>

              {/* Create Folder Button */}
              <Button id={`${id}-new-folder-btn`} variant="outline" onClick={() => setShowFolderManager(true)}>
            ...
            {/* View Mode Switcher */}
            <ViewSwitcher currentView={viewMode} onChange={(view) => setViewMode(view)} options={[
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            {!isFoldersPaneCollapsed && (<div className="w-64 flex-shrink-0 border-r border-gray-200">
                <FolderTreeView key={folderTreeKey} selectedFolder={currentFolder} onFolderSelect={handleFolderSelect} onFolderDeleted={() => {
                    refreshDocuments();
                }} isCollapsed={isFoldersPaneCollapsed} onToggleCollapse={() => setIsFoldersPaneCollapsed(!isFoldersPaneCollapsed)}/>
              </div>)}

            {/* Main Content Area */}
            <div className="flex-1 flex flex-col overflow-hidden">

            {showUpload && (<div ref={uploadFormRef} className="m-4 p-4 border border-gray-200 rounded-md bg-white">
                <DocumentUpload id={`${id}-upload`} userId={userId} entityId={entityId} entityType={entityType} folderPath={currentFolder} onUploadComplete={async () => {
                    setShowUpload(false);
                    await refreshDocuments();
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {isLoading ? (<DocumentsGridSkeleton gridColumns={3}/>) : error ? (<div className="text-center py-4 text-red-500 bg-red-50 rounded-md">
                  {error}
                </div>) : (<>
                  {/* Bulk Actions Toolbar */}
                  {selectedDocumentsForMove.size > 0 && viewMode === 'list' && (<div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          {!entityId && !entityType && (<Button id="bulk-move-button" variant="ghost" size="sm" onClick={() => setShowBulkMoveFolderModal(true)}>
                              <FolderInput className="w-4 h-4 mr-2"/>
                              {t('documents.bulkActions.moveToFolder', 'Move to Folder')}
                            </Button>)}
                          <Button id="bulk-delete-button" variant="ghost" size="sm" onClick={async () => {
                        const count = selectedDocumentsForMove.size;
                        if (!confirm(t('documents.prompts.confirmBulkDelete', {
                            count,
                        ...
                        try {
                            ...
                            toast.success(t('documents.messages.bulkDeleteSuccess', {
                                count,
                            ...
                            toast.error(t('documents.messages.bulkDeleteFailed', 'Failed to delete some documents'));
                        }
                    }}>
                      ...
                      <Button id="bulk-clear-selection-button" variant="ghost" size="sm" onClick={() => setSelectedDocumentsForMove(new Set())}>
                        <X className="w-4 h-4 mr-2"/>
                        {t('documents.bulkActions.clearSelection', 'Clear Selection')}
                      </Button>
                    </div>)}

                  {viewMode === 'list' ? (<DocumentListView documents={documentsToDisplay} selectedDocuments={selectedDocumentsForMove} onSelectionChange={setSelectedDocumentsForMove} onDelete={(doc) => handleDelete(doc)} onClick={(doc) => handleDocumentClick(doc)}/>) : (documentsToDisplay.length > 0 ? (<div className={`grid ${gridColumnsClass} gap-4`}>
                        {renderDocumentCards()}
                      </div>) : (<div className="text-center py-8 text-gray-500 bg-gray-50 rounded-md">
            ...
            {/* Pagination */}
            {documentsToDisplay.length > 0 && totalPages > 1 && (<div className="border-t border-gray-200 p-4">
                <DocumentsPagination id={`${id}-pagination`} currentPage={currentPage} totalItems={totalDocuments} itemsPerPage={pageSize} onPageChange={handlePageChange} onItemsPerPageChange={handlePageSizeChange} itemsPerPageOptions={viewMode === 'grid' ? gridPageSizeOptions : undefined}/>
              </div>)}
          </div>

          {/* Folder Manager Dialog */}
          <FolderManager open={showFolderManager} onClose={() => setShowFolderManager(false)} currentFolder={currentFolder} onFolderCreated={handleFolderCreated}/>

          {/* Folder Selector Modal for New Documents */}
          <FolderSelectorModal isOpen={showDocumentFolderModal} onClose={() => setShowDocumentFolderModal(false)} onSelectFolder={handleDocumentFolderSelected} title={t('documents.folderSelector.newDocumentTitle', 'Select Folder for New Document')} description={t('documents.folderSelector.newDocumentDescription', 'Choose where to save this new document')} namespace={namespace}/>

          {/* Folder Selector Modal for Moving Documents */}
          <FolderSelectorModal isOpen={showMoveFolderModal} onClose={() => {
                setShowMoveFolderModal(false);
                setDocumentToMove(null);
            }} onSelectFolder={handleMoveFolderSelected} title={t('documents.folderSelector.moveTitle', 'Move Document')} description={documentToMove
                ...
                : t('documents.folderSelector.moveDescription', 'Select destination folder')} namespace={namespace}/>

          {/* Folder Selector Modal for Bulk Moving Documents */}
          <FolderSelectorModal isOpen={showBulkMoveFolderModal} onClose={() => {
                setShowBulkMoveFolderModal(false);
            }} onSelectFolder={handleMoveDocuments} title={t('documents.folderSelector.bulkMoveTitle', 'Move Selected Documents')} description={t('documents.folderSelector.bulkMoveDescription', {
                count: selectedDocumentsForMove.size,
                defaultValue: `Select destination folder for ${selectedDocumentsForMove.size} document${selectedDocumentsForMove.size !== 1 ? 's' : ''}`
            })} namespace={namespace}/>

          {/* Preview Modal for Images/Videos/PDFs */}
          {showPreviewModal && previewDocument && previewDocument.file_id && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75" onClick={() => setShowPreviewModal(false)}>
              <div className="relative max-w-7xl max-h-[90vh] w-full mx-4" onClick={(e) => e.stopPropagation()}>
                <button className="absolute top-4 right-4 text-white hover:text-gray-300 z-10" onClick={() => setShowPreviewModal(false)}>
                  <X className="w-8 h-8"/>
                </button>
                {previewDocument.mime_type?.startsWith('image/') && (<img src={previewDocument.preview_file_id ? `/api/documents/${previewDocument.document_id}/preview` : `/api/documents/view/${previewDocument.file_id}`} alt={previewDocument.document_name} className="max-w-full max-h-[90vh] object-contain mx-auto"/>)}
                {previewDocument.mime_type?.startsWith('video/') && (<video src={`/api/documents/view/${previewDocument.file_id}`} controls className="max-w-full max-h-[90vh] mx-auto"/>)}
                {previewDocument.mime_type === 'application/pdf' && (<iframe src={`/api/documents/view/${previewDocument.file_id}`} className="w-full h-[90vh] bg-white" title={previewDocument.document_name}/>)}
        ...
        <div className="document-drawer">
          <Drawer id={`${id}-document-drawer`} isOpen={isDrawerOpen} onClose={() => {
                setIsDrawerOpen(false);
                setDrawerError(null);
            }} isInDrawer={isInDrawer} hideCloseButton={true} drawerVariant="document">
          <div className="flex flex-col h-full">
            <div className="flex justify-between items-center mb-4 pb-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                (selectedDocument.type_name === 'text/plain' ||
                    ...
                    (!selectedDocument.type_name && !selectedDocument.file_id)) && (<Button id={`${id}-download-pdf-btn`} onClick={async () => {
                    if (selectedDocument) {
                ...
                {!isCreatingNew && !isEditModeInDrawer && selectedDocument && (<Button id={`${id}-edit-document-btn`} onClick={() => setIsEditModeInDrawer(true)} variant="outline" size="sm">
                    <Edit3 className="w-4 h-4 mr-2"/>
                    Edit
                  </Button>)}
                {!isInDrawer && (<Button id={`${id}-close-drawer-btn`} onClick={() => {
                    setIsDrawerOpen(false);
                    setDrawerError(null);
                    if (!isCreatingNew) {
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
              {drawerError && (<div className="mb-4 bg-red-50 border border-red-300 text-red-700 px-4 py-3 rounded">
                  {drawerError}
                </div>)}
              <div className="mb-4 relative p-0.5">
                <Input id={`${id}-document-name`} type="text" placeholder="Document Name *" value={isCreatingNew ? newDocumentName : documentName} onChange={(e) => {
            ...
            }} readOnly={!isCreatingNew && !isEditModeInDrawer} className={(!isCreatingNew && !isEditModeInDrawer) ? "bg-gray-100 cursor-default" : ""}/>
              </div>

              <div className="flex-1 overflow-y-auto mb-4 p-2">
                <div className="h-full w-full">
                  {isLoadingContent ? (<div className="flex justify-center items-center h-full">
                      <Spinner size="sm"/>
                    </div>) : isCreatingNew || (selectedDocument && isEditModeInDrawer) ? (<TextEditor key={isCreatingNew ? "editor-new" : `editor-${selectedDocument?.document_id}`} id={`${id}-editor`} initialContent={currentContent} onContentChange={handleContentChange} editorRef={editorRef} searchMentions={searchUsersForMentions}/>) : selectedDocument ? (<RichTextViewer id={`${id}-viewer`} content={currentContent}/>) : (<div className="flex justify-center items-center h-full text-gray-500">
              ...
              <div className="flex justify-end space-x-2">
                <Button id={`${id}-cancel-btn`} onClick={() => {
                setIsDrawerOpen(false);
                setDrawerError(null);
                if (!isCreatingNew) {
                    setIsEditModeInDrawer(false);
                ...
                {(isCreatingNew || isEditModeInDrawer) && (<Button id={`${id}-save-btn`} onClick={isCreatingNew ? handleSaveNewDocument : handleSaveChanges} disabled={isSaving || (!hasContentChanged && !isCreatingNew && documentName === selectedDocument?.document_name)} variant="default" className={isCreatingNew && !newDocumentName.trim() ? 'opacity-50' : ''}>
      ...
      <div className="w-full space-y-4">
        <div className="flex justify-between items-center mb-4">
          <div className="flex space-x-2">
            <Button id={`${id}-new-document-btn`} onClick={handleCreateDocument} variant="default">
              <FileText className="w-4 h-4 mr-2"/>
              {t('documents.newDocument', 'New Document')}
            </Button>
            <Button id={`${id}-upload-btn`} onClick={() => {
            setShowUpload(true);
            ...
            {entityId && entityType && (<Button id={`${id}-link-documents-btn`} onClick={() => setShowSelector(true)} variant="default" data-testid="link-documents-button">
        ...
        {showUpload && (<div ref={uploadFormRef} className="mb-4 p-4 border border-gray-200 rounded-md bg-white">
            <DocumentUpload id={`${id}-upload`} userId={userId} entityId={entityId} entityType={entityType} onUploadComplete={async () => {
                setShowUpload(false);
        ...
        {showSelector && entityId && entityType ? (<DocumentSelector id={`${id}-selector`} entityId={entityId} entityType={entityType} onDocumentsSelected={async () => {
                setShowSelector(false);
                // Refresh the documents list (triggers router.refresh() in entity mode)
                await refreshDocuments();
            }} isOpen={showSelector} onClose={() => setShowSelector(false)}/>) : null}

        {error && (<div className="text-center py-4 text-red-500 bg-red-50 rounded-md">
            {error}
          </div>)}

        {isLoading ? (<DocumentsGridSkeleton gridColumns={gridColumns}/>) : documentsToDisplay.length > 0 ? (<div className={`grid ${gridColumnsClass} gap-4`}>
            {renderDocumentCards()}
          </div>) : (<div className="text-center py-8 text-gray-500 bg-gray-50 rounded-md">
            {t('documents.empty.default', 'No documents found')}
          </div>)}

        {documentsToDisplay.length > 0 && totalPages > 1 && (<div className="mt-4">
            <DocumentsPagination id={`${id}-pagination`} currentPage={currentPage} totalItems={totalDocuments} itemsPerPage={pageSize} onPageChange={handlePageChange} onItemsPerPageChange={handlePageSizeChange} itemsPerPageOptions={viewMode === 'grid' ? gridPageSizeOptions : undefined}/>
          </div>)}

  ...
  const tDoc = (key, options) => t(`${documentKeyPrefix}${key}`, options);
    ...
    const handleFolderCreated = async (folderPath) => {
        try {
            ...
            // Show success toast
            toast.success(tDoc('messages.folderCreated', {
                name: folderPath,
    ...
    const handleMoveDocuments = async (targetFolder) => {
        ...
        try {
            ...
            const folderName = targetFolder || tDoc('folders.root', 'Root');
            toast.success(tDoc('messages.moveDocumentsSuccess', {
                count,
    ...
    const handleDelete = useCallback(async (document) => {
        ...
        try {
            ...
            toast.success(tDoc('messages.deleteSuccess', {
            ...
            const errorMessage = error instanceof Error && error.message
                ? error.message
                : tDoc('messages.deleteFailed', 'Failed to delete document');
            toast.error(errorMessage);
            setError(errorMessage);
        }
    ...
    const handleDisassociate = useCallback(async (document) => {
        ...
        try {
            ...
            setError(tDoc('messages.removeAssociationFailed', 'Failed to remove document association'));
        }
    ...
    const handleMoveFolderSelected = async (folderPath) => {
        ...
        try {
            ...
            toast.success(tDoc('messages.moveDocumentSuccess', {
    ...
    const handleSaveNewDocument = async () => {
        try {
            if (!newDocumentName.trim()) {
                setDrawerError(tDoc('validation.nameRequired', 'Document name is required'));
                return;
            }
            setIsSaving(true);
            setDrawerError(null);
            ...
            setDrawerError(tDoc('messages.createFailed', 'Failed to create document'));
        }
        finally {
            setIsSaving(false);
        }
    ...
    const handleSaveChanges = async () => {
        try {
            ...
            setDrawerError(tDoc('messages.saveFailed', 'Failed to save document'));
        }
        finally {
            setIsSaving(false);
        }
        ...
        const loadContent = async () => {
            if (selectedDocument?.document_id) {
                ...
                try {
                    ...
                    setError(tDoc('messages.loadContentFailed', 'Failed to load document content'));
                    setCurrentContent(DEFAULT_BLOCKS);
                }
                finally {
                    setIsLoadingContent(false);
    ...
    const handleDocumentClick = async (document) => {
        ...
        try {
            ...
            toast.error(tDoc('messages.downloadFailed', 'Failed to download document'));
        }
    };
              ...
              {isLoading ? (<DocumentsGridSkeleton gridColumns={3}/>) : error ? (<div className="text-center py-4 text-red-500 bg-red-50 rounded-md">
                  ...
                  {selectedDocumentsForMove.size > 0 && viewMode === 'list' && (<div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        const count = selectedDocumentsForMove.size;
                        if (!confirm(tDoc('prompts.confirmBulkDelete', {
                            count,
                        ...
                        try {
                            ...
                            toast.success(tDoc('messages.bulkDeleteSuccess', {
                                count,

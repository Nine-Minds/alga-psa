...
import { publishEvent } from '@server/lib/eventBus/publishers';

// Import existing service functions
import * as invoiceService from '@server/lib/services/invoiceService';
import { generateInvoiceNumber } from '@product/actions/invoiceGeneration';
...
import { PDFGenerationService, createPDFGenerationService } from '@server/services/pdf-generation.service';
import { StorageService } from '@server/lib/storage/StorageService';

// Import workflow actions
import { approveInvoice, rejectInvoice, processInvoiceEvent } from '@product/actions/invoiceWorkflowActions';
...
} from '@server/interfaces/invoice.interfaces';

import { IBillingResult, IBillingCharge, IClientContractLineCycle } from '@server/interfaces/billing.interfaces';
import { IClient } from '@server/interfaces/client.interfaces';
import { ISO8601String } from '@server/types/types.d';
import {
  getClientDetails,
  persistManualInvoiceItems,
  calculateAndDistributeTax,
  updateInvoiceTotalsAndRecordTransaction
} from '@server/lib/services/invoiceService';

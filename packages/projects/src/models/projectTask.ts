...
const ProjectTaskModel = {
  addTask: async (knexOrTrx: Knex | Knex.Transaction, tenant: string, phaseId: string, taskData: Omit<IProjectTask, 'task_id' | 'phase_id' | 'created_at' | 'updated_at' | 'tenant' | 'wbs_code' | 'actual_hours'> & { order_key?: string }): Promise<IProjectTask> => {
    return withCoManagedOperationalTransaction(knexOrTrx, tenant, async (trx) => {
      try {
        ...
        const newWbsCode = await ProjectModel.generateNextWbsCode(trx, tenant, phase.wbs_code);
        ...
        const [newTask] = await tenantScopedTable<IProjectTask>(trx, 'project_tasks', tenant)
          .insert({
            ...taskData,
            task_id: uuidv4(),
  ...
  updateTask: async (knexOrTrx: Knex | Knex.Transaction, tenant: string, taskId: string, taskData: Partial<IProjectTask>): Promise<IProjectTask> => {
    return withCoManagedOperationalTransaction(knexOrTrx, tenant, async (trx) => {
      try {
        ...
        const [updatedTask] = await tenantScopedTable<IProjectTask>(trx, 'project_tasks', tenant)
  ...
  updateTaskStatus: async (knexOrTrx: Knex | Knex.Transaction, tenant: string, taskId: string, projectStatusMappingId: string): Promise<IProjectTask> => {
    return withCoManagedOperationalTransaction(knexOrTrx, tenant, async (trx) => {
      try {
        ...
        const newWbsCode = await ProjectModel.generateNextWbsCode(trx, tenant, parentWbs);
  ...
  addChecklistItem: async (knexOrTrx: Knex | Knex.Transaction, tenant: string, taskId: string, itemData: Omit<ITaskChecklistItem, 'checklist_item_id' | 'task_id' | 'created_at' | 'updated_at' | 'tenant'>): Promise<ITaskChecklistItem> => {
    return withCoManagedOperationalTransaction(knexOrTrx, tenant, async (trx) => {
      ...
      const [newItem] = await tenantScopedTable<ITaskChecklistItem>(trx, 'task_checklist_items', tenant)
        .insert({
          ...itemData,
          task_id: taskId,
  ...
  updateChecklistItem: async (knexOrTrx: Knex | Knex.Transaction, tenant: string, checklistItemId: string, itemData: Partial<ITaskChecklistItem>): Promise<ITaskChecklistItem> => {
    return withCoManagedOperationalTransaction(knexOrTrx, tenant, async (trx) => {
      ...
      const [updatedItem] = await tenantScopedTable<ITaskChecklistItem>(trx, 'task_checklist_items', tenant)
  ...
  addTaskResource: async (knexOrTrx: Knex | Knex.Transaction, tenant: string, taskId: string, userId: string, role?: string): Promise<void> => {
    return withCoManagedOperationalTransaction(knexOrTrx, tenant, async (trx) => {
      try {
        ...
        const existingResource = await tenantScopedTable(trx, 'task_resources', tenant)
  ...
  addTaskTicketLink: async (knexOrTrx: Knex | Knex.Transaction, tenant: string, projectId: string, taskId: string | null, ticketId: string, phaseId: string): Promise<IProjectTicketLink> => {
    return withCoManagedOperationalTransaction(knexOrTrx, tenant, async (trx) => {
      try {
        ...
        const existingLink = await tenantScopedTable(trx, 'project_ticket_links', tenant)
          .where({
        ...
        const [newLink] = await tenantScopedTable<IProjectTicketLink>(trx, 'project_ticket_links', tenant)
          .insert({
            link_id: uuidv4(),
            project_id: projectId,
            phase_id: phaseId,

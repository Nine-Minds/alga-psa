...
import {
  ...
  TACTICAL_INSTANCE_URL_SECRET,
  TACTICAL_KNOX_PASSWORD_SECRET,
  TACTICAL_KNOX_TOKEN_SECRET,
  TACTICAL_KNOX_USERNAME_SECRET,
  TACTICAL_WEBHOOK_SECRET,
  type TacticalRmmAuthMode,
} from '../../lib/rmm/tacticalrmm/shared';
// Device sync and its helpers live in a plain lib module: exports from this
// 'use server' file are callable RPC endpoints, so an unguarded sync could not
// live here.
import {
  axiosErrorToMessage,
  buildConfiguredTacticalClient,
  createTacticalAssetRecord,
  extractOsFields,
  extractVitals,
  inferAssetTypeFromTacticalAgent,
  publishRmmSyncEvent,
  runTacticalRmmDeviceSync,
  tenantScopedTable,
  type TacticalDeviceSyncResult,
} from '../../lib/rmm/tacticalrmm/deviceSync';
...
export const syncTacticalRmmDevices = withAuth(async (
  ...
  // attributed to the operator who pressed the button.
  return runTacticalRmmDeviceSync({
    tenant,
    actorUserId: (user as any)?.user_id as string | undefined,
  });
});

...
const state = vi.hoisted(() => ({ category: 'providers' }));
...
vi.mock('./ProviderCredentialsWorkbench', () => ({ ProviderCredentialsWorkbench: () => <div>Google and Microsoft app setup</div> }));
...
describe('integration access by capability', () => {
  it('keeps shared Google and Microsoft email app setup accessible without Pro', () => {
    state.category = 'providers';
    render(<IntegrationsSettingsPage canUseIntegrations={false} />);
    expect(screen.getByText('Google and Microsoft app setup')).toBeInTheDocument();

...
/**
 * Official brand marks for the accounting integrations, rendered inline so they
 * stay crisp at any size and inherit no surrounding color. The source paths are
 * the single-path brand icons (the wordmark is knocked out of the brand-coloured
 * disc), so they read as the real app icons when placed on a white tile.
 */

export type AccountingBrand = 'quickbooks' | 'xero';
...
const MARK_SIZES = {
  sm: { tile: 'h-10 w-10 rounded-lg', glyph: 'h-6 w-6' },
  lg: { tile: 'h-12 w-12 rounded-xl', glyph: 'h-7 w-7' }
} as const;

/**
 * A brand logo presented as an app-style tile: the brand mark on a white face
 * with a hairline ring, so the QuickBooks green and Xero blue stay true in both
 * light and dark themes.
 */
export function AccountingBrandMark({
  brand,
  size = 'sm',
  className
}: {
  ...
  const sizing = MARK_SIZES[size];
  return (
    <div
      className={cn(
        'flex shrink-0 items-center justify-center bg-white shadow-sm ring-1 ring-border',
        sizing.tile,
        className
      )}
      aria-hidden="true"
    >
      {brand === 'quickbooks' ? (
        <QuickBooksGlyph className={sizing.glyph} />
      ) : (
        <XeroGlyph className={sizing.glyph} />
      )}
    </div>
  );
}

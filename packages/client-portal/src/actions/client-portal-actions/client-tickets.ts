...
export const updateClientTicketDescription = withAuth(async (
  user,
  { tenant },
  ticketId: string,
  description: string
  ...
  const canUpdate = await hasPermission(userForPermission, 'ticket', 'update', db);
  if (!canUpdate) {
    throw new Error('Insufficient permissions to update ticket description');
  }
  ...
  await withTransaction(db, async (trx: Knex.Transaction) => {
    ...
    const contact = await trx('contacts')
      .where({ contact_name_id: userRecord.contact_id, tenant })
      .first();
    ...
    const ticket = await trx('tickets')
      .where({ ticket_id: ticketId, tenant, client_id: contact.client_id })
      .first();
    ...
    const updatedAttributes = { ...currentAttributes, description };

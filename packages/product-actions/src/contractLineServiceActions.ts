...
import * as planServiceConfigActions from '@product/actions/contractLineServiceConfigurationActions';
...
export async function getContractLineServicesWithConfigurations(contractLineId: string): Promise<{
  ...
  for (const config of configurations) {
    ...
    if (config.configuration_type === 'Hourly') {
      ...
      const hourlyConfigModel = new (await import('@server/lib/models/contractLineServiceHourlyConfig')).default(trx);

...
import { getEmailNotificationService } from "@server/lib/notifications/email";
...
} from "@server/lib/models/notification";
...
export async function getTemplatesAction(tenant: string): Promise<{
  ...
  const { knex } = await (await import("@server/lib/db")).createTenantKnex();

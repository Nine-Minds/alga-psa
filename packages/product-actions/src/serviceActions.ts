...
import Service, { serviceSchema, refinedServiceSchema } from '@server/lib/models/service'; // Import both schemas
import { IService, IServiceType } from '@server/interfaces/billing.interfaces';
import { withTransaction } from '@alga-psa/shared/db';
import { createTenantKnex } from '@server/lib/db';
import { Knex } from 'knex';
import { validateArray } from '@server/lib/utils/validation';
import { ServiceTypeModel } from '@server/lib/models/serviceType'; // Import ServiceTypeModel
...
export async function getServiceTypesForSelection(): Promise<{ id: string; name: string; billing_method: 'fixed' | 'hourly' | 'usage'; is_standard: boolean }[]> {
   try {
       ...
       // Need to import ServiceTypeModel from '@server/lib/models/serviceType'
       const { ServiceTypeModel } = await import('@server/lib/models/serviceType');
       const { knex: db } = await createTenantKnex();

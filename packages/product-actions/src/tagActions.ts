...
import TagDefinition, { ITagDefinition } from '@server/lib/models/tagDefinition';
import TagMapping, { ITagMapping, ITagWithDefinition } from '@server/lib/models/tagMapping';
import { ITag, TaggedEntityType } from 'server/src/interfaces/tag.interfaces';
import { withTransaction } from '@alga-psa/shared/db';
import { createTenantKnex, getCurrentTenantId } from '@server/lib/db';
import { getCurrentUser } from '@product/actions/user-actions/userActions';
import { hasPermission } from '@server/lib/auth/rbac';
import { throwPermissionError } from '@server/lib/utils/errorHandling';
  ...
  return await withTransaction(db, async (trx: Knex.Transaction) => {
    try {
      ...
      if (existingTag && (existingTag.background_color || existingTag.text_color)) {
        ...
        const { generateEntityColor } = await import('@server/utils/colorUtils');

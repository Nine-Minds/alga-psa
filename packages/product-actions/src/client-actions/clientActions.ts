...
import { unparseCSV } from '@server/lib/utils/csvParser';
import { createDefaultTaxSettings } from '@product/actions/taxSettingsActions';
import { revalidatePath } from 'next/cache';
import { getCurrentUser } from '@product/actions/user-actions/userActions';
import { hasPermission } from '@server/lib/auth/rbac';
import { getClientLogoUrl, getClientLogoUrlsBatch } from '@server/lib/utils/avatarUtils';
...
import { createTag } from '@product/actions/tagActions';
...
export async function exportClientsToCSV(clients: IClient[]): Promise<string> {
  ...
  const exportData = await withTransaction(knex, async (trx: Knex.Transaction) => {
    ...
    const { findTagsByEntityIds } = await import('@product/actions/tagActions');

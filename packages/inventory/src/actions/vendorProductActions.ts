...
export const listVendorProducts = withAuth(
  ...
  ): Promise<Array<IVendorProduct & { service_name: string | null; sku: string | null; vendor_name: string | null }>> => {
...
export const deleteVendorProduct = withAuth(
  async (user, { tenant }, vendorId: string, serviceId: string): Promise<{ removed: boolean }> => {
    await requireVendorPerm(user, 'update');
    const { knex: db } = await createTenantKnex();
    return withTransaction(db, async (trx: Knex.Transaction) => {

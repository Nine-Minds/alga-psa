...
type VendorBillActionError = ActionMessageError | ActionPermissionError;

function vendorBillActionErrorFrom(error: unknown): VendorBillActionError | null {
...
async function withVendorBillActionErrors<T>(work: () => Promise<T>): Promise<T | VendorBillActionError> {
  try {
    return await work();
  } catch (error) {
    const expected = vendorBillActionErrorFrom(error);
...
export const listVendorBills = withAuth(
  ...
  ): Promise<Array<IVendorBill & { vendor_name: string | null; po_number: string | null }> | VendorBillActionError> => {
...
export const getVendorBill = withAuth(
  async (user, { tenant }, billId: string): Promise<VendorBillView | VendorBillActionError> => {
    return withVendorBillActionErrors(async () => {
      await requireBillPerm(user, 'read');
      const { knex: db } = await createTenantKnex();
      return withTransaction(db, async (trx: Knex.Transaction) => {
...
export const createVendorBill = withAuth(
  ...
  ): Promise<IVendorBill | VendorBillActionError> => {
    return withVendorBillActionErrors(async () => {
...
export const createBillFromPo = withAuth(
  async (user, { tenant }, poId: string, billNumber: string): Promise<IVendorBill | VendorBillActionError> => {
    return withVendorBillActionErrors(async () => {
      await requireBillPerm(user, 'create');
      const { knex: db } = await createTenantKnex();
...
export const setVendorBillStatus = withAuth(
  async (user, { tenant }, billId: string, status: VendorBillStatus): Promise<IVendorBill | VendorBillActionError> => {
    return withVendorBillActionErrors(async () => {
...
export const openVendorBillsAging = withAuth(
  async (user, { tenant }): Promise<VendorBillAgingRow[] | VendorBillActionError> => {
    return withVendorBillActionErrors(async () => {
      await requireBillPerm(user, 'read');
      const { knex: db } = await createTenantKnex();
      return withTransaction(db, async (trx: Knex.Transaction) => {

...
export const listPoLandedCosts = withAuth(
  async (user, { tenant }, poId: string): Promise<IPoLandedCost[]> => {
...
export const removePoLandedCost = withAuth(
  async (user, { tenant }, landedCostId: string): Promise<{ removed: boolean }> => {
    await requirePoPerm(user, 'update');
    const { knex: db } = await createTenantKnex();
    return withTransaction(db, async (trx: Knex.Transaction) => {
...
export const applyPoLandedCosts = withAuth(
  async (user, { tenant }, poId: string): Promise<ApplyLandedCostsResult> => {
    await requirePoPerm(user, 'update');
    const { knex: db } = await createTenantKnex();
    return withTransaction(db, async (trx: Knex.Transaction) => {

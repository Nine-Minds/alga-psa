...
describe('loanerActionErrorFrom', () => {
  ...
  it('maps the expected loaner business-rule failures to actionable messages', () => {
    expect(loanerActionErrorFrom(new Error('client_id is required to loan out a unit'))).toEqual({
      ...
      messageKey: 'features/inventory:errors.loaners.clientRequired',
    });
    expect(loanerActionErrorFrom(new Error('location_id is required to return a loaner'))).toEqual({
      ...
      messageKey: 'features/inventory:errors.loaners.returnLocationRequired',
    });
    expect(loanerActionErrorFrom(new Error('loan_due_at must be a valid date'))).toEqual({
      actionError: 'Choose a valid due date.',
      messageKey: 'features/inventory:errors.loaners.dueDateInvalid',
    });
    expect(loanerActionErrorFrom(new Error('Stock unit not found'))).toEqual({
      ...
      messageKey: 'features/inventory:errors.shared.stockUnitNotFound',
    });
  });
  ...
  it('humanizes the snake_case status embedded in loan-out / return guard errors', () => {
    ...
    expect(
      ...
      actionError: "This unit can't be loaned out — it's currently On loan.",
      messageKey: 'features/inventory:errors.loaners.cannotLoanOutStatus.on_loan',
    });

    expect(
      loanerActionErrorFrom(new Error('Unit must be on_loan to return (current status: in_rma)')),
    ).toEqual({
      actionError: "This unit isn't out on loan — it's currently In RMA.",
      messageKey: 'features/inventory:errors.loaners.notOnLoanStatus.in_rma',
    });
  });

  it('leaves a status outside the label table keyless so it degrades to English', () => {
    // Each status has its own whole sentence in the catalogue; an unknown one has none,
    // so the payload must carry no key rather than point at a key that does not resolve.
    expect(
      loanerActionErrorFrom(new Error('Unit must be in_stock to loan out (current status: some_future_status)')),
    ).toEqual({ actionError: "This unit can't be loaned out — it's currently some future status." });
  });
  ...
  it('maps a 22P02 uuid-cast error to a safe message instead of leaking raw Knex SQL', () => {
    ...
    expect(
      ...
      messageKey: 'features/inventory:errors.loaners.invalidReferenceUnitClient',
    });
  });
  ...
  it('maps foreign-key and unique-violation codes, and returns null for unexpected errors', () => {
    expect(loanerActionErrorFrom({ code: '23503' })).toEqual({
      ...
      messageKey: 'features/inventory:errors.loaners.recordInvalid',
    });
    expect(loanerActionErrorFrom({ code: '23505' })).toEqual({
      ...
      messageKey: 'features/inventory:errors.loaners.conflict',
    });
...
describe('restockReturnActionErrorFrom', () => {
  ...
  it('maps the expected restock business-rule failures to actionable messages', () => {
    expect(restockReturnActionErrorFrom(new Error('restocking_fee_cents must be a non-negative integer (cents)'))).toEqual({
      ...
      messageKey: 'features/inventory:errors.restock.feeNonNegative',
    });
    expect(restockReturnActionErrorFrom(new Error('location_id is required to restock this unit'))).toEqual({
      ...
      messageKey: 'features/inventory:errors.restock.locationRequired',
    });
    expect(
      restockReturnActionErrorFrom(new Error('service_id and location_id are required for a non-serialized restock return')),
    ).toEqual({
      actionError: 'Choose a product and location before restocking non-serialized inventory.',
      messageKey: 'features/inventory:errors.restock.productAndLocationRequired',
    });
    expect(
      restockReturnActionErrorFrom(new Error('quantity must be a positive number for a non-serialized restock return')),
    ).toEqual({
      actionError: 'Restock quantity must be greater than zero.',
      messageKey: 'features/inventory:errors.restock.quantityPositive',
    });
    expect(
      restockReturnActionErrorFrom(new Error('This product is serialized; provide unit_id to restock a specific unit')),
    ).toEqual({
      actionError: 'This product is serialized. Choose the specific unit to restock.',
      messageKey: 'features/inventory:errors.restock.serializedUnitRequired',
    });
  });
  ...
  it('humanizes the delivered/returned guard status and keeps the eligibility hint', () => {
    expect(
      ...
      messageKey: 'features/inventory:errors.restock.cannotRestockStatus.on_loan',
    });
  });
  ...
  it('maps a 22P02 uuid-cast error to a safe message (location-oriented copy)', () => {
    expect(restockReturnActionErrorFrom({ code: '22P02' })).toEqual({
      ...
      messageKey: 'features/inventory:errors.restock.invalidReferenceUnitLocation',
    });
  });
  ...
  it('maps constraint codes and returns null for unexpected errors', () => {
    expect(restockReturnActionErrorFrom({ code: '23503' })).toEqual({
      ...
      messageKey: 'features/inventory:errors.restock.recordInvalid',
    });
    expect(restockReturnActionErrorFrom({ code: '23505' })).toEqual({
      ...
      messageKey: 'features/inventory:errors.restock.conflict',
    });

...
export function MaintenanceCommandCenter() {
  ...
  return <div className="h-full min-h-0 overflow-auto bg-[rgb(var(--color-background))] p-4 md:p-6" data-automation-id="maintenance-command-center">
    ...
    <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{([
      { label: 'Overdue', value: aggregates?.overdue ?? 0, accent: 'border-l-red-500', num: aggregates?.overdue ? 'text-red-700' : 'text-[rgb(var(--color-text-900))]', hint: aggregates?.overdue ? 'click to filter' : 'all clear', win:'overdue' as const },
      { label: 'Due today', value: aggregates?.due_today ?? 0, accent: 'border-l-amber-500', num: aggregates?.due_today ? 'text-amber-700' : 'text-[rgb(var(--color-text-900))]', hint: aggregates?.due_today ? 'click to filter' : 'nothing due today', win:'today' as const },
    ...
    {tab === 'queue' && <div className="mb-4 flex flex-wrap gap-2"><div className="relative min-w-[220px] flex-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-[rgb(var(--color-text-500))]" /><Input id="maintenance-search" className="pl-9" placeholder="Search plans, assets, or clients" value={search} onChange={(event) => setSearch(event.target.value)} /></div><select id="maintenance-status-filter" value={status} onChange={(event) => setStatus(event.target.value as AssetMaintenanceOccurrenceStatus | 'all')} className="h-9 rounded-md border border-[rgb(var(--color-border-200))] bg-[rgb(var(--color-card))] px-3 text-sm text-[rgb(var(--color-text-700))]"><option value="open">Actionable</option><option value="all">All statuses</option><option value="completed">Completed</option><option value="skipped">Skipped</option></select><select id="maintenance-client-filter" value={clientId} onChange={(event) => setClientId(event.target.value)} className="h-9 rounded-md border border-[rgb(var(--color-border-200))] bg-[rgb(var(--color-card))] px-3 text-sm text-[rgb(var(--color-text-700))]"><option value="">All clients</option>{clientOptions.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select><select id="maintenance-asset-type-filter" value={assetType} onChange={(event) => setAssetType(event.target.value)} className="h-9 rounded-md border border-[rgb(var(--color-border-200))] bg-[rgb(var(--color-card))] px-3 text-sm text-[rgb(var(--color-text-700))]"><option value="">All asset types</option>{assetTypeOptions.map((item) => <option key={item} value={item}>{item}</option>)}</select><select id="maintenance-plan-type-filter" value={maintenanceType} onChange={(event) => setMaintenanceType(event.target.value)} className="h-9 rounded-md border border-[rgb(var(--color-border-200))] bg-[rgb(var(--color-card))] px-3 text-sm text-[rgb(var(--color-text-700))]"><option value="">All plan types</option>{maintenanceTypeOptions.map((item) => <option key={item} value={item}>{item}</option>)}</select><div className="flex items-center gap-1 rounded-md border border-[rgb(var(--color-border-200))] bg-[rgb(var(--color-card))] px-2"><span className="text-xs font-medium text-[rgb(var(--color-text-500))]">Due</span><input id="maintenance-due-from-filter" type="date" value={dueFrom} onChange={(event) => setDueFrom(event.target.value)} className="border-0 bg-transparent py-1.5 text-sm text-[rgb(var(--color-text-700))] focus:outline-none" /><span className="text-xs text-[rgb(var(--color-text-400))]">–</span><input id="maintenance-due-to-filter" type="date" value={dueTo} onChange={(event) => setDueTo(event.target.value)} className="border-0 bg-transparent py-1.5 text-sm text-[rgb(var(--color-text-700))] focus:outline-none" />{(dueFrom || dueTo) && <button id="maintenance-due-clear" onClick={() => { setDueFrom(''); setDueTo(''); }} className="ml-1 text-xs text-[rgb(var(--color-primary-600))]">Clear</button>}</div><div className="inline-flex h-9 items-center rounded-md border border-[rgb(var(--color-border-200))] bg-[rgb(var(--color-background-100))] p-0.5">{([['list', 'List', List], ['calendar', 'Calendar', CalendarDays]] as const).map(([value, label, Icon]) => <button key={value} id={`maintenance-${value}-toggle`} onClick={() => setMode(value)} className={`inline-flex items-center rounded px-3 py-1 text-sm font-medium transition-colors ${mode === value ? 'bg-[rgb(var(--color-card))] text-[rgb(var(--color-text-900))] shadow-sm' : 'text-[rgb(var(--color-text-500))] hover:text-[rgb(var(--color-text-700))]'}`}><Icon className="mr-1 h-4 w-4" />{label}</button>)}</div></div>}
    <div className="grid min-h-[calc(100vh-420px)] items-stretch gap-4 xl:grid-cols-[minmax(0,44%)_minmax(0,1fr)]"><Card className="flex flex-col"><CardHeader><CardTitle className="text-base">{tab === 'queue' ? 'Actionable occurrences' : tab === 'plans' ? 'Active plan queue (occurrence-backed)' : 'History'}</CardTitle></CardHeader><CardContent className="min-h-0 flex-1 overflow-auto">{isLoading ? <p className="text-sm text-[rgb(var(--color-text-500))]">Loading maintenance…</p> : mode === 'calendar' && tab === 'queue' ? <Calendar occurrences={shown} onSelect={setSelected} /> : <OccurrenceList occurrences={shown} selectedId={selected?.occurrence_id} onSelect={setSelected} emptyMessage={tab === 'history' ? 'Completed and skipped occurrences will appear here once maintenance is closed out.' : tab === 'plans' ? 'Open plan occurrences will appear here as schedules generate work.' : 'No maintenance occurrences match these filters. Clear a filter or widen the due-date range.'} />}</CardContent></Card><Card className="flex flex-col"><CardHeader><CardTitle className="text-base">Occurrence detail</CardTitle></CardHeader><CardContent className="min-h-0 flex-1 overflow-auto">{selected ? <Detail occurrence={selected} onTicket={() => setTicketTarget(selected)} onComplete={() => setCompleteTarget(selected)} onSkip={() => setSkipTarget(selected)} onPause={async () => { await unwrapAssetActionResult(await setSchedulePaused(selected.schedule_id, true)); await refresh(); }} /> : <div className="flex h-full min-h-[320px] flex-col items-center justify-center gap-2 text-center"><CalendarDays className="h-10 w-10 text-[rgb(var(--color-text-300))]" /><div className="text-sm font-medium text-[rgb(var(--color-text-700))]">No occurrence selected</div><p className="max-w-[280px] text-sm text-[rgb(var(--color-text-500))]">Select an occurrence from the queue to see its asset, plan, linked ticket, and execution actions — or clear the filters to see more work.</p></div>}</CardContent></Card></div>
...
function OccurrenceList({ occurrences, selectedId, onSelect, emptyMessage }: { occurrences: AssetMaintenanceOccurrence[]; selectedId?: string; onSelect: (occurrence: AssetMaintenanceOccurrence) => void; emptyMessage?: string }) {
  ...
  return <div className="space-y-4">{Array.from(groups.entries()).map(([client, items]) => <div key={client}>
    ...
    <div className="space-y-1.5">{items.map((occurrence) => <button id={`maintenance-occurrence-${occurrence.occurrence_id}`} key={occurrence.occurrence_id} onClick={() => onSelect(occurrence)} className={`w-full rounded-md border border-l-4 p-3 text-left transition-colors ${rowAccentClass(occurrence)} ${occurrence.occurrence_id === selectedId ? 'border-[rgb(var(--color-primary-300))] bg-[rgb(var(--color-primary-50))]' : 'border-[rgb(var(--color-border-200))] hover:bg-[rgb(var(--color-primary-50))]'}`}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="truncate font-medium text-[rgb(var(--color-text-900))]">{occurrence.schedule_name}</div><div className="flex items-center gap-1.5 truncate text-xs text-[rgb(var(--color-text-600))]"><span className="truncate">{occurrence.asset_name}{occurrence.asset_type ? ` · ${occurrence.asset_type}` : ''}</span>{occurrence.ticket_id && <span className="shrink-0 rounded-full bg-[rgb(var(--color-primary-100))] px-1.5 py-px text-[10px] font-medium text-[rgb(var(--color-primary-700))]">ticket</span>}</div></div><div className="shrink-0 text-right"><div className="text-xs text-[rgb(var(--color-text-600))]">{dateLabel(occurrence.due_date)}</div><div className={`text-xs ${urgencyClass(occurrence)}`}>{urgencyLabel(occurrence)}</div></div></div></button>)}</div>
  </div>)}</div>;
}
...
function Detail({ occurrence, onTicket, onComplete, onSkip, onPause }: { occurrence: AssetMaintenanceOccurrence; onTicket: () => void; onComplete: () => void; onSkip: () => void; onPause: () => void }) { const checklist = checklistEntries(occurrence.maintenance_data); const execution = checklistItems(occurrence.schedule_config); return <div className="flex h-full flex-col gap-5"><div><div className="flex items-center gap-2"><h2 className="text-lg font-semibold text-[rgb(var(--color-text-900))]">{occurrence.schedule_name}</h2><Badge variant={tone(occurrence)}>{occurrence.status === 'open' ? urgencyLabel(occurrence) : occurrence.status}</Badge></div><div className="mt-0.5 text-sm text-[rgb(var(--color-text-600))]">{occurrence.client_name || 'Unassigned client'} · due {dateLabel(occurrence.due_date)}</div></div><div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm"><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Asset</div><Link id="maintenance-detail-asset-link" className="font-medium text-[rgb(var(--color-primary-600))]" href={`/msp/assets/${occurrence.asset_id}`}>{occurrence.asset_name || occurrence.asset_id}</Link></div><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Type</div><div className="font-medium capitalize text-[rgb(var(--color-text-900))]">{occurrence.maintenance_type}</div></div><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Cadence</div><div className="font-medium text-[rgb(var(--color-text-900))]">{cadenceLabel(occurrence.frequency, occurrence.frequency_interval)}</div></div><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Next due</div><div className="font-medium text-[rgb(var(--color-text-900))]">{dateLabel(occurrence.due_date)}</div></div><div className="col-span-2"><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Linked ticket</div>{occurrence.ticket_id ? <Link id="maintenance-linked-ticket" href={`/msp/tickets/${occurrence.ticket_id}`} className="mt-0.5 inline-flex items-center gap-1.5 rounded-full border border-[rgb(var(--color-primary-200))] bg-[rgb(var(--color-primary-50))] px-2.5 py-0.5 text-xs font-medium text-[rgb(var(--color-primary-700))]"><span className="h-1.5 w-1.5 rounded-full bg-[rgb(var(--color-primary-500))]" />{occurrence.ticket_title || occurrence.ticket_id}<span aria-hidden>↗</span></Link> : <div className="text-sm text-[rgb(var(--color-text-400))]">None yet</div>}</div></div>
    {occurrence.status === 'open' && <div><div className="mb-2 text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Execution{execution.length > 0 ? ` 0/${execution.length}` : ''}</div>{execution.length > 0 ? <><ul className="space-y-1.5">{execution.map((item) => <li key={item.key} className="flex items-center gap-2 text-sm text-[rgb(var(--color-text-700))]"><span className="h-4 w-4 shrink-0 rounded border border-[rgb(var(--color-border-300))]" />{item.label}</li>)}</ul><p className="mt-1.5 text-xs text-[rgb(var(--color-text-400))]">Steps are checked off when completing this occurrence.</p></> : <p className="text-sm text-[rgb(var(--color-text-400))]">No checklist on this plan — add steps from the asset&apos;s Maintenance tab to guide technicians.</p>}</div>}
    <div><div className="mb-2 text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Timeline</div><ol className="space-y-3 border-l border-[rgb(var(--color-border-200))] pl-4">{[
      { key: 'created', title: 'Occurrence created', date: occurrence.created_at, tone: 'bg-[rgb(var(--color-text-300))]' },
      ...(occurrence.ticket_id ? [{ key: 'ticket', title: `Ticket linked — ${occurrence.ticket_title || occurrence.ticket_id}`, date: undefined, tone: 'bg-[rgb(var(--color-primary-400))]' }] : []),
      ...(occurrence.status === 'open' ? [{ key: 'due', title: `Due — repeats ${cadenceLabel(occurrence.frequency, occurrence.frequency_interval).toLowerCase()}`, date: occurrence.due_date, tone:dueDaysDelta(occurrence) !== null && dueDaysDelta(occurrence)! < 0 ? 'bg-red-500' : 'bg-amber-400' }] : []),
      ...(occurrence.status === 'completed' ? [{ key: 'done', title: `Completed by ${personName(occurrence.performed_by_first_name, occurrence.performed_by_last_name, occurrence.performed_by)}`, date: occurrence.performed_at, tone: 'bg-emerald-500' }] : []),
      ...(occurrence.status === 'skipped' ? [{ key: 'skipped', title: `Skipped by ${personName(occurrence.closed_by_first_name, occurrence.closed_by_last_name, occurrence.closed_by)}`, date: occurrence.closed_at, tone: 'bg-[rgb(var(--color-text-400))]' }] : []),
    ].map((event) => <li key={event.key} className="relative"><span className={`absolute -left-[21px] top-1 h-2.5 w-2.5 rounded-full ${event.tone}`} /><div className="text-sm font-medium text-[rgb(var(--color-text-800))]">{event.title}</div>{event.date && <div className="text-xs text-[rgb(var(--color-text-500))]">{dateLabel(event.date)}</div>}</li>)}</ol></div>{occurrence.status === 'completed' && <div id="maintenance-completion-audit" className="space-y-2 rounded-md border border-[rgb(var(--color-border-200))] p-3 text-sm"><div className="font-medium text-[rgb(var(--color-text-900))]">Completion evidence</div><div><span className="text-[rgb(var(--color-text-500))]">Performed:</span> {formatMaintenancePerformedDate(occurrence.performed_at)}</div><div><span className="text-[rgb(var(--color-text-500))]">Performed by:</span> {personName(occurrence.performed_by_first_name, occurrence.performed_by_last_name, occurrence.performed_by)}</div>{occurrence.completion_notes && <div><span className="text-[rgb(var(--color-text-500))]">Notes:</span> {occurrence.completion_notes}</div>}{checklist.length > 0 && <div><div className="text-[rgb(var(--color-text-500))]">Checklist:</div><ul className="list-disc pl-5">{checklist.map(({ key, value }) => <li key={key}>{key}: {value}</li>)}</ul></div>}</div>}{occurrence.status === 'skipped' && <div id="maintenance-skip-audit" className="space-y-2 rounded-md border border-[rgb(var(--color-border-200))] p-3 text-sm"><div className="font-medium text-[rgb(var(--color-text-900))]">Skip evidence</div><div><span className="text-[rgb(var(--color-text-500))]">Reason:</span> {occurrence.skip_reason || 'Not recorded'}</div><div><span className="text-[rgb(var(--color-text-500))]">Closed by:</span> {personName(occurrence.closed_by_first_name, occurrence.closed_by_last_name, occurrence.closed_by)}</div><div><span className="text-[rgb(var(--color-text-500))]">Closed:</span> {formatMaintenancePerformedDate(occurrence.closed_at)}</div></div>}{occurrence.status === 'open' && <div className="flex flex-wrap gap-2 border-t border-[rgb(var(--color-border-200))] pt-4">{occurrence.ticket_id ? <Link id="maintenance-open-ticket" href={`/msp/tickets/${occurrence.ticket_id}`} className="inline-flex items-center rounded-md border border-[rgb(var(--color-border-200))] px-3 py-1.5 text-sm"><Plus className="mr-1 h-4 w-4" />Open ticket</Link> : <Button id="maintenance-create-ticket" size="sm" variant="outline" onClick={onTicket}><Plus className="mr-1 h-4 w-4" />Create ticket</Button>}<Button id="maintenance-complete-occurrence" size="sm" onClick={onComplete}><CheckCircle2 className="mr-1 h-4 w-4" />Complete</Button><Button id="maintenance-skip-occurrence" size="sm" variant="outline" onClick={onSkip}><SkipForward className="mr-1 h-4 w-4" />Skip</Button><Button id="maintenance-pause-schedule" size="sm" variant="ghost" onClick={onPause}><Pause className="mr-1 h-4 w-4" />Pause</Button></div>}</div>; }

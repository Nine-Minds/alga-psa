...
export function MaintenanceCommandCenter() {
  ...
  return <div className="h-full min-h-0 overflow-auto bg-[rgb(var(--color-background))] p-4 md:p-6" data-automation-id="maintenance-command-center">
    ...
    <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{([
      { label: 'Overdue', value: aggregates?.overdue ?? 0, accent: aggregates?.overdue ? 'border-l-red-500' : 'border-l-emerald-400', num: aggregates?.overdue ? 'font-bold text-red-700' : 'text-[rgb(var(--color-text-400))]', hint: aggregates?.overdue ? 'click to filter' : 'all clear', win:'overdue' as const },
      { label: 'Due today', value: aggregates?.due_today ?? 0, accent: aggregates?.due_today ? 'border-l-amber-500' : 'border-l-emerald-400', num: aggregates?.due_today ? 'font-bold text-amber-700' : 'text-[rgb(var(--color-text-400))]', hint: aggregates?.due_today ? 'click to filter' : 'nothing due today', win:'today' as const },
    ...
    <div className="grid min-h-[calc(100vh-420px)] items-stretch gap-4 xl:grid-cols-[minmax(0,44%)_minmax(0,1fr)]"><Card className="self-start"><CardHeader><CardTitle className="text-base">{tab === 'queue' ? 'Actionable occurrences' : tab === 'plans' ? 'Active plan queue (occurrence-backed)' : 'History'}</CardTitle></CardHeader><CardContent className="overflow-auto">{isLoading ? <p className="text-sm text-[rgb(var(--color-text-500))]">Loading maintenance…</p> : mode === 'calendar' && tab === 'queue' ? <Calendar occurrences={shown} onSelect={setSelected} /> : <OccurrenceList occurrences={shown} selectedId={selected?.occurrence_id} onSelect={setSelected} emptyMessage={tab === 'history' ? 'Completed and skipped occurrences will appear here once maintenance is closed out.' : tab === 'plans' ? 'Open plan occurrences will appear here as schedules generate work.' : 'No maintenance occurrences match these filters. Clear a filter or widen the due-date range.'} />}{!isLoading && shown.length > 0 && <div id="maintenance-queue-summary" className="mt-4 border-t border-[rgb(var(--color-border-200))] pt-3 text-xs text-[rgb(var(--color-text-500))]">{shown.length} shown · {openCount} open · <button className="font-medium text-[rgb(var(--color-primary-600))] hover:underline" onClick={() => setTab('history')}>{closedCount} in history</button></div>}</CardContent></Card><Card className="flex flex-col"><CardHeader><CardTitle className="text-base">Occurrence detail</CardTitle></CardHeader><CardContent className="min-h-0 flex-1 overflow-auto">{selected ? <Detail occurrence={selected} onTicket={() => setTicketTarget(selected)} onComplete={() => setCompleteTarget(selected)} onSkip={() => setSkipTarget(selected)} onPause={async () => { await unwrapAssetActionResult(await setSchedulePaused(selected.schedule_id, true)); await refresh(); }} /> : <div className="flex h-full min-h-[320px] flex-col items-center justify-center gap-2 text-center"><CalendarDays className="h-10 w-10 text-[rgb(var(--color-text-300))]" /><div className="text-sm font-medium text-[rgb(var(--color-text-700))]">No occurrence selected</div><p className="max-w-[280px] text-sm text-[rgb(var(--color-text-500))]">Select an occurrence from the queue to see its asset, plan, linked ticket, and execution actions — or clear the filters to see more work.</p></div>}</CardContent></Card></div>

...
export function MaintenanceCommandCenter() {
  ...
  return <div className="h-full min-h-0 overflow-auto bg-[rgb(var(--color-background))] p-4 md:p-6" data-automation-id="maintenance-command-center">
    ...
    <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{([
      { label: 'Overdue', value: aggregates?.overdue ?? 0, accent: aggregates?.overdue ? 'border-l-red-500' : 'border-l-emerald-500', num: aggregates?.overdue ? 'text-red-700' : 'text-emerald-700', hint: aggregates?.overdue ? 'click to filter' : 'all clear', win:'overdue' as const },
      { label: 'Due today', value: aggregates?.due_today ?? 0, accent: aggregates?.due_today ? 'border-l-amber-500' : 'border-l-emerald-500', num: aggregates?.due_today ? 'text-amber-700' : 'text-emerald-700', hint: aggregates?.due_today ? 'click to filter' : 'nothing due today', win:'today' as const },

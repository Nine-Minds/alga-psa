...
export function MaintenanceCommandCenter() {
  ...
  return <div className="h-full min-h-0 overflow-auto bg-[rgb(var(--color-background))] p-4 md:p-6" data-automation-id="maintenance-command-center">
    ...
    <div className="grid items-stretch gap-4 xl:grid-cols-[minmax(0,44%)_minmax(0,1fr)]"><Card className="flex flex-col"><CardHeader><CardTitle className="text-base">{tab === 'queue' ? 'Actionable occurrences' : tab === 'plans' ? 'Active plan queue (occurrence-backed)' : 'History'}</CardTitle></CardHeader><CardContent className="flex min-h-0 flex-1 flex-col overflow-auto">{isLoading ? <p className="text-sm text-[rgb(var(--color-text-500))]">Loading maintenance…</p> : mode === 'calendar' && tab === 'queue' ? <Calendar occurrences={shown} onSelect={setSelected} /> : <OccurrenceList occurrences={shown} selectedId={selected?.occurrence_id} onSelect={setSelected} emptyMessage={tab === 'history' ? 'Completed and skipped occurrences will appear here once maintenance is closed out.' : tab === 'plans' ? 'Open plan occurrences will appear here as schedules generate work.' : 'No maintenance occurrences match these filters. Clear a filter or widen the due-date range.'} />}{!isLoading && tab === 'queue' && mode === 'list' && shown.length > 0 && shown.length <= 6 && <div className="flex flex-1 items-center justify-center py-8"><div className="rounded-lg border border-dashed border-[rgb(var(--color-border-200))] px-6 py-4 text-center"><CalendarDays className="mx-auto mb-2 h-6 w-6 text-[rgb(var(--color-text-300))]" /><div className="text-sm font-medium text-[rgb(var(--color-text-600))]">Queue is clear beyond these {shown.length}</div><div className="text-xs text-[rgb(var(--color-text-400))]">Nothing else due in the selected range — widen the due range or check the plan queue.</div></div></div>}
    {!isLoading && shown.length > 0 && <div id="maintenance-queue-summary" className="mt-auto pt-4"><div className="border-t border-[rgb(var(--color-border-200))] pt-3 text-xs text-[rgb(var(--color-text-500))]">{aggregates && !aggregates.overdue && !aggregates.due_today ? <span className="mr-2 font-medium text-emerald-700">All caught up{(() => { const next = shown.filter((o) => o.status === 'open').map((o) => dueDaysDelta(o)).filter((delta): delta is number => delta !== null && delta > 0).sort((a, b) => a - b)[0]; return next ? ` — next due in ${next}d` : ''; })()}.</span> : null}{shown.length} shown · {openCount} open · <button className="font-medium text-[rgb(var(--color-primary-600))] hover:underline" onClick={() => setTab('history')}>{closedCount} in history</button></div></div>}</CardContent></Card><Card><CardContent className="pt-6">{selected ? <Detail occurrence={selected} onTicket={() => setTicketTarget(selected)} onComplete={() => setCompleteTarget(selected)} onSkip={() => setSkipTarget(selected)} onPause={async () => { await unwrapAssetActionResult(await setSchedulePaused(selected.schedule_id, true)); await refresh(); }} /> : <div className="flex h-full min-h-[320px] flex-col items-center justify-center gap-2 text-center"><CalendarDays className="h-10 w-10 text-[rgb(var(--color-text-300))]" /><div className="text-sm font-medium text-[rgb(var(--color-text-700))]">No occurrence selected</div><p className="max-w-[280px] text-sm text-[rgb(var(--color-text-500))]">Select an occurrence from the queue to see its asset, plan, linked ticket, and execution actions — or clear the filters to see more work.</p></div>}</CardContent></Card></div>
...
function Calendar({ occurrences, onSelect }: { occurrences: AssetMaintenanceOccurrence[]; onSelect: (occurrence: AssetMaintenanceOccurrence) => void }) {
  const today = startOfDay(new Date());
...
function Detail({ occurrence, onTicket, onComplete, onSkip, onPause }: { occurrence: AssetMaintenanceOccurrence; onTicket: () => void; onComplete: () => void; onSkip: () => void; onPause: () => void }) { const checklist = checklistEntries(occurrence.maintenance_data); const execution = checklistItems(occurrence.schedule_config); return <div className="flex h-full flex-col gap-5"><div><div className="flex items-center gap-2"><h2 className="text-lg font-semibold text-[rgb(var(--color-text-900))]">{occurrence.schedule_name}</h2><Badge variant={tone(occurrence)}>{occurrence.status === 'open' ? urgencyLabel(occurrence) : occurrence.status}</Badge></div><div className="mt-0.5 text-sm text-[rgb(var(--color-text-600))]">{[occurrence.client_name || 'Unassigned client', occurrence.asset_tag, occurrence.asset_location].filter(Boolean).join(' · ')} · due {dateLabel(occurrence.due_date)}</div></div><div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm"><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Asset</div><Link id="maintenance-detail-asset-link" className="font-medium text-[rgb(var(--color-primary-600))]" href={`/msp/assets/${occurrence.asset_id}`}>{occurrence.asset_name || occurrence.asset_id}</Link></div><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Type</div><div className="font-medium capitalize text-[rgb(var(--color-text-900))]">{occurrence.maintenance_type}</div></div><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Cadence</div><div className="font-medium text-[rgb(var(--color-text-900))]">{cadenceLabel(occurrence.frequency, occurrence.frequency_interval)}</div></div><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]" title="Next due advances from the actual completion date.">Next due</div><div className="font-medium text-[rgb(var(--color-text-900))]">{dateLabel(occurrence.due_date)}</div></div><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Last done</div><div className="font-medium text-[rgb(var(--color-text-900))]">{occurrence.last_maintenance ? dateLabel(occurrence.last_maintenance) : 'Never'}</div></div><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Assignee</div><div className="font-medium text-[rgb(var(--color-text-900))]">{[occurrence.ticket_assignee_first_name, occurrence.ticket_assignee_last_name].filter(Boolean).join(' ') || (occurrence.ticket_id ? 'Unassigned ticket' : 'Assign via ticket')}</div></div><div><div className="text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Linked ticket</div>{occurrence.ticket_id ? <Link id="maintenance-linked-ticket" href={`/msp/tickets/${occurrence.ticket_id}`} className="mt-0.5 inline-flex items-center gap-1.5 rounded-full border border-[rgb(var(--color-primary-200))] bg-[rgb(var(--color-primary-50))] px-2.5 py-0.5 text-xs font-medium text-[rgb(var(--color-primary-700))]"><span className={`h-1.5 w-1.5 rounded-full ${occurrence.ticket_closed ? 'bg-[rgb(var(--color-text-400))]' : 'bg-emerald-500'}`} />{occurrence.ticket_number ? `#${occurrence.ticket_number}` : occurrence.ticket_title || occurrence.ticket_id} · {occurrence.ticket_closed ? 'Closed' : 'Open'}<span aria-hidden>↗</span></Link> : <div className="text-sm text-[rgb(var(--color-text-400))]">None yet</div>}</div></div>
    ...
    <div><div className="mb-2 text-[11px] font-medium uppercase tracking-wide text-[rgb(var(--color-text-500))]">Timeline</div><ol className="ml-1.5 space-y-4 border-l-2 border-[rgb(var(--color-border-200))] pl-5">{[
      ...
      ...(occurrence.status === 'completed' ? [{ key: 'done', title: `Completed by ${personName(occurrence.performed_by_first_name, occurrence.performed_by_last_name, occurrence.performed_by)}`, date: occurrence.performed_at, tone: 'border-emerald-500' }] : []),

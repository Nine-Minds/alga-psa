...
export function MaintenanceCommandCenter() {
  ...
  return <div className="h-full min-h-0 overflow-auto bg-[rgb(var(--color-background))] p-4 md:p-6" data-automation-id="maintenance-command-center">
    ...
    <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{([
      ...
      { label: 'Due today', value: aggregates?.due_today ?? 0, accent: aggregates?.due_today ? 'border-l-amber-500' : 'border-l-emerald-400', num: aggregates?.due_today ? 'font-bold text-amber-700' : 'text-[rgb(var(--color-text-400))]', hint: aggregates?.due_today ? 'click to filter' : 'all clear', win:'today' as const },

...
export const addChecklistItem = withAuth(
  async (user, { tenant }, ticketId: string, input: ChecklistItemInput): Promise<ITicketChecklistItem> => {
...
export const updateChecklistItem = withAuth(
  async (user, { tenant }, itemId: string, input: Partial<ChecklistItemInput>): Promise<ITicketChecklistItem> => {
    if (!(await hasPermission(user, 'ticket', 'update'))) {
      throw new Error('Permission denied: Cannot update ticket checklist');
    }
...
export const deleteChecklistItem = withAuth(
  async (user, { tenant }, itemId: string): Promise<void> => {
    if (!(await hasPermission(user, 'ticket', 'update'))) {
      throw new Error('Permission denied: Cannot update ticket checklist');
    }
...
export const reorderChecklistItems = withAuth(
  async (user, { tenant }, ticketId: string, orderedItemIds: string[]): Promise<void> => {
    if (!(await hasPermission(user, 'ticket', 'update'))) {
      throw new Error('Permission denied: Cannot update ticket checklist');
    }
...
export const setChecklistItemCompleted = withAuth(
  async (user, { tenant }, itemId: string, completed: boolean): Promise<ITicketChecklistItem> => {
    if (!(await hasPermission(user, 'ticket', 'update'))) {
      throw new Error('Permission denied: Cannot update ticket checklist');
    }

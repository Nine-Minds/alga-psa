...
describe('ticket close/checklist/material facade contract', () => {
  ...
  it('uses facade joins for the migrated tenant-table joins', () => {
    expect(sources.materialCatalog).toContain("tenantScopedTable(trx, 'ticket_materials as tm', tenant)");
    expect(sources.materialCatalog).toContain("tenantJoin(");
    expect(sources.materialCatalog).toContain("'service_catalog as sc'");

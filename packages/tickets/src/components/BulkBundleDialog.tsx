...
export default function BulkBundleDialog({ id, isOpen, onClose, initialTicketIds, knownRows, onBundled }: Props): React.JSX.Element {
  ...
  const removeMember = (ticketId: string) => {
    ...
    if (masterId === ticketId) setMasterId(next[0]?.ticket_id ?? null);
  ...
  const performBundle = async () => {
    ...
    setError(null);
    try {
      const result = await bundleTicketsAction({ masterTicketId: masterId, childTicketIds: members.map(m => m.ticket_id).filter((id): id is string => Boolean(id) && id !== masterId), mode: syncUpdates ? 'sync_updates' : 'link_only', ...(needsClosedChoice && closedChoice ? { onClosedMaster: closedChoice } : {}) });

...
describe('ticket origin locale keys', () => {
  ...
  it('T061: English clientPortal locale includes ticket origin keys for internal/client_portal/inbound_email/api/other', () => {
    ...
    expect(locale?.origin?.internal).toBe('Created Internally');
    expect(locale?.origin?.clientPortal).toBe('Created via Client Portal');
    expect(locale?.origin?.inboundEmail).toBe('Created via Inbound Email');
    expect(locale?.origin?.api).toBe('Created via API');
    expect(locale?.origin?.other).toBe('Created via Other');
  });
});

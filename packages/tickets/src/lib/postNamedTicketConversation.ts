...
export async function applyNamedTicketConversationPost(context: NamedConversationPostContext, comment: CoManagedCommentInsert) {
  return applyTicketConversationComment({ trx: context.trx, resource: { tenant: context.ticket.tenant, id: context.ticket.ticketId },
    ...
    canUpdateResponseState: context.canUpdateResponseState,

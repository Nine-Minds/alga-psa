...
describe('email notification service tenant-scoped query contract', () => {
  it('uses structural tenant scoping for tenant-owned notification roots', () => {
    ...
    expect(source).toContain("'tenant_email_settings', tenantId, 'tenant_id'");
    expect(source).toContain("'notification_settings', tenant");
    expect(source).toContain("'tenant_email_templates', tenant");
    expect(source).toContain("'user_notification_preferences', tenant");
    ...
    expect(source).toContain("'notification_logs', tenant");

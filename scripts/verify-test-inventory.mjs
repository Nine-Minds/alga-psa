...
try {
  ...
  result = reconcileDiscovery({ root, candidates: repositoryTestFiles(root),
    collections: manifest.collections, exclusions: manifest.exclusions ?? [] });
  result.scope = 'repository';
  result.runners = manifest.collections.map(({ runner, owner, runtime, mandatory }) => ({ runner, owner, runtime, mandatory }));

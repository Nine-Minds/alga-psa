...
const output = path.join(root, 'test-results/repository-discovery/vitest');
mkdirSync(output, { recursive: true });
const before = testRevision(root);
const failures = [], collections = [];
const runners = JSON.parse(readFileSync(path.join(root, 'scripts/repository-vitest-runners.json'), 'utf8'));
if (process.argv.length !== 2) throw new Error('Repository discovery always collects all registered runners; filters are not allowed.');
...
const result = { schemaVersion: 1, revision: before.revision, sourceRoot: root,
  scope: 'repository-vitest-collection', executionVerified: false,

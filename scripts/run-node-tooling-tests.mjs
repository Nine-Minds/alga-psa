...
let evidence;
try {
  if (process.argv.length !== 2) throw new Error('Usage: node scripts/run-node-tooling-tests.mjs (complete suite; no filters)');
  before = testRevision(root);
  const candidates = repositoryTestFiles(root).filter(isNodeToolingTest);
  const exclusions = JSON.parse(readFileSync(path.join(root, 'scripts/node-test-exclusions.json'), 'utf8'));
  ...
  // Node resolves these workspace exports through dist. Always build from this
  // candidate; a developer's old output must not make the control tests pass.
  for (const workspace of ['@alga-psa/emulator-host', '@alga-psa/emulator-stripe']) {
    const build = spawnSync('npm', ['run', 'build', `--workspace=${workspace}`], { cwd: root, stdio: 'inherit' });
    if (build.status !== 0) throw new Error(`Required emulator build failed: ${workspace} (exit ${build.status})`);
  }

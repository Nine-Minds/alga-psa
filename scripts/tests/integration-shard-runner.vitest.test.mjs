...
test('actual integration runner partitions, executes and rejects missing or stale shard evidence', { timeout: 120_000 }, t => {
  ...
  const run = (script, index = 1, mode = 'full', total = 3) => spawnSync(process.execPath, [path.join(root, 'scripts', script)], {
    ...
    env: { ...process.env, CI: '1', TIER1_BASE_SHA: '', GITHUB_SHA: git('rev-parse', 'HEAD').trim(), INTEGRATION_FULL: 'true', INTEGRATION_JOB_RESULT: 'success', INTEGRATION_SHARD_INDEX: String(index), INTEGRATION_SHARD_TOTAL: String(total) },
  });

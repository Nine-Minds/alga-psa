...
import { captureUpgradeDistribution } from './lib/upgrade-citus.mjs';

async function main() {
  const root = fileURLToPath(new URL('../', import.meta.url));
  const database = process.env.UPGRADE_DB_NAME;
  const backend = process.env.UPGRADE_DB_BACKEND || 'postgres';
  if (!['postgres', 'citus'].includes(backend)) throw new Error('UPGRADE_DB_BACKEND must be postgres or citus');
  ...
  const report: any = { schemaVersion: 1, phase: 'schema-and-retention', status: 'failed', database, backend, source, baseline, output,
  ...
  try {
    ...
    if (backend === 'citus') {
      const setup = knex({ client: 'pg', connection: connection(database!), pool: { min: 0, max: 1 } });
      try {
        await setup.raw('CREATE EXTENSION citus');
        await setup.raw('ALTER DATABASE ?? SET citus.shard_count = 4', [database]);
      } finally { await setup.destroy(); }
      report.limitations.push('Citus topology and business behavior require separate runtime evidence; distribution metadata alone does not prove either.');
    }
    ...
    if (backend === 'citus') report.distribution = { baseline: await captureUpgradeDistribution(db) };

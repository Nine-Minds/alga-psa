...
const input = path.resolve(root, process.argv[2] || 'test-results/workspace-gate-input');
const output = path.join(root, 'test-results/workspace-gate');
...
try {
  ...
  const jobResults = JSON.parse(process.env.WORKSPACE_GATE_JOB_RESULTS || '{}');
  ...
  for (const { suite, shards } of workspaceRequirements) {
    ...
    for (let shard = 1; shard <= shards; shard++) {
      const artifact = shards > 1 ? `${suite}-shard-${shard}` : `${suite}-${source.revision}`;
  ...
  result = evaluateWorkspaceGate({ root, revision: source.revision, jobResults, candidatesBySuite, bundles });
} catch (error) {
  result = { schemaVersion: 1, scope: 'additional-workspace-tests', status: 'failed', suites: [], failures: [] };
...
console.log(`Workspace execution gate: ${result.status}; ${result.suites.filter(suite => suite.status === 'passed').length}/${workspaceRequirements.length} suites complete`);

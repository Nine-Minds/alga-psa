...
const file = 'ee/temporal-workflows/src/__tests__/integration/workflowInvocationPersistence.integration.test.ts';
...
const collected = [{ file: path.join(root, file) }];
...
try {
  ...
  const selection = path.relative(server, collected[0].file);
  const collection = run(['list', selection, `--json=${path.join(output, 'collected-tests.json')}`]);
  if (collection.status !== 0) throw new Error('Citus workflow collection failed');
  const collectedTests = JSON.parse(readFileSync(path.join(output, 'collected-tests.json'), 'utf8'));
  if (!Array.isArray(collectedTests) || !collectedTests.length) throw new Error('Citus workflow collection is empty');
  const result = run(['run', selection, '--coverage.enabled=false', '--reporter=default', '--reporter=json',
    `--outputFile.json=${path.join(output, 'results.json')}`]);
  const report = JSON.parse(readFileSync(path.join(output, 'results.json'), 'utf8'));
  evidence = reconcileExecution({ collected, collectedTests, report, root, suite: 'citus-workflow-runtime',
  ...
  evidence = { schemaVersion: 1, suite: 'citus-workflow-runtime', status: 'failed', failures: [error.message] };
}
